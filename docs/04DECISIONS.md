...
- CUI-2：Code commit **fd99134**（`feat: add apfr correction ui core`・`index.html` +165／-5・`apfrCorrectionUi.test.js` 新規804行）。本docs追記の開始時点でbranch main／HEAD `fd991341c586ef5316c9a85871e243c40e5e7a8e`／origin/main `a8d1b399bb795c84b5ed6ac0ce81186bcb5b11ac`（ahead 1／behind 0＝**Code push未実施**）。**CUI-0／CUI-1／CUI-2のTagはいずれも未作成**であり、Correction UI全体（CUI-0〜CUI-2）としての正式リリース（push→Annotated Tag→Render反映→本番UI確認）は次工程で実施する。
