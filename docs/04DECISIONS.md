...
次工程:
- **Correction UI Core系列（CUI-0〜CUI-2）の正式リリースはComplete**（push・Annotated Tag・Render反映・本番配信コード確認・本番実データread-only検証・**本番認証後の実ブラウザ最終目視確認まで完了**）。**APFR Step C-1A（Compliance Context Foundation）・Step C-1B（Writer/Reviewer Compliance Injection）はCode実装・テスト・commitまでComplete**（詳細は上記「決定（Step C-1A...）」「決定（Step C-1B...）」参照）。**ただしC-1A/B Codeはまだ未push（origin/mainはCUI-2正式リリース時点`186ec637...`のまま）**。
- **APFR Step C-1A + C-1B正式リリースComplete**（dev-check再試行で200/200/200を取得・main push・Annotated Tag `v1.01-apfr-compliance-injection` 作成・tag push・Render自動Deploy反映確認まで完了。詳細は下記追記参照）。**残る次工程＝C-1C以降の選定**（**C-1Cへは自動的に進まない**）。以下を比較対象候補として次工程選定を行う：①**CUI-3**（「訂正済み」バッジ・旧値参考表示。独立工程として本当に必要かは要再評価）②**CUI-4**（APFR直接ジャンプ導線）③残課題2（ITP `7days` field追加要否）④残課題4〜7のUI改善⑤**APFR Step C-1C（Quality Gate Compliance Enforcement）**⑥**APFR Step C-2（payout/epc/approvalRateのFormal Truth数値突合・score接続）**。**特定の1つを自動確定せず、正式な次工程はユーザー承認後に決定する**。`listingPolicy`の正式注入も未着手のまま。
