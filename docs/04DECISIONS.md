...
/members/<single-segment-id>.md              （単一segment・.md拡張子のみ・サブディレクトリ不可）
```

`shared/`ディレクトリは原則非公開であり、現在allowlistされた5ファイルのみが例外（`carouselImageCore.js`／`carouselApproval.js`等の同ディレクトリ内server-onlyファイルは対象外）。`members/<id>.md`はUIが動的なidでfetchするため、個別ファイル列挙ではなく「単一segment・`.md`のみ」というパターンルールで許可している。

4. **Default-Deny原則。** 新しいfile／resourceがrepoへ追加されても、**自動的にpublicになってはならない。** 新しいbrowser assetを追加する場合は以下を必須とする：
   1. browser requirementを確認する（実際にbrowserが要求するか）
   2. server-only informationを含まないことを確認する
   3. `lib/publicStatic.js`のexplicit allowlistへ追加する
   4. static exposure regression test（`staticExposureBoundary.test.js`）を追加・更新する

5. **Internal resourcesは静的配信しない：** `docs/`／`lib/`（allowlist対象外）／`supabase/`／`data/`／`assets/`／`node_modules/`／backup directories／`*.test.js`／`server.js`／`package.json`／`package-lock.json`／root直下の内部MD・内部JSON／`.env*`／`.git*`／logs／その他server-only source。

6. **API boundaryは不可侵。** `/api/*`および必要なserver routesはstatic allowlistの対象ではなく、Express route processingへpass-throughする。Carousel（`POST /api/carousel-image/approval`／`POST /api/carousel-image/generate`／`GET /api/carousel-image/assets`）のDecision 113 access controlは変更しない。

7. **SPA sensitive fallbackを持たない。** blocked internal pathを要求した場合、`index.html`へfallbackして200を返す構造にしない。internal file contentも返さない。実測：blocked pathは**404**となる（漏洩の有無とHTTPステータスが一致する）。

8. **Traversal／encoding耐性。** dot traversal（`..`／`.`／空segment）・percent encoding（`%2e%2e`／`%252e%252e`／`%2F`／`%00`。`%`を含むpathはデコード曖昧性を避けるため一律拒否）・backslash・C0制御文字／DEL・case variation（allowlistはcase-sensitive完全一致）によってinternal fileへ到達できないことをregression testで維持する。

### Architecture Scope

本DecisionはCarousel専用ではない。**ENBISOU AI COMPANY Web server全体のPublic Static Asset Security Boundary**として扱う。ただし、認証方式（`WEB_APP_PASSWORD`／`WEB_SESSION_SECRET`／localStorage UI gate）・API authorization・Carousel Approval Token・billingLock・Budget・Ledger・Storage trust boundaryは変更しない。

将来frontend構成を変更する場合（public directory新設・bundler output・CDN配信等）でも、方式の選択自体は変更可能である。しかし、**public resourceを明示的に限定し、server-only／internal resourceをdefault-denyする**という本Decisionの原則は維持する。本Decisionの変更なしにrepo-root unrestricted static servingへ戻さない。

### 採用理由

- denylist（「これは隠す」）は、将来server-onlyファイルが追加されたときに自動的に公開されてしまうため不採用。allowlistは「browserが実際に要求するassetのみ」を明示するため、新規ファイル追加が既定で安全側に倒れる。
- 実装済みの`express.static`の配信契約（Content-Type／ETag／Last-Modified／Range／conditional GET）をそのまま再利用でき、大規模なfrontend移動・bundler導入・npm package追加が不要だった（既存UIへの影響が最小）。
- SPA fallbackを持たないことで、「内部ファイルの存在有無」が常にHTTPステータス（404）で正しく表現される。

### 却下した案

- **denylist（既知のserver-onlyファイル・ディレクトリを列挙して除外）** — 将来追加されるファイルが既定で公開されてしまう。allowlist（本採用）は逆に安全側へ倒れる。
- **public/ディレクトリ新設＋frontend asset移動** — 既存`index.html`は単一ファイルでCSS/JSを内包しており、移動対象が実質存在しない。大規模移動は既存UIを壊すリスクがあり、Scope Escalation（B判定）に該当するため見送った。
- **SPA catch-all fallback（未知pathをindex.htmlへ200で返す）の追加** — 内部pathの存在有無がHTTPステータスから読み取れなくなり、security testでの区別が失われる。

### PA-3A実測エビデンス

- `staticExposureBoundary.test.js` **211 passed／0 failed**（実HTTP listen 0・実network 0）
- loopback ephemeral portによるend-to-end実測 **36／36**（公開resourceが200・blocked internal resourceが404）
- 既存Carousel回帰7スイート込み合計 **1,077 passed／0 failed**

### 今回実施していないこと（重要）

- **本番deployは行っていない。** 状態は**Implemented / Not Yet Deployed**（未commit・未push・未deploy）。deploy後、本番URLでの再確認が必要。
- **認証方式・Carousel課金安全境界（billingLock／Approval Token／Budget／Ledger／Decision 110〜115／Dual-Key／Storage trust boundary）は一切変更していない。**
- `server.js`／`lib/publicStatic.js`／`staticExposureBoundary.test.js`のコード変更は行っていない（本Decisionはdocs-onlyの正式化）。

---


# Decision 115
## Carousel Real Image Generation Dual-Key Kill Switch（実Image API到達条件をsource gate（`REAL_ENABLED`）とserver-only env gate（`CAROUSEL_IMAGE_REAL_ENABLED`）のANDへ拡張し、deployなしの緊急停止を可能にする・2026-09-08）

**新規Decision番号を採用する理由**：既存Decisionのうち`REAL_ENABLED`に触れているものはDecision 111（budget model）とDecision 112（partial UNIQUE lock）だが、いずれも「実API到達条件の仕組み自体」を扱っていない。本Decisionは`lib/carouselImageClient.js`の実API到達ゲート構造そのものの変更であるため新規採番する。

### 背景

Step C-4のProduction Environment Preflightで、`REAL_ENABLED`が**source constant（`lib/carouselImageClient.js`のvar宣言）のみ**で制御されていることを確認した。これは以下の運用リスクを持つ：

- 緊急停止（無効化）にcommit／push／deployのフルサイクルが必要になる。
- インシデント発生時、Renderのenv変更だけでは実APIを止められない。

C-4はこれを「REAL_ENABLED=true解禁前のRequired Safety Work」として整理し、Decision化は実装後へ延期した（C-5では未実装として記録）。本Decisionはその実装完了を受けた正式化である。

### 決定（正式）

1. **実Image APIへ到達可能なのは、以下2条件を両方満たす場合のみとする。**

```
_sourceRealEnabled === true         （source gate。lib/carouselImageClient.js の編集でのみ変更可能）
  AND
process.env.CAROUSEL_IMAGE_REAL_ENABLED === 'true'   （env gate。厳密一致）
```

2. **env変数名は `CAROUSEL_IMAGE_REAL_ENABLED`。** server-only。`NEXT_PUBLIC_*` 禁止。browserへ値を返さない。ログへ値を出力しない（`lib/carouselImageClient.js` は `console.*` を一切持たない）。
3. **fail-closed contract**：未設定／`''`／`'false'`／`'False'`／`'TRUE'`／`'1'`／`'yes'`／前後に空白を含む文字列（`' true'`／`'true '`）等は**すべてdisabled**。**文字列 `'true'` の完全一致のみenable**（大文字小文字を許容しない・trimしない）。
4. **source gateを削除しない。** envだけで実APIを有効化できる構造にしない（Renderのenv変更経路は本コードよりも変更頻度・変更経路が多く、単独の有効化スイッチにはしない）。
5. **既存 `shared/carouselImageCore.js`（変更禁止）の `assertRealCallAllowed()` は `client.REAL_ENABLED !== true` を直接読む単一のcheckpointである。** そのため `REAL_ENABLED` のexported値そのものをdual-keyの結果にする（`Object.defineProperty` によるgetter/setter化）。getterは `_sourceRealEnabled === true && process.env.CAROUSEL_IMAGE_REAL_ENABLED === 'true'` を返し、setterは既存テストの `client.REAL_ENABLED = true/false` という save/restore パターンをそのまま受理する（= source gate側を切り替えるという既存の意味を維持）。
6. **この設計により、assertRealCallAllowed（reserve直前のgate）と `generateBackground()`／`generateBackgroundRaw()`（provider直前のgate）が常に同一のdual-key値を参照し、決して乖離しない。** これにより、「source=true・env未成立のときにreserve（nonce消費）だけ成功し、provider呼び出し段階でだけ`real_api_disabled`となってbudget/nonceを無駄に消費する」という経路を構造的に排除する。
7. **既存reason語彙 `real_api_disabled` を維持する。** 新しいreasonを追加していない。
8. **既存の他Gate（Session／Origin／Approval Token／billingLock／Budget・Ledger）はいずれも変更しない。** 有料実行のAND条件（Decision 113）に「source gate ∧ env gate」がそのまま合流する。

### 採用理由

- Renderのenv変更＋restartだけで実APIを即座に止められる（commit/push/deployのフルサイクルを待たない）。
- source gateを残すことで、env変数の設定ミス・意図しない設定だけでは実API解禁が起こらない（二重の意図確認を要求する）。
- 既存 `shared/carouselImageCore.js` を変更せずに済む（同ファイルの改変を避けたいという既存方針と両立する）。

### 却下した案

- **env gateのみ（source constantを削除）** — Renderのenv変数だけで実API解禁が可能になり、誤設定時の安全弁が1段減る。
- **generateBackground()／generateBackgroundRaw()側にのみdual-keyを追加し、REAL_ENABLEDのexported値は変更しない** — assertRealCallAllowed（reserve直前）とprovider呼び出し側のgateが乖離し、source=true・env未成立の状態でreserveだけが先に成功してしまう（budget/nonceの無駄な消費）。安全性が本Decisionの目的（緊急停止の実効性）に対して弱くなるため却下。

### 今回実施していないこと（重要）

- **`REAL_ENABLED`（source gate）を`true`へ変更していない。** `_sourceRealEnabled = false` を維持（実測：`lib/carouselImageClient.js` に `var _sourceRealEnabled = false;` が存在することをテストで確認）。
- **Renderへ `CAROUSEL_IMAGE_REAL_ENABLED` を設定していない。**
- **実OpenAI API呼び出し・paid generationは0件のまま。**
- 実装とテストはfake dependency／同一プロセス内の一時的な `process.env` 変更（各テストでsave/restore）のみで完結している（実測：`carouselRealEnabledKillSwitch.test.js` **51 passed／0 failed**、既存4スイート回帰 **carouselImageProduction 465／0・carouselImageRoutes 95／0・carouselAccessControl 82／0・carouselAssetStorage 54／0・carouselExecutionDb 88／0**）。

---

# Decision 114
## Carousel Asset Storage Server-only Trust Boundary（生成画像の永続化をprivate bucket＋server-only privileged uploadへ限定し、閲覧を短命signed URLのみに限定する・2026-09-08）

**新規Decision番号を採用する理由**：Decision 110（carousel_image_executions Server-only Trust Boundary）は**paid execution ledgerという1テーブル専用**の判断であり、同Decision自身が「privileged clientは `lib/carouselExecutionSupabase.js` に隔離し、carousel execution ledger専用とする／他のDB helperへ横展開しない」と明記している。したがってDecision 110をStorage（別リソース・別アクセス経路・別失敗モード）へ準用することはできない。生成画像バイナリという新しい資産に対する新しいtrust boundaryであるため、新規採番する。

### 背景

Phase 2-D時点まで、Carousel Image Productionには**生成画像バイナリを永続化する機構が存在しなかった**。`shared/carouselImageCore.js` が返す `/generated/<caseId>/<outputId>/...` は擬似metadata文字列であり、実ファイル保存ではない（read-only調査で確認：repo全体でSupabase Storage利用0件・画像binary永続化コード0件）。

初回有料生成を行うには、(1) 生成画像を失わずに保存し、(2) 人間がImage Reviewのために閲覧でき、(3) その画像が第三者へ露出しない、という3条件を同時に満たす必要がある。base64のDB JSONB保存・Render filesystem・新規外部S3はいずれも却下し、既存Supabase基盤上のStorageを採用した。

### 決定（正式）

1. **bucketは `carousel-images` 固定**（コード定数。client入力からbucket名を指定させない）。**private（public=false）**とする。
2. **public URLを一切生成しない。** upload成功時の正式metadataは `storagePath` のみ。
3. **browserからの直接uploadを行わない。** uploadはserver privileged clientのみ。
4. **閲覧は短命signed URLのみ**とする。既定300秒・**expiryはserver固定**（clientから指定させない・範囲外要求はDEFAULTへfail-safe）。
5. **asset pathは `carousel/<caseId>/<outputId>/<nonce>/slide-<N>.png`。** nonceをpathへ含めることで、failed persistence／retry／旧generation間のasset collisionを防ぐ。
6. **path構成要素はallowlist検証**（`^[A-Za-z0-9_-]{1,128}$`）。`..`／`/`／`\`／制御文字／空文字は構造的に排除する。slideIndexは1〜19（`lib/carouselCompositor.js` の既存 `SAFE_NAME_RE` と同一範囲）。
7. **uploadは `contentType: 'image/png'` 固定・`upsert: false` 固定。** 同一storagePathへの再アップロードは `storage_collision` として拒否する（同一nonce/slideの再書込みでbilling/persistence truthを曖昧にしないため）。
8. **application側でPNG検証を行う**：`Buffer.isBuffer` 必須／空Buffer拒否／PNG signature（`89 50 4E 47 0D 0A 1A 0A`）厳密一致／`MAX_ASSET_BYTES = 32MB`（`lib/carouselImageNormalize.js` の `MAX_INPUT_BYTES` と同一根拠・同一値）。
9. **正式asset metadata**は `slideIndex／slideId／storagePath／sha256／bytes／format／width／height／aspectRatio／status／imageReviewOk／generatedAt／regenCount`。**raw PNGバイナリをOutput DraftのJSONBへ保存しない。**
10. **width/height/aspectRatioは画像をデコードして検証しない**（decode dependencyを新たに持ち込まない）。1080×1350／4:5はupstream（normalize／compositor）が保証した固定契約値としてそのまま記録する。
11. **順序は `provider成功 → ledger completed → asset persistence`** を維持する。Storage失敗が発生済みの課金を取り消すことはない。
12. **自動delete rollbackを実装しない。** batch uploadが部分失敗しても、成功済みobjectを自動削除しない（削除という新しい危険操作を暗黙に追加しないため）。全枚成功時のみOutput Draftへ `carouselAssets` を正式登録する。
13. **Storage privileged clientはDecision 110のledger専用clientを流用しない。** 将来 `lib/carouselAssetStorageSupabase.js` 等のStorage専用server-only clientを別責務で新設する（env変数名の規約 `SUPABASE_SECRET_KEY`→`SUPABASE_SERVICE_ROLE_KEY` は共通・`NEXT_PUBLIC_*` 禁止）。
14. **raw prompt全文／日本語slide本文／API key／provider raw response／raw Storage errorを、storagePath・metadata・戻り値のいずれにも含めない。**

### 採用理由

- private bucket＋server privileged upload＋短命signed URLの組み合わせは、Decision 110が確立した「client roleに直接権限を与えない」原則とアーキテクチャ上整合する。
- `upsert:false` により、同一nonce/slideの二重書込みが構造的に起こらない。課金台帳（nonce UNIQUE）と同型の冪等性がStorage層でも成立する。
- pathへnonceを含めることで、失敗した実行の残骸と成功した実行の成果物が同一pathで混ざらない。orphan発生時もどの実行由来かをpathだけで特定できる。
- 自動削除を持たないことで、「課金は発生したがassetは消えた」という最悪の不整合を作らない。orphanは残るが、それは観測可能な安全側の状態である。

### 却下した案

- **base64をOutput DraftのJSONBへ保存** — Draft本文と画像バイナリを同一行へ混在させ、行サイズ・転送量・既存Draft契約のいずれも壊す。
- **Render filesystem保存** — Renderのdiskはdeploy/restartで揮発し、multi-instanceで共有されない。課金済み成果物の保存先として不適格。
- **新規外部S3等の追加** — 新しいcredential・新しいtrust boundary・新しい課金経路を増やす。既存Supabase基盤で要件を満たせるため不要。
- **public bucket＋難読URL** — security by obscurityであり、URLが漏れた時点で無制限に閲覧可能になる。

### 今回実施していないこと（重要）

本Decisionは **Implemented / Production Not Activated** である。

**Production Activation Step PA-2（2026-09-08）追記**：Storage専用 server-only privileged client（`lib/carouselAssetStorageSupabase.js`）の実装・`buildProductionDeps()` への配線が完了した（Decision 110のledger専用clientは横展開せず、Decision 110と同一のcredential優先順位規約〔`SUPABASE_SECRET_KEY`優先→`SUPABASE_SERVICE_ROLE_KEY`fallback〕のみを共有する完全に独立したmoduleとして実装）。credential未設定の現行環境では `carouselAssetStorageClient = null` となり、`buildProductionDeps().storageClient` も `null` のまま——`lib/carouselAssetStorage.js`／`lib/carouselAssetAccess.js`（いずれも無変更）が引き続き `storage_unavailable` でfail-closedするため、実Storage networkへの到達は変わらず0のままである。新Decision番号は採番していない（本Decision 114の実装完成として記録）。実測：`carouselAssetStorageSupabase.test.js` **31 passed／0 failed**（新規）。

以下は引き続き**すべて未実施**：

- **`carousel-images` bucketの作成**（実bucketは存在しない）
- **bucketのMIME／file size制限の設定**（`image/png` のみ・32MBを設定予定）
- **Storage policyの確認・設定**
- **実Storage upload**（0件）／**signed URL発行**（0件）
- **Render `SUPABASE_SECRET_KEY`／`SUPABASE_SERVICE_ROLE_KEY` の設定**

実装とテストは fake Storage client のdependency injectionのみで完結している（実測：`carouselAssetStorage` 54 passed／0 failed、`carouselImageRoutes` 95 passed／0 failed、`carouselAssetStorageSupabase` 31 passed／0 failed）。

---

# Decision 113
## Carousel Paid Route Server-side Access Control Boundary（有料実行routeへserver-side signed sessionを必須化し、shared internal access boundaryとして正式化する・2026-09-08）

**新規Decision番号を採用する理由**：既存Decisionにaccess control／authenticationを扱ったものが存在しない。Decision 110はDB credentialのtrust boundary、Decision 111はcost authorizationであり、いずれも「そのHTTP routeへ到達してよい利用者か」を扱っていない。本Decisionは本repo初のserver-side認可境界であるため新規採番する。

### 背景

Phase 2-E Step C-4のread-only調査で、本repoの認証が**server-side authorizationとして機能していない**ことが判明した。

- `POST /api/login` は `WEB_APP_PASSWORD` を照合して `{ok:true}` を返すのみで、**session／cookie／tokenを一切発行しない**。
- ブラウザ側は成功時に `localStorage.setItem('enbisou_auth_v1','true')` を書くだけで、以降のAPI呼び出しに認証情報を**送らない**（サーバも検証しない）。
- したがって認証は「localStorageに文字列があるか」というUI表示ゲートに過ぎず、DevToolsで1行書けば突破でき、そもそもUIを経由せず直接HTTPを叩ける。

Step C-3で実装したCarousel production routesも同水準で公開されており、`REAL_ENABLED=true` 解禁後は **無認証で有料画像生成を起動できる**状態になる（被害はDecision 111の¥100/postとDecision 112の1 completed/outputで上限が付くが、承認済み未公開Draftの数だけ繰り返せる）。これを解禁前に閉じる必要があった。

### 決定（正式）

1. **Carousel有料実行routeの3本にserver-side sessionを必須化する。**
   `POST /api/carousel-image/approval`／`POST /api/carousel-image/generate`／`GET /api/carousel-image/assets`
2. **sessionはstatelessなHMAC-SHA256署名tokenをHttpOnly cookieで運ぶ。** cookie名 `enbisou_session`、token format `s1.<base64url(payload)>.<base64url(sig)>`、payloadは `{v, iat, exp}` のみ（PII／password／secret／roleを載せない）。署名比較は `crypto.timingSafeEqual`。
3. **署名secretは `WEB_SESSION_SECRET`（独立secret・最小16文字）。** `WEB_APP_PASSWORD` をHMAC鍵へ直接流用しない。server-only・`NEXT_PUBLIC_*` 禁止・browserへ値を返さない・ログへ出さない。
4. **cookie属性は `HttpOnly` / `SameSite=Strict` / `Path=/` / `Max-Age=24h`。** `Secure` は**リクエスト自身から判定**する（`x-forwarded-proto: https` → 付与、localhost HTTP → 非付与）。`NODE_ENV` 等の環境判定に依存しない。
5. **POST 2routeにtrusted Origin検証を課す**（CSRF defense in depth）。allowlistは `WEB_TRUSTED_ORIGIN`（カンマ区切り）∪ `RENDER_EXTERNAL_URL`。**1つでも設定されていればlocalhostは許可しない**。何も設定されていない場合のみlocalhostを許可する（未設定＝ローカル開発）。Host headerを無条件に信用して期待originを組み立てない。
6. **Origin欠落は403で拒否する**（無条件acceptしない）。**Referer fallbackは採用しない**（Referrer-Policyで容易に欠落し「無ければ通す」経路が防御を実質無効化するため）。
7. **`GET /api/carousel-image/assets` もsession必須**とする（署名URL発行routeであるため）。書き込みではないためOrigin検証は課さない（cookieが `SameSite=Strict` のためクロスサイトでは送信されない）。
8. **環境変数の有無で認証をskipする経路を作らない。** `WEB_SESSION_SECRET` 未設定なら署名検証が常に失敗し、3routeは常時401で閉じる（実測確認済み）。
9. **session失敗の理由をclientへ返さない。** signature／version／expiry／形状のいずれで落ちても一律 `401 {ok:false, reason:'unauthorized'}`。raw token・raw errorを返さない。
10. **`POST /api/logout` を追加し、cookieを `Max-Age=0` で失効させる。** 既存UIの `logout()` はlocalStorage削除のみだったため、sessionが残り続ける状態を防ぐ。
11. **`localStorage.enbisou_auth_v1` はUI表示制御に留め、server authorityとしない。**
12. **適用範囲は上記3routeのみとする。** 既存API群への一括適用（repo全体のSecurity Migration）は本Decisionのscope外とする。

### 認可の分離（重要）

有料実行は以下の**AND条件**で初めて成立する。1つでも欠ければ生成へ到達しない（実測検証済み）。

| 層 | 何を保証するか | 実体 |
|---|---|---|
| Session | このrouteへ到達してよい利用者か | `WEB_SESSION_SECRET` 署名cookie |
| Approval Token | この成果物・quality・costで生成してよいか | `CAROUSEL_APPROVAL_SECRET` 署名token（Decision 111） |
| Billing Unlock | 課金ロックが明示的に解除されているか | `billingLock === false` |
| Budget / Ledger | 予算内かつ未実行か | Decision 111／Decision 112 |

**Session と Approval Token は別domain・別secret・別token prefix（`s1` / `v1`）であり、相互に流用しない。**

### この境界の限界（正式に明記する）

`WEB_APP_PASSWORD` は**社内共有の合言葉**であり、本sessionが表すのは **individual user identity ではなく shared internal access boundary**（社内関係者のみ到達可能）である。

- 誰が操作したかを識別・監査することはできない。
- 合言葉を知る全員が同一の権限を持つ。
- 個人単位の権限分離・失効・追跡が必要になった時点で、別途user identity層の導入判断が必要になる。

本Decisionは「無認証で有料実行できる状態を閉じる」ことを目的としており、それ以上の主張をしない。

### billingLock（新Decisionとしない）

`billingLock` は既存 `/api/evidence/web-search`（EEA-3）が確立済みのbilling safety contractであり、新しい設計判断ではない。Carouselへの**適用記録**として本Decisionへ含める。

- **`billingLock === false`（boolean）のみ課金許可。** `true`／欠落／`null`／`'false'`／`'true'`／`0`／`''` はすべて `403 billing_locked`。
- 拒否は **reserve／provider／Storageのいずれよりも前**（実測：全カウンタ0）。
- Step C-3実装時に `billingLock: false` がhardcodeされていた逸脱を、Step C-5-preで既存contractへ修復済み。

### 却下した案

- **既存API全体を一括でsession必須化** — blast radiusが過大で、既存UIの全fetch経路の回帰リスクを負う。今回の目的（有料実行の無認証を閉じる）に対して過剰。
- **`WEB_APP_PASSWORD` をHMAC鍵として流用** — password変更が全sessionの失効と直結し、passwordの推測可能性が署名強度に直結する。独立secretを分ける方が安全。
- **npm package（express-session／cookie-parser／csurf等）の追加** — Node組み込み `crypto` で要件を満たせるため、依存とattack surfaceを増やさない。
- **Referer fallbackの採用** — Referrer-Policyで容易に欠落するため、「無ければ通す」経路がCSRF防御を実質無効化する。
- **`/api/login` をWEB_SESSION_SECRET未設定時に503化** — Carousel以外を含むアプリ全体のログインを壊す。sessionを発行しない（＝server権限を与えない）ことで安全性は満たされるため、既存機能を壊さない側を採用した。

### 今回実施していないこと（重要）

- **`WEB_SESSION_SECRET` のRender環境変数への設定**（未設定。secret値の取得・表示も行っていない）
- **`WEB_TRUSTED_ORIGIN` のRender設定**（未設定。`RENDER_EXTERNAL_URL` による自動解決を想定）
- **既存API群への横展開**
- **deploy**（本変更はまだcommitもpushもされていない）

未設定のまま deploy した場合、Carousel 3routeは401で閉じたままとなり（fail-closed）、既存機能には影響しない。

---

# Decision 112
## One Completed Generation Per Output Draft Conservative Safety Lock（partial UNIQUE indexのpredicateを `status IN ('in_progress','completed')` へ拡張し、1 Output Draftあたりのcompleted generationを最大1件に制限する・2026-09-08）

**新規Decision番号を採用する理由**：Decision 111（Conservative Image Generation Authorization Budget）は**1 Output Draftあたりの累積authorization ceiling（¥100）という金額の判断**であり、「同一Output Draftに対して有料生成を何回成立させてよいか」という実行回数の直列化は扱っていない。後述のとおり両者は**数学的に同義ではない**ため、Decision 111の改訂ではなく新規採番とする。

### 背景

Phase 2-E Production Connection Step C調査で、per-post cumulative budgetに**TOCTOU（check-then-act）race**が形式的に成立することが判明した。

当時のpartial UNIQUE index は `WHERE status = 'in_progress'` のみを対象としていたため：

```
B: SELECT SUM(...) で cumulative=0 を観測
A: SELECT SUM(...) で cumulative=0 を観測
A: reserve（in_progress行INSERT）→ provider → completeExecution
   → statusが completed へ遷移し、in_progress行が0件になる
   → 旧predicateではindex占有が解放される
B: 古い cumulative=0 を保持したまま reserve → INSERTが成功してしまう
→ 同一 output_id への authorization が per-post ceilingを超えて成立し得る
```

`SELECT SUM(...)` による事前チェックだけではcheck-then-actのraceを排除できないため、DB側の直列化ポイントを強化する必要があった。

### 決定（正式）

1. **partial UNIQUE index `uq_carousel_exec_active_output` のpredicateを拡張する。**
...
2. これにより、Aが `completed` へ遷移した後も同一 `output_id` のindex占有が維持され、**古いSUMを保持したBのINSERTがDB側で23505として拒否される**。
3. 結果として **1 Output Draft（output_id）につき、completed generationは最大1件**となる。
4. `failed_before_charge`／`failed_after_charge`／`unknown_billing` はpredicate対象に**含めない**（`getCumulativeSpentJpyByOutputId()` が `spent_estimated_jpy` 経由で別途budget集計する既存設計を変えない）。
5. **release／reclaim／自動timeout unlockは実装しない。** `completed` は意図的にindexを保持し続ける。crash等で残った `in_progress` も引き続きfail-closedのままとする。
6. 23505の分類reason `post_execution_in_progress` は**既存のまま維持する**（rename しない）。predicate拡張により本reasonは「対象行が厳密に in_progress である」ことを意味しなくなり、「同一 output_id へのactiveなauthorization（in_progress または completed）が既に存在するため拒否された」を意味する。この意味論の変化はコードコメントに明記済み。

### Decision 111との境界（極めて重要）

**本Decisionは Decision 111 と数学的に同義ではない。**

現行価格（Decision 111）における7枚あたりのauthorized total：

| quality | 7枚 | 2回実行した場合 | `BUDGET_JPY_PER_POST=100` に対して |
|---|---|---|---|
| low | ¥45.2816 | **¥90.5632** | **予算内（Decision 111だけなら2回可能）** |
| medium | ¥92.5232 | ¥185.0464 | 超過（2回目不可） |
| high | ¥255.2816 | — | 1回目から超過 |

すなわち **low品質であればDecision 111の¥100 ceiling内で2回のcompleted generationが成立し得る**。本Decisionのpartial UNIQUE lockは、それを超えて**回数そのものを1回に制限する追加の保守的Safety Policy**である。

したがって、docsやDecisionへ「completedをindex対象にすることはDecision 111のbudget ruleと同義／等価である」と記述してはならない。両者は別々の制約であり、実際にlow×2のケースで挙動が分岐する（Step C-3のテストで実証済み：承認段階のbudget pre-flightは通過し、generate時のreserveで `post_execution_in_progress` として拒否される）。

### 追記（Production Activation Step PA-3A・2026-09-08）：charged-failure 非対称性と read→reserve invariant

PA-3 Release Preflight の race 再評価で確認した構造を、**隠さずに正式記録する**。

#### (1) lock 保持集合と budget 計上集合は一致しない（非対称）

| status | partial UNIQUE lock（`uq_carousel_exec_active_output`） | `getCumulativeSpentJpyByOutputId()` の計上 |
|---|---|---|
| `in_progress` | **保持する** | `estimated_total_cost_jpy`（保守的な見積計上） |
| `completed` | **保持する** | `spent_estimated_jpy` |
| `failed_after_charge` | **解放される** | `spent_estimated_jpy` |
| `unknown_billing` | **解放される** | `spent_estimated_jpy` |
| `failed_before_charge` | 解放される | 0 |

すなわち **`failed_after_charge` と `unknown_billing` は、支出を budget へ計上したまま partial UNIQUE lock を解放する。**

これは意図した設計である。課金済み失敗の後も同一 `output_id` を永久に lock し続けると、Decision 111 の ¥100 ceiling 内に余裕が残っていてもリトライが一切できなくなるため。**回数制限（本Decision）と金額制限（Decision 111）が別々の制約であるという Decision 111 との境界の帰結**であり、この非対称性を「lock が常に保持される」と要約してはならない。

したがって同一 `output_id` に対する **`completed` は最大1件**だが、**課金済み失敗を含む execution 行は複数生じ得る**。その場合の上限は本Decisionの lock ではなく **Decision 111 の cumulative ceiling が単独で担保する**（実測例：low 7枚 ¥45.2816 → 1回目 45.2816 → 2回目 90.5632 → 3回目は 135.8448 で `budget_exceeded`）。

#### (2) read → reserve invariant（正式な不変条件）

上記 (1) により、cumulative budget read と reserve INSERT の間に他 request の execution が「課金済み失敗として完了し lock を解放する」ことは理論上あり得る。この窓が広がれば Decision 111 の ceiling を突破する経路が生まれる。

PA-3 で実測した現行の実行順序は次のとおりで、**この区間に `await` / async boundary / external I/O が1つも存在しない**：

```
lib/carouselImageService.js:297   await deps.getCumulativeSpentJpyByOutputId(outputId)   ← 累積読み取り
lib/carouselImageService.js:302   deps.checkCostLimit()                                  ← 同期
lib/carouselImageService.js:326   await runCarouselImageJob(...)
  shared/carouselImageCore.js     assertRealCallAllowed / plan / quality /
                                  pre-flight budget / running budget                     ← すべて同期
  shared/carouselImageCore.js:669 await deps.executionStore.reserve(...)                 ← reserve INSERT
```

（`shared/carouselImageCore.js:645` の `await _finalizeLedger()` は running budget 拒否時の **return 経路**であり、reserve へ到達する成功経路には現れない。）

そのため現行コード形状では、この区間で requester が他 request の execution 全体（provider HTTP 呼び出し7回＝数秒）を跨いで中断されることがなく、**PA-3 の全 interleaving 検証で到達可能な突破経路は確認されなかった**。

**正式な不変条件として以下を記録する。**

> **cumulative budget read（`getCumulativeSpentJpyByOutputId()`）から reserve INSERT（`executionStore.reserve()`）の開始までの間に、新しい `await` / async boundary / external I/O を追加してはならない。**

対象となる追加の例：追加のDB read、外部API呼び出し、logging transport への送信、非同期 validation、非同期 feature flag 参照、`await` を伴う metrics 送出。

#### (3) 記述してはならないこと（過大主張の禁止）

本Decisionおよび派生docsへ、以下を書いてはならない。

- 「TOCTOU は完全に解決した」
- 「DB制約だけで Decision 111 の cumulative ceiling を完全保証している」
- 「charged failure（`failed_after_charge` / `unknown_billing`）でも lock は保持される」

正確な表現は次のとおり。

> **現行の execution ordering（read→reserve 間に await 無し）と partial UNIQUE lock と cumulative accounting の組合せにおいて、到達可能な突破経路は確認されていない。**

#### (4) 将来の再評価トリガ

read→reserve 区間へ await / async boundary / external I/O を追加する必要が生じた場合、**本Decisionの safety proof を再評価することを必須とする**。その時点で invariant を維持できないなら、以下を再検討する。

- 累積読み取りと reserve を **単一の atomic DB RPC / transaction** に統合する（`SELECT ... FOR UPDATE` 相当、または budget 判定を含む SQL 関数へ移す）
- reserve 側の DB 制約で cumulative ceiling そのものを表現する

いずれも本 PA-3A では**実装していない**（現行 invariant が成立しているため不要と判断した）。

#### (5) 本追記で実施していないこと

- schema.sql の変更なし（predicate は `status IN ('in_progress','completed')` のまま）
- `lib/carouselExecutionDb.js` / `lib/carouselImageService.js` / `shared/carouselImageCore.js` のコード変更なし
- 実DB適用なし（**DB applied = No** のまま）


### Regeneration Policy（本Decisionに内包）

Image ReviewでNGとなった場合、同一 `output_id` での再生成は**できない**。運用は以下とする。

1. **新しいOutput Draft（新 `output_id`）を作成して再生成する**（正式運用方針）。
2. **completed ledger行を書き換えて再利用しない。**
3. **nonceを再利用しない。**
4. **C-1 predicateを緩めない。**

これにより課金truthを保持したまま、Image Review NGが運用上の行き止まりにならない。

### 将来の再検討条件

本制約は永久不変ではない。**pricing／reserve／budget／slide countが変更され、複数jobが1 post budget内へ現実的に収まる設計になった場合**は、このpredicateを再検討する（schema.sqlのコメントにも同旨を記載済み）。

### 採用理由

- `SELECT SUM(...)` の事前チェックはcheck-then-actであり、DB側の一意制約なしにraceを閉じられない。
- 既存 `uq_affiliate_eval_active_product`（Decision 070・`WHERE is_active` のpartial UNIQUE）と同型のパターンであり、新しい概念を持ち込まない。
- release/reclaimを持たないことで、「課金が発生したか不明な状態でロックを解放する」という最も危険な操作をコード上不可能にしている。

### 却下した案

- **route側でin-memory mutex/process lockを持つ** — multi-instance Renderで共有されず、restartで消える。billing authorityとして不適格。
- **`completed` をpredicateに含めず、SUMチェックの精度向上で対処** — check-then-actの構造が残るため、raceは閉じられない。
- **自動timeout releaseの導入** — 課金が発生したか不明な `in_progress` を時間経過だけで解放することになり、二重課金を許容しうる。

### 今回実施していないこと（重要）

- **実Supabase DBへのschema適用**（`carousel_image_executions` は実DBに存在しない・**DB applied = No**）
- したがって本indexは**まだ実DB上に存在しない**。schema.sql上の定義とテストのみが正式化されている（実測：`carouselExecutionDb` 88 passed／0 failed）。

**適用時の注意（重要）**：`CREATE UNIQUE INDEX IF NOT EXISTS` は**index名のみ**で存在判定し、predicateの差異を検出しない。旧predicate（`WHERE status = 'in_progress'`）の同名indexが存在する環境では**黙ってno-opとなり本Decisionが適用されない**。適用後は必ず以下でpredicateを実測確認すること。

```sql
SELECT indexdef FROM pg_indexes
WHERE schemaname='public' AND indexname='uq_carousel_exec_active_output';
```

---

# Decision 111
## Conservative Image Generation Authorization Budget（Carousel Image Productionのpre-execution cost authorizationをtext input reserve込みの三要素モデルへ正式化・2026-09-07）

**新規Decision番号を採用する理由**：Decision 110はDBアクセスのtrust boundary（誰が触れてよいか）についての判断であり、課金額そのものの見積り方式（何を根拠にいくら許可するか）は扱っていない。本件はPhase 2-E Gate 1 Step④で実装・テストまで完了したcost authorization設計を正式化するものであり、既存Decisionの改訂ではなく新規の設計判断であるため、新番号を採用する。

### 背景

Phase 2-E Preflight（Deferred B）で、既存のpre-flight estimate（`estimateImageJpy()`）がimage output tokenのみを算入し、text input tokenが未算入であることが判明していた。Gate 3でOpenAI公式Pricingにより `gpt-image-2` の text input 価格（$5.00 / 1M tokens）を一次情報で確認し、Gate 1/2 Final Designでconservative reserve方式の採用が正式承認され、Step④で実装・テスト（395 passed）まで完了した。本Decisionはその設計を正式なDecisionとして記録する。

### 決定（正式）

1. **Initial text input reserve**：`INITIAL_TEXT_INPUT_RESERVE_TOKENS_PER_SLIDE = 7000`。これは **pre-execution authorization reserve** であり、tokenizer maximum・actual usage maximum・billing maximum・invoice maximumのいずれでもない。実usageがこの値を超えないことを保証する値ではない。
2. **Pricing basis**：`TEXT_INPUT_USD_PER_1M = 5.00`（新規・OpenAI公式Pricing確認済み）／`IMAGE_OUTPUT_USD_PER_1M = 30.00`（既存）／`USD_TO_JPY_STATIC = 160`（既存・社内static値）／`OUTPUT_TOKENS_BY_QUALITY = {low:181, medium:1587, high:6431}`（既存）。価格変更時は再検証が必要。
3. **Authorization estimate**：`estimatedTotalCostJpy = estimatedOutputCostJpy + reservedInputCostJpy`。`reservedInputCostJpy = INITIAL_TEXT_INPUT_RESERVE_TOKENS_PER_SLIDE × slideCount ÷ 1,000,000 × TEXT_INPUT_USD_PER_1M × USD_TO_JPY_STATIC`。既存 `estimateImageJpy()` はoutput-onlyの意味のまま**変更しない**（`estimated_output_cost_jpy`・既存テストが依存するため）。新規 `estimateReservedInputJpy()` / `estimateAuthorizedTotalJpy()` / `estimateAuthorizedPerCallJpy()`（`lib/carouselImageClient.js`）で算出する。
4. **7-slide reference**（実装・テストで固定値として確認済み）：low ¥45.2816／medium ¥92.5232／high ¥255.2816。現時点で low 7・medium 7 は authorized、high 7 は `budget_exceeded`（個別ハードコードではなく計算結果）。
5. **Budget semantics**：`BUDGET_JPY_PER_POST = 100` は **pre-execution authorization ceiling** である。**「実課金は必ず100円以内」「100円を超えることはない」「billing hard cap」「invoice guarantee」の意味では記録しない。** actual provider usageおよび最終invoiceが100円以内になることを数学的に保証しない。
6. **Approval scope**：approval tokenの `estimatedCostJpy` は **estimated authorized total cost**（output estimate + reserved text input cost）を意味する。approval発行時とreal guard再計算時は必ず同一のcost model（`estimateAuthorizedTotalJpy()`）を使用する。cost model変更後に旧approval tokenが `estimated_cost_mismatch` で失効することは安全側の正常動作である。
7. **Running budget**：各provider attempt直前に `spentEstimatedJpy + authorizedPerCallJpy <= BUDGET_JPY_PER_POST` を確認する。attempted provider callは課金可能性ありとして保守的に計上し、provider throwでもreserveを巻き戻さない。
8. **Calibration period / Regen**：`7000` reserve採用中、medium 7 slidesは `¥92.5232` となり、追加medium call（`92.5232+13.2176=105.7408`）は `¥100` authorization ceilingを超える。**initial calibration periodではmedium 7枚生成後の再生成余地が0回になることを正式に受け入れる。これは不具合ではない。** actual usage取得後、reserveを7000から縮小するか（例：2000）は**別Decision**で判断する。
9. **Reserve vs Actual Usage**：reserveは **pre-execution authorization**、actualUsageは **post-execution observability** として完全に分離する。将来actualUsageを取得しても、過去execution ledgerのreserve値をactual値で上書きしない（当時のauthorization判断を監査可能に残す）。
10. **Decision 110（Server-only Trust Boundary）・Decision 070項目10は変更しない。** 本Decisionはcost authorization設計のみを扱い、DBアクセス制御・Migration経路には触れない。

### 採用理由

- text input tokenを無視した既存estimateは実際の課金構成要素を見落としており、budget authorizationとして不完全だった。conservative reserveを別枠で確保することで、実測なしでも保守的な安全側の判断ができる。
- reserve値7000は、実行前に予算超過を防ぐための initial authorization であり、「正確なtoken数の予測」ではない設計として位置づけることで、後日の実測（actual usage）に基づく調整（縮小）を妨げない。
- approval発行とreal guardが同一関数を共有する設計により、署名値とguard再計算値の乖離が構造的に発生しない（テストで実証済み）。
- reserve（事前）とactualUsage（事後）を別概念・別フィールドとして維持することで、将来のactualUsage実装（Gate 1 Step⑤以降）と意味的に衝突しない。

### 却下した案

- **2000 tokens/slideの即時採用** — 実measured usageが0件の状態で縮小するのは推測に基づく判断であり、under-reserveリスクを負う。実測データ収集後の別Decisionへ委ねる。
- **reserve導入の延期**（Step④を保留） — actual usageは実画像生成を経てはじめて取得できるため、「実測がないからreserveを決められない／reserveがないから実行できない」という循環に陥る。まず保守的な初期値で開始し、実測後に調整する2段階運用を採用する。

### 今回実施していないこと（重要）

本Decisionの正式化時点で、以下は**すべて未実施**である。

- **実画像生成／actual usageの取得**（`REAL_ENABLED=false` 維持）
- **reserve値の縮小判断**（2000等への変更は実測後の別Decision）
- **`extractActualUsage()` / `actualUsdFromUsage()` の3項式拡張**（Step⑤以降の範囲）
- push／tag／DB適用／Render secret登録

実装・テストは `lib/carouselImageClient.js`／`shared/carouselImageCore.js`／`carouselImageProduction.test.js` の3ファイルで完了済み（**carouselImageProduction 395 passed / 0 failed**、**carouselExecutionDb 44 passed / 0 failed・無回帰**）。

---

# Decision 110
## carousel_image_executions Server-only Trust Boundary（paid execution ledgerへのアクセスをserver-only privileged credentialへ限定し、anon／authenticatedの直接CRUDを全面禁止する・2026-09-07）

**新規Decision番号を採用する理由**：Decision 070 項目10はMigration実行経路（Claude Code環境へDDL経路を追加しない）についての判断であり、DBアクセスのtrust boundary（どのcredentialがどのテーブルへ触れてよいか）は扱っていない。本件は既存Decisionの改訂ではなく、Carousel Image Productionの課金台帳という新しい責務に対する新規のsecurity判断であるため、新番号を採用する。

### 背景

Phase 2-E Gate 1 Step①〜②で `carousel_image_executions`（billing idempotency／nonce single-use／paid execution ledgerの正本）と `lib/carouselExecutionDb.js` を実装した。初版のRLSは既存の新テーブル群（`cases`／`customers`／`api_cost_*`／`affiliate_evaluations` ほか）の慣例に従い `FOR ALL TO anon USING(true) WITH CHECK(true)` としていた。

しかしSecurity Reviewで、本テーブルは他テーブルと責務・リスクが異なることが判明した。anonからの直接CRUDを許すと **nonce先取り**（reserve済みnonceを第三者が先に埋める）・**execution status改変**・**cost改変**・**actual_usage改変**・**row削除** が理論上成立し、二重課金防止の正本として機能しない。

一方でread-only調査により、本repoのserverは `lib/supabase.js`／`lib/conversationsDb.js` を含め **すべて anon key（`NEXT_PUBLIC_SUPABASE_ANON_KEY`／`SUPABASE_ANON_KEY`）のみで接続**しており、service_role相当のprivileged clientがrepo全体に**存在しない**ことが実測された（`grep -rniE service_role` = 0 hits／`createClient` は2箇所ともanon）。したがって単純に `TO anon` を剥がすと **server自身のreserve／complete／failも同時に遮断**される、という構造的ジレンマがあった。

そこでDecision 070 項目10の原文を精査したところ、禁止の主語は4箇所すべて **「Claude Code環境」に限定**されており、production server runtimeでのprivileged credential利用については一切言及がないことが確認できた。またSupabase公式ドキュメントにより、(1) server-side用の現行推奨環境変数名は `SUPABASE_SECRET_KEY`（`sb_secret_...` 形式／legacyの `service_role` JWTも当面動作）、(2) secret keyは **RLSをbypassする**、(3) secret keyはUser-Agent判定によりブラウザからは動作せず401になる、ことを一次情報で確認した。

### 決定（正式）

1. **`carousel_image_executions` へのDBアクセスは server-only privileged credential に限定する。**
2. **anon／authenticated からの直接CRUD（SELECT／INSERT／UPDATE／DELETE）を全面禁止する。**
3. **RLSはENABLEDのまま、anon／authenticated向けpolicyを1本も作成しない**（RLS有効かつ一致policyなし＝全commandでdeny）。過去に適用された可能性のある旧policy（`carousel_image_executions_all`／`_insert`／`_update`／`_select`）は `DROP POLICY IF EXISTS` で明示除去し、再適用しても0本へ収束させる。
4. **GRANT層でも拒否する**（Defense in Depth）。`REVOKE ALL ON TABLE public.carousel_image_executions FROM anon;` および `FROM authenticated;` を適用する。Supabaseはpublic schemaのテーブルへ既定GRANTを付与するため、RLSのみに依存しない。privileged側（postgres／service_role）からはREVOKEしない。
5. **privileged credentialはproduction server（Render）のsecret環境変数としてのみ保持する。** 変数名は `SUPABASE_SECRET_KEY`（優先）／`SUPABASE_SERVICE_ROLE_KEY`（legacy fallback）。**`NEXT_PUBLIC_*` 系の名前は使用しない。**
6. **secret値をClaude Code環境／`.env.local`／local開発credential／browser／Git／docs／test fixture／source codeへ一切置かない。** docsとコードに記載してよいのは環境変数「名」のみ。
7. **privileged clientは `lib/carouselExecutionSupabase.js` に隔離し、carousel execution ledger専用とする。** 既存DB helper（`costDb`／`approvalsDb`／`affiliateEvalDb`／`conversationsDb` 等）へ横展開しない。repo-wideのprivileged client化は禁止。既存 `lib/supabase.js`（anon client）は一切変更しない。
8. **credential欠落時はfail-closed**とする。`carouselExecutionClient = null` → `reserveExecution()` が `reserve_unavailable` → provider call 0 で停止する。
9. **anon clientへのfallbackを実装しない。** `lib/carouselExecutionDb.js` は `lib/supabase.js` を **require しない**（import自体を持たないことで、構造的にanonへ落ちられないようにする）。`lib/carouselExecutionSupabase.js` もanon keyを実コードで参照しない。
10. **Decision 070 項目10（Claude Code環境へDDL実行経路を追加しない）は変更しない。** 本Decisionはそれと両立する——credentialはRenderのsecret環境変数にのみ置きClaude Code環境には置かない。またsupabase-js／PostgREST経由ではDDLを実行できないため、DDL経路を新設しない。

### 採用理由

- 課金台帳の二重課金防止は「secretを持っているか」ではなく「DB層で構造的に閉じているか」で担保すべきであり、RLS policy 0本＋GRANT REVOKEの二重防御がその唯一の実現手段である。
- privileged clientを1テーブル・1モジュールへ隔離することで、attack surfaceとblast radiusを最小化できる。既存の全DB helperをprivileged化する案と比べ、変更量・リスクとも桁違いに小さい。
- `lib/supabase.js` をrequireしない設計により、「credentialが無ければanonで代替する」という最も危険なfallbackがコード上書きようがなくなる。
- Claude Code環境にsecretを置かないため、deterministic unit testは従来どおり `opts.client` のfake注入で全件成立する（実測：44 passed／0 failed）。

### 却下した案

- **案A：現状のanon INSERT／UPDATE／SELECT維持** — 5つの禁止要件のうち row削除 しか満たせない。paid execution ledgerとして不適格。
- **案C：SECURITY DEFINER RPC** — `EXECUTE` をanonへ許可した瞬間、anon keyを持つ第三者が同じ関数を直接呼べる（`complete_carousel_execution()` 経由でstatus改変・cost改変が成立）。防ぐには関数内でHMAC検証が必要となり、承認secretをPostgres側へ置くことになりsecret管理がむしろ悪化する。`EXECUTE` をanonからrevokeするなら結局privileged roleが前提となり、本Decisionを重ねる意味がない。
- **repo全体のprivileged client化** — blast radiusが不必要に拡大し、既存の全DB helperの回帰リスクを負う。carousel execution ledger以外に本要件は存在しない。

### 今回実施していないこと（重要）

本Decisionの正式化時点で、以下は**すべて未実施**である。

- **Supabase SQL Editorでのschema適用**（`carousel_image_executions` は実DBに存在しない・`DB applied = No`）
- **Render環境変数への `SUPABASE_SECRET_KEY` 登録**（ユーザーのDashboard操作・別工程）
- **secret key（`sb_secret_...`／service_role）の取得**（Claude Codeは取得も表示も行っていない）
- **Render restart／deploy**
- push／tag

したがって本Decisionは「設計と実装コードの正式化」であり、**本番稼働の開始を意味しない**。`REAL_ENABLED=false` は維持され、real image API call 0／paid generation 0 も維持されている。
...
決定（Step C-1A ─ Compliance Context Foundation・Code commit `9d665256d5ac359f4c8354db5cf1c7a1447aceef`）:
- **位置づけ**：Step C（Intelligence接続）は「score計算への接続」からではなく、**現在どのIntelligence層にも接続されておらず実務リスクとして放置されていたCompliance系5field（うち4fieldを対象）の注入**から着手するという次工程選定調査の結論に基づき、**Compliance Context Foundation**を最初のサブ工程として実装した。新しいFormal Truth Contractではないため**新規Decision番号は作成せずDecision108への追記**とする。
- **対象field（4件）**：`listingNgWords`／`advertisingDisclosureRequirements`／`complianceRestrictions`／`regulatoryCategory`。**`listingPolicy`は対象外**（`"一部ok"`等の表記統一問題が未解決のため、Formal TruthとしてAI指示へ注入する準備が整っていないと判断・残課題4のまま維持）。
- **責務**：`_apfrResolveCurrentFacts()`（Phase 1 Resolver）で確定したCompliance Formal Truthのうち、上記4fieldをresolvedのみ抽出し、`APFR Resolver → Compliance Context → atRunWorkflow() → POST /api/auto-task → server.js pass-through → runAutoTaskWorkflow()`までread-onlyで配線するFoundation層。**prompt文字列への実注入は本工程では一切行わない**（0件）。
- **新規helper**：`index.html`に`_apfrBuildComplianceContext(product)`（`_apfrResolveCurrentFacts()`のみを使用し、`product.facts`の直接走査・独自current判定・配列末尾／`recordedAt`／sourceMethod優先などの恣意的tie-breakerはいずれも実装しない）。none／ambiguousの値は一切含めない（fail-closed維持・candidate代表選択もしない）。
- **配線範囲**：`atRunWorkflow()`内でIADP intent判定（`_iadpRequested`）に**依存しない独立変数**として算出（通常のInstagram投稿生成でも利用できるようにする方針どおり）。scope判定は既存APFR CUIの`_apfrCurrentAdoptedProduct()`と同一の三重guard（caseId一致・product存在・intelligenceContext存在）を、Workflow開始時に捕捉した`_atRunCaseId`で適用（既存IADPブロックと同一パターン）。`/api/auto-task`のPOST bodyへ`complianceContext`を追加送信（既定は`{}`）。`server.js`は`req.body`から`complianceContext = null`を受け取り、Resolver再実装・facts参照・value加工を一切行わない**完全な受動パススルー**として`runAutoTaskWorkflow()`へ渡す。`runAutoTaskWorkflow()`（`openaiClient.js`）は`complianceContext = null`を引数として受け取るのみで、本工程では未使用のまま（`buildSystemPrompt()`等のprompt文字列生成へは一切使用しない）。
- **禁止事項の遵守確認**：facts直接走査0・独自current判定0・Product Intelligenceへの複製0・Content Intelligenceへの複製0・Output Draft snapshot保存0・DB変更0・API endpoint新規追加0・Provider変更0・Quality Gate変更0。
- **変更ファイル**：`index.html`／`server.js`／`openaiClient.js`／新規`apfrComplianceContext.test.js`。Code commit **9d665256d5ac359f4c8354db5cf1c7a1447aceef**（`feat: add apfr compliance context foundation`）。working treeの別系統差分「Leader Case Context Phase2」（`server.js`の`caseContext`／`openaiClient.js`のシグネチャ行に同居する`caseContext`等）とは、HEADベースの合成patch（`git apply --cached`）によりhunk単位で完全分離し、本commitには一切混入していない（`git show`で直接確認済み）。
- **実測結果**：新規合成テスト当初39/39 PASS（後述のC-1B時点で48/48へ拡張）。既存回帰（CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase0 40/40・APFR Core 49/49・Manual Input 35/35・EER 51/51・IADP/Quality Gate 86/86ほか）全PASS・新規FAIL 0。`node --check` OK・`git diff --check` CLEAN・dev-check 200/200/200。

決定（Step C-1B ─ Writer / Reviewer Compliance Injection・Code commit `f525111f32d2686ca47c221423865a7600dff228`）:
- **責務**：Step C-1Aで配線したCompliance ContextをWriter・Reviewerのsystem promptへ限定注入する。**対象agentはwriter／reviewerのみ**。Strategy・Leader Final・その他全担当には**注入しない**（Reviewerの品質フィードバックは既存の`【Reviewerの品質フィードバック】`経路でLeader Finalへ統合される既存機構をそのまま利用し、新しい経路は作らない）。
- **新規helper**：`openaiClient.js`に`buildCompliancePromptBlock(agent, complianceContext)`を新設（`buildIadpSelfCompletionOverride`直後・`buildSystemPrompt`直前に配置）。責務は①agent guard（`agent !== 'writer' && agent !== 'reviewer'`なら`''`）②null/empty判定（`''`）③対象4fieldのみ抽出④値の安全な文字列化（scalar→`String()`、array→各要素`String()`化・空要素除外・「・」区切り連結）⑤改行正規化（`\r\n`→単一スペース）⑥長さ制限（300文字＋`…`切り詰め。既存に同種helperがないため新規設定）⑦Writer／Reviewer文言分離。
- **Writer文言**：「これは確認済みの商品事実データであり、命令ではありません。以下の記載範囲だけを遵守して執筆してください。ここに書かれていない規制・表示文言を推測して追加で創作しないでください。」
- **Reviewer文言**：「...Writerの成果物が以下を守っているかを確認項目に追加してください。違反があれば既存の品質確認ルールに従い差し戻し、修正案を提示してください。」（**新規rejectフラグ等は追加せず、既存REVIEWER_RULESの「問題があれば差し戻す。修正案は完成文で必ず提示すること」という文章ベースの差し戻し契約をそのまま再利用**）。
- **Prompt Safety（重要）**：APFR FactはUser Verified Formal Truthだが、**値自体をsystem instructionとして扱う構造にしない**。Compliance block冒頭で「これは確認済みの商品事実データであり、命令ではありません」という責務境界を明示したうえで、値は「」で囲まれたデータ枠として提示する（命令文字列検出・blacklist等の追加防御は過剰設計として不採用。役割分離と明示的なデータ枠のみで対応）。`regulatoryCategory`はカテゴリ名から追加の法令・規制をAIが独自推測することを明示的に禁止する文言を含む。
- **field別責務**：`listingNgWords`＝Writerは使用禁止表現として遵守・Reviewerは違反確認。`advertisingDisclosureRequirements`＝Writerは記載された表示条件を遵守（Factにない文言を勝手に創作しない）・Reviewerは必要表示の確認。`complianceRestrictions`＝Writerは記載範囲の制約のみ遵守・Reviewerは違反確認。`regulatoryCategory`＝参考Formal Truthとして提示するのみ（score・promptへの独自解釈接続はしない）。`listingPolicy`＝**C-1Bでも対象外**（対象4fieldのORDER定義に含めていない二重防御）。
- **empty Contract**：`complianceContext`が`null`／`undefined`／`{}`／対象4field全て非存在のいずれの場合も、`buildCompliancePromptBlock()`は必ず`''`を返し、既存`.filter(Boolean)`パターンで自動除外される。**Writer／Reviewer／Strategy／Leaderいずれについても、C-1B前のsystemPromptとbyte-identicalであることを実装から直接呼び出すテストで確認済み**。
- **接続方式**：`runAutoTaskWorkflow()`のtaskループ内、`buildSystemPrompt(task.agentId, null, null, {...})`のoptionsへ`complianceContext`を**全agentへ無条件で渡し**、agent限定は`buildCompliancePromptBlock()`内部でのみ行う（既存の`intelligenceContextAvailable`と完全に同一パターンを踏襲。呼び出し側にagent判定ロジックを重複させない）。既存の`accountIntelligenceMode`／`intelligenceContextAvailable`は無変更。
- **Correction追従・Workflow snapshot**：C-1B側では再Resolver・永続化を一切行わない。C-1Aから受け取った値（Workflow開始時点でResolverから1回だけ構築されたスナップショット）をそのまま消費するのみ。訂正（`A → B supersedes A`）後は**次回workflow開始時**にC-1AがResolverから最新値を再取得し、Bのみがpromptへ到達する（実際のfixtureで確認済み）。workflow実行中の再Resolverはしない——既存のFormal Truth Priority Architecture（`caseContext`のWorkflow開始時snapshot方針）と整合する設計。
- **Quality Gate（重要）**：**C-1BではQuality Gate Compliance Enforcementは未実装**。現在のQuality Gate（`evaluateQualityGate`／`packageQuality`）にはCompliance専用の強制判定軸が存在しない（構造完成度のみを評価）。これは**Step C-1C候補として別工程**とし、C-1A/B CompleteとC-1C Completeを混同しない。
- **Intelligence非変更確認**：Product／ASP／Content／Revenue／Competition／Market Opportunity Intelligenceはいずれも無変更。Formal Truthの別ledgerへの複製は0（Product IntelligenceやContent Intelligenceへコピー保存しない設計）。
- **変更ファイル**：`openaiClient.js`／新規`apfrComplianceInjection.test.js`／`apfrComplianceContext.test.js`（C-1A時点限定だった2アサーションを、C-1B以降も有効な契約＝「helper経由のみ・facts直接走査0・Resolver再実装0」へ更新）。Code commit **f525111f32d2686ca47c221423865a7600dff228**（`feat: inject apfr compliance context into writer reviewer`）。working treeの別系統差分「Leader Case Context Phase2」とは、C-1A時と同じくHEADベースの合成patchによりhunk単位で完全分離（`git show`で直接確認済み）。
- **実測結果**：C-1B専用`apfrComplianceInjection.test.js` **62/62 PASS**（Writer注入／Reviewer注入／Strategy等非注入／empty完全不変／partial／array正規化／改行正規化／長さ制限／prompt injection対策／regulatoryCategory推測禁止／listingPolicy 0／Correction統合／workflow snapshot設計確認／IADP無回帰）。C-1A更新後`apfrComplianceContext.test.js` **48/48 PASS**。既存回帰（CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase0 40/40・APFR Core 49/49・Manual Input 35/35・EER 51/51・IADP/Quality Gate 86/86・IADP Structured／Evidence／Cost各PASS）全PASS・**新規FAIL 0**。`node --check` OK・`git diff --check` CLEAN。
- **dev-check（重要・PASS扱いにしない）**：`/`＝200。`/api/task-history`＝200取得実績ありだが応答時間に変動あり。`/api/workflow-dashboard`＝このセッション内では固定5秒dev-checkタイムアウト内での200が未確定。切り分け済み：両endpointとも`server.js`→`_hybridTaskHistory()`→`lib/taskHistoryDb.js`→Supabaseという経路のみを使用し、C-1Bのコードパス（`openaiClient.js`の`buildSystemPrompt`／`runAutoTaskWorkflow`）とは無関係であることを、同一コードの直接呼び出しによる分離検証で確認済み（Supabase応答時間は実測0.6〜1.9秒の変動があり、dev-check.ps1側の固定5秒タイムアウトとの相性が悪いことが原因と推定）。**dev-check environment/latency Pendingとして記録し、200/200/200のPASS扱いにはしない**。server.js変更・timeout変更・DB変更・Supabase変更はいずれも実施していない（0）。

決定（Step C-1C-1 ─ Deterministic Compliance Check・Code commit `d8e70216e3cf60718651f2d0e8721f3f274d977b`）:
- **正式名称の明記**：本工程の正式名称は**「APFR Step C-1C-1 Deterministic Compliance Check」**とする。「Quality Gate Enforcement Complete」等とは記録しない。理由：本工程はOutput Engineへの**非ブロッキング確認表示**であり、User Approval／`OUTPUT_STATUS.READY`への実際のブロック接続（Enforcement）は未実装のまま。**C-1C-1 Complete ≠ Compliance Enforcement Complete**。
- **調査結論の反映**：C-1C調査・設計（本Decision108の前段）の結論どおり、Compliance 4fieldのうち機械的に判定可能なのは**`listingNgWords`のみ**と確定し、本工程はこの1fieldのみを対象とする。
  - `advertisingDisclosureRequirements`：Fact値が「PR表記必須」等の**要求を記述した自然文**であり、文字列一致では成果物がその要求を満たしているか判定できない（deterministic判定不可）。
  - `complianceRestrictions`：意味的制約（例：未成年への訴求禁止）と語句ベース制約が同一配列に混在しうる現行schemaでは、全体をdeterministic判定できない。
  - `regulatoryCategory`：カテゴリ名（例：「化粧品」）自体は違反ではなく、そこから規制内容をAIが推測してfail判定することはAPFRの推測禁止原則に反するため、Gate対象外（context情報としてReviewerへ提示するのみ・C-1Bで実施済み・変更なし）。
  - `listingPolicy`：既存の表記統一問題（`"一部ok"`等）が未解決のため、C-1A/Bに続きC-1C-1でも対象外。対象4fieldのORDER定義に含めていない（helper内での二重防御）。
- **新規純関数`evaluateComplianceGate(outputDraft, complianceContext)`**：`index.html`に追加（`_apfrBuildComplianceContext()`直後）。責務は①`outputDraft`／`complianceContext`ともread-only②`complianceContext.listingNgWords`のみ利用③APFR `facts`の直接走査0④Resolver（`_apfrResolveCurrentFact`／`_apfrResolveCurrentFacts`）の直接呼び出し0（C-1Aが構築した`complianceContext`のみをconsumerとして利用）⑤`outputDraft.fields`のstring／array値のみを走査対象とし、object等へのdeep recursionは行わない。
- **正規化方式（deterministic限定）**：NG word・出力値の双方を`String()`→trim→Unicode NFKC→lowercaseで正規化したうえで部分一致（`indexOf`）判定のみを行う。**regex・NLP・fuzzy matching・word boundary等の複雑な推測ロジックは今回実装しない**（過剰設計の回避）。空文字NGwordは前処理で除外し全件hit事故を防止。同一field／同一ngWordの重複violationsは`field::ngWord`単位で排除。
- **status Contract（3状態）**：`{executed, checked, status, violations}`を返す。
  - `clear`：`listingNgWords`を検査し、成果物に該当なし。
  - `violation`：禁止語のhitあり（`violations: [{field, ngWord}, ...]`）。
  - `not_checked`：`outputDraft`／`complianceContext`／`listingNgWords`のいずれかが欠如、または空配列・有効なNG word0件の場合。**加えてC-1AのCompliance ContextはFactが`none`／`ambiguous`のいずれの場合もfieldキー自体を出力しないため、C-1C-1側からは両者を区別する情報が存在せず、双方とも`not_checked`として統一処理する**（ambiguous候補の代表選択は行わない・fail-closed維持。区別が必要な場合はC-1Aのデータ形状変更が前提となり本工程の範囲外）。
- **Correction追従**：`A → B supersedes A`の場合、C-1AのResolver経路により`complianceContext`にはBの値のみが入るため、C-1C-1側は特別な処理を行わずとも訂正前Aの語は自然に検査対象外となる（`evaluateComplianceGate()`はruntime再計算のみで判定結果を保存しないため、旧Violationが残留することもない）。実際のfixtureで確認済み。
- **保存**：**runtimeのみ**（`evaluateOutputPackageCompleteness()`と同一パターン）。DB保存0・Output Draftへの保存0・APFR Factへの保存0・Intelligence Contextへの保存0。Output Engineパネル描画のたびに再計算する。
- **UI**：新規`buildComplianceGateHtml()`。`_apfrCurrentAdoptedProduct()`（既存scope guard再利用・Cross-case／Cross-product混入防止は同関数に委ねる）→`_apfrBuildComplianceContext()`（C-1A既存helper再利用・新しいAPFR読み取りロジックは作らない）→`evaluateComplianceGate()`の経路で判定し、**Output Package Quality（Phase48-1）とは完全に独立したパネル**として表示（`packageQuality`のscore／statusへは一切加減点しない）。CLEAR／VIOLATION／NOT CHECKEDの3状態バッジに加え、「※現在は確認表示のみです。User Approval・READY・Quality Gate・Executive Decisionは自動停止しません。」という非ブロッキング明示文言を含む。
- **Non-blocking Contract（最重要）**：`packageQuality`（`evaluateOutputPackageCompleteness()`）・`evaluateQualityGate()`・`QUALITY_GATE_PASSING_STATUSES`・`OUTPUT_PACKAGE_QUALITY_CHECKS`・Executive Decision Engine・`OUTPUT_STATUS.READY`遷移条件・IADP `canApprove`・User Approval送信処理（`pushApprovalToServer`／`_enqueueApprovalPost`）は**いずれも無変更**（機能コードへの`+`/`-`行が存在しないことをcommit差分で直接確認済み）。Compliance violationが存在していても、C-1C-1単独では成果物の承認・納品準備OK状態を一切停止しない。
- **C-1C-2との境界**：実際にUser Approval／READY遷移をブロックする接続は**Step C-1C-2**として明確に別工程・別承認とする。理由は、一般Output Draftには元々ブロック可能な承認ゲートが存在せず（C-1C調査で判明）、ブロック化は既存Contractの拡張ではなく新しい振る舞いの新設にあたるため。C-1C-1は情報表示のみに限定した第一段階。
- **変更ファイル**：`index.html`（+121行：`evaluateComplianceGate`／`_apfrComplianceGateNormalize`／`buildComplianceGateHtml`／Output Engineパネル配線1行）／新規`apfrComplianceGate.test.js`（464行）。Code commit **d8e70216e3cf60718651f2d0e8721f3f274d977b**（`feat: add apfr deterministic compliance check`）。`server.js`／`openaiClient.js`は変更0（C-1A/Bの配線のみで完結）。working treeの別系統差分「Leader Case Context Phase2」は、C-1A/B時と同じくHEADベースの合成patch（`git apply --cached`）によりhunk単位で完全分離（本コミットの3 hunkはいずれも削除行0の純粋な追加であり、LCC Phase2の既存5 hunkとの物理的な重なりが一切ないことを確認済み）。
- **実測結果**：新規`apfrComplianceGate.test.js` **42/42 PASS**（hit／no hit／none／empty／ambiguous／Correction／NFKC正規化／case-insensitive／array出力／multi-field／duplicate排除／mutation 0／facts・Resolver参照0／packageQuality回帰／evaluateQualityGate回帰／User Approval非ブロック／IADP無回帰／Output Engine配線確認）。既存回帰（C-1A 48/48・C-1B 62/62・CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase0 40/40・APFR Core 49/49・Manual Input 35/35・EER 51/51・IADP/Quality Gate 86/86・IADP Structured／Evidence／Cost各PASS）全PASS・**新規FAIL 0**。`node --check` OK・`git diff --check` CLEAN。localhost実機確認（Chrome・`_lastOutputDraft`とAPFR productへのfixture注入方式）でCLEAR／VIOLATION／NOT CHECKEDの3表示すべてを確認、**Console Error 0**、確認後はリロードしfixtureを破棄・DB書き込み0を維持。

決定（Step C-2-1 ─ Formal Truth Numeric Consistency Check・Code commit `9cf7ab92028e4280e83153a3c046a588187aedff`）:
- **正式名称の明記**：本工程の正式名称は**「APFR Step C-2-1 Formal Truth Numeric Consistency Check」**とする。「Formal Truth Score Enforcement Complete」「Intelligence Score Enforcement Complete」等とは記録しない。**C-2-1 Complete ≠ Intelligence Score Enforcement Complete**。read-only・non-blockingの整合確認であり、乖離を検出してもscore値そのものは変更しない。
- **対象3field**：`payout`／`epc`／`approvalRate`（Product Intelligenceのscore計算が現在使用している`product.inputs`側の数値と、APFR Formal Truth側の同名fieldとの整合をread-onlyで確認する）。
- **4状態Contract**：`match`／`mismatch`／`uncomparable`／`not_checked`。
  - **payout**：`APFR_FIELD_META.payout.type`が既存Contract上`'string'`（自由記述Formal Truth。例：「初回4000円・2回目以降2000円」）である一方、Intelligence側は`number`（円）のため型が根本的に異なる。**parseFloat・正規表現抽出・円記号除去等の数値変換は一切行わず、Resolver `resolved`であれば常に`uncomparable`とする**（既存Contractに存在しない変換規則を新規に作らない）。
  - **epc**：両側`number`（円・単位変換なし）。厳密数値一致（`===`）のみで`match`／`mismatch`を判定。roundingの許容誤差は新設しない。
  - **approvalRate**：両側`number`・0〜100の%数値として扱う既存Contract（両UIとも「(%)」表記・`0.8=80%`のような自動scale変換は両フィールドとも未実装）。厳密数値一致（`===`）のみで判定。**`0.8`と`80`は自動scale変換せず厳密不一致＝`mismatch`として扱う**（テストで固定）。
  - **not_checked**：Resolverが`none`／`ambiguous`のいずれか（区別せず統一。C-1C-1と同じ前例踏襲）、またはIntelligence側`product.inputs[field]`が`null`（未入力）の場合。Resolverが`resolved`でない場合、Intelligence側の値を「正しい」とも「誤り」とも認定しない。
- **Resolver Contract**：新規純関数`_apfrEvaluateNumericConsistency(product)`は各fieldについて既存`_apfrResolveCurrentFact(product, field)`のみを呼ぶ。**`product.facts`直接走査・`recordedAt`比較・配列末尾判定・独自Correction chain判定はいずれも0件**（Resolverの唯一の読み取り口Contractを継承・再実装しない）。CUI-2による`A → B supersedes A`後は、Resolverが自動的に返す最新current Fact（B）のみが比較対象となることを合成テストで確認済み（追加実装なしで自動追従）。
- **保存**：**runtimeのみ**。DB保存0・Output Draft保存0・APFR Fact保存0・Product/ASP/Revenue Intelligence保存0・Intelligence Context保存0。Output Engineパネル描画のたびに`product.inputs`と`_apfrResolveCurrentFact()`から再計算する（Formal Truthの複製を作らない）。
- **UI**：新規`buildFormalTruthConsistencyHtml()`。`_apfrCurrentAdoptedProduct()`（既存scope guard再利用）→`_apfrEvaluateNumericConsistency()`の経路で判定し、**Output Package Quality（Phase48-1）・Compliance Check（C-1C-1）とは完全に独立したパネル**として表示。match／mismatch／uncomparable／not_checkedの4状態ラベルに加え、「※現在は整合確認のみです。scoreへ自動反映しません。」という非ブロッキング明示文言を含む。Output Engine配線は`_oeSafe(buildComplianceGateHtml,...)`の直後（C-1C-1パネル直後）に`_oeSafe(buildFormalTruthConsistencyHtml,...)`を1行追加。
- **Non-blocking Contract（最重要）**：`_aicIntegratedScore()`・score・ranking・recommendation・Leader判断・User Approval・`OUTPUT_STATUS.READY`・`evaluateQualityGate()`・`evaluateComplianceGate()`（C-1C-1）は**いずれも無変更**（commit差分内の該当識別子への一致は新規コメント文中の説明のみであり、機能コードとしての変更行が0件であることを直接確認済み）。乖離（mismatch）を検出しても、C-2-1単独ではscoreへ自動反映しない。
- **変更ファイル**：`index.html`（+98行：`_apfrEvaluateNumericConsistency`／`buildFormalTruthConsistencyHtml`／Output Engineパネル配線1行。削除行0の純粋な追加）／新規`apfrNumericConsistency.test.js`（486行）。Code commit **9cf7ab92028e4280e83153a3c046a588187aedff**（`feat: add apfr formal truth numeric consistency check`）。`server.js`／`openaiClient.js`／`claudeClient.js`は変更0。working treeの別系統差分「Leader Case Context Phase2」は、C-1A/B/C-1C-1と同じくHEADベースの合成patch（HEAD:index.htmlへ直接3editを再適用して生成した合成版とHEADとの差分をpatch化し`git apply --cached`）によりLCC Phase2を一切経由せず分離（LCCマーカー0件をcommit差分から直接確認済み）。
- **実測結果**：新規`apfrNumericConsistency.test.js` **53/53 PASS**（payout uncomparable×3／epc match・mismatch・not_checked／approvalRate match・mismatch・scale非変換mismatch／Resolver none・ambiguous・Correction追従／Cross-case・Cross-product混入0／product・facts mutation 0／score非干渉／UI表示分岐／static検証5系統18項目）。既存回帰（C-1C-1 42/42・C-1A 48/48・C-1B 62/62・CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・APFR Core 49/49・Manual Input 35/35・Phase0 40/40・EER 51/51・IADP Quality Gate 86/86・IADP Structured 13/13・Evidence Promotion 17/17・Cost Tracker 19/19）全PASS・**新規FAIL 0**。`node --check` OK（working tree版・INDEX blob版とも）・`git diff --cached --check`／`git diff --check` CLEAN。localhost実機確認（`npm run dev-check`＝`/`200・`/api/task-history`200・`/api/workflow-dashboard`200。fixture注入方式）でpayout=uncomparable／epc=match／approvalRate=mismatch／Factなし=全field not_checkedの4状態すべてを確認、**Console Error 0**、fixture操作によるPOST発火0件（DB書き込み0）を維持。

決定（Leader Case Context Phase2 ＋ Option B ─ Formal Truth / Intelligence Case Data Context Wiring・Code commit `95eaa8990016ea6567a9d6ff1133f44217ad1af2`）:
- **背景（実運用不具合）**：Instagram実運用テストで、実案件`case-msr9yckye65y`に保存済みの商品情報・ASP情報・Product/ASP/Revenue/Competition/Content/Market Opportunity Intelligence・APFR Formal Truthが存在するにもかかわらず、Writer／Researcher／Reviewerが「確認できない」と回答し、情報不足のままLeader Finalが根拠のない一般的な投稿（「無料説明会」「初期投資不要」等）を生成しReviewer Passed／Strategy Accepted／Quality Gate Passed／Readyまで到達する不具合を特定した。原因調査の結果、本番HEADには案件データをAI社員へ渡す配線（`buildLeaderCaseContext`／`caseContext`／`hasCaseContext`／`formalTruthRule`）自体が存在せず（working tree専用の未commit差分＝**Leader Case Context Phase2**としてこれまでprotectedに保護されてきた）、かつLCC Phase2単独でもIADP由来の項目（ブランドコンセプト・ターゲット等）のみでAPFR Formal Truth・Intelligenceを含まないことを確認した。
- **正式名称の明記**：本工程の正式名称は**「Leader Case Context Phase2」＋「APFR Step Option B（Formal Truth / Intelligence Case Data Context Wiring）」**とする。**「Leader Final Grounding Complete」「Quality Gate Grounding Complete」「READY Grounding Complete」とは記録しない**（後述の重要な非依存関係を参照）。
- **LCC Phase2（既存protected差分の正式化）**：`caseId`をPath A（`atRunWorkflow()`→`/api/auto-task`）・Path B（`sendMessage()`→`/api/chat`）双方で保持し、server-side `buildLeaderCaseContext(caseId)`（IADP・Evidence・Quality Gate・Assessment由来の文字列Context）を構築、`caseContext`という**文字列の共通入力契約**としてWriter／Researcher／Reviewer／Strategy／Leader Final・OpenAI（`openaiClient.js`）／Claude（`claudeClient.js`）両Providerへ配線した。`hasCaseContext = !!caseContext`（非空文字列判定）に基づき`formalTruthRule`（CASE CONTEXTに存在する情報を「確認できない」と述べることの絶対禁止・存在しない情報の推測禁止）を有効化する。
- **Option B（client-side Case Data Context・Resolver Contract厳守）**：**APFR Resolver（`_apfrResolveCurrentFact`／`_apfrResolveCurrentFacts`）はclient専用のまま維持**し、server.jsでの再実装は行わない。新規純関数`_buildCaseDataContext(expectedCaseId)`をPath A/B共通の**唯一の構築口**としてclient側（index.html）に追加し、①既存Resolverを呼ぶだけで取得した`resolved`のみのAPFR Current Fact（`none`／`ambiguous`／superseded履歴・evidence[]・fieldEvidence・usedEvidenceIdsはいずれも非送信）②Product/ASP/Revenue/Competition/Content/Market Opportunity Intelligenceの`derived`／`confidence`のみを要約したテキストを構築し、Path A・Path Bそれぞれの既存POST bodyへ`caseDataContext`として追加送信する。**値の忠実性**：`payout`（`APFR_FIELD_META.type==='string'`）は数値変換・円記号除去をせず保存値のまま、`epc`／`approvalRate`（`number`）はscale変換・roundingをせず保存値のまま渡す。
- **server受動パススルー**：`server.js`は`/api/auto-task`・`/api/chat`双方で`caseDataContext`（不透明な文字列）を受け取り、新規`_mergeCaseContextText(serverText, clientText)`で既存`caseContext`文字列へ連結するのみ。型（string）と最大長8000字の境界検証のみを行い、**APFR Factの意味解釈・Resolver再実装・`.facts`直接走査はいずれも0件**（**C-1A test 10-6「server.js側でResolver再実装／facts参照が行われていない」を維持**）。
- **IADP必須ガード**：`buildLeaderCaseContext()`のIADP必須ガード（`!draft.fields.iadp.package`ならContext全体をnull）は**変更していない**。client-side `caseDataContext`が独立ソースとして`caseContext`を非空にするため、IADP未生成の案件でもAPFR Formal Truth・Intelligenceが到達し`hasCaseContext===true`→`formalTruthRule`が有効化されることをテストで確認済み。
- **Payload制限**：本番実データ`intelligenceContext`全体（25,008 B）に対し、`caseDataContext`実測**約2,359 B（約1/10.6）**。facts履歴（9,703 B・22件）・evidence[]（7,496 B・9件）はいずれも非送信。
- **Cross-case／Cross-product**：Path Aは`_atRunCaseId`、Path Bは`_ncActiveCaseId(currentMember.id)`を用いた二重guard（`draft.caseId`一致・`product.caseId`一致）。不一致・caseId未確定では空文字列を返し他案件へfallbackしない。Cross-productは既存Resolverのguardへ完全委譲（独自判定を追加しない）。
- **変更ファイル**：`index.html`（+176行：`_buildCaseDataContext`他helper群・Path A/B POST body追加）／`server.js`（+268行：req.body受領2箇所・`_mergeCaseContextText`・連結2箇所）／`openaiClient.js`・`claudeClient.js`（**Option Bによる変更0**・LCC Phase2既存差分のみ・`caseDataContext`参照0件を機械検証）／新規`apfrCaseDataContext.test.js`（577行）。Code commit **95eaa8990016ea6567a9d6ff1133f44217ad1af2**（`feat: wire case context and apfr formal truth to ai workflow`）。
- **実測結果**：新規`apfrCaseDataContext.test.js` **80/80 PASS**（Formal Truth到達10項目・Resolver status別4項目・payload制限4項目・Intelligence6モジュール到達・Cross-case/Cross-product4項目・server受動パススルー6項目・openaiClient/claudeClient無変更4項目・LCC Phase2既存配線維持6項目・IADP必須ガード無変更/既存Contract非干渉8項目・Resolver再実装0確認5項目・mutation0・fail-open5項目）。既存回帰（C-1C-1 42/42・C-1A 48/48・C-1B 62/62・C-2-1 53/53・CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・APFR Core 49/49・Manual Input 35/35・Phase0 40/40・EER 51/51・IADP Quality Gate 86/86・IADP Structured 13/13・Evidence Promotion 17/17・Cost Tracker 19/19）全PASS・**新規FAIL 0**。`node --check`（server.js／index.html inline／openaiClient.js／claudeClient.js）OK・`git diff --check`／`git diff --cached --check` CLEAN。
- **実AI E2E（Path B・本番案件`case-msr9yckye65y`）**：Output Draftへ書き込まないPath B（`/api/chat`）を用い、本番Draftの現在地を変更せずに実AI E2Eを実施した（Path Aは`getOutputDraft({caseId})`が`updated_at`最新の1件を返すため、実行すると対象案件のCurrent Draftが新Draftへ切り替わる既存動作があり、**今回は未実行**）。Writer／Researcher／Reviewer／Strategyの4社員すべてでContext到達を実証：ResearcherがFormal Truth全21項目（productName／aspName／landingUrl／payout="5000"／epc=34.24／approvalRate=100／regulatoryCategory=医薬部外品／listingNgWords等）を正確に列挙、ReviewerがserverサイドLCC Context（Profile Bio／ターゲット）とclient側Option B Context（`advertisingDisclosureRequirements`／`regulatoryCategory`／`listingNgWords`／`complianceRestrictions`／Revenue Intelligence）を**2ソース同時に引用**し植え込んだ捏造4種（「初期投資不要」「必ず結果が出ます」等）を全て違反として検出、Strategyが6 Intelligenceモジュールを正確な値で引用し判断材料が無い軸（自動化可能性）は「判定材料なし」と明示（数値創作0）。**生成側3社員（Writer／Researcher／Strategy）の捏造0・Formal Truth矛盾0**。API実行5回・実測コスト**約¥11.5**。
- **Data Safety（E2E前後で実測確認）**：本番Draft`out_1787613075018`のAPFR facts（22件）・`intelligenceContext`はE2E前後でJSON**完全一致**、IADP・EER（3件）とも維持。E2Eに伴い更新されたのは`cost-logs.json`（ローカルCost Gate state・AI実行に伴う既存の通常挙動）のみで、APFR Fact・EER・IADP・商品情報の変更は**0**。
- **重要な非依存関係（本Decisionの最重要事項）**：**LCC Phase2 + Option B Complete ≠ Leader Final Grounding Complete ≠ Quality Gate Grounding Complete ≠ READY Grounding Complete**。以下は今回**未検証・未実装**であり、Completeと記録しない：①Path A（Auto Task）でのLeader Final実AI E2E（Path Bはdispatcher専用でありLeader Final成果物生成の実測ではない）②Quality Gate実AI観察③READY実AI観察④`LEADER_FINAL_PROMPT`の「情報不足でも完成品を作れるならLeader自身で作る」ルールに対するfail-closed修正⑤Quality Gate Grounding Enforcement（根拠のない事実が残ったままPassedになる状況を防ぐ仕組み）。今回解消したのは「保存済み案件データがAI社員へ届かない」問題のみであり、Leader Finalが情報不足時に一般論で成果物を捏造しうる構造自体（`LEADER_FINAL_PROMPT`の既存ルール）は変更していない。
...
- Step C-1A：Code commit **9d665256d5ac359f4c8354db5cf1c7a1447aceef**（`feat: add apfr compliance context foundation`）。変更3ファイル（`index.html`／`server.js`／`openaiClient.js`）＋新規`apfrComplianceContext.test.js`。
- Step C-1B：Code commit **f525111f32d2686ca47c221423865a7600dff228**（`feat: inject apfr compliance context into writer reviewer`）。変更1ファイル（`openaiClient.js`）＋新規`apfrComplianceInjection.test.js`＋`apfrComplianceContext.test.js`のtest契約更新。
- docs：docs commit **3793752c179682c7a3f631ec0d5942e872ff79de**（`docs: record apfr compliance injection`）。
- Step C-1A／C-1Bとも、working treeの別系統差分「Leader Case Context Phase2」（`openaiClient.js`に同居する`caseContext`／`hasCaseContext`／`formalTruthRule`等）は、HEADベースの合成patch（`git apply --cached`）を用いてhunk単位で完全分離し、commitへ一切混入させていない（`git show`によるcommit内容の直接確認・LCCマーカー0件を確認済み）。
- **正式リリース**：dev-check再試行で**200/200/200**を取得（`/`・`/api/task-history`・`/api/workflow-dashboard`いずれも200。前回セッションのdev-check environment Pendingは今回クリーンな状態からの再試行で再現せず解消）。上記3commitを**main push済み**（`186ec637...`ベースから`b2dc180..3793752` main→main）、**HEAD／origin/main とも `3793752c179682c7a3f631ec0d5942e872ff79de`（ahead/behind 0/0）**。**正式Tag `v1.01-apfr-compliance-injection`（Annotated・message「APFR Step C-1A/C-1B Compliance Context and Writer Reviewer Injection Complete」・target `3793752...`）を作成しtag push済み**。**Render**：main push後の自動Deployで本番反映済み。本番トップ200・`/api/task-history`200・`/api/workflow-dashboard`200を確認、本番配信HTMLに`_apfrBuildComplianceContext`が存在することを確認し新コードの反映を確認済み。**Writer/Reviewerへの実際のCompliance Block注入は、実AI API課金を伴うため今回は本番実行による確認を行っていない**（C-1B専用テスト62件が実関数`buildSystemPrompt`/`buildCompliancePromptBlock`を直接呼び出して検証済みであることをもって代替。実運用での確認は今後の実案件で行う）。
- Step C-1C-1：Code commit **d8e70216e3cf60718651f2d0e8721f3f274d977b**（`feat: add apfr deterministic compliance check`）。変更2ファイル（`index.html`／新規`apfrComplianceGate.test.js`）。`server.js`／`openaiClient.js`変更0。working treeの別系統差分「Leader Case Context Phase2」は、C-1A/B時と同じくHEADベースの合成patchによりhunk単位で完全分離（本コミットの3 hunkはいずれも削除行0の純粋な追加でLCC既存5 hunkとの物理的重なりなし）。
- docs：docs commit **a2bd95af130d8b7d500e567ba346688cc17c1c21**（`docs: record apfr deterministic compliance check`）。
- **正式リリース**：正式リリース前最終検証（C-1C-1専用42/42・C-1A 48/48・C-1B 62/62・既存回帰全PASS・新規FAIL0・`node --check` OK・`git diff --check` CLEAN・LCC混入0再確認）を実施したうえで、クリーンなlocalhost状態からdev-checkを実行し**200/200/200を取得**。上記2commitを**main push**（`650384c..a2bd95a`）、**HEAD／origin/main とも `a2bd95af130d8b7d500e567ba346688cc17c1c21`（ahead/behind 0/0）**。**正式Tag `v1.01-apfr-deterministic-compliance-check`（Annotated・target `a2bd95a...`）を作成しtag push済み**（既存Tagとの命名重複なしを事前確認）。**Render**：自動Deployで本番反映済み。本番トップ200・`/api/task-history`200・`/api/workflow-dashboard`200を確認、本番配信コードに`evaluateComplianceGate`／`buildComplianceGateHtml`とOutput Engineへの配線の存在を確認済み（起動エラー0）。**実案件データによるCLEAR/VIOLATION/NOT CHECKED表示の本番確認は、合言葉認証で保護されており認証情報を持たないこと、および不要なAI API課金・本番Fact汚染を避ける方針のため今回は未実施**（C-1C-1専用テスト42件が実関数を直接呼び出して検証済みであることをもって代替。実運用での確認は今後の実案件登録時に行う）。
- Step C-2-1：Code commit **9cf7ab92028e4280e83153a3c046a588187aedff**（`feat: add apfr formal truth numeric consistency check`）。変更2ファイル（`index.html`／新規`apfrNumericConsistency.test.js`）。`server.js`／`openaiClient.js`／`claudeClient.js`変更0。working treeの別系統差分「Leader Case Context Phase2」は、C-1A/B/C-1C-1と同じくHEADベースの合成patchによりhunk単位で完全分離（LCCマーカー0件をcommit差分から直接確認）。
- docs：docs commit **38ac9aa34e963d6168c44f46d010289ab646b11d**（`docs: record apfr formal truth numeric consistency check`）。
- **正式リリース**：正式リリース前最終検証（C-2-1専用53/53・既存回帰15スイート全PASS・新規FAIL0・LCC混入0・server/openaiClient/claudeClient変更0・Non-blocking Contract無変更を`git show`で直接再確認）ののち、クリーンなlocalhost状態からdev-checkを実行し**200/200/200を取得**。上記2commitを**main push**（`d68658b..38ac9aa`）、**HEAD／origin/main とも `38ac9aa34e963d6168c44f46d010289ab646b11d`（ahead/behind 0/0）**。**正式Tag `v1.01-apfr-formal-truth-numeric-consistency`（Annotated・target `38ac9aa...`）を作成しtag push済み**（既存Tagとの命名重複なしを事前確認）。**Render**：自動Deployで本番反映済み。本番トップ200・`/api/task-history`200・`/api/workflow-dashboard`200を直接curl取得で確認、本番配信コードに`_apfrEvaluateNumericConsistency`／`buildFormalTruthConsistencyHtml`とOutput Engineへの配線の存在を確認済み（起動エラー0・LCCマーカー0件）。**実案件データによるmatch/mismatch/uncomparable/not_checked表示の本番確認は、合言葉認証で保護されており認証情報を持たないこと、および不要なAI API課金・本番Fact汚染を避ける方針のため今回は未実施**（C-2-1専用テスト53件＋localhost fixture確認が実関数を直接呼び出して検証済みであることをもって代替。実運用での確認は今後の実案件登録時に行う）。
- Leader Case Context Phase2 + Option B：Code commit **95eaa8990016ea6567a9d6ff1133f44217ad1af2**（`feat: wire case context and apfr formal truth to ai workflow`）。変更5ファイル（`index.html`／`server.js`／`openaiClient.js`／`claudeClient.js`／新規`apfrCaseDataContext.test.js`）。openaiClient.js／claudeClient.jsはLCC Phase2既存差分のみでOption Bによる変更0（`caseDataContext`参照0件を機械検証）。実AI E2E（Path B・実案件`case-msr9yckye65y`・API 5回・実測¥11.5）でContext到達・捏造0・Formal Truth矛盾0を確認。**docs commitは本追記が該当（push/Tag/Render未実施）**。

次工程:
- **Correction UI Core系列（CUI-0〜CUI-2）の正式リリースはComplete**（push・Annotated Tag・Render反映・本番配信コード確認・本番実データread-only検証・**本番認証後の実ブラウザ最終目視確認まで完了**）。**APFR Step C-1A + C-1B正式リリースはComplete**（push・Annotated Tag `v1.01-apfr-compliance-injection`・Render反映まで完了）。**APFR Step C-1C-1（Deterministic Compliance Check）も正式リリースComplete**（push・Annotated Tag `v1.01-apfr-deterministic-compliance-check`・Render反映まで完了。詳細は上記追記参照）。**C-1C-1 Complete ≠ Compliance Enforcement Complete**（User Approval／READYへの実ブロック接続はStep C-1C-2として引き続き別工程・未着手）。
- **残る次工程＝C-1C-2以降の選定**（**C-1C-2（User Approval / READY Enforcement）へは自動的に進まない**）。**APFR Step C-2-1（Formal Truth Numeric Consistency Check）は正式リリースComplete**（push・Annotated Tag `v1.01-apfr-formal-truth-numeric-consistency`・Render反映・本番配信コード確認まで完了。実案件データによる本番確認は合言葉認証のため未実施）。**C-2-1 Complete ≠ Intelligence Score Enforcement Complete**——mismatch検出結果をscore／confidence／warning等へ実際に接続するかどうかは**Step C-2-2として別設計工程**で保留し、今回は自動的に進まない。以下を比較対象候補として次工程選定を行う：①**CUI-3**（「訂正済み」バッジ・旧値参考表示。独立工程として本当に必要かは要再評価）②**CUI-4**（APFR直接ジャンプ導線）③残課題2（ITP `7days` field追加要否）④残課題4〜7のUI改善⑤**APFR Step C-1C-2（User Approval / READY Enforcement候補）**⑥**APFR Step C-1C-3（advertisingDisclosureRequirements／complianceRestrictionsの構造化サブschema検討・APFR schema変更を伴うため別Decision）**⑦**APFR Step C-2-2（C-2-1のmismatch検出結果をscore／confidence／warningへ接続するか否かの設計）**⑧**Leader Final Grounding（`LEADER_FINAL_PROMPT`の情報不足時fail-closed修正・Path A実AI E2Eでの観察が前提）**⑨**Quality Gate Grounding Enforcement（根拠のない事実が残ったままPassedになる状況を防ぐ仕組み）**。**Leader Case Context Phase2 + Option BはCode commit Complete**（`95eaa89`・push/Tag/Render未実施）。**特定の1つを自動確定せず、正式な次工程はユーザー承認後に決定する**。`listingPolicy`の正式注入も未着手のまま。
- Step C開始の前提となるContract（Current Fact Resolver／Correction／ambiguity fail-closed／legacy Fact handling）はPhase 1で正式確定済みであり、**CUI-0によりCorrection Contractがduplicate policy側でも成立**、**CUI-1で現在値が可視化**、**CUI-2で訂正操作が実装Complete**となり、**C-1A/BによりCompliance Formal TruthのWriter/Reviewer promptへの実接続が初めてComplete**した。
- **Release状態（確定）**：Correction UI Core系列（CUI-0〜CUI-2）＝push済み・docs push済み・正式Tag作成済み・tag push済み・Render反映済み・本番認証後最終目視確認Complete（リリース確認Pending 0）。**Step C-1A/B＝Code commit 2件（`9d66525`／`f52511`）＋docs commit（`3793752`）・main push済み（HEAD/origin/main `3793752...`・ahead/behind 0/0）・正式Tag `v1.01-apfr-compliance-injection`（Annotated・target `3793752...`）作成・tag push済み・Render反映確認済み**（リリース確認Pending 0）。

追記日: 2026-09-02（Decision 108・**Instagram Grounding修正3件のdocs catch-up記録 ＋ APFR Product Content Formal Truth Extension（Decision108追記・新Decisionなし）**。■ Instagram Grounding修正3件＝いずれも push済・Render Auto-Deploy Live・本番read-only確認済で現production baseline（origin/main = `82a1e24681064c19104dc2cf0cc1dc41318f5ab0`）。(1) Output Draft Parser P2（Code commit `3d2a28a` `fix: parse instagram carousel leader final sections`）＝Leader FinalのInstagramカルーセル `### ` セクション（`【N枚目】`スライド／画像生成Prompt）をOutput Draftへ正しくparseし slides 10／imagePrompts 10 の構造化出力接続を修正。(2) Caption Wiring（Code commit `88939c9` `fix: wire instagram carousel caption from leader final section`）＝Output Draftの `fields.caption` をLeader Final全文ではなく `### キャプション` セクション本文（`【広告】…#PR`・214字）へ配線。セクション不在の旧形式は従来どおり finalText へ後方互換fallback。(3) Mobile Review Grounded Wiring（Code commit `82a1e24681064c19104dc2cf0cc1dc41318f5ab0` `fix: use output draft fields as source for mobile review carousel`）＝Mobile Review／Mobile Approval／Publishing preview が旧テンプレ再生成ではなくOutput Draft正本（`fields.slides`／`fields.imagePrompts`／`fields.caption`／`fields.cta`／`fields.hashtags`）を source として使用（`sourceMeta.source = 'outputDraftFields'`）。テンプレ由来の非グラウンディング文言（「知らないと損する」「9割が見落とす」等）が Mobile Review 表示に出ないことを確認。※本3件は「新規リリース」ではなく、公式MDの記録が gpt-5.4-mini Blocking Fix（`4f1fef2`）で止まっていたため実Git/production truthへdocsを追いつかせる catch-up。Mobile Approval／Publishing／Instagram投稿はいずれも未実施。■ APFR Product Content Formal Truth Extension（Code commit `db833f27cbc6a0b8394cc371357065f213765bff` `fix: add product content fields to apfr formal truth`・**Stage-1＝未push・未Render・本番未反映**）＝`APFR_FIELD_META`／`APFR_FIELD_ORDER` へ消費者向け商品情報4 field（`productDescription`:string／`productFeatures`:array／`productUsage`:string／`targetUser`:string・group「商品内容」）を**追加のみ**。既存21 fieldの名称・型・順序は不変（`APFR_FIELD_ORDER` = 25 field）。変更関数0（`validateApfrRecord`／Current Fact Resolver／Manual Input UI／Correction UI／Compliance Context／Case Data Context いずれも無変更）。Fact昇格Contract（Decision108）は新fieldでも既存のまま適用＝`a8_screen_user_verified` または `advertiser_lp_user_verified` ＋ `verificationStatus:'user_verified'` ＋ `verifiedBy:'user'` ＋ `verifiedAt` が必須で、`manual_user_input` 単独・`web_retrieved`（EEA verified Evidence）だけでは `classification:'fact'` へ昇格不可。新4 fieldは `APFR_COMPLIANCE_CONTEXT_FIELDS`／`APFR_COMPLIANCE_PROMPT_FIELD_ORDER`（各4 field）へは追加せず商品内容とCompliance情報を責務分離。DB schema／API／migration変更0。新規 `apfrProductContentFields.test.js`（68/68）＋既存 `apfrListingScope.test.js` の旧「APFR_FIELD_ORDER末尾固定」1 assertionを「listing/compliance field群が順序内に保持」へ最小同期。deterministic対象10 test（apfrProductContentFields 68／apfrCurrentFactResolver 70／apfrCurrentFactUi 78／apfrCaseDataContext 80／apfrCorrectionUi 105／apfrComplianceContext 48／apfrListingScope 74／p1BlockingFix 60／outputDraftCarouselParser 69／mobileReviewGroundedWiring 38）全PASS・新規FAIL 0。`leaderFinalGrounding` はpost-commitで 52/53（残存FAILは `20-2a` のみ・既知恒久FAIL・53/53とは記録しない）。実データ（`case-msr9yckye65y`／プラファスト／A8.net）で `_apfrResolveCurrentFacts()` = **resolved 21／none 4（新4 field）／ambiguous 0**・既存22 Fact mutation 0。**新4 fieldへの実データ user_verified Fact登録は未実施**。EEA verified Evidence と APFR Formal Fact は別Contract（同一視しない）。EER（現実世界の行為記録）と APFR（商品事実）も責務分離。21/21も25/25も**正式Complete Gateではなく表示上のcoverage summary**で、Quality Gate／READY／Approval／IADP status は本field追加だけで変更しない。**Version1 Final Complete ／ Version1.1 Connected AI Company 開発中・Phase54 Complete 維持・Phase55 未着手（今回のcatch-up/Extensionでも変更なし・Phase55を開始しない）**。**現Git**：HEAD = `db833f27cbc6a0b8394cc371357065f213765bff`（local・本docs更新commitが以降の最新HEAD）／origin/main = `82a1e24681064c19104dc2cf0cc1dc41318f5ab0`／ahead 1・behind 0（`db833f2` 未push）。**Grounding完全保証・Fully Compliant・投稿品質保証・永久解決 等とは記録しない**。）

追記日: 2026-08-31（Decision 108・**Leader Final 実行モデルを gpt-4.1-nano → gpt-5.4-mini へ変更（Blocking Fix・docs-only Stage-1 → 正式リリースComplete・Decision108追記・stage-2 docs同期 2026-08-31）**。Instagram第1投稿の実AI Path A（前工程調査・workflow `wf-1788103235478`／case `case-msr9yckye65y`）で、**Reviewerの否認・`LEADER_FINAL_REVIEWER_REJECT_RULE`・formalTruthRule（正本参照義務）・IADP Scope Boundary（IADP値を商品事実へ意味変換しない）がいずれもLeader Final promptへ正しく供給されていた**にもかかわらず、旧 `gpt-4.1-nano` のLeader Finalがそれらを遵守せず、CASE CONTEXTに存在しない商品事実（植物由来・肌に優しい・安全性・口コミ・継続効果・キャンペーン等）を最終成果物へ再生成した。出力が `LEADER_FINAL_PROMPT` のテンプレート見出しを逐語コピーし末尾が「※※※」の反復で崩壊していたことから、**供給経路の欠落ではなくLeader Final実行モデルの能力不足が直接原因**と判定した。**修正方針**：既存Contract・Prompt構造（`LEADER_FINAL_PROMPT`・question組み立て・Reviewer/Strategy供給・reject遵守Contract・Grounding Block）を1文字も変更せず、**通常Leader Finalの実行モデルだけを最小変更で引き上げる**。**Code commit ①`41634f3e67311bf04c3c0ad6c39ebb7a6c6c05b6`（`fix: run leader final on dedicated gpt-5.4-mini model`）**：`openaiClient.js` へ専用定数 `LEADER_FINAL_MODEL='gpt-5.4-mini'` を新設（`EEA_WEB_SEARCH_MODEL` と同じ「別経路は別定数」方式）。`callOpenAI()` に任意の `options.model`（未指定時は従来どおり `OPENAI_MODEL`）を追加し、**通常Leader Final（`_iadpMode=false`）のときだけ** `LEADER_FINAL_MODEL` を指定する。新規 `leaderFinalModel.test.js`（50/50）。**グローバル `OPENAI_MODEL='gpt-4.1-nano'` は無変更**。各AI社員・Company Brain・`leaderSummary`・`strategyConsolidate` 等の既存OpenAI呼び出しは全て従来モデルのまま。**Provider は OpenAI のまま（変更なし）**。**IADP用Leader Final（`structuredOutput`／8192 tokens）は今回の対象外**（別責務・別検証のため従来モデル）。Reviewer=Claude／Strategy=Claude も無変更。**Code commit ②`6b9b2caf013c17cb5cf76d28407a173cb71aee8d`（`fix: register gpt-5.4-mini pricing in cost tracker`）**：`costTracker.js` の `MODEL_PRICES` へ `gpt-5.4-mini` を登録——input `$0.75 / 1M tokens`、cached input `$0.075 / 1M tokens`、output `$4.50 / 1M tokens`（本表の既存形式＝USD/100万トークンと同一・換算不要）。**料金計算構造（`calculateOpenAICost()`・cached按分・USD→JPY）は一切変更していない**（`gpt-5.6-terra` と同じ按分ロジックがそのまま機能）。新規 `costTrackerGpt54Mini.test.js`（8/8・純関数のみ・`cost-logs.json` 非書き込み）。**実AI Path A E2E 実績（1回・正式記録）**：gpt-5.4-mini 変更後、実AI Path A E2Eを **1回** 実施し、**未確認商品事実0件を実測**。workflowId `wf-1788126291305`／caseId `case-msr9yckye65y`／Leader Final `modelUsed = gpt-5.4-mini`（実測）／Reviewer `claude-haiku-4-5`／Strategy `claude-opus-4-8`／他OpenAI担当モデル変更なし。**未確認商品事実0件**（31語の機械走査・画像生成プロンプトも対象）。キャプション冒頭 `【広告】` あり・`#PR` あり。**Reviewer否認 → Strategy分類A（Formal Truthで解消／該当表現削除の方針明示）→ Leader Final遵守**。テンプレート逐語コピー症状消滅・「※※※」生成崩壊症状消滅。**この結果は1回の実測結果として記録する**——Blocking解消Complete／Grounding Complete／Fully Compliant／deterministic guarantee／永続的に再発しない／完全解決 とは**記録しない**。**Cost 再計算**：前回E2E Leader Final の実測token（`api_cost_events` から取得・input `7,969`／output `1,745`）を新料金表で再計算 → **USD `$0.013829` / JPY `¥2.21`**。ただし cached token は `api_cost_events` に保持されないため、**¥2.21 は全inputを通常input単価で計算した上限側の値**として記録する。**`cost-logs.json` は runtime累積ファイルであり正式化対象外**（deterministic test 実行時に既存テスト基盤の `resetCostTracker()` が上書きしたが、durable正本は Supabase `api_cost_events`・無傷。編集・restore・stage・commit いずれもしない）。**テスト記録**：`leaderFinalModel.test.js` 50/50・`costTrackerGpt54Mini.test.js` 8/8・`costTracker.eea8.test.js` 19/19・P1／Reviewer／APFR／IADP関連回帰は新規FAIL 0。**`leaderFinalGrounding.test.js` は 52/53**——現在のFAILは `20-2a`（commit後は `openaiClient.js` がHEAD差分から消えるため、Option F由来のdiff-state static guardが構造上PASSしない既知制約。handover記録済み）。**`53/53 PASS` とは書かない**・機能的な新規回帰とは扱わない。**P2（別系統・今回スコープ外・未修正）**：slides 10→1／imagePrompts取込失敗／targetAudience・benefitの断片化／packageQuality表示乖離。これは Output Draft parser 側の別系統であり、今回のLeader Finalモデル変更とは混同しない。このP2を理由に今回のCode変更をFAIL扱いしない。docs Stage-1中にP2を実装しない。**Decision**：新規Decision番号は作成せず Decision 108 へ追記（P1・C-3-1・Compliance Restrictions と同じ扱い）。**Version1 Final Complete／Version1.1 Connected AI Company 開発中・Phase54 Complete 維持・Phase55 未着手（変更なし・今回のBlocking FixでPhase55を開始しない）**。**stage-2 docs同期時点**：docsのみ変更（Code変更は上記2commitで完了済み・test変更0〔新規追加のみ〕・DB write 0・AI API追加実行0・Render再deploy 0・Tag再作成0）。**【stage-2 docs同期・正式リリース実績（2026-08-31）】**Code commit ①`41634f3e67311bf04c3c0ad6c39ebb7a6c6c05b6` ②`6b9b2caf013c17cb5cf76d28407a173cb71aee8d` ＋ docs Stage-1 commit `4f1fef23b141de0a0b585efd7fb5df83a8ef43b6`（`docs: formalize leader final gpt-5.4-mini blocking fix`）を **main push済**（HEAD＝origin/main＝`4f1fef23b141de0a0b585efd7fb5df83a8ef43b6`・ahead/behind 0/0）。**Annotated Tag `v1.01-leader-final-model-blocking-fix`**（tag object `2ba54d0ae1f500726f1a0ba8514b936298000376`・peeled commit `4f1fef23b141de0a0b585efd7fb5df83a8ef43b6`・message `Leader Final gpt-5.4-mini blocking fix`）**local／remote反映済**（既存Tag `v1.01-instagram-p1-issue-ab-release` は不変・新規Tag再作成なし）。**Render Service `ai-company`（branch `main`・Trigger Auto-Deploy）Deploy `4f1fef2`・Status Live**——ユーザーが Render Dashboard 実画面で **deployed commit `4f1fef2` / Live** を確認済み（Deploy日時 2026-08-31 07:52:51 GMT+9・Duration 19.0s・Manual Deploy／再deployなし）。**本番read-only health確認 PASS**：`GET /`・`/api/auth-required`・`/api/task-history`・`/api/workflow-dashboard`・`/api/workflow-config`〔`leader.provider: openai` ＝ Provider不変〕・`/api/claude-status`〔`ready`〕・`/api/members`・`/api/cases`・`/api/approvals`・`/api/output-drafts`・`/api/claude-cost`・`/api/cost` すべて 200／対象case `case-msr9yckye65y` の Output Draft・IADP `iadp_1787060839814_izhakb`・affiliate `プラファスト`／`A8.net`・intelligence facts 22件 が intact／起動エラー・fatal signal なし。**有料AI追加実行 0**（本番read-only GET のみ・実AI Path A E2E は deploy前の1回のみ・本番deploy後の追加実行なし）。**今回のstage-2で確認できた範囲は「①対象Codeを含むcommitがRenderへdeployされた ②Render Dashboardで `4f1fef2` / Live を確認した ③本番read-only endpointが正常 ④deploy前の実AI Path A E2E 1回では未確認商品事実0件だった」の4点まで**——**本番実AI E2E確認済／本番でgpt-5.4-mini動作を再実測済／Blocking解消Complete／Grounding Complete／Fully Compliant／deterministic guarantee／永続的に再発しない／完全解決 とは記録しない**（この4点を混同しない）。**Leader Final gpt-5.4-mini Blocking Fix は正式リリースComplete**。**本stage-2同期はdocsのみ変更**（Code変更0・test変更0・DB write 0・AI API実行0・Render再deploy 0・Tag再作成0）。**正式リリース対象は `4f1fef2` で完了済みであり、本stage-2 docs同期はそのリリース実績を後追いで正式記録する工程**）

追記日: 2026-08-29／30（Decision 108・**Instagram実運用 P1〜Issue A/B 修正 4commit ─ 実AI Path A E2E PASS・docs正式化 Stage-1 → 2026-08-30 正式リリースComplete（main push・Annotated Tag `v1.01-instagram-p1-issue-ab-release`・Render Auto-Deploy `e935056` Live・本番UI確認済）**。Instagram実運用1周目で確定したP1 Blocking Issue、およびその過程で判明したIssue A／Issue Bについて、**Code実装4commitがlocalhost実AI Path A E2Eで検証完了**した。**Instagram実運用 P1〜Issue A/B 修正は正式リリースComplete（stage-2 docs同期・2026-08-30）**。**【stage-2 docs同期・正式リリース実績】**Code修正4commit（`ba4d82e`／`c35b534`／`d8a09f1`／`08d15e3`）＋docs commit `e935056`（`docs: record instagram p1 and issue ab fixes`）を **main push済**（HEAD＝origin/main＝`e935056ab48049aeb60fa9b39e340c6168b40b69`・ahead/behind 0/0）。**Annotated Tag `v1.01-instagram-p1-issue-ab-release`**（tag object `6d957a8d097c3cd1f43e5875594ae27b812aa768`・peeled commit `e935056ab48049aeb60fa9b39e340c6168b40b69`）**local／remote反映済**（Tag再作成なし）。**Render Service `ai-company`（branch `main`・Trigger Auto-Deploy）Deploy `e935056`・Status Live**（2026-08-30 07:40:12 GMT+9・Duration 22.1s・Manual Deploy／再deployなし）。**本番AI COMPANYの認証後UIをユーザーが実画面で正常表示確認済**（対象case `case-msr9yckye65y`／Package `iadp_1787060839814_izhakb`／最新確認Draft `out_1787999232715`・caseId binding正常・Cross-case混線なし・致命的UI崩壊なし・Render本番read-only API取得も正常）。**P1〜Issue A/B 正式リリースComplete**。本stage-2同期はdocsのみ変更（Code変更0・test変更0・DB write 0・AI API実行0・Render再deploy 0・Tag再作成0）。**正式対象Code commit（4件・push済・正式リリース済）**：①`ba4d82e96668322bb57342927c692a0af6e9d76d`（`fix: restore instagram publishing safety gates`）＝**P1-1 Output Draft caseId Binding**＋**P1-2 Reviewer/Strategy → Leader Final stage-1**（reviewerText/strategyTextの600→1200文字化・`LEADER_FINAL_REVIEWER_REJECT_RULE`追加）。新規`p1BlockingFix.test.js`。②`c35b534d4d4165b541ed0d8a186f3a41567aa443`（`fix: scope listing ng words to listing ads only`）＝**listingNgWords Channel Scope Fix**。新規`apfrListingScope.test.js`。③`d8a09f1c39c0a9e3a2c598769e5aac15689cbfd3`（`fix: mark iadp values as account design not product facts`）＝**IADP Scope Boundary（Option E＋B）**。新規`iadpScopeBoundary.test.js`。④`08d15e3080d2e50113640671927dbc16ca9c29c8`（`fix: supply main task reviewer review to leader final`）＝**Issue A Option D**。新規`mainReviewerSupply.test.js`。**①P1-1 Output Draft caseId Binding**：`createOutputDraft()`がcaseIdを持たないDraftを返すため、生成直後Draftでは`_apfrCurrentAdoptedProduct()`の三重guard（caseId一致）が必ず失敗し、complianceContextが`{}`となってCompliance CheckがNOT CHECKEDへ、さらに`_apfrEvaluateMobileApprovalCompliance()`もnot_checked→fail-open→`blocked:false`となり**C-1C-2b-1 Mobile Approval Enforcementが生成直後Draftに対して発火しなかった**（復元経路`_outputDraftFromServerRow()`は`row.case_id`を設定済みのためF5後のみ正常という不一致）。`atRunWorkflow()`の既存carry-forwardブロック先頭でWorkflow開始時に捕捉済みの`_atRunCaseId`をbindingする1行のみ追加（`createOutputDraft()`本体は無変更＝他呼び出し経路へ副作用0）。**実AI E2E実測**：新規Draft`caseId=case-msr9yckye65y`・`restored:false`・Cross-case混線なし・Compliance Context 4field取得・Mobile Approval Enforcement `evaluated:true`。**②listingNgWords Channel Scope Fix**：A8.net公式定義上`listingNgWords`は「リスティング広告（検索連動型広告）で入札・出稿を禁止する検索キーワード」であり、**Instagram organic投稿本文の禁止語ではない**。実案件の実値`["商品名","法人名"]`をInstagram本文へsubstring適用していたため、①本文中の「商品名」という語自体がviolationとなりMobile Approvalが誤blockされ、②Leader Finalが「商品名はNGワード」と誤解しFormal Truthの`productName`（プラファスト）をIADPアカウント名（ナチュラルエッセンス）へ置換する二次被害が発生していた。**正式仕様**：**Formal Truth（APFR Fact）は`["商品名","法人名"]`のまま保持し書き換えない**（Correction・schema変更いずれも0。将来リスティング広告機能で使用）。Instagram organic出力タイプのときだけ**Compliance evaluation inputから当該fieldのみを除外**し、既存Contractどおり`not_checked`となる（detector本体`evaluateComplianceGate()`／`_apfrEvaluateDisclosureMarkers()`／`_apfrEvaluateComplianceAssessment()`／`_apfrBuildComplianceContext()`はいずれも無変更）。`not_checked`はAssessmentの`unchecked`へ明示され握りつぶさない。他チャネル（lp／flyer等）は従来どおり評価する。**実AI E2E実測**：violation 0件・Mobile Approval誤block 0件・`unchecked:["listing_ng_words"]`。**③IADP Scope Boundary（Option E＋B）**：`fields.iadp.package.finalProfile`の`brand.brandConcept`・`account.bio`等がserver側`_leaderCaseContextToText()`により【CASE CONTEXT｜…（Formal Truth）】ヘッダ下へフラット列挙されていたため、AI社員が「自然由来」（アカウントのブランドコンセプト）を対象商品の成分・特性の事実として転用するClass C（意味誤用）が発生していた。値はCASE CONTEXTに実在するため既存`formalTruthRule`の「記載のない情報を断定しない」では防げなかった。**正式仕様**：IADPの`accountName`／`username`／`bio`／`brandConcept`／`target`／採用ジャンル等は**媒体アカウント側の設計・方針**であり、対象商品の**成分・効能・効果・適性・品質・安全性・実施中キャンペーン・特典の商品事実ではない**。**Option E**＝`server.js _leaderCaseContextToText()`へ用途境界の見出し（`■ アカウント設計（IADP｜運用する媒体アカウント側の設計・方針です。対象商品の…商品事実ではありません）`）を追加（値・順序・件数は不変・増えたのは見出し1行のみ・IADP値0件時は見出しを出さない）。**Option B**＝`openaiClient.js _buildFormalTruthRuleText()`の【絶対禁止】へ意味変換禁止を1項追加（**単一ソースのためSNS／Writer／Reviewer／Strategy＋Leader Finalの全Agentへ同時適用**・Agentごとの重複追加なし）。**過剰禁止を避けるため、アカウント自体の説明・世界観・投稿トーンとしての利用は明示的に許可**（例「自然由来の美容と健康情報を発信するアカウントです」は適切）。**実AI E2E実測**：「自然由来」の商品成分・特性への転用**0件**（出現はアカウント説明のみ）、IADP targetの商品適性転用**0件**、未確認キャンペーン・特典・お試し・割引**0件**。**④Issue A Option D（main-task Reviewer → Leader Final供給）**：Path AではReviewerが**2系統**存在する——(a)Auto Task担当行として実行される**main-task Reviewer**（例`at-task-3`）と、(b)後処理で自動生成される**post-process Reviewer**（`at-postprocess-reviewer`）。main-taskは`workflowTasks.map()`生成物で`isPostProcess`プロパティを持たない（undefined）ため既存の`find(t => t.agentId===[reviewer] && t.isPostProcess)`にマッチせず、`reviewerText`は常に(b)だけを指していた。結果、停止・確認要求を出した(a)の本文は`memberReplies`内に他担当と横並びで埋もれ「AI社員の社内検討内容（正式回答ではない）」扱いとなり、`LEADER_FINAL_REVIEWER_REJECT_RULE`の対象外だった。**修正**：新変数`mainReviewerTask`（`reviewer`／`!isPostProcess`／`completed`／`result`有）を追加し、Leader Final questionへ**専用ラベル`【Reviewer（品質レビュー担当）の作業中レビュー】`**で追加供給（通常分岐・IADP分岐の両方）。上限は既存`LEADER_FINAL_POSTPROCESS_TEXT_MAX=1200`を再利用。reject遵守Contractの発火条件を`(reviewerText || mainReviewerText)`へ拡張（緩和ではなく拡張）。**既存の`reviewerTask`／`strategyTask`取得行は1文字も変更せず**、post-process Reviewerと統合・置換もしない（両方をLeader Finalへ渡す）。`memberReplies`の構造・内容・件数も無変更。**Issue A 実AI E2E結果＝PASS**：Reviewerが「CASE CONTEXTに存在しない商品固有情報（成分・処方・特徴）について確認が必要」と要求→Strategyが「CASE CONTEXTに存在しない情報であり、正本ルールに従い『使わずに成立する投稿』として作成済み。事実未確認の成分・効能・特典は一切含めていません」と解決→Leader Finalが**「商品の成分・効能・効果については記載せず、Formal Truthに基づく事実のみを掲載しています」と明示**し、実際に未確認事実を使用せず完成稿を生成。「Reviewer確認済み」「Compliance Check完了」等の**虚偽断定は0件**（むしろ「Reviewer・Strategy・Compliance担当へ詳細確認と最終承認を進めます」と未完了扱い）。**永久block 0**・Strategy override正常動作。**deterministic block／停止語のsubstring・regex判定／新Enforcementはいずれも追加していない**——Reviewer停止を無条件blockにすると、商品成分等がFormal Truthに構造上存在しない案件（APFRはASPプログラム事実のみを保持）が永久blockになるため。`shared/leaderRuleEngine.js`の`reviewerSignal: null`は偽陽性回避の既存設計判断を尊重し**維持**（Rule Engineはbyte一致で無変更）。`buildStrategyConsolidatePrompt()`のStrategy override権限（caseContextRule）も**維持**。**Not Required維持**：**APFR Compliance Restrictions deterministic化＝Not Required**（判定C）・**APFR Step C-3-1 Grounding Detection＝Not Required**（判定D）をいずれも維持し、新detectorは追加していない。**Class B残件（P3既知事項・正式リリースを止めない）**：最新E2E残存は`#美肌`・`#敏感肌`・`#保湿`の**ハッシュタグ3件のみ**で、**本文中の未確認商品事実は0件**。read-only調査の結論：(a)`#敏感肌`はH3（商品適性示唆）で医薬部外品案件では要注意だが、(b)`#美肌`はH2（悩み・関心タグ）、`#保湿`はH4境界だが投稿テーマとも読め、(c)**deterministic denylistは偽陽性・商品ごとの差・保守コストの観点から採用しない**（`listingNgWords`以外を機械Gate化しないC-1C-1判断および`leaderRuleEngine.js`「既存NGキーワード判定は使用しない（偽陽性回避）」方針と整合）、(d)根本原因の一因は**`LEADER_FINAL_PROMPT`の「ハッシュタグ20個程度」という量指示**（Formal Truthから根拠を持てるタグは`productName`／`productCategory`／`regulatoryCategory`／IADP`accountName`＋広告開示で最大6個程度であり、残りを一般美容タグで埋める生成圧力になる）。**将来必要ならPrompt Rule追加＋量指示緩和をP3として検討する**（今回実装しない）。**第1投稿運用方針**：Class BハッシュタグはMobile Approval時に人間が目視確認し不要・不適切なタグを削除できるため、第1投稿を止めない。**productName**：最新E2Eで`プラファスト`が**4件正常使用**され、`ナチュラルエッセンス`はアカウント名としてのみ使用（商品名への誤使用0件）。前回E2Eで観測された商品名の過剰回避（0件）は**再発なし**。追加修正不要。**テスト**：新規4スイート（`p1BlockingFix` 60/60・`apfrListingScope` 74/74・`iadpScopeBoundary` 84/84・`mainReviewerSupply` 72/72）すべてPASS。既存回帰も新規FAIL0（`apfrComplianceGate` 42/42・`apfrComplianceAssessment` 84/84・`apfrDisclosureDetection` 66/66・`apfrApprovalEnforcement` 76/76・`apfrComplianceUiScope` 60/60・`apfrComplianceContext` 48/48・`apfrComplianceInjection` 62/62・`apfrCaseDataContext` 80/80・`iadpQualityContractRouting` 86/86 ほか）。**`leaderFinalGrounding.test.js`は52/53**（FAIL`20-2a`のみ＝Option F正式commit後に`openaiClient.js`のHEAD差分が0となるため構造上PASS不可の既知diff-state static guard制約。**53/53 PASSとは記録しない**）。released testの追随修正は`apfrComplianceInjection.test.js`（listingNgWordsのpromptラベル変更に伴う期待文字列3行）と`p1BlockingFix.test.js`（reject遵守Contract発火条件の拡張に伴うE-7・検証を弱めずE-7bを新規追加）の2件のみで、**test削除・skip・条件緩和は一切行っていない**。`server.test.js`は実行していない。**実AI費用**：Path A E2E計3回（listingNgWords検証・IADP Scope Boundary検証・Issue A検証）で約¥2.31（Leader chat各1回＋workflow各1回）。**過大表現の禁止**：以下は**記録しない**——Compliance Enforcement完全完成／Fully Compliant／Grounding Enforcement Complete／全ての広告表現が法的に保証済み／自動公開安全性100%保証。今回正式化するのは**P1修正・listingNgWords Channel Scope・IADP Scope Boundary・Issue A Option Dの各実AI E2E PASSとMobile Approval Enforcement正常動作まで**である。**Decision**：新規Decision番号は作成せず、P1-1／listingNgWords／IADP Scope Boundary／Class B調査はDecision108へ、Issue A（Reviewer→Leader Final供給・Approval Contractに接続しない情報供給のみ）もDecision108へ追記する（**Decision109（Mobile Approval Compliance Enforcement Contract）は変更なし**——本工程はEnforcementを追加せず、Mobile Approvalの`canApprove`条件も無変更のため）。**Version1 Final Complete／Version1.1 Connected AI Company開発中・Phase54 Complete維持・Phase55未着手（変更なし）**。**stage-2 docs同期時点**：docsのみ変更（Code変更0・test変更0・DB write 0・AI API実行0・Render再deploy 0・Tag再作成0）。push／Tag／Render Auto-Deployは正式リリースで実施済み・本同期での再実行なし。**現在地フロー**：C-1C-2a-1正式リリース → C-3-1 Not Required → Compliance Restrictions Not Required → 実運用1周目でP1発見 → P1修正 → listingNgWords Channel Scope → IADP Scope Boundary → Issue A Option D → **本docs正式化 Stage-1** → main push（HEAD＝origin/main＝`e935056`）→ Annotated Tag `v1.01-instagram-p1-issue-ab-release`（local／remote反映）→ Render Auto-Deploy（`e935056`・Live・2026-08-30 07:40 GMT+9）→ 本番UI確認（ユーザー実画面・`case-msr9yckye65y`）→ **P1〜Issue A/B 正式リリースComplete** → **stage-2 docs同期（本更新）** → 現在ここ。）

追記日: 2026-08-28（Decision 108・**APFR Compliance Restrictions deterministic化 ─ Not Required 正式化（docs-only stage-1）**。前工程のread-only調査（Opus・C-1系列 read-only）により、`complianceRestrictions` をdeterministic Compliance Detectorへ直接利用する設計は不適切と確定したため、**APFR Compliance Restrictions deterministic化＝Not Required（Detectorを新設しない）として正式化**した（**判定C**：deterministic対象は `listingNgWords` へ責務分離する）。**実測根拠（実案件プラファストの `complianceRestrictions` 実値）**：`A8.netのルール遵守`／`広告表示必須`／`法律関連の禁止事項遵守`／`リスティング違反禁止` の**4値すべてが単純禁止語ではなく directive／obligation／自然言語制約**であり、Type A（単一禁止語）・Type B（禁止フレーズ）の実データは0件。**substring実測（既存 `evaluateComplianceGate()` と同一の NFKC→lowercase→trim→substring 方式を仮適用）**：①残存表現（`#敏感肌`／「自然由来」／「肌に優しい」）の検出＝**0件（false negative 100%）**／②正しいCompliance遵守表明「A8.netのルール遵守のうえ作成」＝`violation` 誤検出／③正しい広告開示「広告表示必須の規定に従い【広告】と明記」＝`violation` 誤検出／④`広告表示必須` は禁止表現ではなく **positive obligation** であり「存在したら違反」となる substring detector では**意味が逆転**する。この時点で直接Detector化は不採用。**Source of Truth 責務分離（正式維持）**：**`listingNgWords`＝deterministic禁止語のSource of Truth**（閉じた語彙・既存 `evaluateComplianceGate()`）／**`advertisingDisclosureRequirements`＝開示義務trigger**（自然文値そのものを substring 検索せず、Formal Truth値の存在をtriggerに別途定義した閉じたmarker集合〔【広告】／【PR】／#広告／#PR／#プロモーション／定型文〕を検査する C-1C-1b whitelist marker detector）／**`complianceRestrictions`＝自然言語Compliance制約のSource of Truth**（既存 C-1B prompt injection で Writer／Reviewer へ伝達・semantic判断は AI／Reviewer の責務）／**`regulatoryCategory`＝参考情報**（カテゴリ名から追加の法令・規制を推測しない）。**C-1C-1 既存判断は変更しない**：「deterministic対象は `listingNgWords` のみ／`complianceRestrictions`・`regulatoryCategory`・`listingPolicy` は自然言語・意味的判断を要するため機械Gate化しない」という正式判断は、今回の実データ調査でも正しかったと確認された。**`advertisingDisclosureRequirements` が C-1C-1b で deterministic 化済みなのは自然文値そのものの substring 検索ではなく別 whitelist 方式のため、同じ成功パターンを `complianceRestrictions` へそのまま適用できない**。**残存3表現（`#敏感肌`／「自然由来」／「肌に優しい」）は現在の Formal Truth だけでは deterministic に NG 判定不能**（`listingNgWords` 一致なし・`complianceRestrictions` 一致なし・`regulatoryCategory` から法令知識を推測することは禁止・semantic判断が必要）。**C-3-1 Grounding Detection＝Not Required と今回の Compliance Restrictions deterministic化＝Not Required により、残存3表現は Grounding側・Compliance側の双方で安全な機械判定が成立しない**ことが確定。現状は Option F／Reviewer 等の AI 側責務に残す。**Compliance Assessment 変更0**（`_apfrEvaluateComplianceAssessment()` へ新Detectorを追加しない）。**Mobile Approval 変更0**（既存spine「Compliance Assessment blocked→Mobile Approval不可→Publishing Ready不可→markInstagramPublished不可」を維持）。**Quality Gate 変更0**（`complianceRestrictions` を `evaluateQualityGate()` へ接続しない）。**READY 変更0**（`complianceRestrictions` を `OUTPUT_STATUS.READY` へ接続しない）。**新規観察事項（別課題・今回は修正しない）**：実案件 `listingNgWords = ['商品名', '法人名']` は「Instagram投稿本文の禁止語」というより「リスティング広告における商品名・法人名キーワード入札禁止カテゴリ」を意味している可能性があり、現行 C-1C-1 detector が Instagram 成果物本文へ substring 検索する適用先との間に**チャネル意味ズレの可能性**がある。**今回この意味ズレは修正しない**（`listingNgWords` の意味変更／field rename／schema変更／detector変更／scan対象変更／ASPデータ変更／既存Fact訂正は禁止）。別課題としてのみ記録する。**Option D（deterministicに止めたい語句は `listingNgWords` へ登録する）の責務原則自体は維持候補だが、現在の `listingNgWords` field が Instagram 投稿用禁止語を正式に保持する Contract なのかは未確認のため「Option D 完全採用」とはまだ記録しない**。正式記録は「`complianceRestrictions` detector＝Not Required／deterministic禁止語の既存SoT＝`listingNgWords`／`listingNgWords` のチャネル意味定義には別途確認事項あり」までとする。**Decision**：新規Decision番号は作成せず Decision108 へ追記（本調査は APFR／Compliance Contract の責務境界の設計判断のため）。**Decision109（Mobile Approval Compliance Enforcement Contract）は変更なし**。**今回は docs-only stage-1（Code変更0・test変更0・DB変更0・AI API実行0・push未実施・新規Tag 0・Render操作0）**。**「Compliance Complete」「Compliance Enforcement Complete」とは記録しない**——正式意味は「調査の結果、`complianceRestrictions` の deterministic Detector 化は Not Required と判断した」である。次工程はまだ自動確定しない（技術的第一候補＝「`listingNgWords` チャネル意味定義確認調査」・ChatGPT確認待ち）。**現在地フロー**：C-1C-2a-1 正式リリースComplete → Quality Gate Grounding Enforcement 調査 → C-3-1 Grounding Detection＝Not Required → **APFR Compliance Restrictions deterministic化＝Not Required** → 現在ここ。）

追記日: 2026-08-28（Decision 108・**APFR Step C-3-1 Grounding Detection ─ Not Required 正式化（docs-only stage-1）**。前工程のread-only調査（Opus・Quality Gate Grounding Enforcement調査に続くC-3-1調査）により、deterministic Grounding Detectionを新設しても実際に発生したGrounding問題を有効に検出できないことが確定したため、**APFR Step C-3-1 Grounding Detection＝Not Required（実装しない）として正式化**した（**判定D**）。**決定的根拠（Option F前の実AI E2Eで確認されたGrounding問題15件）**：**Class A（Formal Truth直接矛盾。例「Formal Truth 報酬4000円 vs 成果物 8000円」）＝0件**／**Class B（Formal Truthに照合先fieldが存在しないContext外具体的捏造。自然由来成分・植物エキス・肌にやさしい成分設計・敏感肌・乾燥肌・悩みを解消・肌本来の美しさ・肌改善・今だけお得なキャンペーン・キャンペーン告知・特典あり等）＝12件**／**Class C（Formal Truth意味誤用。`approvalRate=100`→「100%審査通過済み」／`cookieWindowDays=90`→「90日間のクッキー期間中に購入すると特典あり」／`mobileOptimized=true`→「スマホ最適化済みで簡単購入」。**値自体はFormal Truthと一致しており、値の不一致ではなく意味・文脈の転用**）＝3件**。したがってFormal Truth Contradiction Detectionを実装しても実測15件を1件も検出できない。**APFR Formal Truth全21fieldを調査した結果、C-3-1の安全なdeterministic対象候補＝0件**（APFR＝ASPアフィリエイトプログラムの事実〔ASP名・Program ID・商品名・報酬・EPC・確定率・Cookie期間・技術対応・compliance制約等〕／Grounding問題の中心＝消費者向けマーケティング主張〔商品成分・効果・肌への適性・キャンペーン・特典・評価的表現〕でdomainが一致しない）。**claim extraction不成立**：X1（Formal Truth値との競合値検索）＝false positive過多（例`approvalRate=100`に対し「100人に聞きました」を誤検出）／X2（field-specific keyword proximity）＝実測Class Cは本文にfield labelを含まず検出不能／X3（値完全一致をsupported扱い）＝**禁止**（実測Class C 3件をすべて「supported」と誤肯定する）／X4（claim extraction／contradiction detection分割）＝claim extraction自体がsemantic判断のためdeterministicに成立しない。**status Contract**：Grounding用に`clear`／`supported`／`grounded`／`fully_grounded`を**新設しない**（「値が一致している」「矛盾が見つからない」ことを「Groundingされている」と読み替えると誤保証。特に実測Class CはFormal Truth値が完全一致しているのに意味誤用が発生している）。**C-3系列（仮提案の C-3-1 Grounding Detection／C-3-2a Grounding Assessment／C-3-2a-1 Grounding UI／C-3-2b Grounding Enforcement／C-3-3 Grounding E2E）は実装系列として取り下げ**（設計履歴として「調査した結果Not Requiredとなった」ことは正式記録として残す・単純削除は禁止）。**Leader Final Grounding現在地は変更なし**：Option F実AI E2E＝**A**／Leader Final Grounding＝**B**（軽微残存：`#敏感肌`・「自然由来のスキンケア」・「自然由来の美容と健康を追求するあなたに」・「肌に優しい医薬部外品」等）。**Grounding Complete／Fully Grounded／No Hallucination Guaranteed／Grounding Enforcement Completeとは記録しない**。**Option F責務**：今後もGeneration Preventionとして維持（deterministic Detectionではない）。正式構造＝Case Context／Formal Truth→Generation Prevention（Option F）→Reviewer／Strategy等AI評価→Leader Final。Groundingのdeterministic post-generation Detectorは新設しない。**Quality Gate変更0**（`packageQuality.status`のみを見る構造Gateであり通常Instagram投稿の公開spineを止めない・`evaluateQualityGate()`へGroundingを接続しない）。**READY変更0**（AI生成完了状態・主条件`integratedCount > 0`・Grounding接続しない・READY Grounding Enforcementも現時点では実装しない）。**Mobile Approval変更0**（`canApprove`へ新しいGrounding blockerを追加しない・現行spine「Compliance Assessment blocked→Mobile Approval不可→Publishing Ready不可→markInstagramPublished不可」を維持）。**C-2-1 Numeric Consistency変更0**（`product.inputs`とFormal Truthの照合であり成果物本文を評価しない・C-3とは別責務）。**C-2-2＝引き続き未着手・保留**。**Marketing Claim Formal Truth＝現時点では新設しない**（Formal Truth schemaを増やしても、成果物中のどの表現が「事実主張」かをdeterministicに抽出するclaim extraction問題が残る／allowlist方式＝false positive過多／denylist方式＝Complianceと同一責務）。**将来候補（実装・正式採用はしない）**：残存する軽微表現の一部（`#敏感肌`・「自然由来」・「肌に優しい」）はGroundingよりComplianceの責務に近く、Formal Truthには既に`regulatoryCategory`／`complianceRestrictions`が存在するが、既存C-1C-1は「`advertisingDisclosureRequirements`／`complianceRestrictions`／`regulatoryCategory`／`listingPolicy`は自然言語・意味的判断を要するため機械Gate化しない」と正式判断済み——**この判断は今回変更しない**。将来候補としてのみ「C-1系列：complianceRestrictions deterministic化可否調査」を記録する（まだ実装・正式採用しない）。**Decision**：新規Decision番号は作成せずDecision108へ追記（本C-3-1調査はAPFR／Formal Truth／Grounding境界の設計判断のため）。**Decision109（Mobile Approval Compliance Enforcement Contract）は変更なし**。**今回はdocs-only stage-1（Code変更0・test変更0・DB変更0・AI API実行0・push未実施・新規Tag 0・Render操作0）**。次工程はまだ自動確定しない（技術的第一候補＝「Compliance Restrictions deterministic化可否調査」・ChatGPT確認待ち）。**現在地フロー**：C-1C-2a-1 正式リリースComplete → Quality Gate Grounding Enforcement 調査（＝Quality Gate接続は不採用と判定）→ C-3-1 Grounding Detection 調査 → **C-3-1 Grounding Detection＝Not Required** → 現在ここ。）

追記日: 2026-08-27（Decision 108・**APFR Step C-1C-2a-1 Compliance UI Scope Correction 正式リリースComplete（stage-2 docs同期）**。**【stage-2 docs同期・正式リリース実績】** APFR Step C-1C-2a-1 は正式リリースComplete。Code commit `112dafd6e9ff76f737a6240e2dee346656cfbed6`（`feat: scope compliance ui to scannable outputs`）／stage-1 docs commit `a97109bb2b840b464037b22719d92c00ad3cca62`（＝正式Tag target）を **main push済み**（正式リリース時 HEAD＝origin/main＝`a97109bb2b840b464037b22719d92c00ad3cca62`・ahead/behind 0/0）。**Annotated Tag `v1.01-apfr-compliance-ui-scope-correction`**（target `a97109bb2b840b464037b22719d92c00ad3cca62`）**push済み**（新規Tagは作成せず既存Tag targetも不変）。**Render自動Deploy反映確認済み**：本番 `/` 200・`/api/task-history` 200・`/api/workflow-dashboard` 200・fatal/startup error 0、本番配信 `index.html` に `_apfrComplianceHasScannableContent` と正式「NOT CHECKED」文言が存在、**本番配信 `index.html` とローカルHEADの `index.html` はbyte単位IDENTICAL確認済み**。**最新 `leaderFinalGrounding.test.js` 実測＝52/53 PASS（FAIL: `20-2a` のみ）**——`20-2a` はOption F正式commit後に `openaiClient.js` のHEAD差分が0となるため構造上PASS不可の既知diff-state static guard制約であり、C-1C-2a-1の機能FAILではない。**53/53 PASSとは記録しない**（stage-1実装中の途中値 49/53〔FAIL 18-1・20-1・20-2a・20-2b〕は `index.html` 未commit時のdiff-state由来で、正式commit後に 18-1・20-1・20-2b は解消済み）。**今回の正式化はCompliance UI Scope Correction＝正式リリースCompleteまで**——Compliance Enforcement全体Complete／Quality Gate Grounding Complete／READY Grounding Complete／Grounding Enforcement Complete／IADP Approval Enforcement Complete／server-side Enforcement Completeとは記録しない。新規Decision番号は作成せずDecision108へ追記（Decision109の責務は不変）。**次工程候補の第一＝Quality Gate Grounding Enforcement（未着手・調査/設計工程から開始）**。——以下は正式リリース前のstage-1時点の記録：APFR Step C-1C-2b-2（IADP Approval Enforcement Not Required・本Decision109へ追記済み）の調査過程で、IADPアカウント設計フェーズのdraftを表示すると、投稿成果物が存在しないにもかかわらずCompliance Check UIが「禁止語：CLEAR」「広告開示：MISSING」「総合：BLOCKED」等の確定判定に見える表示false positiveを出すことが判明した。**根本原因**：Output Engineは全output type共通で`buildComplianceGateHtml()`を無条件表示しており（呼び出しは[index.html:43255]の1箇所のみ・output type分岐なし）、`evaluateComplianceGate()`・`_apfrEvaluateDisclosureMarkers()`はobject型フィールド（`fields.iadp`）をdeep recursionせず対象外とするため、IADP-onlyの状態でも「検査対象テキストが0件」のまま`clear`（NGワード側）や`missing`（広告開示側）という確定的な戻り値を返していた——「検査して問題なし／不足あり」ではなく「そもそも検査対象が無かった」だけなのに、確定判定のように表示されていた。**責務判断**：投稿成果物が存在する場合、Detector（C-1C-1／C-1C-1b）・Assessment（C-1C-2a）は正しく機能していることを実測確認済みであるため、**Detector Contract・Assessment Contractは変更しない**。問題はUI表示スコープのみと判定した（本工程着手前の調査で判定A：UI表示スコープのみの最小修正で安全に是正可能、と確定）。**新規helper**：`_apfrComplianceHasScannableContent(outputDraft)`を追加し、`outputDraft.fields`直下にstringまたはarrayの値が1件でも存在するかを判定する（既存2 detectorが内部で使用する型判定`typeof value==='string' || Array.isArray(value)`と完全に同一の浅い形状チェックのみ。NGワード検索・広告marker検索・output type判定・IADP存在判定・product判定はいずれも行わない）。**IADP-only表示**：scannable contentが0件の場合のみ、禁止語／広告開示／総合の3項目すべてを「NOT CHECKED」表示へ差し替え、「投稿成果物などの検査対象テキストがまだ存在しないため、Compliance Checkは未実施です。」という補足を表示する（CLEARでもBLOCKEDでもないことを明示）。**内部判定は完全に無変更**：`evaluateComplianceGate()`・`_apfrEvaluateDisclosureMarkers()`・`_apfrEvaluateComplianceAssessment()`の呼び出し（無条件で必ず実行される）・戻り値はいずれも変更していない（static検証で確認済み）。表示直前でUI層のみ上書きする設計。**lifecycle carry-forward対応**：`fields.iadp`と投稿成果物（caption/slides/hashtags等）は同一Output Draftへ併存し得るため、**「IADP存在」ではなく「scannable content存在」を判定軸とした**（IADP存在 ≠ IADP-only）。ブラウザ実測で、IADPと投稿成果物が併存する場合はscannable content=trueとなり通常のCompliance表示（実際の判定結果）が正しく表示されることを確認済み——**false negativeを防止し、本物の投稿Complianceを隠さない**ことを最優先した設計。**Mobile Approval Enforcement境界（C-1C-2b-1）**：変更0。投稿成果物が存在する場合の`Compliance Assessment blocked → Mobile Approval不可`という正式spineは維持され、新helperは`_apfrEvaluateMobileApprovalCompliance()`・`approveInstagramPackage()`のいずれにも接続していない（static検証で確認済み）。**IADP Approval境界（C-1C-2b-2）**：C-1C-2b-2 = Not Requiredの判断は維持。`_iadpApproveDesign()`・`accountCreationReadiness`・IADP Approval stateはいずれも変更0。**Quality Gate / READY境界**：`evaluateQualityGate()`・`OUTPUT_STATUS.READY`は変更0。今回のUI補正をQuality Gate Grounding EnforcementやREADY Grounding Enforcementの解決として記録しない。**テスト**：新規`apfrComplianceUiScope.test.js` **60/60 PASS**（helperのfalse positive/false negative耐性・IADP-only表示・lifecycle carry-forward・投稿成果物あり時の表示維持・Mobile Approval/IADP Approval/Quality Gate/READY/server-side変更0のstatic検証を含む）。**released test追随修正**：`apfrDisclosureDetection.test.js`のtest 40が、`buildComplianceGateHtml()`本体を固定6000文字windowで切り出していたため、今回の正規UI追加で検証対象文言（「確認表示のみ」・オフセット6613文字目）がwindow外へ押し出され1件FAILした。原因は表示文言の削除ではなく固定長切り出しという測定方法のアーティファクトであることを実測確認し、既存test suite内の標準パターン（`\n}\n`終端検出＋fallback。`apfrComplianceGate.test.js`のtest 16-1が採用する方式と同一）へ統一する形で追随修正した。**assertion内容・ラベル・検証Contractは1文字も変更しておらず、変更は`end`/`body`算出の2行のみ**（assert行の変更差分は0件をcommit内で機械確認済み）。既存回帰：`apfrApprovalEnforcement` 76/76・`apfrComplianceAssessment` 84/84・`apfrDisclosureDetection` 66/66・`apfrComplianceGate` 42/42・`apfrComplianceContext` 48/48・`apfrComplianceInjection` 62/62・`apfrCaseDataContext` 80/80・`iadpQualityContractRouting` 86/86・`apfrCurrentFactResolver` 70/70・`apfrCore` 49/49、他既存重要回帰も新規FAIL0。**`leaderFinalGrounding.test.js` 49/53**（FAIL: 18-1・20-1・20-2a・20-2b。Option F commit時点のCode diff状態を固定するstatic guardが今回の正規`index.html`変更を検出した既知の非機能FAILであり、C-1C-2a-1機能のFAILではない。**53/53 PASSとは記録しない**。test修正・assertion削除はいずれも行っていない）。`node --check`・`git diff --cached --check`いずれもCLEAN。**localhost**：`/`200・`/api/task-history`200・`/api/workflow-dashboard`200。**fixture確認**：ブラウザ実行環境で5パターン（A. IADP-only→NOT CHECKED/NOT CHECKED/NOT CHECKED、B. IADP+投稿成果物→通常Compliance表示、C. 投稿+disclosure missing→MISSING/BLOCKED維持、D. 投稿+disclosure satisfied→SATISFIED/CLEAR維持、E. listing violation→VIOLATION/BLOCKED維持）を実測確認。Console Error0・fixture実行によるPOST発火**0件**・DB write0・AI API実行0。**Code commit `112dafd6e9ff76f737a6240e2dee346656cfbed6`（`feat: scope compliance ui to scannable outputs`）。対象3ファイルのみ（`index.html`／新規`apfrComplianceUiScope.test.js`／`apfrDisclosureDetection.test.js`）・push/Tag/Render未実施**。working treeの別系統runtime差分（`cost-logs.json`／`data/conversations/_meta.json`・既存untracked7件）は本Code commit・本docs追記いずれにも不混入。新規Decision番号は作成せず、C-1C-2a-1はC-1C-2aの表示スコープ補正でありEnforcement Contract変更ではないため、C-1C-2a記録済みのDecision108へ追記（C-1C-2b-1/C-1C-2b-2のApproval Enforcement Contract系はDecision109に分離済みであり、責務境界に従い本追記はDecision108側とする）。**今回の正式化はCompliance UI Scope Correction Code Implementation Completeまでであり、Compliance Enforcement全体Complete／Quality Gate Grounding Complete／READY Grounding Complete／Grounding Enforcement Complete／IADP Approval Enforcement Complete／server-side Enforcement Completeとは記録しない**。**Version1 Final Complete／Version1.1 Connected AI Company開発中・Phase54 Complete維持・Phase55未着手（変更なし）**）。以前: 追記日: 2026-08-27（Decision 108・**APFR Step C-1C-2a Compliance Assessment Aggregation Code Implementation Complete**。既存C-1C-1（listingNgWords Deterministic Compliance Check）とC-1C-1b（Advertising Disclosure Detection）はそれぞれ独立したdetectorとして`buildComplianceGateHtml()`内で個別に表示されるのみで、両者を横断した1つの総合Assessmentは存在しなかった。この2 detectorの結果を1つのCompliance Assessmentへ集約する`APFR_COMPLIANCE_ASSESSMENT_ITEMS`（item識別子定数）と新規純関数`_apfrEvaluateComplianceAssessment(outputDraft, complianceContext)`を`index.html`のみへ追加した。**本設計の前提となる調査結論**：本工程着手前にQuality Gate Grounding Enforcementの調査・設計を実施し、「Quality GateへCompliance結果を直接集約する案（Option A/B/C/E）は、`apfrCaseDataContext.test.js`・`apfrComplianceGate.test.js`・`apfrDisclosureDetection.test.js`・`apfrNumericConsistency.test.js`の4つの正式リリース済みtestが固定している『`evaluateQualityGate()`は既存シグネチャのまま・Compliance参照0・Disclosure参照0』というContractを破壊するため不採用」と判断し（Option D：Quality Gateは構造Gateのまま維持し別Gateを新設）、その具体化として本C-1C-2aを新規の集約層として分離実装した。**今回の正式化はCompliance Assessment Aggregation Completeまでであり、Compliance Enforcement Complete／Quality Gate Grounding Complete／READY Grounding Complete／User Approval Enforcement Complete／accountCreationReadiness Enforcement Completeとは記録しない**。**Existing Detector再利用**：`evaluateComplianceGate(outputDraft, complianceContext)`と`_apfrEvaluateDisclosureMarkers(outputDraft, complianceContext)`を「呼ぶだけ」とし、listingNgWords検索・広告marker検索・`_apfrComplianceGateNormalize()`・`APFR_DISCLOSURE_ACCEPTED_MARKERS`・hashtag token判定・APFR Resolver（`_apfrResolveCurrentFact`／`_apfrResolveCurrentFacts`）・`.facts`直接走査のいずれも独自再実装していない（static検証で確認済み）。**status Contract**：`clear`（blockers0件かつ少なくとも片方のdetectorが実判定済み）／`blocked`（`evaluateComplianceGate().status==='violation'`または`_apfrEvaluateDisclosureMarkers().status==='missing'`のいずれか1件以上）／`not_checked`（両detector not_checked・outputDraft/complianceContext不正・例外）の3状態。**blockers**は既存detector結果の要約のみ（`{type: 'listing_ng_words'|'advertising_disclosure', label, detail}`）で、新しい意味判定・独自ルールは一切追加していない（test 14でblocker detailの件数がdetector結果の`violations.length`と機械的に一致することを検証）。**unchecked Contract**：not_checked項目を握りつぶさず`unchecked[]`へ保持する。例えば`compliance clear + disclosure not_checked`は`status=clear・unchecked=['advertising_disclosure']`となり、**`clear`は「全Compliance完全確認済み」を意味しない**ことをUI（「未検査項目あり」表示）でも明示。**fail-open/fail-closed**：明確な違反（violation/missing）のみblockedとし、判定不能（not_checked）は自動blockせずuncheckedへ保持する。例外時もclearを返さずnot_checkedへfail-openする（誤ってclearを詐称しない）。**Non-blocking Contract（全て変更0を実装・テスト双方で確認）**：`evaluateQualityGate()`／`evaluateOutputPackageCompleteness()`／`QUALITY_GATE_PASSING_STATUSES`／`OUTPUT_STATUS.READY`／`approveInstagramPackage()`／`_iadpApproveDesign()`／`pushApprovalToServer()`／`_enqueueApprovalPost()`／`accountCreationReadiness`（`shared/instagramAccountDesignQuality.js`）／Executive Decision Engine／Publishing Ready。今回のblockedはCompliance Assessment上の状態のみで、これらのいずれも停止しない。UIにも「※現在は確認表示のみです（検出・集約のみ）。User Approval・READY・Quality Gate・Executive Decisionは自動停止しません。」を明示。**重要な構造ギャップ（次工程への申し送り）**：`accountCreationReadiness`はIADP専用のenforcement spineとして既に複数Gate結果（Reviewer/Strategy/Quality Gate）を集約し`ready`/`conditional`/`not_ready`を決定する実績があるが、非IADP案件（アフィリエイト記事等）には同等のEnforcement接続先が現状存在しない。この構造ギャップは**Step C-1C-2bの設計課題として未着手のまま残す**。**Numeric Consistency境界**：`_apfrEvaluateNumericConsistency()`（C-2-1）への参照・変更は0、`mismatch`/`uncomparable`をblockerに含めない（Step C-2-2として別工程のまま分離維持）。**Leader Final Grounding境界**：Option F（Leader Final Grounding）はprompt-based preventionでありdeterministic Grounding detectorは実コード調査（`grep "function.*[Gg]rounding"`）で存在しないことを確認済みのため、「Grounding clear」「Grounding blocked」等のstatusは新設していない。Prompt遵守をAssessment PASSと見なしていない。**UI**：既存Compliance Checkパネル内へ「禁止語：CLEAR/VIOLATION/NOT CHECKED」「広告開示：SATISFIED/MISSING/NOT CHECKED」に加え「総合：CLEAR/BLOCKED/NOT CHECKED」を独立表示し、blocked時はblocker理由（例：「・リスティングNGワード違反：禁止語違反：1件」）、unchecked項目がある場合は「未検査項目あり：リスティングNGワード」等を明示。**保存**：runtime onlyの純関数（DB・Output Draft・APFR Fact・Intelligence Context・Compliance Context・EER・IADP assessmentContext・Quality Gate fieldへの書き込みは0・呼び出しのたび再計算）。**テスト**：新規`apfrComplianceAssessment.test.js` **83/83 PASS**（status Contract 9パターン〔clear／blocked／not_checked各組み合わせ〕・fail-open4件〔null/undefined/malformed/例外〕・blockers要約検証・unchecked保持検証・Existing Detector再利用のstatic検証（コメント文中の説明語への誤ヒットを避けるため行頭コメントを除去したコード部分のみを対象とする方式へ実装過程で修正）・Numeric Consistency非接続・Quality Gate/READY/User Approval/accountCreationReadiness変更0のstatic検証を含む）。既存回帰：`apfrComplianceGate` 42/42・`apfrDisclosureDetection` 66/66・`apfrComplianceContext` 48/48・`apfrComplianceInjection` 62/62・`apfrCaseDataContext` 80/80・`apfrNumericConsistency` 53/53・`apfrCurrentFactResolver` 70/70・`apfrCore` 49/49・`iadpQualityContractRouting` 86/86・`apfrManualInputUi` 35/35・`apfrReadoptCarryOver` 40/40・`apfrCorrectionDuplicate` 65/65・`apfrCorrectionUi` 105/105・`apfrCurrentFactUi` 78/78・`externalExecutionRecord.eer1` 51/51、他既存重要回帰も新規FAIL0。**`leaderFinalGrounding.test.js` 49/53**（FAIL: 18-1・20-1・20-2a・20-2b。これはOption F commit時点のCode diff状態を固定するstatic guardが、今回の正規`index.html`変更（C-1C-2a）を検出した既知の非機能FAILであり、C-1C-2a機能のFAILではない。**53/53 PASSとは記録しない**。test修正・assertion削除・PASSへの書き換えはいずれも行っていない）。`node --check`・`git diff --cached --check`いずれもCLEAN。**localhost**（`npm run dev-check`）：`/`200・`/api/task-history`200・`/api/workflow-dashboard`200・Console Error0。**fixture確認**：ブラウザ実行環境（javascript_tool）で`_apfrEvaluateComplianceAssessment()`・`buildComplianceGateHtml()`を一時的な状態差し替え（finally節で確実に復元）を用いて直接呼び出し、clear／blocked／not_checked／clear+uncheckedの4状態のHTML描画を実測確認。fixture実行によるPOST発火**0件**（GETリクエストのみを実測確認）・DB write0・AI API実行0・実行後は状態を完全復元。**Code commit `659e82ceefb899e794b08872b58d2820b357c1df`（`feat: aggregate apfr compliance assessment`）。対象2ファイルのみ（`index.html`／新規`apfrComplianceAssessment.test.js`）・push/Tag/Render未実施**。working treeの別系統runtime差分（`cost-logs.json`／`data/conversations/_meta.json`・既存untracked7件）は本Code commit・本docs追記いずれにも不混入。新規Decision番号は作成せずDecision108へ追記。**Version1 Final Complete／Version1.1 Connected AI Company開発中・Phase54 Complete維持・Phase55未着手（変更なし）**）。以前: 追記日: 2026-08-26（Decision 108・**APFR Step C-1C-1b Advertising Disclosure Detection Code Implementation Complete**。既存C-1C-1（Deterministic Compliance Check）は`listingNgWords`のみをdeterministicに確認しており、`advertisingDisclosureRequirements`はC-1Aが構築するCompliance ContextとしてWriter/Reviewer/Leader Finalへ到達していたにもかかわらず`evaluateComplianceGate()`では一度も評価されていなかった（同関数は`complianceContext.listingNgWords`のみを見て他3fieldに触れない実装だった）。Leader Final Grounding Option F実AI E2Eでも、`advertisingDisclosureRequirements`が存在するにもかかわらず【広告】／#PR／#広告等の広告明示が成果物へ反映されない事象を確認しており、この不足を可視化するため**APFR Step C-1C-1bとしてAdvertising Disclosure Detectionを追加した**。**今回の正式化はAdvertising Disclosure Detection Completeまでであり、Compliance Enforcement Complete／Compliance Gate Complete／Quality Gate Grounding Complete／READY Grounding Complete／User Approval Enforcement Completeとは記録しない**。**実装**（`index.html`のみ・`evaluateComplianceGate()`の兄弟関数として追加）：新規`APFR_DISCLOSURE_ACCEPTED_MARKERS`（whitelist方式：ラベル【広告】／【PR】、ハッシュタグ#広告／#PR／#プロモーション、明示文章「この投稿はアフィリエイト広告を利用しています」「この投稿はプロモーションを含みます」「この投稿には広告が含まれます」の8種）と新規純関数`_apfrEvaluateDisclosureMarkers(outputDraft, complianceContext)`。既存`_apfrComplianceGateNormalize()`（String化→NFKC→lowercase→trim）をそのまま再利用し新しい自然言語解析・fuzzy判定・AI判定は追加していない。ハッシュタグは単純`indexOf()`部分一致ではなく`/#([^\s#]+)/g`によるtoken境界完全一致で判定し、`#profile`を`#PR`として誤検出しない設計。`#アフィリエイト`／`#案件`／`#提供`／文章中の単なる「広告」「PR」の無境界部分一致は単独でsatisfiedにしない（deterministic判定で誤って適法・適切と認定しないことを最優先したwhitelist設計）。**status Contract**：`satisfied`（requirementあり＋Marker1件以上検出）／`missing`（requirementあり＋Marker0件）／`not_checked`（requirementなし・none・ambiguous・empty array・invalid input・fields不正・例外時）の3状態。`complianceContext.advertisingDisclosureRequirements`が有効なnon-empty arrayとして存在する場合のみ`requirementPresent=true`とし、C-1Aの既存Contract上none/ambiguousはキー自体が出力されないため自然に`not_checked`となる（C-1Aのshapeは変更0）。**Non-blocking Contract（全て変更0を実装・テスト双方で確認）**：`evaluateComplianceGate()`／`evaluateOutputPackageCompleteness()`／`evaluateQualityGate()`／`OUTPUT_STATUS.READY`／`approveInstagramPackage()`／`_iadpApproveDesign()`／`pushApprovalToServer()`／`_enqueueApprovalPost()`／Executive Decision Engine／Publishing Ready／APFR Resolver／C-1A Compliance Context／C-1B Compliance Injection／Leader Final Grounding Option F。missingでもQuality Gate／READY／User Approval／Publishing Readyをブロックしない。例外時はsatisfiedを返さずnot_checkedへfail-open。**UI**：既存Compliance Checkパネル内へ「禁止語：CLEAR/VIOLATION/NOT CHECKED」と並べて「広告開示：SATISFIED/MISSING/NOT CHECKED」を独立表示。**Instagramタイアップ投稿ラベル境界**：Paid Partnershipラベルはプラットフォーム上の外部UI操作でありOutput Draft文字列からは実際に設定されたか確認できないため、本Detectionのsatisfied条件には含めず、架空の「設定済み」判定を作らない（新規Fact作成0・EER変更0・DB保存0）。UIには「Instagram投稿時はタイアップ投稿ラベルの設定も別途確認してください（本Detectionでは検証していません）」という補足のみ表示（status !== not_checkedの場合のみ）。**保存**：runtime onlyの純関数（DB・Output Draft・APFR Fact・Intelligence Context・Compliance Context・EERへの書き込みは0・呼び出しのたび再計算）。**テスト**：新規`apfrDisclosureDetection.test.js` **66/66 PASS**（振る舞い27件＋static検証39件。Accepted Marker各種・missing・not_checked・NFKC正規化（全角＃ＰＲ）・hashtag token境界・`#profile`誤検出なし・`#アフィリエイト`／`#案件`／`#提供`単独では非satisfied・「広告表示義務について」等の無境界部分一致では非satisfied・mutation0・fetch0・DB write0・AI API0・Existing Compliance Gate/Quality Gate/READY/User Approval変更0のstatic検証を含む。実装過程で発見・修正した自己参照バグ〔テスト自身の検索文字列が自分自身へ誤ってマッチしていた〕1件を含む）。既存回帰：`apfrComplianceGate` 42/42・`apfrComplianceContext` 48/48・`apfrComplianceInjection` 62/62・`apfrCaseDataContext` 80/80・`apfrCurrentFactResolver` 70/70・`apfrCore` 49/49・`iadpQualityContractRouting` 86/86・`apfrManualInputUi` 35/35・`apfrReadoptCarryOver` 40/40・`apfrCorrectionDuplicate` 65/65・`apfrCorrectionUi` 105/105・`apfrCurrentFactUi` 78/78・`apfrNumericConsistency` 53/53・`externalExecutionRecord.eer1` 51/51、他既存重要回帰も新規FAIL0。**`leaderFinalGrounding.test.js` 49/53**（FAIL: 18-1・20-1・20-2a・20-2b。これはOption F commit時点のCode diff状態を固定するstatic guardが、今回の正規`index.html`変更（C-1C-1b）を検出した既知の非機能FAILであり、C-1C-1b機能のFAILではない。**53/53 PASSとは記録しない**。test修正・assertion削除・PASSへの書き換えはいずれも行っていない）。`node --check`・`git diff --cached --check`いずれもCLEAN。**localhost**（`npm run dev-check`）：`/`200・`/api/task-history`200・`/api/workflow-dashboard`200・Console Error0。**fixture確認**：ブラウザ実行環境（javascript_tool）で`_apfrEvaluateDisclosureMarkers()`・`buildComplianceGateHtml()`を一時的な状態差し替え（finally節で確実に復元）を用いて直接呼び出し、satisfied／missing／not_checkedの3状態のHTML描画を実測確認。fixture実行によるPOST発火**0件**（GETリクエストのみを実測確認）・DB write0・AI API実行0・実行後は状態を完全復元。**Code commit `e23c0df88d7aca485f576124677735aa080441ee`（`feat: detect advertising disclosure markers`）。対象2ファイルのみ（`index.html`／新規`apfrDisclosureDetection.test.js`）・push/Tag/Render未実施**。working treeの別系統runtime差分（`cost-logs.json`／`data/conversations/_meta.json`・既存untracked7件）は本Code commit・本docs追記いずれにも不混入。新規Decision番号は作成せずDecision108へ追記。**Version1 Final Complete／Version1.1 Connected AI Company開発中・Phase54 Complete維持・Phase55未着手（変更なし）**）。以前: 追記日: 2026-08-26（Decision 108・**Leader Final Grounding Option F Code Implementation Complete・実AI E2E Validated**。Path A実AI E2Eで、Leader Case Context Phase2 + Option B（前回追記）によりContext自体はLeader Finalまで正常に到達しているにもかかわらず、Writerが情報不足を正しく検出して停止したのに**Leader Finalがその停止判断を乗り越えてCASE CONTEXT外の具体的事実を捏造し完成品を生成する**問題を発見した（**LCC Phase2 + Option Bの失敗ではない**。Context Wiring自体は正常。問題はLeader Final Grounding層に限定）。**Option F修正前の実測**：Context外具体的捏造12件（自然由来成分・植物エキス・肌にやさしい成分設計・敏感肌・乾燥肌・悩みを解消・肌本来の美しさ・肌改善・今だけお得なキャンペーン・キャンペーン告知・特典あり等）、Formal Truth意味誤用3件（`approvalRate=100`→「100%審査通過済み」、`cookieWindowDays=90`→「90日間のクッキー期間中に購入すると特典あり」、`mobileOptimized=true`→「スマホ最適化済みで簡単購入」）、架空キャンペーンあり、架空特典あり、Writer停止判断の不正上書きあり。packageQuality=100/complete・Quality Gate=passed・READY相当（**捏造成果物でも現行Quality Gate/READYロジックは素通しすることを実測で確認**）。**根本原因**：`LEADER_FINAL_PROMPT`／`ACCOUNT_INTELLIGENCE_LEADER_FINAL_PROMPT`は`buildSystemPrompt()`を経由しない独立の固定System Prompt定数のため、Writer/Researcher/Reviewer/Strategyに適用される`formalTruthRule`完全版（CASE CONTEXTに存在しない情報を断定禁止とする中核ルール）がLeader Finalへ届いていなかった。加えて既存`LEADER_FINAL_PROMPT`には「情報不足でもLeader自身が完成品を作れると判断すれば生成可」というルールが存在し、これが「AI社員が正しく停止→Leaderが独自補完→捏造完成品生成」を許していた。**Option F実装**（`openaiClient.js`）：①`buildSystemPrompt()`内にあったformalTruthRuleのinline定義を`_buildFormalTruthRuleText(hasCaseContext)`という純関数へ抽出し単一ソース化（本文の意味は無変更・`buildSystemPrompt()`はこの関数を呼ぶだけ・Leader Finalも同じ関数を再利用・二重管理なし）。②新規純関数`_buildLeaderFinalGroundingBlock(caseContext, ruleFacts)`を追加し、`runLeaderFinalResponse()`でcaseContext存在時のみquestion側へ付加（`LEADER_FINAL_PROMPT`／`ACCOUNT_INTELLIGENCE_LEADER_FINAL_PROMPT`定数本体はいずれも無変更・caseContextなし経路は既存question不変でfail-open維持）。③Formal Truthのfield名・意味・用途を維持し別概念へ読み替えてはいけないという意味維持条項を新設（`approvalRate`／`cookieWindowDays`／`mobileOptimized`の実測誤用パターンを具体例として明示）。④既存`shared/leaderRuleEngine.js`の`evaluateLeaderRuleFacts()`をそのまま再利用し（`shared/leaderRuleEngine.js`自体・新規schemaいずれも変更0）、`informationInsufficient.count > 0`の場合のみ「情報不足と判断した担当社員が本来提供すべきだった具体的事実をLeader Finalが代わりに創作してその担当の判断を上書きしてはならない」というfail-closed文を追加（count===0では追加せず、ruleFacts null/undefinedでも例外を出さず既存挙動を維持）。**変更0**：`index.html`／`server.js`／`claudeClient.js`／`shared/leaderRuleEngine.js`／APFR Resolver／Quality Gate／READY／Compliance Gate／DB schema。**テスト**：新規`leaderFinalGrounding.test.js`（AI API呼び出し0・fetch0・DB write0・runtimeデータ変更0の合成テスト）**53/53 PASS**。既存`apfrCaseDataContext.test.js`はtest 40のみ、旧: inline formalTruthRule定義の直接文字列一致検証を、新: `_buildFormalTruthRuleText()`経由でhasCaseContextがformalTruthRuleを有効化するという同一Contractへ検証対象を追随させる修正（Contract弱化ではない）で**80/80 PASSへ復旧**。既存回帰：`apfrComplianceContext` 48/48・`apfrComplianceInjection` 62/62、他既存18スイート全PASS・新規FAIL 0。`node --check`・`git diff --check`いずれもCLEAN。**実AI E2E**（本物のPath A workflow・安全なcandidateOnly方式・本番Draft`out_1787613075018`read-only取得→`_buildCaseDataContext()`→実サーバー側Path A workflow→Leader Final→`buildOutputDraftFromLeaderFinal(candidateOnly:true, 独立targetDraft)`→`evaluateQualityGate()`・通常UIの`atRunWorkflow()`／`pushOutputDraftToServer()`は未使用・API6回・実測約16.4円）：**Option F後はContext外具体的捏造0件・Formal Truth意味誤用0件・Formal Truth直接矛盾0件・架空キャンペーン0件・架空特典0件・Writer停止判断の不正上書き0件**（Writerは情報不足を正しく報告し続け〔【現状仮説】【確認したいこと】〕、Leader FinalはA8.net提携・報酬5,000円・医薬部外品・スマホ対応済み・効果効能表現回避等、Formal Truth範囲内で成果物を作成）。**ただし軽微な残存あり**：`#敏感肌`・「自然由来のスキンケア」・「自然由来の美容と健康を追求するあなたに」・「肌に優しい医薬部外品」等、Formal Truthに直接裏付けのない評価的・示唆的表現が残った（具体的数値・キャンペーン・特典・効能の断定とは質的に異なるが、厳密なGrounding上は軽微な残存）。**正式判定：Leader Final Grounding＝B（軽微な表現問題のみ）・Option F実AI E2E＝A（成功）**。**「Leader Final Grounding Complete」「Grounding Enforcement Complete」とは記録しない**。**Data Safety**：E2E前後でoutputId・updated_at同一、facts22件→22件完全一致、intelligenceContext完全一致、IADP完全一致、EER3件→3件完全一致、User Approval approved→approved、output_drafts write0・APFR/EER/IADP/User Approval変更0（task_history／conversations／messages／cost-logs.jsonへの実行履歴追加は許可済みruntime副作用として区別）。**重要な非依存関係（本追記後も維持）**：Option F実AI E2E成功 ≠ Compliance Enforcement Complete（`advertisingDisclosureRequirements`はCASE CONTEXTへ到達しているが成果物に【広告】／#PR／#広告等の明示表示なし・`#アフィリエイト`は存在・別課題として分離）。Option F実AI E2E成功 ≠ Quality Gate Grounding Complete（現行Gateは構造充足中心・Option F前は捏造成果物でも100/complete/passedとなったことを実証済み・Option FでもQuality Gateロジック自体は変更0）。Option F実AI E2E成功 ≠ READY Grounding Complete（READYは`integratedCount > 0`が主条件・Option FでREADYロジック変更0）。**Code commit `04e28a08793218ceaf59dd8fa228333bb58fcd0c`（`feat: ground leader final in formal truth context`）。対象3ファイルのみ（`openaiClient.js`／`apfrCaseDataContext.test.js`／新規`leaderFinalGrounding.test.js`）・push/Tag/Render未実施**。commit前の最終テストでstage後に`leaderFinalGrounding.test.js`が一時的にFAIL（`git diff --name-only`がunstaged差分のみを見るためstage後にopenaiClient.js等が変更対象一覧から消えた既知のtestハーネス限界。Option F本体の機能的後退ではない）→同ファイル内のみ`git diff --name-only HEAD`（staged+unstaged合算）へ検出方法を修正し、assertion削除・Contract弱化なしで**53/53 PASSへ復旧**したうえでcommit。working treeの別系統差分（`cost-logs.json`／`data/conversations/_meta.json`・既存untracked7件）は本Code commit・本docs追記いずれにも不混入。**Version/Phase変更なし**（Version1 Final Complete／Version1.1 Connected AI Company開発中・Phase54 Complete維持・Phase55未着手）。**本docs追記工程はdocs更新のみでCode/DB/API変更0・新規push/Tag/Render操作0**）。以前: 追記日: 2026-08-25（Decision 108・**Leader Case Context Phase2 + Option B（Formal Truth / Intelligence Case Data Context Wiring）Code Implementation Complete**。Instagram実運用テストで判明した「保存済み商品情報・Intelligence・APFR Formal Truthが存在するのにWriter/Researcher/Reviewerが『確認できない』と回答し、情報不足のままLeader Finalが根拠のない一般的投稿を生成してReady到達する」不具合を原因調査したうえで解消した。**原因**：本番HEADにはCase Context配線（`buildLeaderCaseContext`／`caseContext`／`hasCaseContext`／`formalTruthRule`）自体が存在せず（working tree専用の未commit差分＝**Leader Case Context Phase2**としてこれまで保護されてきた）、かつLCC Phase2単独でもAPFR Formal Truth・Intelligenceを含まない（IADP由来項目のみ）ことを特定。**LCC Phase2**（既存protected差分の正式化）＝`caseContext`という文字列の共通入力契約をPath A／Path B・OpenAI／Claude両Providerへ配線し`hasCaseContext`で`formalTruthRule`を有効化。**Option B**（新規実装）＝新規純関数`_buildCaseDataContext(expectedCaseId)`をPath A/B共通の唯一の構築口としてclient側へ追加し、既存APFR Resolver（`_apfrResolveCurrentFacts()`）を呼ぶだけでresolvedのみのFormal Truthと6 Intelligenceモジュールのderived/confidence要約を構築、server.jsは新規`_mergeCaseContextText()`で不透明な文字列として連結するのみ（**Resolver再実装0・`.facts`直接走査0・C-1A test 10-6維持**）。payoutの数値変換・approvalRateのscale変換はいずれも行わず保存値を忠実に渡す。IADP必須ガードは変更せず、client側Contextが独立ソースとして`hasCaseContext===true`を成立させることを確認。**Payload**：intelligenceContext全体25,008 Bに対し実測約2,359 B（約1/10.6）。**実AI E2E**（Path B・実案件`case-msr9yckye65y`・本番Draft無変更・API 5回・実測¥11.5）でWriter/Researcher/Reviewer/Strategyの4社員全てでContext到達を実証（Researcherが Formal Truth全21項目を正確に列挙・Reviewerがserver側/client側2ソースを同時引用し植え込んだ捏造4種を全検出）。**生成側3社員の捏造0・Formal Truth矛盾0**。本番Draft`out_1787613075018`のfacts22件・intelligenceContextはE2E前後で完全一致、APFR Fact/EER/IADP変更0。テスト：新規`apfrCaseDataContext.test.js` **80/80 PASS**、既存回帰16スイート全PASS・新規FAIL0。`node --check` OK・`git diff --check` CLEAN。**Code commit `95eaa8990016ea6567a9d6ff1133f44217ad1af2`（`feat: wire case context and apfr formal truth to ai workflow`）済み・push/Tag/Render未実施**。**重要な非依存関係**：**LCC Phase2 + Option B Complete ≠ Leader Final Grounding Complete ≠ Quality Gate Grounding Complete ≠ READY Grounding Complete**。Path A Leader Final実AI E2E・Quality Gate実AI観察・READY実AI観察・`LEADER_FINAL_PROMPT`のfail-closed修正・Quality Gate Grounding Enforcementはいずれも**未検証・未実装**（Path A実行は本番Draftの現在地を切り替える既存動作があるため今回意図的に未実施）。**Version1 Final Complete／Version1.1開発中は変更なし・Phase54 Complete維持・Phase55未着手**。次工程はLCC Phase2 + Option B正式リリース前最終検証（Leader Final GroundingやQuality Gate Groundingへは先に進まない）。新規Decision番号は作成せずDecision108へ追記）。以前: 2026-08-25（Decision 108・**APFR Step C-2-1 正式リリースComplete**。正式リリース前最終検証（C-2-1専用`apfrNumericConsistency.test.js` 53/53・既存回帰15スイート——C-1C-1 42/42・C-1A 48/48・C-1B 62/62・CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・APFR Core 49/49・Manual Input 35/35・Phase0 40/40・EER 51/51・IADP Quality Gate 86/86・IADP Structured 13/13・Evidence Promotion 17/17・Cost Tracker 19/19——全PASS・新規FAIL0・`node --check` OK・`git diff --check` CLEAN・Code commit `9cf7ab92028e4280e83153a3c046a588187aedff`をgit showで直接再確認しLCC混入0（`caseContext`／`autoTaskCaseContext`／`buildLeaderCaseContext`／`hasCaseContext`／`formalTruthRule`／`handleLeaderDispatch`／`triggerStrategyConsolidate`／`triggerLeaderSummary`いずれも0件）／`server.js`・`openaiClient.js`・`claudeClient.js`変更0／`_aicIntegratedScore()`・User Approval・`OUTPUT_STATUS.READY`・`evaluateQualityGate()`・`evaluateComplianceGate()`への機能的変更0（コメント文中の説明的言及のみ）を確認）を実施したうえで、クリーンなlocalhost状態から`dev-check`を実行し**200/200/200を取得**。Release判定＝A（release可能）と判断し、Code commit `9cf7ab92028e4280e83153a3c046a588187aedff` ＋ docs commit `38ac9aa34e963d6168c44f46d010289ab646b11d`の2件を**main push**（`d68658b..38ac9aa`）、**HEAD／origin/main とも `38ac9aa34e963d6168c44f46d010289ab646b11d`（ahead/behind 0/0）**。**正式Tag `v1.01-apfr-formal-truth-numeric-consistency`（Annotated・message「APFR Step C-2-1 Formal Truth Numeric Consistency Check Complete」・target `38ac9aa...`＝リリース前docs正式化commit。既存C-1A・C-1B・C-1C-1と同一の命名・targeting方式を踏襲）を作成しtag push済み**（既存Tagとの命名重複なしを事前確認）。**Render**：main push後の自動Deployで本番反映済み。本番トップ200・`/api/task-history`200・`/api/workflow-dashboard`200を直接curl取得で確認、本番配信コードに`_apfrEvaluateNumericConsistency`／`buildFormalTruthConsistencyHtml`／Output Engine配線（`_oeSafe(buildFormalTruthConsistencyHtml,...)`）がいずれも存在し、LCC Phase2マーカー0件を本番HTML直接検索で確認（起動エラー0）。**本番確認の範囲（区別して記録）**：①**Complete**＝本番エンドポイント3件の200確認・新関数のデプロイ反映確認・Output Engine配線の反映確認・起動エラー0・LCCマーカー0件。②**未実施**＝実案件データによるmatch/mismatch/uncomparable/not_checked表示の本番確認——本番URLが合言葉認証で保護されており認証情報を持たないため実施せず、また不要なAI API課金や本番Fact汚染を避ける方針のため今回はテストデータ登録も行っていない。C-2-1専用テスト53件＋localhost fixture確認（4状態すべて実測・Console Error 0・DB書き込み0）が実関数を直接呼び出して検証済みであることをもって代替し、実運用での確認は今後の実案件登録時に行う（**FAIL扱いにはしない**）。**Non-blocking Contractの維持**：`_aicIntegratedScore()`／score／ranking／recommendation／User Approval／`OUTPUT_STATUS.READY`／`evaluateQualityGate()`／`evaluateComplianceGate()`はいずれも無変更のまま。**正式名称は「APFR Step C-2-1 Formal Truth Numeric Consistency Check」とし、「Formal Truth Score Enforcement Complete」「Intelligence Score Enforcement Complete」とは記録しない**（**C-2-1 Complete ≠ Intelligence Score Enforcement Complete**。mismatch検出結果をscoreへ実際に接続するかはStep C-2-2として引き続き別工程・未着手）。**Data Safety**：DB変更0・API変更0・Supabase設定変更0・APFR Fact変更0。**Version1 Final Complete／Version1.1開発中は変更なし・Phase54 Complete維持・Phase55未着手**。**C-2-2／C-1C-2／C-1C-3／CUI-3／CUI-4はいずれも今回開始していない**。次工程はユーザー承認後に選定する。新規Decision番号は作成せずDecision108へ追記）。以前: 2026-08-25（Decision 108・**APFR Step C-2-1 Formal Truth Numeric Consistency Check Code Implementation Complete**。Product Intelligenceのscore計算が現在使用している`product.inputs`側の数値（`payout`／`epc`／`approvalRate`）と、APFR Formal Truth側の同名Fact（`_apfrResolveCurrentFact()`経由）の数値整合をread-onlyで確認する新規純関数`_apfrEvaluateNumericConsistency(product)`と表示関数`buildFormalTruthConsistencyHtml()`を実装（Code commit `9cf7ab92028e4280e83153a3c046a588187aedff`）。**4状態Contract＝match／mismatch／uncomparable／not_checked**。`payout`は`APFR_FIELD_META.payout.type`が既存Contract上`'string'`（自由記述Formal Truth）でありIntelligence側`number`と型が根本的に異なるため、parseFloat・正規表現抽出・円記号除去等の数値変換を一切行わず常に`uncomparable`とする（既存Contractにない変換規則を新規に作らない）。`epc`・`approvalRate`は両側`number`で厳密数値一致（`===`）のみを採用し、`0.8`と`80`のような自動scale変換は行わない（既存Contract通り）。Resolverが`none`／`ambiguous`のいずれか、またはIntelligence側が`null`の場合は`not_checked`へ統一（C-1C-1と同じ前例踏襲・fail-closed維持）。**`product.facts`直接走査・独自Correction chain判定はいずれも0件**（既存Resolver`_apfrResolveCurrentFact()`のみを消費）。CUI-2による訂正（`A → B supersedes A`）後は追加実装なしで最新current Fact（B）のみへ自動追従することを合成テストで確認済み。**UI**：Output Package Quality（Phase48-1）・Compliance Check（C-1C-1）とは完全に独立した新規パネル「Formal Truth 数値整合確認（Step C-2-1）」をOutput Engineへ追加（`_oeSafe(buildComplianceGateHtml,...)`直後）。**Non-blocking Contract（最重要）**：`_aicIntegratedScore()`／score／ranking／recommendation／Leader判断／User Approval／`OUTPUT_STATUS.READY`／`evaluateQualityGate()`／`evaluateComplianceGate()`（C-1C-1）はいずれも無変更（commit差分内の該当識別子への一致は新規コメント文中の説明のみであることを直接確認）。**判定結果は保存しない（runtimeのみ）**：DB・Output Draft・APFR Fact・Product/ASP/Revenue Intelligence・Intelligence Contextいずれへも保存0。**正式名称は「APFR Step C-2-1 Formal Truth Numeric Consistency Check」とし、「Formal Truth Score Enforcement Complete」「Intelligence Score Enforcement Complete」とは記録しない**（**C-2-1 Complete ≠ Intelligence Score Enforcement Complete**。mismatch検出結果をscore／confidence／warningへ実際に接続するかはStep C-2-2として別設計工程・未着手）。working treeの別系統差分「Leader Case Context Phase2」は、C-1A/B/C-1C-1と同じくHEADベースの合成patch（HEAD:index.htmlへ直接3editを再適用して生成した合成版とHEADとの差分をpatch化し`git apply --cached`でINDEXへのみ適用）によりLCC Phase2を一切経由せず分離し、LCCマーカー（`caseContext`／`autoTaskCaseContext`／`buildLeaderCaseContext`／`hasCaseContext`／`formalTruthRule`／`handleLeaderDispatch`／`triggerStrategyConsolidate`／`triggerLeaderSummary`）0件をcommit差分・INDEX blob双方から直接確認済み。テスト：新規`apfrNumericConsistency.test.js` **53/53 PASS**。既存回帰（C-1C-1 42/42・C-1A 48/48・C-1B 62/62・CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・APFR Core 49/49・Manual Input 35/35・Phase0 40/40・EER 51/51・IADP Quality Gate 86/86・IADP Structured 13/13・Evidence Promotion 17/17・Cost Tracker 19/19）全PASS・**新規FAIL 0**。`node --check` OK（working tree版・INDEX blob版とも）・`git diff --cached --check`／`git diff --check` CLEAN。localhost実機確認（`npm run dev-check`＝`/`200・`/api/task-history`200・`/api/workflow-dashboard`200。fixture注入方式でDB書き込み0・**Console Error 0**）でpayout=uncomparable／epc=match／approvalRate=mismatch／Factなし=全field not_checkedの4状態すべてを確認。**Code commit `9cf7ab92028e4280e83153a3c046a588187aedff`済み・push/Tag/Render未実施**。本docsはこの正式記録のみ・DB/API/Fact変更0。次工程はC-2-1正式リリース前最終検証（**C-2-2へは先に進まない**）。新規Decision番号は作成せずDecision108へ追記）。以前: 2026-08-24（Decision 108・**APFR Step C-1C-1 正式リリースComplete**。正式リリース前最終検証を実施：C-1C-1専用テスト42/42・C-1A 48/48・C-1B 62/62・既存回帰（CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase0 40/40・APFR Core 49/49・Manual Input 35/35・EER 51/51・IADP/Quality Gate 86/86・IADP Structured/Evidence/Cost各PASS）全PASS・新規FAIL0・`node --check` OK・`git diff --check` CLEAN・Code commit `d8e70216`をgit showで直接再確認しLCC混入0／server.js・openaiClient.js変更0／User Approval・READY・packageQuality・evaluateQualityGateへの機能的変更0を確認したうえで、クリーンなlocalhost状態から`dev-check`を実行し**200/200/200を取得**（`/`／`/api/task-history`／`/api/workflow-dashboard`）。Release blocker判定＝A（release可能）と判断し、Code commit `d8e70216` ＋ docs commit `a2bd95a`の2件を**main push**（`650384c..a2bd95a`）、**HEAD／origin/main とも `a2bd95af130d8b7d500e567ba346688cc17c1c21`（ahead/behind 0/0）**。**正式Tag `v1.01-apfr-deterministic-compliance-check`（Annotated・message「APFR Step C-1C-1 Deterministic Compliance Check Complete」・target `a2bd95a...`＝リリース前docs正式化commit。既存CUI-2／C-1A・C-1Bと同一の命名・targeting方式を踏襲）を作成しtag push済み**（既存Tagとの命名重複なしを事前確認）。**Render**：main push後の自動Deployで本番反映済み。本番トップ200・`/api/task-history`200・`/api/workflow-dashboard`200を確認、本番配信コードに`evaluateComplianceGate`／`buildComplianceGateHtml`が存在し、Output Engineパネルへの配線（`_oeSafe(buildComplianceGateHtml,...)`）も本番配信コード内に存在することを確認（起動エラー0）。**本番確認の範囲（区別して記録）**：①**Complete**＝本番エンドポイント3件の200確認・新関数のデプロイ反映確認・Output Engine配線の反映確認・起動エラー0。②**未実施**＝実案件データによるCLEAR／VIOLATION／NOT CHECKED表示の本番確認——本番URLが合言葉認証で保護されており認証情報を持たないため実施せず、また不要なAI API課金や本番Fact汚染を避ける方針のため今回はテストデータ登録も行っていない。C-1C-1専用テスト42件が実関数`evaluateComplianceGate`/`buildComplianceGateHtml`を直接呼び出して検証済みであることをもって代替し、実運用での確認は今後の実案件登録時に行う（**FAIL扱いにはしない**）。**Non-blocking Contractの維持**：`packageQuality`／`evaluateQualityGate()`／`OUTPUT_STATUS.READY`遷移／IADP `canApprove`／User Approval送信処理はいずれも無変更のまま。**C-1C-1 Complete ≠ Compliance Enforcement Complete**（実ブロック接続はStep C-1C-2として引き続き別工程・未着手）。**Data Safety**：DB変更0・API変更0・Supabase設定変更0・timeout変更0・fallback追加0・APFR Fact変更0。**Version1 Final Complete／Version1.1開発中は変更なし・Phase54 Complete維持・Phase55未着手**。**C-1C-2／C-1C-3／CUI-3／CUI-4／APFR Step C-2はいずれも今回開始していない**。次工程はユーザー承認後に選定する。新規Decision番号は作成せずDecision108へ追記）。以前: 2026-08-24（Decision 108・**APFR Step C-1C-1 Deterministic Compliance Check Code実装Complete**。C-1C調査・設計の結論に基づき、Compliance 4fieldのうち機械的に判定可能な`listingNgWords`のみを対象とした非ブロッキングDeterministic Compliance Checkを実装（Code commit `d8e70216e3cf60718651f2d0e8721f3f274d977b`）。新規純関数`evaluateComplianceGate(outputDraft, complianceContext)`はC-1Aが構築した`complianceContext`のみをconsumerとして利用し、APFR `facts`の直接走査・Resolverの直接呼び出しはいずれも0件。NFKC正規化＋lowercase＋部分一致によるdeterministic判定のみを行い、regex・NLP・fuzzy matching等の推測ロジックは実装していない。`clear`／`violation`／`not_checked`の3状態を返し、C-1Aの設計上`none`と`ambiguous`は区別する情報自体が存在しないため両方とも`not_checked`として統一処理（fail-closed維持）。`advertisingDisclosureRequirements`（自然言語要求）・`complianceRestrictions`（意味的制約と禁止語の混在）・`regulatoryCategory`（カテゴリ名自体は違反ではない）・`listingPolicy`（表記統一問題未解決）はいずれも今回のGate対象外（理由は各field個別に明記・Reviewerの責務のまま）。新規`buildComplianceGateHtml()`はOutput Package Quality（Phase48-1）とは完全に独立したパネルとして表示し、「現在は確認表示のみ」の非ブロッキング文言を含む。`packageQuality`／`evaluateQualityGate()`／`OUTPUT_STATUS.READY`遷移／IADP `canApprove`／Executive Decision Engine／User Approval送信処理はいずれも無変更（commit差分に機能コードとしての変更行が存在しないことを直接確認）。判定結果はruntime計算のみで保存しない（DB・Output Draft・APFR Fact・Intelligence Contextいずれへも保存0）。**正式名称は「APFR Step C-1C-1 Deterministic Compliance Check」とし、「Quality Gate Enforcement Complete」とは記録しない**（実際のUser Approval／READYブロック接続はStep C-1C-2として別工程・未着手）。working treeの別系統差分「Leader Case Context Phase2」は、今回のcommitの3 hunkがいずれも削除行0の純粋な追加でありLCC既存5 hunkとの物理的重なりが一切ないことを確認したうえで、HEADベースの合成patchによりhunk単位で完全分離（混入0）。テスト：新規`apfrComplianceGate.test.js` **42/42 PASS**（hit／no hit／none／empty／ambiguous／Correction／NFKC正規化／case-insensitive／array出力／multi-field／duplicate排除／mutation 0／facts・Resolver参照0／packageQuality回帰／evaluateQualityGate回帰／User Approval非ブロック／IADP無回帰／Output Engine配線確認）。既存回帰（C-1A 48/48・C-1B 62/62・CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase0 40/40・APFR Core 49/49・Manual Input 35/35・EER 51/51・IADP/Quality Gate 86/86・IADP Structured/Evidence/Cost各PASS）全PASS・**新規FAIL0**。`node --check` OK・`git diff --check` CLEAN。localhost実機確認（Chrome・fixture注入方式）でCLEAR／VIOLATION／NOT CHECKEDの3表示・**Console Error 0**を確認、確認後はリロードしてfixtureを破棄（DB書き込み0）。**Version1 Final Complete／Version1.1開発中は変更なし・Phase54 Complete維持・Phase55未着手**。次工程＝**C-1C-1正式リリース前最終検証**（dev-check再試行・push判断・Tag判断・Render・本番確認。**C-1C-2へは先に進まない**）。**Code commit済み・push/Tag/Render未実施**。本docsはこの正式記録のみ・DB/API/Fact変更0。新規Decision番号は作成せずDecision108へ追記）。以前: 2026-08-24（Decision 108・**APFR Step C-1A / C-1B 正式リリースComplete**。正式リリース前最終検証を実施：C-1A専用テスト48/48・C-1B専用テスト62/62・既存回帰（CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase0 40/40・APFR Core 49/49・Manual Input 35/35・EER 51/51・IADP/Quality Gate 86/86・IADP Structured/Evidence/Cost各PASS）全PASS・新規FAIL0・`node --check` OK・`git diff --check` CLEAN・LCC Phase2混入0（C-1A/Bとも再確認）を確認したうえで、クリーンなlocalhost状態から`dev-check`を再試行し**200/200/200を取得**（前回セッションで観測されたSupabase応答時間変動によるenvironment Pendingは今回再現せず）。Release blocker判定＝A（release可能）と判断し、Code commit 3件（`9d66525`／`f52511`／`3793752`）を**main push**（`b2dc180..3793752`）、**HEAD／origin/main とも `3793752c179682c7a3f631ec0d5942e872ff79de`（ahead/behind 0/0）**。**正式Tag `v1.01-apfr-compliance-injection`（Annotated・message「APFR Step C-1A/C-1B Compliance Context and Writer Reviewer Injection Complete」・target `3793752...`）を作成しtag push済み**（既存Tagとの命名重複なしを事前確認）。**Render**：main push後の自動Deployで本番反映済み。本番トップ200・`/api/task-history`200・`/api/workflow-dashboard`200を確認、本番配信HTMLに新関数`_apfrBuildComplianceContext`が存在することを確認し新コードの反映を確認済み（起動エラー・contract破損なし）。**本番確認の範囲（区別して記録）**：①**Complete**＝本番エンドポイント3件の200確認・新コード反映確認・起動エラー0。②**未実施**＝Writer/Reviewerへの実際のCompliance Block注入を実AI API呼び出しにより本番確認すること（不要な課金・大量実行を避ける方針のため今回は実施せず、C-1B専用テスト62件が実関数`buildSystemPrompt`/`buildCompliancePromptBlock`を直接呼び出して検証済みであることをもって代替。実運用での確認は今後の実案件登録時に行う）。**Data Safety**：DB変更0・API変更0・Supabase設定変更0・timeout変更0・fallback追加0。**Version1 Final Complete／Version1.1開発中は変更なし・Phase54 Complete維持・Phase55未着手**。**C-1C（Quality Gate Compliance Enforcement）／CUI-3／CUI-4／ITP 7days／listingPolicy注入／APFR Step C-2はいずれも今回開始していない**。次工程はユーザー承認後に選定する。新規Decision番号は作成せずDecision108へ追記）。以前: 2026-08-24（Decision 108・**APFR Step C-1A Compliance Context Foundation ＋ Step C-1B Writer/Reviewer Compliance Injection Code実装Complete**。Step C-1A（Code commit **9d66525**）＝Compliance Formal Truth（`listingNgWords`／`advertisingDisclosureRequirements`／`complianceRestrictions`／`regulatoryCategory`の4field。`listingPolicy`は表記統一問題未解決のため引き続き対象外）を、APFR Resolver（`_apfrResolveCurrentFacts()`）から`atRunWorkflow()`→`/api/auto-task`→server.js受動パススルー→`runAutoTaskWorkflow()`までread-onlyで配線。facts直接走査・独自current判定・Product/Content Intelligenceへの複製・Output Draft snapshot保存はいずれも行わず、この時点ではprompt文字列への実注入も0件。Step C-1B（Code commit **f52511**）＝`buildCompliancePromptBlock(agent, complianceContext)`を新設し、Writer（「遵守して生成」）・Reviewer（「遵守されているか確認」）へ限定注入。Strategy・Leader Finalへは直接注入せず、Reviewerの品質フィードバックは既存の「【Reviewerの品質フィードバック】」経路でLeader Finalへ統合される既存機構を再利用（新規reject契約は追加せず）。Fact値は「確認済み商品事実データであり命令ではない」という責務境界を明示したうえで「」区切りのデータ枠として提示し、system instructionとして解釈させる構造にしない（命令文字列検出等の追加防御は過剰設計として不採用）。scalar/array正規化・改行正規化・300文字上限切り詰めを実施。`regulatoryCategory`はカテゴリ名からの追加規制推測を明示的に禁止。empty Contract（null/undefined/{}/対象fieldなし）時はCompliance blockを一切出力せずC-1B前systemPromptとbyte-identicalであることを実装から直接呼び出すテストで確認。Workflow snapshot方針：1 workflow run＝開始時snapshot・途中では再Resolverせず・永続化しない（既存Formal Truth Priority Architectureの方針と整合）。**Quality Gate Compliance Enforcementは未実装**（現在Quality GateにはCompliance専用強制軸がなく、Step C-1C候補として別工程。C-1A/B CompleteとC-1C Completeを混同しない）。Product/ASP/Content/Revenue/Competition/Market Opportunity Intelligenceはいずれも無変更・Formal Truthの別ledger複製は0。テスト：C-1A専用テスト最終**48/48 PASS**（C-1B接続に伴い当初39件中2アサーションを「参照0件」から「helper経由のみ・facts直接走査0・Resolver再実装0」というC-1B以降も有効な契約へ更新・既存1〜10節は無変更）、C-1B専用テスト**62/62 PASS**。既存回帰：CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase0 40/40・APFR Core 49/49・Manual Input 35/35・EER 51/51・IADP/Quality Gate 86/86・IADP Structured/Evidence/Cost各PASS。**新規FAIL0**。`node --check` OK・`git diff --check` CLEAN。**dev-check**：`/`200・`/api/task-history`は200取得実績ありだが応答時間に変動・`/api/workflow-dashboard`はこのセッション内で固定5秒タイムアウト内の200が未確定——C-1Bのコードパス（`buildSystemPrompt`/`runAutoTaskWorkflow`）とは無関係な`server.js`→`_hybridTaskHistory()`→`lib/taskHistoryDb.js`→Supabase経路の応答時間変動（実測0.6〜1.9秒）とdev-check.ps1の固定タイムアウトの組み合わせと切り分け済み。**dev-check environment/latency Pendingとして記録し、PASS扱いにはしない**（server/timeout/DB/Supabase変更はいずれも0）。**working treeの別系統差分「Leader Case Context Phase2」はC-1A/Bとも一切混入していない**（HEADベース合成patchによるhunk単位分離・`git show`で直接確認済み）。**Version1 Final Complete／Version1.1開発中は変更なし・Phase54 Complete維持・Phase55未着手**。次工程＝**C-1A + C-1B正式リリース前最終検証**（dev-check再試行・push判断・Tag判断・Render・本番確認。**C-1Cへは先に進まない**）。**Code commit 2件（`9d66525`／`f52511`）済み・push未実施・Tag未作成・Render未反映**。本docsはこの正式記録のみ・DB/API/Fact変更0。新規Decision番号は作成せずDecision108へ追記）。以前: 2026-08-24（Decision 108・**APFR Correction UI Core 本番認証後最終目視確認Complete**。ユーザー本人が本番URLへ認証後ログインし、実ブラウザでCorrection UIを目視確認した：①resolved Fact行（例：広告表示義務）に「訂正」ボタンが表示されていることを確認、②「訂正」押下でCorrection modeへ正常遷移したことを確認、③Correction mode内に現在の値（Current Fact）が正常表示されることを確認、④訂正対象fieldが固定表示され他fieldへの変更ができないことを確認、⑤新しい値の入力欄が**空の状態**で表示され旧値の自動コピーがないことを確認、⑥「訂正をやめる」ボタンが正常表示されることを確認、⑦押下後は通常の「情報を1件登録」モードへ正常復帰しCorrection Targetの画面残留がないことを確認。**今回「確認済みFactとして登録」は押していないため、Correction Fact登録0・本番Fact変更0・DB書き込み0を維持**。**Console Errorの追加実測は今回提供されたスクリーンショット確認では未実施**（画面動作の目視Completeとは区別して記録する。既存テスト・localhost実機確認でのConsole Error 0記録は不変・変更なし）。これにより**Correction UI Core系列（CUI-0〜CUI-2）に残っていた唯一のリリース確認Pending＝本番認証後の実ブラウザ最終目視確認がComplete**となり、**Correction Contract／Current Fact Resolver／CUI-0／CUI-1／CUI-2／Correction UI Core正式リリース／本番認証後最終目視＝すべてComplete**。**Correction UI Core系列に残るリリース確認Pendingは0**。**次工程＝APFR Correction UI後の次工程選定・調査**（CUI-3／CUI-4／ITP `7days` field／APFR Step Cの必要性を比較する工程。いずれも未着手・ユーザー承認なしに自動開始しない）。**本docs更新のみ・Code変更0・DB変更0・API変更0・Fact登録0・新規Tag作成0・既存Tag変更0・Render操作0・Leader Case Context Phase2不混入**。新規Decision番号は作成せずDecision108へ追記）。以前: 2026-08-24（Decision 108・ASP Product Fact Record（APFR）・**APFR Correction UI Core CUI-0〜CUI-2 正式リリースComplete**。Code commit **fd99134**・docs commit **186ec63** を**main pushし、HEAD／origin/main とも `186ec6371676e0ad9ab49368f2899bf9e4155f90` へ同期（ahead/behind 0/0）**。**正式Tag `v1.01-apfr-correction-ui-core`（Annotated・target `186ec637...`・message「APFR Correction UI Core CUI-0 to CUI-2 complete」）を作成しtag push済み**。**Render**：main push後の通常Deployで本番反映済み（本番トップ200／`/api/task-history`200／`/api/workflow-dashboard`200）。本番配信コードに`_apfrCorrectionTargetFor`／`_apfrStartCorrection`／`_apfrCancelCorrection`／`_apfrValidateCorrectionTarget`／`_apfrBuildCorrectionHeaderHtml`がいずれも存在し、`buildLeaderCaseContext`は本番0件のまま（**LCC Phase2は引き続き本番未リリース**）。**本番検証（区別して記録）**：①**Complete**＝本番配信コードを本番Supabase実データ（`case-msr9yckye65y`／プラファスト22 Fact）へ**read-onlyで適用**し、Resolver結果`resolved`21／`none`0／`ambiguous`0、訂正ボタン21件（`disabled`混入なし）、`listingNgWords`でのCorrection mode生成・submit直前検証の正常動作・History22件既定閉・**facts不変・DB書き込み0**を確認。②**Pending**＝本番URLが合言葉認証画面で停止しており認証情報未提供のため、**認証後の実ブラウザ操作による最終目視確認のみ未実施**（Correction UI正式リリースの失敗ではない。「本番ログイン後の実画面で確認済み」とは記録しない）。テスト（CUI-2 105/105・CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase 0 40/40・APFR Core 49/49・Manual Input UI 35/35・EER 51/51・IADP/Quality Gate 86/86・IADP Structured 13/13・Evidence 17/17・Cost 19/19、新規FAIL 0、dev-check 200/200/200、`git diff --check` CLEAN）は正式リリース前に再確認済み。**Data Safety**：Correction Fact登録0・本番Fact変更0・DB schema変更0・API変更0・`server.js`変更0・Render設定変更0・環境変数変更0。**正式現在地＝Correction Contract／Current Fact Resolver／CUI-0／CUI-1／CUI-2すべてComplete・APFR Correction UI Core CUI-0〜CUI-2＝正式リリースComplete**。次工程＝**本番認証後のCorrection UI最終目視確認**（ユーザー本人が実施。Correction Fact登録ボタンは押さない）。新規Decision番号は作成せずDecision108へ追記。**本docs更新のみ・Code/DB/API変更0・新規push/Tag/Render操作0・Leader Case Context Phase2不混入**）。以前: 2026-08-23（Decision 108・**CUI-2 Correction UI Core 正式化Complete**。Code commit **fd99134**（`feat: add apfr correction ui core`・`index.html` +165/-5・`apfrCorrectionUi.test.js` 新規804行）。**CUI-2は、Resolverで`resolved`となった現在Factをユーザー操作によって正式なCorrection Recordとして訂正するUI**である。正式フローは`Resolver`→`resolved` current Fact→Current Fact UI「訂正」→**Correction Target**（`_apfrCorrectionTarget`＝`caseId`/`productIdentifier`/`field`/`currentFactId`の4項目のみ・**Fact本文は保持せずFormal Truthの複製を作らない**）→Manual Input UIの訂正モード→**submit直前のResolver再検証**→`supersedesFactId`自動付与→**既存APFR Core**→append-only保存→Resolver再評価→新Factがcurrent／旧FactはHistory。**status別契約**：`resolved`のみ訂正可（対象はResolverの`currentFact`のみ）／`none`はCorrectionでなく通常新規登録で`supersedesFactId`を付けない／**`ambiguous`は訂正ボタン非表示・Correction禁止・候補の代表選択なし**（fail-closedをUIでも維持）。**stale Target対策（重要）**：開始時だけでなく**submit直前にもResolverを再実行**し`status==='resolved' && currentFact.factId === target.currentFactId`を要求。不一致時は**登録停止・append 0・Target破棄**とし、**古いFactへ訂正をつないで`branched_chain`を生成する事故を防止**（実測確認済み）。**scope安全性**：Cross-case／Cross-product／Cross-field をいずれも禁止し、fieldは**UI固定（`selected`＋`disabled`）＋submit時の一致再検証の二重防御**。product scope比較は既存の`String()`厳密比較を流用。**append-only維持**（旧Factの編集・削除・置換・`superseded:true`等のmutationは実装なし）。**CUI-0のduplicate policyをそのまま再利用**しCUI-2独自判定は0（検証順序は stale再検証→Core duplicate判定。**stale時はTarget破棄／duplicate等の通常failure時はTarget保持**して再試行可能）。**User Verification維持**（Correction専用Validaterなし・`manual_user_input`やチェック未了は訂正でもfact昇格しない・**AI自動訂正禁止**）。`supersedesFactId`の代入はコード全体で**1箇所のみ**、かつ**再検証を通過したTargetからのみ**。追加state 1・追加helper 5（`_apfrCorrectionTargetFor`／`_apfrStartCorrection`／`_apfrCancelCorrection`／`_apfrValidateCorrectionTarget`／`_apfrBuildCorrectionHeaderHtml`）・変更関数5（`_apfrBuildCurrentFactsHtml`／`_apfrBuildPanelHtml`／`_apfrOnFormChange`／`_apfrRegisterFromUi`／`_apfrBuildFieldOptionsHtml`の任意引数対応）で、**Core 3関数・Resolver 4関数・`_apfrBuildHistoryHtml`はいずれも無変更**。新規テスト`apfrCorrectionUi.test.js` **105/105 PASS**（22ケース群）、既存回帰全PASS（CUI-1 78/78・CUI-0 65/65・Resolver 70/70・Phase 0 40/40・APFR Core 49/49・Manual Input UI 35/35・EER 51/51・IADP Quality 86/86・IADP Structured 13/13・Evidence 17/17・Cost 19/19）・**新規FAIL 0**・`node --check` OK・`git diff --check` CLEAN・dev-check 200/200/200・localhost合成fixtureで**Console Error 0**および通常Manual Input UI無回帰を確認。**実DBへのCorrection Fact登録0件・本番`case-msr9yckye65y`の22 Factは無操作**。**DB変更0・API変更0・`server.js`変更0**。新規Decision番号は作成せずDecision108へ追記。残課題1は「**Correction UI Core系列（CUI-0〜CUI-2）の実装として解消**」へ更新。**⚠ CUI-2 Completeは「Correction UI Coreの完成」であり、CUI-3／CUI-4／Step C／Product・ASP Intelligence接続／EEA問題／Quality Gate・Hold問題／Leader Case Context Phase2 のCompleteをいずれも意味しない**。**Correction UI Core系列（CUI-0〜CUI-2）は実装Complete。ただし正式Tag・正式リリースは未実施**。**本docs更新のみ・Code変更0・DB/API/Fact変更0・Code push未実施・docs push未実施・Tag未作成・Render未操作・Leader Case Context Phase2不混入**）。以前: 2026-08-23（Decision 108・**CUI-1 Current Fact / History UI 正式化Complete**。Code commit **1cf3b2e**（`feat: add apfr current fact ui`・main push済・Render自動Deploy反映確認済・**Tag未作成**）。Step B以降「登録済み情報」が`product.facts`を全件フラット表示していたため、本番`listingNgWords`で旧`["法人名"]`と訂正`["商品名","法人名"]`が同じ見た目で並び**どちらが現在値か画面上で判断できない**状態だった問題を解消した。**Current Fact UI Contract**として、現在値一覧は**`_apfrResolveCurrentFacts(product)`の結果のみを使用**し、UI側が配列末尾・`recordedAt`最新・`sourceMethod`・`value`・配列順から独自にcurrentを決めることを**禁止**（**ResolverはUIにおいてもcurrent判定の唯一の口**。実装の静的検証をテストへ組み込み）。`resolved`のみ`currentFact`を「現在の情報」に表示し**旧Factを混在させない**、`none`は「○ 未登録」で21フィールドの充足状態を可視化、`ambiguous`は**`currentFact`を表示せず**理由と候補件数のみ示し**候補の代表表示もしない**（Ambiguity Contractのfail-closedをUIでも維持）。**全件フラット表示を廃止し「現在値一覧＋折りたたみHistory（`<details>`・既定閉）」へ分離**。Historyはappend-onlyの全Factを保持表示し、「現在値」「過去の記録」の区別は**Resolver結果から動的導出のみ**で**旧Factへ`status:'superseded'`等を保存しない**。通常表示は21行固定のため**Factが増えても通常画面が履歴件数に比例して長くならない**。**boolean日本語表示**を同時実装（表示・入力selectのlabelとも日本語化。`option value`は`'true'`/`'false'`のまま、**保存値は`boolean`のまま不変**。`productLinkAvailable`＝利用可/利用不可、`reviewRequired`＝あり/なし、`mobileOptimized`・`itpSupported`・`linkManagerSupported`＝対応/非対応）。`listingPolicy`の`"一部ok"`は**normalizeせず保存値のまま表示**（訂正は将来Correction UIでユーザー本人が実施）。**listingNgWords実例**：現在値一覧は`["商品名","法人名"]`のみ、Historyは旧・新2件保持、**旧FactのDB変更・削除・mutationは0件**。CUI-1専用テスト**78/78 PASS**、既存回帰全PASS（CUI-0 65/65・Resolver 70/70・Phase 0 40/40・APFR Core 49/49・Manual Input UI 35/35・EER 51/51・IADP Quality 86/86・IADP Structured 13/13・Evidence 17/17・Cost 19/19）・**新規FAIL 0**・dev-check 200/200/200・localhost実機で**Console Error 0**。旧`_apfrBuildFactsListHtml()`は完全置換（残存参照0件）。新規Decision番号は作成せずDecision108へ追記（既存Resolver Contractの可視化工程のため）。あわせて**陳腐化した2記述を訂正**（Phase 1の「Resolver UI未接続」→CUI-1で表示専用接続済み／Step C開始条件の「UI・Intelligence・score計算のいずれにも未接続」→UIへは接続済み・Intelligence/scoreへは未接続）、Step Bの「登録済みFact一覧」にCUI-1で置換された旨の参照を追加。残課題3（boolean日本語表示）は**CUI-1でComplete**へ更新、残課題1は「CUI-1 Complete／**CUI-2 Correction UI Core 未実装**」へ更新、残課題2・4〜9は未解決のまま維持。**docs更新のみ・Code/DB/API/Fact変更0・Tag未作成・push未実施・Render追加操作なし・Leader Case Context Phase2不混入**）。以前: 2026-08-22（Decision 108・**CUI-0 Correction-aware Duplicate Policy 正式化Complete**。Code commit **9ad76f8**（`fix: support apfr correction duplicates`）。Phase 1で正式化したCorrection Contractが表現する多段訂正のうち、`A(value=1)`→`B(value=2, supersedes A)`→`C(value=1, supersedes B)`という**「元の値への正式な差し戻し訂正」が`_apfrRecordsEqual()`の比較に`supersedesFactId`が含まれていなかったため`duplicate_record`で誤拒否されていた**問題を解消した。**`supersedesFactId`をduplicate identityへ追加**し、`C`は`A`と`value`が同じでも訂正関係が異なるため別の正式Correction Recordとして扱う。一方で**全9項目（`caseId`/`productIdentifier`/`aspName`/`field`/`classification`/`sourceMethod`/`sourceReference`/`value`/`supersedesFactId`）が一致する完全同一Correction Recordは従来どおり拒否**し、**duplicate防止自体は弱めていない**。未設定は既存`aspName`／`sourceReference`と同じ`|| null`方式に揃え、**property未存在／`undefined`／`null`／`''`をすべて「訂正関係なし」として同一扱い**、**通常Record同士のduplicate判定はCUI-0以前から不変**（CUI-0前実装との機械比較で全差分パターン一致を確認）。**chain異常（orphan／self／cross-field／cross-case／cross-product／branched／circular）の判定はduplicate関数の責務ではなくPhase 1 Resolverが`ambiguous`＋`currentFact:null`のfail-closedで処理する**責務境界を明記（orphan参照Recordは登録が通るがResolverが`orphan_reference`でambiguousを返しFormal Truth読み取りは汚染されないことを実測）。**append-only原則は不変**で旧Factのmutation・削除・`superseded`書き込みは一切なく、`A→B→C`はFact総数3件として全件保持。CUI-0専用テスト**65/65 PASS**（差し戻し訂正成功・Resolverが`explicit_chain`でCをresolved・recordedAt逆順でもchain優先・4段訂正整合・二重登録拒否維持）。既存回帰全PASS（Resolver 70/70・Phase 0 40/40・APFR Core 49/49・Manual Input UI 35/35・EER 51/51・IADP Quality 86/86・IADP Structured 13/13・Evidence 17/17・Cost Tracker 19/19）・**新規FAIL 0**・dev-check 200/200/200。main push済み・**Render自動Deployにより本番配信コードへの反映を確認済み**。新規Decision番号は作成せずDecision108へ追記（既存Correction Contractのduplicate policy側の補完のため）。残課題1は「Correction Contract／Resolver／Duplicate Policy＝Complete、**Correction UI＝未実装**」へ更新、残課題2〜9は未解決のまま維持。**docs更新のみ・Code/DB/API/Fact変更0・CUI-0用Tag未作成・push未実施・Render手動操作なし・Leader Case Context Phase2不混入**）。以前: 2026-08-22（Decision 108・**Phase 0 再Adopt時Fact消失防止＋Phase 1 Current Fact Resolver Contract 正式化Complete**。**Phase 0**（Code commit **d69ff60**）＝商品再Adopt時に`product`が丸ごと置換され登録済みFactが全消失する潜在的データ損失リスクを`_apfrCarryOverFacts()`で解消。**同一caseId かつ 同一productIdentifier のみcarry-over**・Cross-case/Cross-product は0・deep clone・入力非破壊・順序と訂正履歴を維持。合成テスト40/40 PASS。**Phase 1**（Code commit **46c51ef**）＝read-only純関数`_apfrResolveCurrentFact()`／`_apfrResolveCurrentFacts()`を追加し、**Current Fact Resolver Contract／Correction Contract／Ambiguity Contract（fail-closed）／Legacy Fallback／Step C開始条件**を正式化。解決順序は①明示訂正chain（`supersedesFactId`）優先 → ②明示関係が皆無の場合のみ`recordedAt`最大 → ③一意決定不能は`ambiguous`＋`currentFact:null`。母集団はcaseId/productIdentifier/field完全一致かつ`validateApfrRecord()` validのみ（Cross-case・Cross-product・invalid Factは除外）。**明示chainと独立legacyの並存は`multiple_chain_terminals`でambiguous**（どちらも勝手に選ばない）。**timestamp collisionは恣意的tie-breaker（factId辞書順・配列順・sourceMethod・value）を使わずambiguous**。**sourceMethodによる自動優先順位は設けない**。ambiguous reasonは12種を実装・記録。**既存プラファスト22 Factはmigration不要**で`listingNgWords`はlegacy fallbackにより新Fact`["商品名","法人名"]`をresolved、旧Factは`candidates`に残存。本番相当fixture（21 field/22 records）で**resolvedCount=21・noneCount=0・ambiguousCount=0**。合成テスト70/70 PASS・実装との等価性を機械比較で確認。既存回帰（APFR Core 49/49・Manual Input UI 35/35・EER 51/51・IADP Quality 86/86・IADP Structured 13/13・Evidence 17/17・Cost Tracker 19/19）**全PASS・新規FAIL 0**・dev-check 200/200/200。**Resolverはread-only・UI未接続・Step C未接続・DB書き込み0**。本Contractは既存Decision108のenum・Fact昇格条件・保存先・責務境界・duplicate policy・append-onlyをいずれも変更しない読み取り規則の補完のため**新規Decision番号は作成せずDecision108へ追記**と判断。残課題1は「Contract明文化Complete／Correction UIは未実装」へ更新し、残課題2〜9は未解決のまま維持。**本追記はdocs更新のみ・Code/DB/API/Fact変更0・Tag/Push/Render未実施・Leader Case Context Phase2不混入**）。以前: 2026-08-22（Decision 108・**プラファスト本番実運用検証Complete**。対象実案件`case-msr9yckye65y`／productIdentifier`["プラファスト","a8.net"]`へユーザー本人が21フィールドを1件ずつ登録し、21/21カバー・Fact総22レコード（`listingNgWords`訂正履歴1件含む）・Contract違反0件・Cross-case混入0件・Persistence確認済み・IADP 100/Complete／Quality Gate Passed／Reviewer Passed／Strategy Accepted／User Approval Approved／EER 3件executed／Evidence 9件の無回帰をすべて実測確認。AI推測によるFact昇格0件・`manual_user_input`単独からのFact昇格0件。Claude Codeは読み取り専用確認のみでFact登録0件。残課題9件を分離記録。**docs commitのみ・Code/DB/API変更0・Tag/Push/Render未実施**）。以前: 2026-08-21（**Contract設計正式化＋Step A Core（Code commit 3113e53）／Step B Manual Input UI（Code commit 1e8de4f）正式リリースComplete**。実商品APFR登録0件・プラファスト未登録・localhost実機検証済み・既存回帰なし。docs release commit・Annotated Tag・main push・tag push・Render反映まで完了）。以前: **Contract設計正式化Complete（コード未実装）**。EER-1〜EER-4完了時点のIADP/Quality Gate/Evidence/User Approval/Account Creation Readiness/External Executionはいずれも無変更。docs commitのみ・Tag/Pushなし。
