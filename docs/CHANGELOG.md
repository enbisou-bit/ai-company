...
## ASP Product Fact Record（APFR）─ Step A Core／Step B Manual Input UI **正式リリース**（2026-08-21）／プラファスト **本番実運用検証Complete**（2026-08-22）／**Phase 0 再Adopt時Fact消失防止＋Phase 1 Current Fact Resolver＋CUI-0 Correction-aware Duplicate Policy＋CUI-1 Current Fact / History UI＋CUI-2 Correction UI Core**（2026-08-22〜23・Decision108）
...
- **残課題（Completeとは分離）**：①同一fieldへの複数Fact存在時の**正本選択ContractはPhase 1で正式明文化・実装Complete、Correction-aware Duplicate PolicyはCUI-0でComplete、Current Fact・History UIはCUI-1でComplete、`supersedesFactId`を付与するCorrection UI CoreはCUI-2でComplete**。すなわち**Correction UI Core系列（CUI-0〜CUI-2）は実装Complete**（現在地の内訳＝Correction Contract／Resolver／Duplicate Policy／Current Fact・History UI／Correction UI Core＝すべて**Complete**）。**ただし正式Tag・正式リリースは未実施**で、実データ上の`listingNgWords`は引き続きlegacy fallbackで解決（後付けmigrationは行わない方針を維持）②③**boolean値の日本語表示はCUI-1でComplete**（残課題から解消）②ITP「7days」の専用保存field不在（`itpSupported`はbooleanのみ）③boolean値の日本語表示④`listingPolicy`「一部ok」の表記統一⑤APFRフィールド選択UI（巨大select・スクロール・検索性）⑥APFR直接ジャンプ導線⑦入力省力化（21項目1件ずつの負担）⑧EEA問題（Decision101・別途未解決）⑨Quality Gate／Hold制御問題（別問題）。**APFR実運用Complete≠EEA問題Complete／≠Quality Gate・Hold問題Complete**。
