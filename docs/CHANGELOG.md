...
- **Render**：Service `ai-company`（branch `main`・Trigger Auto-Deploy）＝**Build successful／Deploy succeeded／Status Live**（Source `02f8f6c`）。Phase 2-Bで追加したnative依存 **sharp ^0.35.4** と **opentype.js ^2.0.0** のRender/Linux本番buildが初めて成功し、**sharp／opentype.js由来のblocking errorなし**。
- **本番挙動の変更はLeader Final 2件のみ**：`d164e6e`＝Reviewer/Strategyが指示したCompliance上の削除・言い換えを、Formal Truth記載を理由に上書きしないことをLeader Finalプロンプトへ明文化。`2f478b2`＝post-process feedbackのtruncate上限を1200→2400へ拡張（Reviewer全文 約2118字の【修正指示】ブロックが切断されLeader Finalへ未達だった実測への対策）。
- **非接続（dormant）**：carousel image production一式は `server.js`（`carousel` 参照0）・`index.html`（同0）から未接続。**real image API call 0／paid generation 0／`REAL_ENABLED=false`／`generated/` 不存在**をリリース後も維持。本リリースは「画像生成機能の本番稼働」ではなく、**課金前guard構造とdeterministic描画基盤を本番ブランチへ正式に載せた段階**である。
- **Phase 2-E前提（Deferred A〜D・未解決）**：(A) `consumeNonce()` がguardから未接続（1 approval token＝1 nonceがN枚を一括承認する構造のため、consume不在では同一tokenの並行実行で二重課金が成立しうる）／(B) input text token costが未算入（repo内にinput価格定数なし・`actualUsdFromUsage()` もoutput-only・`result.actualUsage` 未配線）／(C) quality空文字がplan側でmediumへ暗黙fallback（real pathはguardが `ctx.quality` を検証するため既にfail-closed）／(D) nonce storeがin-memoryでrestart・deploy・crashで消失しmulti-instance非共有。**`REAL_ENABLED=true` 解禁にはGate 1（実装）／Gate 2（正式仕様確定）／Gate 3（OpenAI公式のinput token価格確認）／Gate 4（ユーザー承認）が必要**。詳細は docs/02PHASE_PROGRESS.md。
- **Version1 Final Complete 維持／Version1.1 Connected AI Company 開発中 維持／Phase54 Complete 維持／Phase55 未着手**。**新Decisionなし**。

---

## ASP Product Fact Record（APFR）─ Step A Core／Step B Manual Input UI **正式リリース**（2026-08-21）／プラファスト **本番実運用検証Complete**（2026-08-22）／**Phase 0 再Adopt時Fact消失防止＋Phase 1 Current Fact Resolver＋CUI-0 Correction-aware Duplicate Policy＋CUI-1 Current Fact / History UI＋CUI-2 Correction UI Core**（2026-08-22〜23）／**APFR Correction UI Core CUI-0〜CUI-2 正式リリースComplete**（2026-08-24・Decision108）／**本番認証後Correction UI最終目視確認Complete**（2026-08-24・Decision108）／**Step C-1A Compliance Context Foundation ＋ Step C-1B Writer/Reviewer Compliance Injection 正式リリースComplete**（2026-08-24・Decision108・Tag `v1.01-apfr-compliance-injection`）／**Step C-1C-1 Deterministic Compliance Check 正式リリースComplete**（2026-08-24・Decision108・Tag `v1.01-apfr-deterministic-compliance-check`）／**Step C-2-1 Formal Truth Numeric Consistency Check 正式リリースComplete**（2026-08-25・Decision108・Tag `v1.01-apfr-formal-truth-numeric-consistency`）／**Leader Case Context Phase2 + Option B Code Implementation Complete**（2026-08-25・Decision108・Code commit `95eaa899`）／**Leader Final Grounding Option F Code Implementation Complete**（2026-08-26・Decision108・Code commit `04e28a08`）／**APFR Step C-1C-1b Advertising Disclosure Detection Code Implementation Complete**（2026-08-26・Decision108・Code commit `e23c0df8`）／**APFR Step C-1C-2a Compliance Assessment Aggregation Code Implementation Complete**（2026-08-27・Decision108・Code commit `659e82ce`）／**APFR Step C-1C-2b-1 Mobile Approval Enforcement Code Implementation Complete**（2026-08-27・**新規Decision 109**・Code commit `46b37dc2`）／**APFR Step C-1C-2b-2 IADP Approval Enforcement ─ Not Required正式化**（2026-08-27・Decision109追記・Code変更0）／**APFR Step C-1C-2a-1 Compliance UI Scope Correction 正式リリースComplete**（2026-08-27・Decision108追記・Code commit `112dafd6`・stage-1 docs commit `a97109bb`・main push済み・Annotated Tag `v1.01-apfr-compliance-ui-scope-correction`（target `a97109bb`）push済み・Render反映確認済み〔本番3endpoint 200・本番 `index.html` とローカルHEAD byte-identical〕・最新 `leaderFinalGrounding.test.js` 52/53 PASS〔FAIL `20-2a` のみ・既知制約・53/53 PASSとは記録しない〕・Compliance Enforcement全体／Quality Gate Grounding／READY Grounding Completeとは記録しない）／**APFR Step C-3-1 Grounding Detection ─ Not Required 正式化**（2026-08-28・Decision108追記・docs-only stage-1・Code変更0・実測15件中 Class A 0件／B 12件／C 3件・Formal Truth 21field中 deterministic対象0件・claim extraction不成立・`clear`／`supported`等は新設しない・C-3系列〔C-3-1〜C-3-3〕は実装系列として取り下げ・Leader Final Grounding＝B／Option F E2E＝A維持・Grounding Complete等とは記録しない・Quality Gate／READY／Mobile Approval／C-2-1 変更0・C-2-2 未着手）／**APFR Compliance Restrictions deterministic化 ─ Not Required 正式化**（2026-08-28・Decision108追記・docs-only stage-1・Code変更0・判定C・実案件 `complianceRestrictions` 4値すべて directive／obligation・Type A/B 実データ0件・substring実測で残存3表現 検出0件〔false negative 100%〕かつ遵守表明／正しい開示を violation 誤検出・`広告表示必須` は positive obligation で意味逆転・SoT分離〔`listingNgWords`＝deterministic禁止語／`complianceRestrictions`＝自然言語制約〕・C-1C-1判断は維持・`_apfrEvaluateComplianceAssessment()`／Mobile Approval／Quality Gate／READY 変更0・`listingNgWords` チャネル意味ズレは別課題・Option D 完全採用とは記録しない・Compliance Complete／Compliance Enforcement Complete とは記録しない）／**Instagram実運用 P1〜Issue A/B 修正 4commit ─ 実AI Path A E2E PASS・docs正式化 Stage-1 → 正式リリースComplete**（2026-08-29 Stage-1／2026-08-30 Stage-2・Decision108追記・Code commit `ba4d82e`／`c35b534`／`d8a09f1`／`08d15e3`＋docs commit `e935056`・**main push済・Annotated Tag `v1.01-instagram-p1-issue-ab-release`（local／remote）・Render Auto-Deploy `e935056` Live（2026-08-30 07:40 GMT+9）・本番UI確認済**・P1-1 caseId Binding／listingNgWords Channel Scope／IADP Scope Boundary（Option E＋B）／Issue A Option D の各実AI E2E PASS・Mobile Approval Enforcement正常・Class BハッシュタグはP3既知事項・`leaderFinalGrounding` 52/53〔FAIL `20-2a` のみ・53/53とは記録しない〕・Compliance Enforcement完全完成／Fully Compliant／Grounding Enforcement Complete とは記録しない）／**Leader Final gpt-5.4-mini Blocking Fix — docs-only Stage-1 → 正式リリースComplete**（2026-08-31 Stage-1／2026-08-31 Stage-2・Decision108追記・Code commit `41634f3`／`6b9b2ca`＋docs commit `4f1fef2`・**main push済（HEAD＝origin/main＝`4f1fef23b141de0a0b585efd7fb5df83a8ef43b6`・ahead/behind 0/0）・Annotated Tag `v1.01-leader-final-model-blocking-fix`（object `2ba54d0ae1f500726f1a0ba8514b936298000376`／peeled `4f1fef2`・local／remote）・Render Auto-Deploy `4f1fef2` Live（2026-08-31 07:52:51 GMT+9・19.0s・Render Dashboard実画面確認済）・本番read-only health確認 PASS**・実AI Path A E2E 1回で未確認商品事実0件を実測〔deploy前〕・本番実AI E2E確認済／本番でgpt-5.4-mini動作を再実測済／Blocking解消Complete／Grounding Complete／deterministic guarantee とは記録しない）／**Instagram Grounding修正3件 docs catch-up（Output Draft Parser P2 `3d2a28a`／Caption Wiring `88939c9`／Mobile Review Grounded Wiring `82a1e24681064c19104dc2cf0cc1dc41318f5ab0`・いずれもStage-2完了済＝push済・Render Live・本番確認済・現production baseline origin/main=`82a1e24681064c19104dc2cf0cc1dc41318f5ab0`）＋APFR Product Content Formal Truth Extension（Code commit `db833f27cbc6a0b8394cc371357065f213765bff`・Stage-1＝未push・未Render・本番未反映・Decision108追記・新Decisionなし）**（2026-09-02・`APFR_FIELD_META`／`APFR_FIELD_ORDER` へ `productDescription`:string／`productFeatures`:array／`productUsage`:string／`targetUser`:string を追加のみ・既存21 field不変・`APFR_FIELD_ORDER`=25 field・変更関数0・Fact昇格Contract不変〔`a8_screen_user_verified`／`advertiser_lp_user_verified`＋`user_verified`＋`verifiedBy:user` 必須・`manual_user_input`単独／`web_retrieved`はfact不可〕・Compliance責務分離維持・DB/API/schema/migration 0・新規 `apfrProductContentFields.test.js` 68/68＋`apfrListingScope.test.js` 1 assertion同期・deterministic対象10 test PASS・新規FAIL 0・`leaderFinalGrounding` 52/53〔`20-2a`のみ既知FAIL・53/53とは記録しない〕・実データ `case-msr9yckye65y` resolved21/none4/ambiguous0・既存22 Fact mutation 0・新4 field Fact登録は未実施・21/21も25/25もComplete Gateではなく表示summary・Quality Gate/READY/Approval/IADP status変更なし・Phase54 Complete維持/Phase55未着手・HEAD `db833f27cbc6a0b8394cc371357065f213765bff`／origin/main `82a1e24681064c19104dc2cf0cc1dc41318f5ab0`／ahead1 behind0・Grounding完全保証/Fully Compliant/投稿品質保証 等とは記録しない）
...
- **Leader Final 実行モデル gpt-4.1-nano → gpt-5.4-mini（Blocking Fix・docs-only Stage-1 → 正式リリースComplete）**（2026-08-31・Decision108追記・Code commit `41634f3`／`6b9b2ca`＋docs commit `4f1fef2`・main push済・Annotated Tag `v1.01-leader-final-model-blocking-fix`・Render Auto-Deploy `4f1fef2` Live（19.0s・Render Dashboard確認済）・本番read-only health確認 PASS）：実AI Path A で Reviewer否認・`LEADER_FINAL_REVIEWER_REJECT_RULE`・formalTruthRule・IADP Scope Boundary がLeader Finalへ供給済みだったにもかかわらず旧 gpt-4.1-nano が未確認商品事実を再生成したため、Contract・Prompt無変更で通常Leader Finalの実行モデルだけを最小変更で引き上げた（`LEADER_FINAL_MODEL='gpt-5.4-mini'`・`OPENAI_MODEL` 無変更・他OpenAI担当／Provider／IADP用Leader Final 無変更）。`gpt-5.4-mini` を `MODEL_PRICES` に登録（input $0.75／cached $0.075／output $4.50 per 1M・料金計算構造無変更）。実AI Path A E2E 1回で未確認商品事実0件を実測（`wf-1788126291305`／`case-msr9yckye65y`／31語機械走査／`【広告】`・`#PR` あり／Reviewer否認→Strategy分類A→Leader Final遵守／テンプレート逐語コピー・「※※※」症状消滅）。Blocking解消Complete／Grounding Complete／deterministic guarantee とは記録しない。前回E2E Leader Final の実測token（`api_cost_events`・input 7,969／output 1,745）を新料金表で再計算＝$0.013829／¥2.21（cached非保持のため上限側）。`cost-logs.json` は runtime累積ファイルで正式化対象外。`leaderFinalModel.test.js` 50/50・`costTrackerGpt54Mini.test.js` 8/8・`costTracker.eea8.test.js` 19/19・`leaderFinalGrounding` 52/53〔FAIL `20-2a`・53/53とは記録しない〕・新規回帰0。P2（slides 10→1／imagePrompts取込失敗／targetAudience・benefit断片／packageQuality表示乖離）は Output Draft parser 側の別系統・今回スコープ外・未修正。新規Decision番号なし・Decision108追記。Version／Phase変更なし。stage-2 docs同期：Code 2commit＋docs Stage-1 commit `4f1fef2` を main push済・HEAD＝origin/main＝`4f1fef23b141de0a0b585efd7fb5df83a8ef43b6`・ahead/behind 0/0・Annotated Tag `v1.01-leader-final-model-blocking-fix`（object `2ba54d0ae1f500726f1a0ba8514b936298000376`・peeled `4f1fef2`）local／remote反映済・Render Auto-Deploy `4f1fef2` Live（2026-08-31 07:52:51 GMT+9・Duration 19.0s）をユーザーがRender Dashboard実画面で確認済・本番read-only health確認 PASS・有料AI追加実行0。Leader Final gpt-5.4-mini Blocking Fix は正式リリースComplete（本番実AI E2E確認済／本番でgpt-5.4-mini動作を再実測済 とは記録しない・正式リリース対象は `4f1fef2` で完了済みで本stage-2はその後追い記録）。
- **Instagram実運用 P1〜Issue A/B 修正 4commit ─ 実AI Path A E2E PASS・docs正式化 Stage-1 → 正式リリースComplete（2026-08-29 Stage-1／2026-08-30 Stage-2：main push・Annotated Tag `v1.01-instagram-p1-issue-ab-release`・Render Auto-Deploy Live・本番UI確認済）**：実運用1周目で確定したP1 Blocking Issueと、その過程で判明したIssue A／Issue Bを4commitで修正し、localhost実AI Path A E2Eで検証した。**①`ba4d82e` P1-1 Output Draft caseId Binding ＋ P1-2 stage-1**：`createOutputDraft()`がcaseIdを持たないDraftを返すため、生成直後Draftで`_apfrCurrentAdoptedProduct()`の三重guardが必ず失敗し、Compliance CheckがNOT CHECKED、さらにMobile Approval Enforcementも`not_checked→fail-open→blocked:false`で**事実上無効**だった。`atRunWorkflow()`で`_atRunCaseId`をbindingする1行のみ追加（`createOutputDraft()`本体無変更）。あわせてreviewerText/strategyTextの600→1200文字化と`LEADER_FINAL_REVIEWER_REJECT_RULE`を追加。E2E実測：Draft `caseId=case-msr9yckye65y`・`restored:false`・Compliance Context 4field取得・Enforcement `evaluated:true`。**②`c35b534` listingNgWords Channel Scope Fix**：A8.net公式定義上`listingNgWords`はリスティング広告の出稿キーワード制約であり、Instagram organic投稿本文の禁止語ではない。実値`["商品名","法人名"]`を本文へsubstring適用していたため、本文中の「商品名」でMobile Approvalが誤blockされ、さらにLeader Finalが`productName`（プラファスト）をIADPアカウント名へ置換する二次被害が起きていた。**Formal Truthは書き換えず**（Correction・schema変更0）、Instagram organicのときだけ評価inputから当該fieldを除外（detector本体は無変更・`unchecked`へ明示）。他チャネルは従来どおり。E2E実測：violation 0・誤block 0。**③`d8a09f1` IADP Scope Boundary（Option E＋B）**：IADPの`brandConcept`／`bio`等が【CASE CONTEXT】の「Formal Truth」ヘッダ下にフラット列挙され、AIが「自然由来」を商品成分の事実へ転用するClass Cが発生していた。**Option E**＝`server.js`で用途境界の見出しを付与（値・順序不変）、**Option B**＝`_buildFormalTruthRuleText()`の【絶対禁止】へ意味変換禁止を1項追加（単一ソースで全Agent＋Leader Finalへ同時適用）。**アカウント自体の説明としての利用は明示的に許可**（過剰禁止の回避）。E2E実測：商品成分への転用0・target適性転用0・未確認キャンペーン0。**④`08d15e3` Issue A Option D**：Path AのReviewerは**main-task**（`at-task-3`・`isPostProcess`未設定）と**post-process**（`at-postprocess-reviewer`）の2系統があり、停止判断を出したmain-taskは`memberReplies`に埋もれ`reviewerText`に入らないため`LEADER_FINAL_REVIEWER_REJECT_RULE`の対象外だった。新変数`mainReviewerTask`を追加し、専用ラベル`【Reviewer（品質レビュー担当）の作業中レビュー】`でLeader Final questionへ追加供給（既存`reviewerTask`／`strategyTask`取得行は無変更・post-processと統合しない・`memberReplies`構造も無変更）。**deterministic block・停止語判定・新Enforcementは追加しない**（Reviewer停止の無条件block化は、商品成分等がFormal Truthに構造上存在しない案件を永久blockするため）。`reviewerSignal`はnull維持・Rule Engineはbyte一致で無変更・Strategy override権限も維持。**Issue A E2E＝PASS**：Reviewerが確認要求→Strategyが「使わずに成立する投稿」として解決→Leader Finalが「商品の成分・効能・効果については記載せず」と明示し未確認事実を使用せず完成。虚偽断定0件・永久block 0。**Not Required維持**：Compliance Restrictions deterministic化（判定C）・C-3-1 Grounding Detection（判定D）とも維持し新detectorは追加していない。**Class B残件（P3既知事項）**：残存は`#美肌`・`#敏感肌`・`#保湿`のハッシュタグ3件のみで**本文中の未確認商品事実は0件**。deterministic denylistは偽陽性・保守コストの観点から採用せず、根本原因の一因は`LEADER_FINAL_PROMPT`の「ハッシュタグ20個程度」という量指示（Formal Truthから根拠を持てるタグは最大6個程度）。将来Prompt Rule追加＋量指示緩和をP3として検討する。**第1投稿は止めない**（Mobile Approval時に人間が目視で削除可能）。**productName**：`プラファスト`4件正常使用・`ナチュラルエッセンス`はアカウント名としてのみ使用・過剰回避の再発なし。追加修正不要。**テスト**：新規4スイート（60/74/84/72）全PASS・既存回帰 新規FAIL0・`leaderFinalGrounding` **52/53**（FAIL`20-2a`のみ・既知制約・**53/53とは記録しない**）。追随修正は`apfrComplianceInjection`（期待ラベル3行）と`p1BlockingFix`（E-7拡張＋E-7b新規追加）の2件のみでtest削除・skip・条件緩和0。`server.test.js`未実行。**実AI費用**：Path A E2E計3回で約¥2.31。**Version1 Final Complete／Version1.1開発中・Phase54 Complete維持・Phase55未着手**。**Compliance Enforcement完全完成／Fully Compliant／Grounding Enforcement Complete／広告表現の法的保証／自動公開安全性100%保証 とはいずれも記録しない**。新規Decision番号は作成せずDecision108へ追記（**Decision109は変更なし**——Enforcement追加なし・`canApprove`無変更のため）。
