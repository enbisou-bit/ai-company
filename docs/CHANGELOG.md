...
## ASP Product Fact Record（APFR）─ Step A Core／Step B Manual Input UI **正式リリース**（2026-08-21）／プラファスト **本番実運用検証Complete**（2026-08-22）／**Phase 0 再Adopt時Fact消失防止＋Phase 1 Current Fact Resolver＋CUI-0 Correction-aware Duplicate Policy＋CUI-1 Current Fact / History UI＋CUI-2 Correction UI Core**（2026-08-22〜23）／**APFR Correction UI Core CUI-0〜CUI-2 正式リリースComplete**（2026-08-24・Decision108）／**本番認証後Correction UI最終目視確認Complete**（2026-08-24・Decision108）／**Step C-1A Compliance Context Foundation ＋ Step C-1B Writer/Reviewer Compliance Injection 正式リリースComplete**（2026-08-24・Decision108・Tag `v1.01-apfr-compliance-injection`）／**Step C-1C-1 Deterministic Compliance Check 正式リリースComplete**（2026-08-24・Decision108・Tag `v1.01-apfr-deterministic-compliance-check`）／**Step C-2-1 Formal Truth Numeric Consistency Check Code Implementation Complete**（2026-08-25・Decision108・Code commit `9cf7ab92028e4280e83153a3c046a588187aedff`）
...
- **残課題（Completeとは分離）**：①同一fieldへの複数Fact存在時の**正本選択Contract・Correction-aware Duplicate Policy（CUI-0）・Current Fact・History UI（CUI-1）・Correction UI Core（CUI-2）はすべてComplete、かつCorrection UI Core系列（CUI-0〜CUI-2）は正式リリース・本番認証後最終目視ともComplete**（現在地の内訳＝Correction Contract／Resolver／Duplicate Policy／Current Fact・History UI／Correction UI Core／本番認証後最終目視＝すべて**Complete**）。実データ上の`listingNgWords`は引き続きlegacy fallbackで解決（後付けmigrationは行わない方針を維持）②ITP「7days」の専用保存field不在（`itpSupported`はbooleanのみ）③boolean値の日本語表示（**CUI-1でComplete**）④`listingPolicy`「一部ok」の表記統一（**Step C-1A/BともAI指示への注入は引き続き対象外**）⑤APFRフィールド選択UI（巨大select・スクロール・検索性）⑥APFR直接ジャンプ導線⑦入力省力化（21項目1件ずつの負担）⑧EEA問題（Decision101・別途未解決）⑨Quality Gate／Hold制御問題（別問題）⑩**Deterministic Compliance Check（listingNgWordsのみ）はStep C-1CでComplete**。ただしUser Approval／READYへの実ブロック接続は**Step C-1C-2として未着手**⑪`advertisingDisclosureRequirements`／`complianceRestrictions`の構造化サブschema検討（Step C-1C-3候補・APFR schema変更を伴うため別Decision・未着手）⑫**APFR Step C-2-1（payout/epc/approvalRateのFormal Truth数値突合・**Code Implementation Complete**）／**APFR Step C-2-2（C-2-1のmismatch検出結果をscore／confidence／warningへ接続するか否かの設計・未着手。C-2-1 Complete ≠ Intelligence Score Enforcement Complete）****。**APFR実運用Complete≠EEA問題Complete／≠Quality Gate・Hold問題Complete／≠Compliance Enforcement（C-1C-2）Complete**。
