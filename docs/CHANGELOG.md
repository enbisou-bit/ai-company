...
- **残課題（Completeとは分離）**：①同一fieldへの複数Fact存在時の**正本選択Contract・Correction-aware Duplicate Policy（CUI-0）・Current Fact・History UI（CUI-1）・Correction UI Core（CUI-2）はすべてComplete、かつCorrection UI Core系列（CUI-0〜CUI-2）は正式リリースComplete**（現在地の内訳＝Correction Contract／Resolver／Duplicate Policy／Current Fact・History UI／Correction UI Core＝すべて**Complete**）。**残タスクは本番認証後の実ブラウザ最終目視確認のみ（ユーザー本人が実施）**。実データ上の`listingNgWords`は引き続きlegacy fallbackで解決（後付けmigrationは行わない方針を維持）②③**boolean値の日本語表示はCUI-1でComplete**（残課題から解消）②ITP「7days」の専用保存field不在（`itpSupported`はbooleanのみ）③boolean値の日本語表示④`listingPolicy`「一部ok」の表記統一⑤APFRフィールド選択UI（巨大select・スクロール・検索性）⑥APFR直接ジャンプ導線⑦入力省力化（21項目1件ずつの負担）⑧EEA問題（Decision101・別途未解決）⑨Quality Gate／Hold制御問題（別問題）。**APFR実運用Complete≠EEA問題Complete／≠Quality Gate・Hold問題Complete**。
