...
## PRG Core ＋ PRG UI Contract U2 ─ 本番反映 ＋ Controlled Production Verification（Level 1／Level 2）（2026-09-18〜19・Decision119追記・Tag未作成）

> 記録日: 2026-09-18〜19。**Phase54 Complete維持・Phase55未着手・Version変更なし。** **状態区分：Pushed ／ Render Auto-Deploy Verified（ユーザー実測）／ Production Read-only Smoke PASS ／ PRG Core ＝ Production Verified ／ U2 ＝ Production Verified は「表示・canonical GET・Claim選択UI」まで（送信側は `deterministic test verified / production POST未実証`）。** B1 は Production COMPLETE（維持）。**観測主体の区別：** Push・Smoke・オフライン事前検証は Claude Code 実行、Render Dashboard 確認・Level 1・Level 2 の Console 操作は **ユーザー実施・ユーザー実測**（Claude Code は production の直接観測も production mutation も行っていない）。正確な時刻は未記録。

### Push ／ Render ／ Smoke

- **Push**：`git push origin main`。最初の1回は実行環境（Claude Code の auto mode 権限制限）でブロックされ remote は変化せず、auto mode を終了したうえでユーザーの「一度だけ許可」確認を経て再実行した1回のみが remote へ反映された（force 無し・自動回避なし）。`e5ce21a..9fef9dc8`（fast-forward）。push前後で HEAD ＝ `9fef9dc8d79a24b0e1c94ace9f998a47e1af4426`、push後は origin/main も同値・ahead-behind 0/0・staged 0・Protected 10件 changed 0。push対象2 commits：`da24cf29`（Code：`feat: guard partial evidence resolution updates`・7 files）→ `9fef9dc8`（Docs：`docs: sync evidence safety and resolution guard status`）。**旧 docs commit `19c50d41` は push 前の amend（Release Preflight の結果反映）で `9fef9dc8` に置換されており、現在の履歴には含まれない。**（後述の「Release Preflight」節の `19c50d41` は amend 前の値。）
- **Render Auto-Deploy（ユーザー実測）**：service `ai-company`／branch `main`／deployed commit `9fef9dc`／trigger Auto-Deploy／status Live／`node server.js`・port 10000・`Your service is live`／起動失敗・crash なし。Claude Code は Render を read-only で確認する手段（CLI 未インストール・`RENDER_API_KEY` 未設定・認証済みセッション無し）を持たないため、新規 install・token 発行・credential 抽出を行わず、Dashboard 確認をユーザーへ依頼した。
- **Production Read-only Smoke（Claude Code・GET 3回・認証なし・retry 0）**：実行前にコードで各 endpoint の DB write 0・AI/provider 到達 0 を確認（`GET /`＝`res.sendFile` のみ／`GET /api/auth-required`＝`WEB_APP_PASSWORD` の有無のみ／`GET /api/session-status`＝`requireSession()` が token 無しで即 401・cookie 発行なし）。結果：`GET /` 200（redirect 無し）／`GET /api/auth-required` 200（`{"required":true}`）／未認証 `GET /api/session-status` 401（`{"ok":false,"reason":"unauthorized"}`）。credential・session 生成 0。

### Controlled Production Verification ─ 設計上の整理

- **Production-only Unknown #1**：deterministic test はすべて fake Supabase client（in-memory）、CAS R-a は SELECT のみのため、**実 Supabase/PostgREST 上で `writeCanonicalContentEvidence` の条件付き UPDATE（CAS）が契約どおり動くか**が未証明だった。他の契約（completeness／cross-case／full_replace／partial_update の計算／stale 判定）は Regression Gate（12ファイル・1,010/1,010）で証明済みのため、本番で再証明しない。
- **第一投稿を使わない方針**：「テストしてから戻す」（rollback 依存）を採用せず、専用 disposable scope の新規作成（削除・rollback しない）とした。第一投稿 `case-value-1788410623` ／ `out_1788413020275` は Level 2 中 GET を含め一切アクセスしない。
- **POST 回数の確定（設計時の訂正）**：当初「最小2回」としたが誤りで、**合計3回**が正しい。`planCanonicalResolution` は保存済み Output Draft 行が無いと `resolution_requires_saved_output_draft`（409）で拒否するため、Resolution の前に行作成 POST が必要（①行作成 ②`full_replace` ③`partial_update`）。`output_drafts.case_id` は `TEXT NOT NULL` で外部キー無し・`POST /api/output-drafts` は `cases` テーブルを参照しないため、「case 作成」という別 mutation は存在せず、1回の POST で case_id の記録と行作成が同時に成立する。
- **fixture 設計上の発見（コード精査）**：`general_practice`／`public_statistic` は `market` へ写像され Tier1〜6 の独立ソース 2件以上が必要／`law_regulation` は Tier1（`.go.jp`／`.gov`）または Tier4 の単一ソースで足りる。また Claim 単位の grounding 判定（`evaluateVerifiedPromotion`）は **`sourceMethod: manual_user_input` でも `sourceUrl` 必須**。`classifySourceTrust` は URL の**ホスト名文字列のみ**を判定し実 fetch しない。よって最小 fixture は `law_regulation`・各 Claim に Evidence 1件（`.go.jp` の URL・`manual_user_input`・`verificationStatus: user_verified`）とした。`proposedClaimText` は数字・因果表現を含めない（`detectUnsupportedClaimElements` 対策）。
- **応答の読み方の注意**：`POST /api/output-drafts` は `upsert` 失敗等で **HTTP 200 のまま `ok:false`** を返す経路があるため、合否は `status` と `ok` の**両方**で判定した。
- **paid provider 到達 0**：Resolution 経路（`lib/contentEvidenceResolutionGuard.js`／`lib/contentEvidenceResolutionService.js`／`shared/contentClaimPlanning.js`／`shared/contentEvidence.js`／`shared/evidenceAcquisition.js`／`lib/outputDraftsDb.js`）に `openaiClient`／`claudeClient`／`costTracker`／`axios` への参照は無い。Web Search は別 route で、Resolution submit からは呼ばれない。実行方式は Web Search／承認フローを経由しないため、通常UIではなくユーザー本人の認証済みブラウザの Console から `fetch`（same-origin・既存セッションのみ・secret 直書き無し・retry 無し・loop 無し）で行った。

### Level 1 ─ 本番UI（ユーザー実測・表示・canonical GET・Claim選択のみ）

- 実行前にコードで境界を確認：「既存Evidenceを確認」（`ce-canon-load-btn`）＝GET のみ／Claim checkbox（`ce-canon-select-{claimId}` → `_ceToggleCanonicalClaim`）＝ローカル state のみ（fetch 0）／Content Evidence 機能に紐づく `beforeunload`／`visibilitychange` 無し／POST を送る唯一のボタン「Evidence確定（serverへ送信）」（`ce-submit-btn`）は Plan 作成後の別セクションにのみ表示。
- 結果：canonical Claim **CI-01（洗顔時の摩擦）／CI-02（洗顔後の保湿）／CI-03（日常の紫外線対策）** が server canonical として表示され `revision: (なし)`（既存 baseline の revision 未設定と一致）。CI-01 のみ選択（1/3）→ CI-01＝selected・CI-02／CI-03＝unselected。続けて CI-02・CI-03 を追加選択（3/3）を確認。**「選択したClaimでPlanを作成」「Evidence確定（serverへ送信）」は押していない。** Resolution POST・canonical mutation・Output Draft 保存・Writer 再実行・AI API・Web Evidence 0。
- **Evidence 6件そのものは UI で再カウントしていない**（B1 本番実測の正式 baseline としてのみ扱う）。Level 1 は canonical GET ＋ Claim 選択 UI の確認であり、partial_update／full_replace の POST 成功確認ではない。

### Level 2 ─ 専用 disposable scope での CAS write 検証（ユーザー実測・Console）

- **対象**：`case-prg-verification-20260918` ／ `out-prg-verification-20260918`（第一投稿の ID は一切含まない）。**3 POST を 1 本ずつ「実行→結果確認→STOP」で進めた**（一括実行しない）。POST 2・POST 3 の前に、実サーバー module（`validateResolutionRequest`→`resolveContentEvidenceSubmission`→`planCanonicalResolution`）に対する**オフライン検証**（DB・ネットワーク・ファイル書込なし・標準入力インライン実行）で受理形を確認した（POST 1 は通常保存で PRG 経路に入らないことをコード読解で確認）。

  | POST | endpoint／内容 | 結果 | CAS 述語（実DB） |
  |---|---|---|---|
  | 1 | `POST /api/output-drafts`：`{outputId, caseId, type:'value', fields:{}}`（`contentEvidenceCandidates` 無し＝PRG経路に入らない通常保存） | `200`／`ok:true` | ― |
  | 2 | 同 endpoint：`resolutionMode:'full_replace'`・`targetClaimIds:['TEST-01','TEST-02']`・`expectedRevision:null`・candidates 2件（TEST-01＝`caa.go.jp`／TEST-02＝`mhlw.go.jp`・`law_regulation`・`manual_user_input`・`user_verified`） | `200`／`ok:true`／`contentEvidenceCount:2`・`contentClaimsCount:2`・`resolutionMode:'full_replace'`／revision **`rev-eeffa38d4820371bf05ca41cacce6cab`** | `content_evidence_origin->>revision` **IS NULL** |
  | 3 | 同 endpoint：`resolutionMode:'partial_update'`・`targetClaimIds:['TEST-01']`・`expectedRevision:'rev-eeffa38d4820371bf05ca41cacce6cab'`・candidate は TEST-01 のみ（出典を `meti.go.jp` へ・文言を「（更新後）」へ変更） | `200`／`ok:true`／件数 2／2・`resolutionMode:'partial_update'`／revision **`rev-e8ab5c08e1ebb6b8c34ac008f505965c`**（POST 2 の値から変化） | `content_evidence_origin->>revision` **= `rev-eeffa38d…`** |

- **成功が含意すること（コード実測に基づく）**：`writeCanonicalContentEvidence` は更新行数がちょうど 1 のときだけ `ok:true` を返し（0 行→`cas_conflict`→409／2 行以上→500）、ルートは canonical write 失敗時に 409／500 で即 return する。したがって POST 2・POST 3 の成功は、実本番DBでそれぞれ CAS UPDATE がちょうど 1 行に作用したことを意味する。POST 3 は DB から読んだ既存 revision が `rev-eeffa38d…` であることを前提に通過している（事前の stale 照合と CAS 述語の二重）ため、POST 2 の書込が永続化されていたことの間接的な独立証拠にもなる。**→ Production-only Unknown #1（実DBでの CAS UPDATE）は解消。**
- **オフライン事前検証で確認した内容（POST 3）**：validate／resolve／plan 通過・新 Claim `grounded`・`mode: partial_update`・`expectedRevision` 一致・非対象 TEST-02 の Claim／Evidence が deep-equal で保持・TEST-01 が更新（Claim 文言・出典）・Claim 順序保持・新 revision の形式（`rev-[0-9a-f]{32}`）・旧 revision と異なり `previousRevision` が旧値と一致・`scope: partial`・planner が入力 row を破壊しない。
- **証明していないこと（正確な限界）**：①**DB の読み戻し（readback）は未実施** ― 「TEST-02 保持・TEST-01 更新」は応答（件数 2/2）・オフライン検証・CAS 成功の含意からの推論であり、DB 実データの直接確認ではない ②**U2 の送信側ブラウザ挙動（`resolutionScope` 判定・POST 直前の再GET・payload 組立）は本番で未実証**（`deterministic test verified` のみ・`contentEvidenceUiResolutionContract` 84/84。Level 2 は Console からの直接 `fetch`） ③本番での negative 経路（実409 発火・cross-case・incomplete 等）は**意図的に未実施**（JS 層の判定は決定的テストで証明済み、`.eq()` 不一致→0 行は標準的な DB 動作） ④第一投稿 canonical の無変更は「アクセスしていない」ことによる構成上の保証で、読み戻しによる確認ではない。
- **disposable scope の状態（削除しない・検証用データとして残置）**：`output_drafts` に 1 行（`out-prg-verification-20260918`）。canonical は Claim 2 件（TEST-01（更新後）・TEST-02）・Evidence 2 件（いずれも `law_regulation`・`user_verified`・本文に「検証用・実在の主張ではありません」と明記）。`type:'value'` を保存し、`content_value` はサーバーが決定的に再計算して保存（無料）。`cases` テーブルには行を作っておらず、`output_drafts` を全件列挙する reader も無い（`case_id`／`output_id` 指定のみ）ため案件一覧等には現れない。**DELETE／rollback／manual DB edit／cleanup はすべて 0。以後この scope を実運用（Instagram 投稿・Writer 等）に使わない。**

### 判定（正式）

- **PRG Core：Production Verified**（実本番DBで CAS write の `IS NULL`／`= revision` 両分岐、`full_replace`／`partial_update` の server 経路が成立）。
- **U2：Production Verified は「表示・canonical GET・Claim選択UI」まで。送信側ブラウザ挙動は `deterministic test verified / production POST未実証`。**「U2 送信側まで本番検証済み」とは記録しない。
- **第一投稿（`case-value-1788410623` ／ `out_1788413020275`）：変更禁止を継続**（Level 1 では canonical GET と Claim 選択の表示のみ・Level 2 では一切アクセスしていない）。
- **今回実施していないこと**：Tag 作成／追加の readback GET／U2 送信側の production POST／cleanup／Publishing／Instagram 投稿／AI API／Web Evidence／画像生成。
- **次工程**：自動実行しない。ユーザー判断待ち（候補：disposable scope 限定の readback GET の要否／U2 送信側の production 実証の要否）。

---

## 【push前の記録】Evidence-Based Content Value Quality（CV-3a〜CV-4c-3B）／Output Draft 認可境界（S2・S3-B1b）／Safety Foundation B1／Partial Resolution Guard Core ＋ UI Contract U2 — Docs Synchronization（2026-09-18・Decision119・Tagなし）

> ※本節以降の「Release Preflight」節までは **push 前（2026-09-18）の時点の記録**であり、履歴として保持する。文中の「未push／Not Deployed／ahead 1・2」等は当時の状態で、**現在の状態は直上の節**を参照。

> 記録日: 2026-09-18。**Phase54 Complete維持・Phase55未着手・Version変更なし**。**（push前の状態区分）PRG Core + U2 は Code Commit Complete ／ Not Pushed ／ Not Deployed ／ production 実機確認 pending だった（現在は直上の節のとおり反映・検証済み）。** 公式docsは docs commit `e37324e`（2026-09-10）で停止しており、今回 `e37324e..da24cf29` の未同期 Code Commit 15件を Git 実測（`git log --reverse --name-only`）で再構築した。**「14件」という前回調査の件数は PRG commit 作成前の値であり、実測は 15件**。

### 未同期 Code Commit 一覧（`e37324e` → `da24cf29`・時系列）

| # | commit | 日付 | 工程 | 主な変更 |
|---|---|---|---|---|
| 1 | `df99437` | 09-10 | CV-3a/CV-3b Core | `shared/contentEvidence.js`／`shared/contentValueQuality.js`（6軸）新規 |
| 2 | `07d7027` | 09-10 | CV-4b server-side Content Value | `lib/contentValueService.js`・`supabase/schema.sql`（`content_type`／`content_value` 列）・`server.js`・`lib/outputDraftsDb.js` |
| 3 | `79161b8` | 09-11 | Output Draft Authentication S2 | `GET /api/output-drafts` の `requireSession()` 化・401 と 200-empty の区別・restore flow・`outputDraftAuth.test.js` 新規 |
| 4 | `2e3fde7` | 09-13 | Internal API Authorization Boundary S3-B1b | 有料AI実行・破壊的 route の認証境界・`apiAuthBoundary.test.js` 新規 |
| 5 | `72bb578` | 09-14 | Cost Tracker Test Isolation | `COST_TRACKER_STORAGE_PATH` 分離で `server.test.js` が Protected の本番 `cost-logs.json` へ書かないことを保証 |
| 6 | `d4ad96a` | 09-14 | CV-4c-1 Activation Wiring | `reviewerNovelty` の canonical source を `fields.contentReviewerSignal.novelty` の enum whitelist に限定 |
| 7 | `1a81d9b` | 09-14 | CV-4c-2 Claim Intent Planning Core | `shared/contentClaimPlanning.js`（Claim Intent schema／Safety Filter／query plan／EEA candidate adapter） |
| 8 | `201cc17` | 09-14 | CV-4c-3 Evidence Approval State Machine | `shared/contentEvidenceApproval.js`・UMD 化・`lib/publicStatic.js` allowlist 追加・承認UI |
| 9 | `edd096d` | 09-14 | CV-4c-3B Server Canonical Resolution | `lib/contentEvidenceResolutionService.js`・`contentEvidenceBrowserReality.test.js`（browser reality regression guard）ほか |
| 10-13 | `6dfc335`／`50ca7de`／`5644449`／`4ec5249` | 09-15〜16 | Content Evidence UI 修正 | 承認パネル入口・スクロール・Intent draft の再描画耐性・`intentId` 伝播と Resolution 入力の保持 |
| 14 | `e5ce21a` | 09-17 | Safety Foundation B1 | canonical 3列・`lib/contentEvidenceCanonical.js`・`supabase/content_evidence_canonical_backfill.sql` |
| 15 | `da24cf2` | 09-18 | PRG Core + UI Contract U2 | `lib/contentEvidenceResolutionGuard.js` 新規ほか7ファイル（+1,933/-46） |

### CV-3a / CV-3b — Evidence-Based Content Value Quality Core

- **背景**：Quality 判定系が一度も本文の意味を読んでおらず、presence-only の `packageQuality` で薄い一般論が score 100 / complete を通過していた。
- **CV-3a `shared/contentEvidence.js`**：1 record = 1 claim × 1 source の schema／enum／validator。`shared/evidenceAcquisition.js` の `classifySourceTrust()`／`evaluateVerifiedPromotion()` を read-only 再利用。`isVerifiedContentEvidence()` は推測で verified にしない。cross-case guard（別 caseId の Evidence を sufficient に寄与させない）。`contradicts` を supports として数えない。AI 生成物（generated_hypothesis／ai_interpretation）は **enum に含めず型レベルで登録不能**。
- **CV-3b `shared/contentValueQuality.js`**：6軸（evidenceGrounding／specificity／informationGain／actionability／saveValue／nonGeneric）。独立 gate（evidenceGrounding・nonGeneric）と score（残り4軸）を分離し、「構造チェックを score に含めると常に 100 になる」既知の失敗を回避。status 語彙は既存（complete／almost_ready／needs_work／insufficient）のまま。LLM 呼び出しなし（semantic signal は外部注入）。

### CV-4b / CV-4c-1 — server-side Content Value ＋ content_type SoT

- `lib/contentValueService.js` が **client 供給 `contentValue` を一切保存せず** server 側で再計算する。
- 投稿種別の唯一の SoT は `output_drafts.content_type`（`value`／`bridge`／`product`）。**atomic first-write-wins**・downgrade 不可・client の `contentType` は「初回宣言の候補」に過ぎない。DB 側 CHECK 制約でも許容値を拘束（defense in depth）。
- 未宣言（NULL＝`unknown`）や Evidence 不足は **fail-closed**（`status='insufficient'`）、DB 列未追加・接続失敗は **fail-open**（Output Draft 本体保存を巻き込まない）。
- CV-4c-1 で `reviewerNovelty` の canonical source を `fields.contentReviewerSignal.novelty` の enum whitelist 検証のみに限定（トップレベル値は参照しない）。

### S2 / S3-B1b / Test Isolation — 認可境界と本番状態汚染防止

- **S2（`79161b8`）**：`GET /api/output-drafts` を `requireSession()` 化。401 と「200 だが空」を明確に区別し、login 後の再取得を成立させる。
- **S3-B1b（`2e3fde7`）**：有料AI実行（provider 到達）と破壊的操作の route に認証境界を敷く。テストは実 listen 0・実 provider 0・実DB 0 の source assertion ＋ middleware 直呼びで検証。
- **Test Isolation（`72bb578`）**：`server.test.js` の `resetCostTracker()` が Protected の本番 `cost-logs.json` へ書き込まないことを実測保証（`COST_TRACKER_STORAGE_PATH` を require 前に test 用 path へ）。

### CV-4c-2 / CV-4c-3 / CV-4c-3B — Evidence Grounding の順序強制と承認境界

- **順序強制**：「Writer が先に主張を作り、あとから Evidence を探して正当化する」構造を禁止。Claim Intent（問い）→ Evidence 取得 → Evidence が支持する範囲でのみ claim 確定。
- **Safety Filter**：medical／treatment／cure／diagnosis 等は事前に fail-closed で遮断。finalize 時の新しい wording も server 側で再検証（多層防御）。
- **承認境界**：Web Search 実行には常に独立した明示承認（ユーザーのボタン操作）が必須。`NO_AUTO_TRIGGER_EVENTS`＝Auto Task 実行／Writer 実行／Output Draft 保存／Case 開始は**承認として扱わない**。同一 plan の二重実行防止（plan fingerprint）、retry は failed からのみ（自動 retry を起こさせない）。
- **browser reality guard**：`contentEvidenceBrowserReality.test.js` が `require`／`module` を持ち込まない真のブラウザ相当 `vm.Context` で UMD ロードを検証（「Node で通ってブラウザで落ちる」再発防止）。
- **server canonical resolution（CV-4c-3B）**：`claimType`／`supportType`／`verificationStatus`／`reliability`／`sourceTier`／`grounded` はすべてルールベース関数の出力のみを採用し、client 値を信用しない。`shared/contentEvidence.js` は `lib/publicStatic.js` の配信 allowlist に含めず、ブラウザへ公開しない。

### Safety Foundation B1（`e5ce21a`）— canonical 列と書込経路の単一化

- canonical 3列 `content_evidence`／`content_claims`／`content_evidence_origin` を `fields`（client が丸ごと送って JSONB 全置換される列）の外側へ分離。
- **通常保存（fields／reviewState／content_type 宣言）は canonical 3列を一切変更しない。** canonical write は Evidence Resolution 成功時の `writeCanonicalContentEvidence()` のみ（server.js 内の呼び出しは 1 か所）。
- 保存前に `output_id` の既存 row を読み、`case_id` 不一致は 409、読み取り失敗は 503（書き込まない）。DB 側も `output_id` ＋ `case_id` 両一致条件で二重防御。
- Writer context ／ Content Value 評価入力は canonical 優先（列未設定の既存 row のみ legacy fields を read-only 参照）。legacy fields は削除・変更しない（rollback／監査の source）。
- `supabase/content_evidence_canonical_backfill.sql` は STEP 0（列確認）→ STEP 1（事前SELECT）→ 条件付き UPDATE → rollback 手順の順で、canonical 3列がすべて NULL かつ record の caseId が row の case_id と一致する row だけを対象にする。

### B1 本番到達点（ユーザー実測・引継ぎ事実／本セッションでは production access 0）

- canonical 3列 migration 適用済み。第一投稿 caseId `case-value-1788410623`／outputId `out_1788413020275` に controlled backfill 実施：**Evidence 6件（verified 6）／Claims 3件（CI-01／CI-02／CI-03・grounded）**、`origin.mode='legacy_fields_backfill'`・`version` 1.0.0・`recordedAt` 2026-09-16T07:05:00.941Z・`backfilledAt` 2026-09-16T22:17:54.304311+00:00。canonical を持つ row は 1件、legacy fields 削除 0、canonical と legacy の件数一致 true。
- **Post-Deploy Controlled Save Verification：COMPLETE。** 通常保存後も canonical Evidence 6／Claims 3／origin／fields は不変、`content_type` は NULL 維持。変化したのは `content_value.evaluatedAt` と `updated_at` のみ＝**通常保存で canonical を壊さないことを本番で確認済み**。
- **CAS 方式 R-a：read-only production verification PASS。** `content_evidence_origin->>revision` を CAS 条件に採用。第一投稿には revision キーが未存在のため `revision IS NULL` が一致（`revision is.null` ＋ `mode=legacy_fields_backfill` で 1 row／否定条件で 0 row）。production SELECT 2回のみ・write 0。**migration 0・backfill 0。**

### Partial Resolution Guard Core（`da24cf2`）

- **契約**：`resolutionMode`（`full_replace`／`partial_update`）・`targetClaimIds`・`expectedRevision` を必須化。client の宣言は server 再構築値との照合にのみ使う。
- **完全性検証（いずれも request 全体を拒否・canonical write 0）**：部分成功（resolved.errors あり）／grounded でない Claim／target 外の Claim／Claim を伴わない Evidence／Evidence の無い Claim／`contradicts`／`evidenceId` 重複／cross-case／`full_replace_would_drop_claims`。
- **mode 別計算**：`full_replace` は既存 Claim ID をすべて含む場合のみ許可（暗黙削除の禁止）。`partial_update` は対象 Claim だけを置換し、対象外 Claim と Evidence は順序を保って保持する。
- **CAS**：`content_evidence_origin->>revision` が `IS NULL`（初期）または `expectedRevision` 一致の row だけを更新。更新行数 **1＝成功／0＝`cas_conflict`（HTTP 409 `canonical_revision_conflict`）／2以上＝`invariant_multiple_rows`（HTTP 500 `canonical_write_invariant_violation`・成功扱いにしない）**。
- **次 revision**：直前 revision（無ければ `initial`）と次 canonical の fingerprint の連鎖ハッシュ（`rev-<sha256 32桁>`）。同一内容でも直前 revision が変われば別値になり、成功後に旧 request を再利用できない。
- **処理順**：`validateResolutionRequest` → `planCanonicalResolution` → CAS canonical write → `upsertOutputDraft` → Content Value 再計算。CAS 失敗時は fields／content_value を含め何も書き込まない。

### PRG UI Contract U2（`da24cf2`・index.html）

- 既存 canonical がある場合、**行番号からの claimId 自動採番を禁止**（既存 Claim を誤って置き換える危険が実測されたため）。新規 Claim Intent フォームと新規 Plan は無効化する。
- 既存 canonical Claim を GET で読み、ユーザーが**更新対象 Claim を明示選択**する。選択した canonical の `claimId` をそのまま `intentId` に使う（topic／question／順番から推測しない）。
- **POST 直前に canonical を再GET**し、Claim 集合・revision・`loadSeq`・`outputId` のいずれかが変化していれば **POST しない**（`canonical_changed_since_selection`）。
- mode は選択数のみから決定：一部選択＝`partial_update`／全選択＝`full_replace`。**既存 canonical がある scope での新規 Claim 追加は U2 に含めない**（別工程）。Option E・`content_type` 変更も含めない。
- 409 は自動 retry せず revision を差し替えて再送しない。成功時も stale 扱いにして次操作前の再確認を要求。error code ごとに表示を出し分け、表示は code／claimId／error 名のみ（Evidence 本文・URL は表示しない）。`_ceSubmitInFlight` で 1 操作 1 POST。

### テスト（引継ぎ実測・本セッションでは実行 0）

- 前セッションの NUL 修正後に対象3スイートのみ実行：`contentEvidenceUiResolutionContract` **84/84**・`contentEvidenceResolutionGuard` **110/110**・`contentEvidenceCanonicalColumns` **127/127**（いずれも exit 0）。
- **既存11スイート 991件は NUL 修正後に再実行していない。** release前Preflightで実行する。「991件 再PASS」とは記録しない。

---

## Release Preflight ─ Historical Baseline Recovery ＋ New Regression Gate Design / Execution（2026-09-18・Decision変更なし・Tagなし）

> 記録日: 2026-09-18。**Phase54 Complete維持・Phase55未着手・Version変更なし**。**（push前の状態区分）Release Preflight（Regression Gate差替）完了 ／ Code Commit `da24cf29` は当時なお Not Pushed ／ Not Deployed だった（その後 push・Render Deploy・Smoke・Level 1／2 まで実施済み。現在の状態は本節より上の節を参照）。**

### Release Preflight（Git / Safety / Contract review）

- read-only実測（**push前・docs commit amend前の値**）：branch `main`／HEAD `19c50d411d73f237c9ba434843ad0761ae51b21c`（のち amend で `9fef9dc8` に置換）／origin/main `e5ce21a5a8e5ec9079e2f4dad4a1e2ce7954500d`／ahead-behind `2/0`／staged `0`。Protected 10件 baseline一致（changed 0）。当時の未push 2 commits（`da24cf29` Code／`19c50d41` Docs）の親子関係・順序・merge混入なしを確認。Decision 119存在・Decision 120不存在。`index.html` NUL=0／CR=0／BOM問題なし／trailing whitespace=0／EOF改行あり／`git diff --check` PASS。PRG Core（`validateResolutionRequest`→`planCanonicalResolution`→CAS write→upsert→Content Value の順序・canonical write 呼び出し1箇所・CAS失敗時は`return`で通常保存へ進まない）／U2契約（既存Claim明示選択・ID推測禁止・POST直前再GET・stale検出・409自動retry禁止）／B1回帰（canonical/legacy分離・通常保存不変・Resolution経路のみ書込）をコードから確認。external side effects 0。

### 既存「11スイート／991件」baseline の正式判定：NOT RECOVERABLE

- 当初引継ぎ上は「既存11スイート 991 passed / 0 failed（release前Preflightで再実行）」と記録されていたが、read-only調査（全commit message・全docs過去版のpickaxe検索・git notes／stash／reflog・`claude-cost-logs.json`等ログ系ファイル・`package.json`）の結果、**正確な11ファイル名・991の内訳を裏付ける一次記録は repository 内に存在しないことを確認した**。
- pickaxeで検出された過去の「991」出現は、すべてコミットハッシュ（`504b991`／`fd99134`関連文字列）の一部であり、テスト件数としての「991」は0件。「11スイート」の初出は当時の（amend 前・現在は `9fef9dc8` に置換済みの）`19c50d41` 自身のみで、それより古い記録はない。
- 唯一の実測baseline記録は `df99437`（2026-09-10）commit body の「既存安全 **8 suites** 1,204/0」だが、スイート数（8≠11）・件数（1,204≠991）・対象領域（Carousel Image Production・PA-25期）のいずれも一致せず、直接の根拠にはできない。
- 静的 `assert(` 呼び出し数（65／97／107）は実行結果（84／110／127）と系統的に異なることを実測し、**assertion数の合算による11ファイルの逆算は原理的に不成立**と結論した。
- 以上により、旧991 baselineは「誤りだった」「存在しなかった」と断定はしない。**正確には「repository の記録からは独立に復元できない」**。今後この数値を release gate の正式基準として扱わない。

### New Regression Gate（release change set 由来・ファイル名明示manifest・実測1,010/1,010）

- **設計方針**：「過去に何本だったか」ではなく、`origin/main..da24cf29` の実変更（`server.js`／`lib/outputDraftsDb.js`／`lib/contentEvidenceResolutionGuard.js`新規／`index.html`）と、その依存関係（`lib/outputDraftsDb.js#writeCanonicalContentEvidence` の呼び出し元1箇所、CEブロック内のPlan/Approval UI同居、`lib/supabase.js` への到達経路）から Change → Runtime Dependency → Safety Contract → Test Coverage を組み立てた。11本・991件・54本全実行のいずれにも合わせていない。
- **REQUIRED（5ファイル・487 assertions）**：

  | file | covered contract | result |
  |---|---|---|
  | `contentEvidenceResolutionGuard.test.js` | PRG全契約（full_replace/partial_update/CAS/cross-case/completeness/反証/重複/drop保護） | 110/110 |
  | `contentEvidenceCanonicalColumns.test.js` | B1全契約（canonical/legacy分離・通常保存不変・単一書込経路・cross-case・legacy保全） | 127/127 |
  | `contentEvidenceUiResolutionContract.test.js` | U2全契約（明示選択・ID推測禁止・再GET・stale・no-retry・error code） | 84/84 |
  | `contentValueWiring.test.js` | content_type first-write-wins・client改竄防止・CV再計算・route auth | 98/98 |
  | `outputDraftAuth.test.js` | `requireSession()`／401 vs 200-empty／restore flow | 62/62 |

- **SUPPORTING（7ファイル・523 assertions）**：`contentEvidenceApproval.test.js` 172/172（承認state machine・U2と同一scriptブロック同居のため波及検出）・`contentEvidenceServerResolution.test.js` 31/31（Resolution serviceの入力側）・`contentEvidenceWriterContext.test.js` 19/19（Writer canonical優先）・`contentEvidenceBrowserReality.test.js` 22/22（ブラウザ実環境UMDロード）・`contentEvidence.test.js` 79/79（Evidence core・`df99437`の実績値79/0と一致）・`contentValueQuality.test.js` 141/141（6軸評価core・`df99437`当時104/0から後続機能追加で増加。退行ではない）・`contentClaimPlanning.test.js` 65/65（Claim Intent／Safety Filter）。
- **合計：12ファイル・1,010 assertions / 1,010 passed・0 failed。** 実行はすべて `node <filename>.test.js` によるファイル名明示・個別実行（`npm test`／wildcard／directory-wide／auto discovery は不使用）。
- **Supabase Fail-Closed Gate**：`contentValueWiring.test.js` は `lib/outputDraftsDb.js` を通じて `lib/supabase.js` を実ロードするため、実行直前に `SUPABASE_URL`／`SUPABASE_ANON_KEY`／`SUPABASE_SERVICE_ROLE_KEY`／`SUPABASE_SECRET_KEY`／`NEXT_PUBLIC_SUPABASE_URL`／`NEXT_PUBLIC_SUPABASE_ANON_KEY` が `process.env` 上ですべて UNSET であることを確認したうえで実行した。実行ログにも `[Supabase] 環境変数未設定 — ローカルfallbackモードで動作します` と出力され、fallback動作を実測確認。**`.env.local` ファイル自体にはSupabase関連キーが存在するが、この test の require chain には `dotenv` 読込が無いため到達しない**（値・文字数はdocsに記載しない）。今後この test を Gate に含める場合は、同じ fail-closed precheck を毎回実施する。
- **`server.test.js` の扱い**：**Regression Gate から除外（NOT RUN）。** `require('./server')` は `app.listen` を `require.main === module` でガードしているためport bindは発生しないが、`conversationHistory.js` 経由で Protected `data/conversations/_meta.json` へ実際に `writeFileSync` する経路を持つ（現在のProtected差分 `{}`→`{"user-hist-1":"leader"}` は過去の `server.test.js` 実行痕跡と推定）。今回のPRG/U2/B1変更に対する固有coverageは0件（contract matrixヒット0）のため、安全上の理由で除外し、必要coverageは他11ファイルで確保した。

### Coverage Gaps（設計時点の確認結果）

- GAP-A（既存safe testでcoverage可能）：該当なし（すべてREQUIRED/SUPPORTINGで直接coverage）。
- GAP-B（testは存在するが安全性未確認）：`contentValueWiring.test.js` のSupabase到達性 → fail-closed precheckで解消済み。
- GAP-C（coverageなし・新test必要）：なし。GAP-D（unsafeなtestにしかcoverageがない）：なし。

### 除外した主要test（54本中、変更範囲に近いもの）

- `server.test.js`（上記の理由で除外）。`carouselImageRoutes.test.js`等 `lib/outputDraftsDb` 共有carousel系7本（変更関数`writeCanonicalContentEvidence`を呼ばずno runtime dependency）。`apiAuthBoundary.test.js`（今回route自体のauthは無変更）。`staticExposureBoundary.test.js`（`lib/publicStatic`は今回無変更）。apfr系16本・iadp系3本・costTracker系3本・leaderFinal系2本（unrelated domain）。

### 今回（Release Preflight）実施していないこと

- docs commit 0（本記録は次工程のdocs commitで反映）／`git push` 0／`git tag` 0／Render 0／env変更 0。
- production access 0／production DB 0／DB write 0／migration 0／backfill 0。
- Evidence Resolution production実行 0／Output Draft production保存 0／AI API 0／Web Evidence 0／Image API 0／paid generation 0／Publishing 0／Instagram投稿 0。
- コードファイル変更 0（今回変更したのはdocsのみ）。`npm test`／wildcard／directory-wide test／auto discoveryは不使用。
- 「PRG 本番反映済み」「U2 実機確認済み」「Release Preflight PASS = Production Complete」等の表現は、**当時（push前の Release Preflight 時点）は**記録しなかった。当時 PRG Core + U2（`da24cf29`）は Not Pushed／Not Deployed／production 実機未確認だった（現在は本節より上の節のとおり push・Render Deploy・Smoke・Level 1／2 を実施し、PRG Core ＝ Production Verified、U2 ＝ 表示・選択まで Production Verified）。

### 今回実施していないこと（重要）

- docs commit 0／`git push` 0／`git tag` 0／Render 操作 0／env 変更 0。
- production DB read 0／write 0／migration 0／backfill 0／rollback 0。
- Evidence Resolution 0／Output Draft 保存 0／Writer 実行 0／Web Evidence 0／Recovery Search 0／OpenAI API 0／Claude API 0／画像生成 0／Publishing 0／Instagram 投稿 0。
- コードファイル変更 0（今回の変更は docs のみ）。test 実行 0（`server.test.js`／`npm test` を含む）。
- 「PRG 本番反映済み」「U2 実機確認済み」「Evidence Resolution 全体 Complete」「canonical 破壊不可を保証」等の過大表現は記録しない。
...
- **経緯**：PA-22F（3回目相当のcontrolled paid generation・trial case `case-carousel-trial-1788992344384`／`out_1788992344384`・7 assets／published=false維持）→ PA-22G人間Image Review **B — FAIL**（slide 5/6/7の背景にUI/checklist/一覧表様の情報構造）。
- **PA-22H root cause（read-only）**：slide別 `fields.imagePrompts[i]` の肯定的日本語表現（「注意アイコン」「チェックリスト風デザイン」「一覧表風レイアウト」等）がSAFE_SUFFIXの否定制約と同一prompt内で意味衝突。加えて7 slidesが独立生成で投稿全体の視覚方向を共有する層が不在。F1/F2/F3は実画像PASS・FAILはF4のみ。
- **PA-23**：Option B+C統合設計（read-only）。**PA-24**：4ファイル限定実装（`shared/carouselBackgroundSanitizer.js`新規285行／`shared/carouselVisualDirection.js`新規76行／`shared/carouselImageCore.js` の `buildBackgroundPrompt` 三層化・+36/-5／`carouselImageProduction.test.js` +212）。三層 = Background Sanitizer（意味衝突除去）＋Post-level Visual Direction（全slides共有・固定constant）＋SAFE_SUFFIX（既存19句維持）。
- **PA-25 Part A**：code reviewでASCII substring誤爆bug（`table`→"comfortable" 等・`UI`→"build"/"guide" 等・`drawing`→"withdrawing"・単漢字「絵」→「絵になる」）を発見→ `_termRegex`（ASCII語のみ `(?<![A-Za-z0-9])…(?![A-Za-z0-9])` で語境界化・日本語は部分一致維持）で是正。あわせてソース内 raw control byte（NUL/DEL）を除去し `file(1)` = "JavaScript source, UTF-8 text" へ復元。新規テスト T-S3b 追加。判定 B → 修正 → 再検証。
- **PA-25 Part B**：explicit-path stage（4件のみ）→ Code commit `8ff9f8d`（`feat: harden carousel background visual consistency`・4 files・+604/-5）。
- **PA-25 Part C**：Decision 118「Carousel Background Generation Control Boundary」正式採用（Decision最大番号 117→118・衝突なし）＋ docs同期（04/02/01/06）→ Docs commit `docs: formalize carousel background control boundary`（本コミット）。
- **ローカル安全テスト**：8 safe suites **1,204 assertions / 0 fail**（`carouselImageProduction` 593 含む）。F1/F2/F3 回帰 PASS。PA-22G slide5/6/7 fixture（実データ由来）で `icon`／`checklist`／`table`／`一覧` 系の残存なし。禁止スイート（`costTracker.eea8` / `server` / `iadpStructuredOutput`）は実行せず。
- **Protected**：`cost-logs.json`（`005d632c…`）／`data/conversations/_meta.json`（`b1f27d5f…`）は開始前から dirty の既知 Protected。PA-25 による内容変更なし。
- **正確な状態**：deterministic prompt-control implementation complete ／ local safe tests PASS ／ **production real-image validation pending**（3回目の実画像trial未実施）。「実画像でPASS」「UI/icon生成完全防止」「Visual Consistency完全解決」「provider outputがdeterministic」等は記録しない。
- **未実施**：git push／git tag／Render操作／Runtime Env変更／approval発行／generate POST／Image API／paid generation／execution reserve／DB write／Storage write／Publishing／Instagram投稿／EER — すべて0。

---

## Production Activation Step PA-19〜PA-21 — Option F Fix Release Formalization Complete（2026-09-10・Decision117・Tag `v1.01-carousel-image-production-option-f`）

> 記録日: 2026-09-10。**Phase54 Complete維持・Phase55未着手**。**状態区分：Release Formalized（push済み・Render Live確認済み・tag済み）**。`Source Gate=true`・`Runtime Env Gate=unset`・`Effective REAL=false`・Image API call 0・paid generation 0を維持。

- **目的**：PA-18で正式commit済みのOption F修正をmainへ安全にpushし、Render本番反映を確認したうえでrelease formalization（CHANGELOG・tag）まで完了する。

### PA-19 — Release Precheck（read-only）

`git merge-base --is-ancestor origin/main HEAD`=exit0でfast-forward可能を確認。push対象2commits・8 pathsを実測（Code 4件／Docs 4件・Protected混入0）。Option Fのスコープ（F-1〜F-4）が禁止対象（server.js／webSession／Supabase／DB／package）に触れていないことを再確認。`require()`実行で3モジュール（core/renderer/font）が正常ロードすることを確認し、startup安全性を評価。

### PA-20 — Main Push + Production Verification

`git push origin main`実行：`ea47fc4..4064752`（fast-forward・force不使用）。push後`HEAD=origin/main=4064752`・ahead0/behind0。本番read-only smoke：`GET /`=200・`/api/auth-required`=200・Carousel 3route unauthenticated=401・static boundary（`/server.js`等8件=404・allowlist4件=200）。**この時点ではHTTP挙動だけでは新commitのRender反映を直接証明できなかった**（今回の変更ファイルはすべてstatic配信対象外で、`index.html`自体も未変更のため）。

### PA-21 — Production Deployment Confirmation + Release Formalization

**Render Dashboardをユーザーが直接確認**：Service `ai-company`・branch `main`・最新Deploy commit `4064752`・status **Live**・deployment time **2026-09-10 06:49 JST**・commit message一致・Auto-Deploy経由（Manual Deploy不使用・Environment変更なし）。このユーザー実測を正式Evidenceとして採用し、release成立と判定した。

- **Production smoke再実施**：全項目PASS（PA-20と同一結果）。
- **既存7 assets不変確認**：`GET /api/output-drafts`／`GET /api/approvals`（未認証read-only）で`built_at`／`updated_at`／`carouselAssets`件数(7)／`carouselGeneration`（model:gpt-image-2・quality:low・generatedAt）／`published:false`が**PA-15B-2生成時点と完全一致**することを再確認。
- **CHANGELOG正式化**：`docs/CHANGELOG.md`へ正式release entry追加（PA-15B-2〜PA-21の経緯・Root Cause・Option F・Decision117・release evidence・既存7 assetsの状態を記録）。
- **Annotated Tag**：既存tag衝突なしを確認後、`v1.01-carousel-image-production-option-f`をHEAD（`4064752`）へ作成・push。

### 実施していないこと（重要）

Runtime Env Gate再有効化・`CAROUSEL_IMAGE_REAL_ENABLED`再追加・新Output Draft作成・新approval発行・generate POST・追加のImage API call・追加のpaid generation・Publishing・EER Executed・Instagram投稿・Phase55開始——**すべて0**。**修正版（Option F）による実画像の再生成・目視確認はまだ実施していない**（既存7 assetsは修正前のもの）。

---


## Production Activation Step PA-14〜PA-18 — First Controlled Paid Generation ＋ Overlay Defect Root Cause ＋ Option F Fix（2026-09-09・Decision 117）

> 記録日: 2026-09-09。**Phase54 Complete維持・Phase55未着手**。**状態区分：First Paid Generation Executed（技術的成功・Publishing見送り）／Root Cause Identified／Fix Implemented ＋ Committed／Decision 117正式採用**。`Source Gate=true`・`Runtime Env Gate=unset`（PA-17Aで削除済み）・`Effective REAL=false`・実OpenAI/Image API call 1 job（PA-15B-2のみ）・以降は0を維持。

- **目的**：PA-13で有効化したSource Gateに続き、Runtime Env Gateを一時的に有効化して初回のControlled Paid Generationを実施する。生成結果に描画欠陥を発見したため、read-onlyでroot causeを特定し、修正を実装・commitする。

### PA-14 — Runtime Env Gate Activation Precheck（read-only）

DB/Storage/Render secretsの依存関係を再確認し、対象Output Draft（`case-value-1788410623`／`out_1788413020275`）のfreshnessを未認証GET（`/api/output-drafts`／`/api/approvals`）で実測：7 slides・`published:false`・Mobile Approval `approved`・`carouselAssets`未生成——を確認。budget再確認：low ¥45.2816／medium ¥92.5232／high ¥255.2816（¥100 ceiling超過のため7枚構成では選択不可）。初回quality=**low**を推奨。

### PA-15A — Runtime Env Gate Activation

ユーザーがRender Dashboardで`CAROUSEL_IMAGE_REAL_ENABLED=true`を設定・再デプロイ。Deploy succeeded / Live確認後、read-onlyで既存API無傷（`GET /`=200・`/api/auth-required`=200）・session gate健在（`GET assets`無session=401）・static boundary無傷を確認。Effective REALの直接観測は「env値を露出しない」設計上不可能（Decision 115どおり）。

### PA-15B-1／PA-15B-2 — First Controlled Paid Generation

ユーザーがログイン→approval発行（`quality:low`・estimatedCostJpy **¥45.2816**・budget内）→**generateを1回のみ実行**（billingLock:false・retry 0）。

**結果（技術的成功）**：HTTP 200・`generated:7`・`published:false`。7 assets登録（1080×1350／4:5／PNG・`status:ready`）・signed URL取得成功。

**目視確認で発見した欠陥**：①日本語overlayの文字切れ・文字欠け（slide 1/2/3/7）、②白文字×明るいAI背景による低視認性（slide1背景輝度235・コントラスト約1.2:1）、③AI背景へのチェックボックス状UI要素混入（slide1）。**この時点でPublishingを見送り**、`out_1788413020275`は失敗例Evidenceとして保持（Decision 112 completed lockにより再生成せず）。

### PA-16 — Overlay Quality Root Cause Investigation（完全read-only）

`wrapText`は正常（全対象で文字保持100%・安全幅内）と実測確認したうえで、真因を2件特定：

1. **path serialization NaN混入**：`shared/carouselFont.js`が使うopentype.js 2.0.0の`roundDecimal()`が、座標の小数部が指数表記になるほど微小（ほぼ整数、例`357.00000000000006`）な場合にNaNを返し、SVG path dへ文字列`NaN`が混入。librsvg（sharp）はその時点でpath解析を中断し**以降のグリフを無警告で描画しない**（fail-open）。本番9文字列中4件で発生を実測。
2. **readability層の不在**：`slideTheme()`のdark theme `bg`は本番overlayで塗られておらず、白文字がAI背景の明度に無防備に依存。

テストギャップ：既存の`data-text`属性照合（P2B-10）はSVG構文レベルの検証で、`<path d>`の内容とは独立のため、path破損を検出できなかった。

### PA-17A — Runtime Env Gate Deactivation

ユーザーがRender Dashboardで`CAROUSEL_IMAGE_REAL_ENABLED`を削除・再デプロイ。Deploy succeeded / Live確認。以降`Effective REAL=false`を維持。

### PA-17B — Option F Fix Implementation

- **F-1**（`shared/carouselFont.js`）：`textToPathData()`で`toPathData()`へ渡す前に、path commandの座標（x/y/x1/y1/x2/y2）を精度2へ量子化。指数表記文字列が組み立てられる経路を構造的に発生させない。実測：NaN混入 4/10 → **0/10**。
- **F-2**（同ファイル）：pre-serialization（非finite座標を`invalid_path_coordinate`でthrow）／post-serialization（直列化後の`d`に不正トークンが残っていれば`invalid_path_data`でthrow）の二層fail-closed。既存`missing_glyph`／`unsupported_weight`と同一error contract。
- **F-3**（`shared/carouselRenderer.js`）：dark theme限定のdeterministic gradient scrim（固定値のみ・AI画像解析0）。実測コントラスト：最悪ケース（純白背景）で1.2:1 → **4.60〜4.65:1**（WCAG AA 4.5:1達成）。light theme（slide3等）には非適用を確認。同一入力→完全同一SVGのdeterminism確認。
- **F-4**（`shared/carouselImageCore.js`）：`SAFE_SUFFIX`へ`no UI elements`／`no checkboxes`／`no icons`／`no watermark`／`no frames`を追加（既存14句は維持）。Output Draft本文・imagePromptは無変更。
- **新規テスト**：`carouselImageProduction.test.js`へT1〜T11（11ケース）を追加（NaN regression／pre-serialization invariant／raster visibility／geometric containment／scrim存在・determinism・layer順／既存contract維持／SAFE_SUFFIX反映／dark-theme contrast実測）。

### PA-18 — Fix Review / Commit / Decision 117 Formalization

- 変更4ファイルの最終diff review完了（F-1〜F-4がPA-16設計どおり・node_modules/opentype.js無改変・Output Draft本文無変更・budget/approval/HMAC/session/Origin/billingLock/ledger/Storage/static allowlist無変更を確認）。
- **回帰**：新規57件を含め`carouselImageProduction` **520 passed/0**。既存回帰7スイート合計 **611 passed/0**（総計**1,131 passed/0**）。
- **Code commit**：`770aa10`（`fix: harden deterministic carousel overlay rendering`）。対象4ファイルのみ（`shared/carouselFont.js`／`shared/carouselRenderer.js`／`shared/carouselImageCore.js`／`carouselImageProduction.test.js`）。
- **Decision 117正式採用**：Carousel Dark-Theme Deterministic Readability Boundary（F-3を正式契約化。詳細は`docs/04DECISIONS.md`参照）。

### 実施していないこと（重要）

git push・tag・Render deploy（PA-18内では）・Runtime Env Gate再有効化・新Output Draft作成・2回目のapproval発行・2回目のgenerate・追加のImage API call・追加のpaid generation・Publishing・EER Executed・Instagram投稿・Phase55開始——**すべて0**。`out_1788413020275`の7 assetsは無変更のまま保持。

---


## Production Activation Step PA-8〜PA-13 — Production Infrastructure Connected ＋ Source Gate Enabled（2026-09-09・新Decision採番なし）

> 記録日: 2026-09-09。**Phase54 Complete維持・Phase55未着手**。**状態区分：Infrastructure Connected（DB／Storage／Render secrets）／Source Gate Enabled（code）／Runtime Env Gate Unset（Render）／Effective REAL Generation = Disabled**。`CAROUSEL_IMAGE_REAL_ENABLED`未設定・実OpenAI/Image API 0・paid generation 0を維持。

- **目的**：PA-8（read-only precheck）で確定した安全な実行順に沿って、DB／Storage／Render secretsを本番へ接続し（ユーザー実施）、read-only fail-closed検証（PA-12）を経てから、Dual-Key Kill Switchのsource gateのみを有効化する（PA-13）。

### PA-9〜PA-11（ユーザー実施・本番環境操作）

- **PA-9 DB apply**：`supabase/schema.sql:718`〜`892`の`carousel_image_executions`ブロックのみをSupabase SQL Editorで適用。実測：table_exists=true・RLS enabled=true・anon/authenticated policy=0・privilege=0・`nonce` UNIQUE=true・`uq_carousel_exec_active_output`のindexdefが`WHERE (status = 'in_progress' OR status = 'completed')`（Decision 112の拡張predicateどおり・no-op trap未発動）。
- **PA-10 Storage setup**：`carousel-images` bucketを作成。Public bucket=OFF・32MB上限・`image/png`のみ許可・Storage policy=0（コード側の期待値と一致）。
- **PA-11 Render secrets**：`SUPABASE_SECRET_KEY`／`CAROUSEL_APPROVAL_SECRET`／`WEB_SESSION_SECRET`／`WEB_TRUSTED_ORIGIN`（`https://ai-company-l45x.onrender.com`）を追加し再デプロイ。**`CAROUSEL_IMAGE_REAL_ENABLED`は意図的に未設定のまま**。Deploy succeeded / Liveを確認。

### PA-12 — Production Fail-Closed Verification（Claude Codeによるread-only検証）

- Render Live：`GET /`=200・`GET /api/auth-required`=200（既存API無傷）
- Public Static Allowlist本番実測：allowed 7件（`/`・`/index.html`・shared 5件）=200／blocked internal 13件（`/server.js`・新規Carousel lib各種・`/supabase/schema.sql`・`/package.json`・`/cost-logs.json`・`/data/conversations/_meta.json`・`/docs/04DECISIONS.md`等）=404
- Carousel 3route（session cookie無し・`credentials:'omit'`）：`GET assets`／`POST approval`／`POST generate`（bogus token・`billingLock:false`）**すべて401 `{ok:false,reason:'unauthorized'}`**。login・approval発行・provider到達は一切なし
- privileged client（`carouselExecutionSupabase.js`／`carouselAssetStorageSupabase.js`）のkey優先順位を再確認：`SUPABASE_SECRET_KEY`優先→`SUPABASE_SERVICE_ROLE_KEY`fallback。anon key（`NEXT_PUBLIC_SUPABASE_ANON_KEY`等）は実行コード上0参照
- 本番DBのrecord数・Storageのasset数は、read-only query手段（credential）が無いため**直接確認できず**（cannot confirm・推測でゼロと断定していない）
- 回帰：8スイート合計**1077 passed / 0 failed**（PA-6基準と同一）

### PA-13 — Source Gate Enable（code変更）

- **変更**：`lib/carouselImageClient.js`の`_sourceRealEnabled`を`false → true`（1行＋説明コメント3行のみ）。dual-keyのAND条件（`isRealGenerationEnabled() = _sourceRealEnabled===true && process.env.CAROUSEL_IMAGE_REAL_ENABLED==='true'`）・env gate自体は無変更。
- **付随テスト修正**：source gateの値をリテラル文字列`var _sourceRealEnabled = false;`固定で検証していた3箇所の旧assertion（`carouselRealEnabledKillSwitch.test.js` 5a／`carouselImageProduction.test.js` P2D-7・P2E-S6／`carouselAccessControl.test.js`）を、本来の検証意図（①変数宣言自体が削除されていないこと＝正規表現`/var _sourceRealEnabled = (true|false);/`で確認、②各テストブロック自身のsave/restoreが`client.REAL_ENABLED`のruntime値を正しく元へ戻したこと＝直上のassertionが引き続き担保）に沿う形へ最小修正。
- **回帰**：**1074 passed / 0 failed**（PA-12基準1077から、削除した旧assertion3件の分のみ減。新規FAIL 0）。
- **Code commit**：`8dee915`（`chore: enable carousel image source gate`）。対象4ファイルのみ（`lib/carouselImageClient.js`・`carouselRealEnabledKillSwitch.test.js`・`carouselImageProduction.test.js`・`carouselAccessControl.test.js`）。
- **無変更**：model／pricing／budget／approval／HMAC／session／Origin／billingLock／cumulative budget／execution reserve／ledger／Storage／static allowlist／Output Draft／Publishing／EER。
- **Effective REAL generation**：**disabled のまま**（Render `CAROUSEL_IMAGE_REAL_ENABLED`が未設定のため、source gateのみでは起動しないdual-key設計どおり）。

### 実施していないこと（重要）

`CAROUSEL_IMAGE_REAL_ENABLED=true`設定・Render Environment変更（Claude Codeからは0件）・approval token本番発行・実画像生成・OpenAI Image API call・paid generation・Publishing・EER Executed・Instagram投稿・Phase55開始——**すべて0**。

---


## Production Activation Step PA-6〜PA-7 — Main Push Complete ＋ Render Production Verification PASSED ＋ Release Formalization（2026-09-08・新Decision採番なし）

> 記録日: 2026-09-08。**Phase54 Complete維持・Phase55未着手**。**状態区分：Deployed / Production Verified（code）・Release Formalized（docs）**。実DB適用0・bucket未作成・Render Carousel secret未設定・実Storage 0・実OpenAI/Image API 0・paid generation 0・`REAL_ENABLED=false`・`CAROUSEL_IMAGE_REAL_ENABLED` 未設定を維持。

- **目的**：PA-4で確定したcode/docs commitを `main` へpushし、Render Auto-Deployと本番security boundary（Decision 116）をread-only検証したうえで（PA-6）、CHANGELOG正式記録とAnnotated Tagにより一連のCarousel Production Safety / Activation Foundationをrelease formalizeする（PA-7）。

### PA-6 — Main Push + Render Deploy Verification

- **Push**：`git push origin main`（fast-forward `02f8f6c..341eaa7`）。push前後で `git fetch` により`origin/main`の最新状態を実測し、非fast-forward・force pushは一切使用していない。push後 `HEAD=origin/main=341eaa758b6d4b77a2dbc5dc6ae10bbc14f6cc14`・ahead/behind 0/0。
- **Render Auto-Deploy確認**：本番URL（`https://ai-company-l45x.onrender.com`）へのread-only GETで挙動を実測（Render Dashboard自体への直接アクセスは行っていない）。`GET /`=200・`GET /api/auth-required`=200（既存API契約無傷）・`index.html`の`Last-Modified`がpush直後の時間帯と一致。
- **Decision 116 Production Verification（PASSED）**：blocked internal paths **8/8=404**（`/docs/04DECISIONS.md`／`/docs/06HANDOVER_NEXT_CHAT.md`／`/lib/webSession.js`／`/lib/carouselAssetStorageSupabase.js`／`/supabase/schema.sql`／`/server.js`／`/data/conversations/_meta.json`／`/package.json`）。追加検証として`/shared/carouselImageCore.js`／`/shared/carouselApproval.js`／`/lib/publicStatic.js`／`/node_modules/express/package.json`／`/%2e%2e/server.js`／`/docs/`も404を確認。public allowlist resources=200（`/`・`/index.html`・`shared`配下2件）。
- **Carousel route fail-closed確認**：`GET /api/carousel-image/assets`（session cookie無し）=**401 `{ok:false,reason:'unauthorized'}`**。POST（approval／generate）は今回実行していない。
- **禁止操作実績**：DB write 0／SQL Editor 0／bucket作成 0／Storage操作 0／Render env・secret変更 0／`REAL_ENABLED=true` 0／`CAROUSEL_IMAGE_REAL_ENABLED`設定 0／Image API 0／paid generation 0／Publishing 0／Instagram投稿 0／tag作成 0（PA-6時点）。

### PA-7 — Release Formalization

- **CHANGELOG**：`docs/CHANGELOG.md`へ正式release entry「Carousel Image Production ─ Production Safety / Activation Foundation」を追加（05DOC_UPDATE_PROTOCOLの既存形式・粒度を維持）。Decision 110〜116の実装内容・PA-6本番検証結果・Carousel Production Activation=NOT YETである旨を明記。
- **docs同期**：`docs/01PROJECT_STATUS.md`／`docs/02PHASE_PROGRESS.md`（本セクション）／`docs/06HANDOVER_NEXT_CHAT.md`（security backlog状態を「Implemented / Not Yet Deployed」→「Deployed / Production Verified」へ更新）を同期。`docs/04DECISIONS.md`は変更していない（Decision 110〜116の内容自体はPA-3Bまでに正式化済みのため、PA-6のdeploy verificationをDecision本文へ重複記載しない）。
- **Tag**：既存tag命名規則（`v1.01-<feature-slug>`。直近の`v1.01-carousel-image-production-phase-2d`）に整合させ、Annotated Tag **`v1.01-carousel-image-production-safety-release`** をdocs commit（HEAD）へ作成・push。
- **過大主張の禁止**：「Carousel Production Activated」「Real Image Generation Released」「DB Applied」「Storage Ready」「Paid Generation Ready」「End-to-End Production Complete」とは記録していない。正確には**Carousel Image Production safety / activation foundationがリリースされ、real paid generationは引き続き無効**という状態。
- **code変更0**：`server.js`／`index.html`／`supabase/schema.sql`／`shared/*`／`lib/*`／`*.test.js`はPA-6・PA-7を通じて無変更。

### 実測テスト（push前・PA-4時点の値を維持）

`carouselImageProduction` 465／0・`carouselImageRoutes` 95／0・`carouselAccessControl` 82／0・`carouselAssetStorage` 54／0・`carouselExecutionDb` 88／0・`carouselRealEnabledKillSwitch` 51／0・`carouselAssetStorageSupabase` 31／0・`staticExposureBoundary` 211／0 ＝ **合計1,077／0**（PA-6・PA-7ではcode変更が無いため再実行は必須要件ではなく、変更差分をgit diffで確認したうえで値を維持）。

---


## Production Activation Step PA-3A — Decision 112 Invariant Formalization ＋ Static Root Exposure Correction（2026-09-08・新Decision採番なし）

> 記録日: 2026-09-08。**Phase54 Complete維持・Phase55未着手**。**状態区分：Implemented / Not Yet Deployed。** 未commit・未push・未deploy。実DB適用0・bucket未作成・Render変更0・実Storage 0・実OpenAI/Image API 0・paid generation 0・`REAL_ENABLED=false`・`CAROUSEL_IMAGE_REAL_ENABLED` 未設定を維持。

- **目的**：PA-3（Release Preflight）が判定 A / READY FOR COMMIT だったのに対し、commit 前に確定した2件を処理する。【A】Decision 112 へ charged-failure 非対称性と read→reserve invariant を正式追記。【B】`express.static(path.join(__dirname))` による repo root 全体の静的公開を修正。

### 【A】Decision 112 追記（追記のみ・コード変更なし）

- **charged-failure 非対称性**：partial UNIQUE index `uq_carousel_exec_active_output` が lock を保持するのは `in_progress` と `completed` のみ。`failed_after_charge` と `unknown_billing` は `getCumulativeSpentJpyByOutputId()` が `spent_estimated_jpy` を **budget へ計上したまま lock を解放する**。`failed_before_charge` は計上0・lock解放。`in_progress` は `estimated_total_cost_jpy` として保守的に計上。
  - 結果として同一 `output_id` の **`completed` は最大1件**だが、**課金済み失敗を含む execution 行は複数生じ得る**。その場合の上限は本Decisionの lock ではなく **Decision 111 の cumulative ceiling が単独で担保する**（low 7枚：45.2816 → 90.5632 → 3回目 135.8448 で `budget_exceeded`）。
- **read→reserve invariant（正式記録）**：`lib/carouselImageService.js:297` の cumulative read から `shared/carouselImageCore.js:669` の reserve INSERT まで、実測で **`await` / async boundary / external I/O が1つも存在しない**（`checkCostLimit()` は同期、`runCarouselImageJob()` 本体は reserve まで同期実行。`carouselImageCore.js:645` の `await _finalizeLedger()` は running budget 拒否時の return 経路であり成功経路には現れない）。この区間へ新しい await を追加してはならない、を不変条件として記録した。
- **過大主張の禁止**：「TOCTOU完全解決」「DB制約だけで Decision 111 を完全保証」「charged failure でも lock 保持」と書かない。正確には **現行 execution ordering と partial UNIQUE lock と cumulative accounting の組合せで、到達可能な突破経路が確認されていない**。
- **将来の再評価トリガ**：read→reserve 区間へ await / async boundary / external I/O を追加する場合は safety proof を再評価必須。維持できないなら atomic DB RPC / transaction（`SELECT ... FOR UPDATE` 相当、または budget 判定を含む SQL 関数）を再検討する。**PA-3A では実装していない**。
- **無変更**：`supabase/schema.sql`（predicate は `status IN ('in_progress','completed')` のまま）／`lib/carouselExecutionDb.js`／`lib/carouselImageService.js`／`shared/carouselImageCore.js`。**DB applied = No** のまま。

### 【B】Static Root Exposure Correction

- **調査（修正前・実測）**：`index.html`（2,416,174 bytes・単一ファイルにCSS/JS内包）を全走査し、ブラウザが要求する local asset を確定した。`<script>/<link>/<img>/<source>/<iframe>` タグ、`src=`／`href=`（二重引用符・単一引用符）、`fetch()`、`XMLHttpRequest.open()`、CSS `url()`、dynamic `import()`、manifest / service worker をすべて確認。
  - 結果：**`shared/` の5ファイル（`<script src>`）と `members/<id>.md`（`fetch`）のみ**。CSS・画像・icon・font・favicon・manifest への参照は **0件**。`assets/fonts/*.otf` は sharp が server 側で読むだけでブラウザからは要求されない。
  - `server.js` に SPA catch-all（`app.get('*')` 等）は **存在しない**。
- **採用方式**：**A — allowlist（dedicated public boundary）**。新規 `lib/publicStatic.js` が許可判定を担い、許可された path のみを既存 `express.static` へ委譲する。denylist は将来追加される server-only ファイルが自動公開されるため採用しない。大規模なファイル移動・frontend build system 導入・npm package 追加・URL変更は **不要**（Scope Escalation なし）。
- **公開する asset**：`/`（→`index.html`）／`/index.html`／`/shared/agentResultNormalizer.js`／`/shared/evidenceAcquisition.js`／`/shared/iadpIntelligenceContext.js`／`/shared/instagramAccountDesign.js`／`/shared/instagramAccountDesignQuality.js`／`/members/<id>.md`（単一segment・`.md` のみ・サブディレクトリ不可）。
- **公開しない**：`docs/`／`lib/`／`shared/`（上記5ファイル以外。`carouselImageCore.js`・`carouselApproval.js` を含む）／`supabase/`／`data/`／`assets/`／`backup-dup-candidates-20260714/`／`node_modules/`／`server.js`／`package.json`／`package-lock.json`／`*.test.js`／`*.log`／`*.err`／`*.ps1`／root直下の内部MD・内部JSON／dotfile（`.env*`／`.git*`）。
- **攻撃面**：traversal（`..`／`.`／空segment／末尾スラッシュ）・percent-encoding（`%2e%2e`／`%252e%252e`／`%2F`／`%00`。`%` を含む path はデコード曖昧性を避けるため一律拒否）・backslash・C0制御文字／DEL・case variation（allowlist は case-sensitive 完全一致のため `/SERVER.js` 等が case-insensitive filesystem 経由で通らない）をすべて拒否。`extensions:false`（`/server` → `server.js` 補完の無効化）・`redirect:false`・`dotfiles:'ignore'`。
- **404 と fallback**：SPA fallback が無いため、内部path要求は index.html の 200 ではなく **404**（漏洩の有無と HTTPステータスが一致する）。
- **維持したもの**：許可asset の配信契約（Content-Type／ETag／Last-Modified／Range／conditional GET／`/`→`index.html`）は既存 `express.static` へ委譲したまま無変更。`/api/*`・`/webhook`・Carousel の3route（approval／generate／assets）は素通りして従来どおり後段で処理される。認証契約（`WEB_APP_PASSWORD`／`WEB_SESSION_SECRET`／localStorage UI gate）と Carousel 課金安全境界（billingLock／Approval Token／Budget／Ledger／Decision 110/111 logic／Dual-Key／Storage trust boundary）は **一切変更していない**。
- **新Decision**：PA-3A時点では**採番していない**（security bug fix扱い）。**Production Activation Step PA-3B（2026-09-08）でユーザー承認を得て Decision 116（Public Static Asset Allowlist Boundary）として正式採用済み**（`docs/04DECISIONS.md` 参照・docs-onlyの正式化・code変更0）。

### 変更ファイル

- `lib/publicStatic.js`（**新規**・169行）
- `server.js`（require 1行追加＋static登録の差し替え。他ロジック無変更）
- `staticExposureBoundary.test.js`（**新規**）
- `docs/04DECISIONS.md`（Decision 112 追記＋ヘッダ）／`docs/01PROJECT_STATUS.md`／`docs/02PHASE_PROGRESS.md`／`docs/06HANDOVER_NEXT_CHAT.md`（security backlog → Resolved）

### 実測テスト

- `staticExposureBoundary` **211 passed／0 failed**（新規。実HTTP listen 0／実network 0／`server.js` は require せずソース文字列として検証）
- loopback ephemeral port による end-to-end 実測 **36/36**（公開10path が 200・内部26path が 404）
- 既存回帰7スイート：`carouselImageProduction` 465／0・`carouselImageRoutes` 95／0・`carouselAccessControl` 82／0・`carouselAssetStorage` 54／0・`carouselExecutionDb` 88／0・`carouselRealEnabledKillSwitch` 51／0・`carouselAssetStorageSupabase` 31／0 = **866／0**
- **総計 1,077 passed／0 failed**。`node --check` 全OK・`git diff --check` 0 errors。

### 今回実施していないこと

commit／push／tag／DB write／SQL Editor／migration／bucket作成／Storage操作／policy・RLS変更／Render変更／secret設定・表示／OpenAI・Image API／paid generation／`REAL_ENABLED=true`／`CAROUSEL_IMAGE_REAL_ENABLED` 設定／Publishing／Instagram投稿／npm install／package変更／CHANGELOG更新。Protected Working Tree 9件は無変更（`cost-logs.json` md5 `005d632cd528b2c39e0ec9801c4b8fc8` 維持）。

---


## Production Activation Step PA-2 — Storage Privileged Production Client（2026-09-08・Decision 114実装完成・新Decision採番なし）

> 記録日: 2026-09-08。**Phase54 Complete維持・Phase55未着手**。**状態区分：Implemented / Production Not Activated。** bucket未作成・Render `SUPABASE_SECRET_KEY`等未設定・実Storage upload 0・paid generation 0。

- **目的**：Decision 114（Carousel Asset Storage Server-only Trust Boundary）で正式化済みのStorage trust boundaryを、実際のserver-only privileged Supabase clientとして実装し、production dependencyへ配線した。
- **新規**：`lib/carouselAssetStorageSupabase.js`。責務はStorage専用privileged client生成**のみ**（upload/signed URLのbusiness logicは既存の `lib/carouselAssetStorage.js`／`lib/carouselAssetAccess.js` に残置・無変更）。
- **credential優先順位**：`SUPABASE_URL`（fallback `NEXT_PUBLIC_SUPABASE_URL`）／`SUPABASE_SECRET_KEY`優先→`SUPABASE_SERVICE_ROLE_KEY`fallback。`NEXT_PUBLIC_*`・anon keyは一切参照しない。Decision 110と同一の命名規約を踏襲しつつ、**Decision 110のledger専用client（`lib/carouselExecutionSupabase.js`）は横展開せず**、完全に独立したmoduleとして実装（module責務の分離を維持）。
- **fail-closed**：credential不足時 `carouselAssetStorageClient = null`。`createClient()` 自体はネットワークリクエストを発生させないlazy clientのため、module require・`buildProductionDeps()` 呼び出しだけでは実Storage networkへ到達しない。
- **client生成config**：`persistSession:false`／`autoRefreshToken:false`／`detectSessionInUrl:false`（`lib/carouselExecutionSupabase.js` と同一のserver-only設定）。
- **secret非露出**：exportは `carouselAssetStorageClient`／`isPrivilegedClientConfigured()` の2つのみ。`console.*` を一切使用しない。
- **`buildProductionDeps()` 配線**：`storageClient: null` の直書きを `assetStorageSupabase.carouselAssetStorageClient` へ差し替え。credential未設定のこのテスト/開発環境では引き続き `null` を返す（fail-closed維持を実測確認）。
- **bucket自動作成・Storage policy変更は一切行っていない**（本Stepの責務外）。
- **既存route（approval／generate／assets）のcontract・session／Origin／billingLock／approval token／budget／ledger／REAL_ENABLED dual-keyはすべて無変更**。
- **テスト**：新規 `carouselAssetStorageSupabase.test.js` **31 passed／0 failed**（credential優先順位マトリクス6パターン・secret非露出・server-only config・buildProductionDeps配線・fake clientでの既存upload/signed URL契約への非破壊確認・bucket自動作成/policy変更コード0・@supabase/supabase-js以外への依存0）。
- **既存回帰**：`carouselImageProduction` 465／0、`carouselImageRoutes` 95／0、`carouselAccessControl` 82／0、`carouselAssetStorage` 54／0、`carouselExecutionDb` 88／0、`carouselRealEnabledKillSwitch` 51／0（PA-1のテストが参照していた `storageClient: null` の直書き文字列チェックをPA-2配線後の実態に合わせて更新）。合計 **866 passed／0 failed**。
- **変更ファイル**：`lib/carouselAssetStorageSupabase.js`（新規）／`lib/carouselImageRoutes.js`（storageClient配線・関連コメント更新のみ）。`lib/carouselExecutionSupabase.js`／`lib/carouselAssetStorage.js`／`lib/carouselAssetAccess.js`／`lib/carouselImageClient.js`／`shared/carouselImageCore.js`／`shared/carouselApproval.js`／`lib/carouselExecutionDb.js`／`lib/carouselExecutionStore.js`／`lib/webSession.js`／`server.js`／`supabase/schema.sql` はすべて無変更。
- **Decision**：新規採番せず、**Decision 114（`04DECISIONS.md`）へ「Production client implementation completed」を追記**。
- **未実施**：`carousel-images` bucket作成、bucket MIME／size制限設定、Storage policy設定、Render `SUPABASE_SECRET_KEY`／`SUPABASE_SERVICE_ROLE_KEY` 設定、実Storage upload、signed URL実発行。

---

## Production Activation Step PA-1 — Real Image Generation Dual-Key Kill Switch（2026-09-08・Decision 115）

> 記録日: 2026-09-08。**Phase54 Complete維持・Phase55未着手**。**状態区分：Implemented / Production Not Activated。** `REAL_ENABLED=false`（source gate）維持・Render env未設定・実OpenAI API 0・paid generation 0。

- **目的**：`REAL_ENABLED` がsource constantのみで制御されていたため、緊急停止にcommit/push/deployのフルサイクルが必要だった（Step C-4 finding）。source gateとenv gateのdual-keyへ拡張し、deployなしの緊急停止を可能にした。
- **実API到達条件**：`_sourceRealEnabled === true`（source gate）**AND** `process.env.CAROUSEL_IMAGE_REAL_ENABLED === 'true'`（env gate・厳密一致）。
- **fail-closed**：未設定／`''`／`'false'`／`'False'`／`'TRUE'`／`'1'`／`'yes'`／前後空白付き文字列はすべてdisabled。`'true'`の完全一致のみenable。
- **source gateは削除していない**（envだけで実解禁できる構造にしない）。
- **`REAL_ENABLED` をgetter/setter（`Object.defineProperty`）化**：既存 `shared/carouselImageCore.js`（変更禁止）の `assertRealCallAllowed()` が `client.REAL_ENABLED !== true` を直接読む唯一のcheckpointであるため、そのexported値自体をdual-keyの結果にした。これにより reserve直前のgate（assertRealCallAllowed）と provider直前のgate（generateBackground/generateBackgroundRaw）が常に一致し、「source=trueのみでreserveが先に成功し、providerだけがreal_api_disabledで失敗してbudget/nonceを浪費する」経路を構造的に排除した。
- **後方互換**：setterは既存テストの `client.REAL_ENABLED = true/false` というsave/restoreパターンをそのまま受理する（source gate側の切替という既存の意味を維持）。
- **既存reason `real_api_disabled` を維持**（新reason追加なし）。
- **新規export**：`isRealGenerationEnabled()`／`CAROUSEL_IMAGE_REAL_ENABLED_ENV`。
- **変更ファイル**：`lib/carouselImageClient.js` のみ（+193/-18行）。`shared/carouselImageCore.js`／`shared/carouselApproval.js`／`lib/carouselExecutionDb.js`／`lib/carouselExecutionStore.js`／`lib/carouselAssetStorage.js`／`lib/carouselAssetAccess.js`／`lib/webSession.js`／`server.js`／`supabase/schema.sql` はすべて無変更。
- **既存testの追随**：`carouselImageProduction.test.js`／`carouselImageRoutes.test.js`／`carouselAccessControl.test.js` の `client.REAL_ENABLED = true` によるsave/restoreブロックへ、`process.env.CAROUSEL_IMAGE_REAL_ENABLED` の一時設定・復元を追加（dual-key化に伴う必要な追随。既存テストの意図・assertionは変更していない）。
- **テスト**：新規 `carouselRealEnabledKillSwitch.test.js` **51 passed／0 failed**（source/env全パターンの安全マトリクス・dual-key成立時のみgate通過・OPENAI_API_KEY存在下でもdual-key不成立ならAPI到達0・env値の非露出・process.env非汚染を確認）。
- **既存回帰**：`carouselImageProduction` **465／0**、`carouselImageRoutes` **95／0**、`carouselAccessControl` **82／0**、`carouselAssetStorage` **54／0**、`carouselExecutionDb` **88／0**（合計 **835 passed／0 failed**）。
- **未実施**：`REAL_ENABLED=true` 化・Render `CAROUSEL_IMAGE_REAL_ENABLED` 設定・実OpenAI API呼び出し・paid generation・commit／push／tag／deploy。

---

## Carousel Image Production Phase 2-E Production Connection（C-1〜C-5-pre・2026-09-08・Decision 112／113／114）

> 記録日: 2026-09-08。**Phase54 Complete維持・Phase55未着手**（Carousel Image Productionを理由にPhase55へ進めない）。
> **状態区分：Implemented / Production Not Activated。** コード実装とdeterministic testは完了しているが、**実Supabase DB適用0・実Storage 0・bucket未作成・Storage policy未設定・Render secret未設定・未commit・未push・未deploy・`REAL_ENABLED=false`・paid generation 0**。

### C-1 TOCTOU Conservative Safety Lock（Decision 112）

- **問題**：partial UNIQUE index `uq_carousel_exec_active_output` が `WHERE status = 'in_progress'` のみを対象としていたため、Aが `completed` へ遷移した瞬間にindex占有が解放され、古い `SELECT SUM(...)` を保持したBのreserveが成功しうるTOCTOU raceが形式的に成立していた。
- **修正**：predicateを **`WHERE status IN ('in_progress', 'completed')`** へ拡張（`supabase/schema.sql`）。
- **帰結**：**1 Output Draft（output_id）につきcompleted generationは最大1件**。`failed_before_charge`／`failed_after_charge`／`unknown_billing` はpredicate対象外（`getCumulativeSpentJpyByOutputId()` の `spent_estimated_jpy` 集計で別途制御する既存設計を変更しない）。
- **reason名**：23505分類 `post_execution_in_progress` は**維持**（rename しない）。意味は「同一output_idへのactiveなauthorization（in_progress または completed）が既に存在する」へ拡張された旨をコードコメントへ明記。
- **release／reclaim／自動timeout unlockは実装しない**（`completed` は意図的にindexを保持し続ける）。
- **変更ファイル**：`supabase/schema.sql`／`lib/carouselExecutionDb.js`（コメントのみ）／`carouselExecutionDb.test.js`。
- **テスト**：`carouselExecutionDb.test.js` **88 passed／0 failed**（67→88・21 assertions増）。fake clientでpartial UNIQUE相当の判定を実際の行状態から再現し、TOCTOUシナリオ（B観測→A reserve→A completed→B reserve拒否）を実証。

### C-2 Supabase Storage Helper（Decision 114）

- **新規**：`lib/carouselAssetStorage.js`。
- bucket定数 **`carousel-images`**（client入力から指定不可）。path **`carousel/<caseId>/<outputId>/<nonce>/slide-<N>.png`**（nonceを含めることでretry／旧generation間のcollisionを防ぐ）。
- path-safety：caseId／outputId／nonce は allowlist `^[A-Za-z0-9_-]{1,128}$`（`..`／`/`／`\`／制御文字／空文字を構造的に排除）。slideIndexは1〜19（`lib/carouselCompositor.js` の `SAFE_NAME_RE` と同一範囲）。
- upload：**`contentType: 'image/png'` 固定・`upsert: false` 固定**。409衝突は `storage_collision`。
- 検証：`Buffer.isBuffer` 必須／空Buffer拒否／**PNG signature厳密一致**／`MAX_ASSET_BYTES = 32MB`（`lib/carouselImageNormalize.js` の `MAX_INPUT_BYTES` と同根拠・同値）。
- 成功metadata：`slideIndex／slideId?／storagePath／sha256／bytes／format／width／height／aspectRatio／status`。**画像をデコードして寸法検証しない**（decode dependencyを追加しない・1080×1350／4:5はupstream保証の固定契約値）。
- **public URLを生成しない。自動delete rollbackを実装しない**（batch部分失敗時、成功済みobjectは残置＝orphan許容）。
- server-only boundary：privileged clientを自前生成せず **`opts.client` のdependency injection専用**。
- **テスト**：`carouselAssetStorage.test.js` **54 passed／0 failed**（実Storage接続0・fake client のみ）。

### C-3 Production Routes

- **新規**：`lib/carouselImageService.js`（HTTP非依存の service層・`{status, body}` を返す）／`lib/carouselImageRoutes.js`（Express adapter＋`buildProductionDeps()`）／`lib/carouselAssetAccess.js`（signed URL helper）。`server.js` へ route登録を14行追加。
- **routes**：`POST /api/carousel-image/approval`（`{caseId, workflowId?, outputId, quality}`）／`POST /api/carousel-image/generate`（正式authorityは `{approvalToken}`）／`GET /api/carousel-image/assets`（`?caseId=&outputId=`）。
- **既存helperの再利用**：`core.planCarouselImageJob()`（scope／approval／published／slides／quality）・`core.draftFingerprint()`・`client.estimateAuthorizedTotalJpy()`・`approval.issueApprovalToken()／verifyApprovalToken()`・`core.runCarouselImageJob()`（reserve→provider→normalize→composite→ledger）。**二重ロジックを作っていない。**
- **canonical reconstruction**：`slideCount`／`estimatedCostJpy`／`draftFingerprint`／`nonce`／`alreadySpentEstimatedJpy` はすべてserver側で再構築し、clientから信用しない。
- **generate時のmandatory revalidation**：署名／TTL／nonce／caseId／outputId／draftFingerprint／quality／slideCount／estimatedCostJpy を、**現在のcanonical stateから再計算したexpected**と照合。stale時はproviderCalls=0で停止。
- **順序**：`token/canonical検証 → budget → reserve → provider → normalize → overlay → 全枚成功 → actual usage → ledger terminal update → Storage persistence → Output Draft carouselAssets write → response`。**billing ledger truthをStorage successより後ろへ移動しない。**
- **Output Draft metadata**：既存の bare-array 契約（`fields.carouselAssets` 配列＋`fields.carouselAspectRatio`）を維持し、job-level provenance（nonce／model／quality／generatedAt）は新siblingキー **`fields.carouselGeneration`** へadditiveに格納（`slides`／`caption`／`cta`／`hashtags` は不変）。
- **失敗時の非破壊**：Storage失敗（`asset_persistence_failed`）でも metadata write失敗（`asset_metadata_write_failed`）でも、**ledgerのcompletedを取り消さず、Storage objectも自動削除しない**。
- **published は常に false 維持**（生成成功≠Publishing・Decision 107 EER境界を保持）。
- **signed URL**：`GET assets` はcanonical `fields.carouselAssets` に登録済みの `storagePath` のみをauthorityとし、`carousel/<caseId>/<outputId>/` のexact prefix一致を検証（foreign asset／path traversalは `asset_scope_mismatch`）。expiryは**server固定300秒**（clientから指定不可）。応答に `storagePath` 自体を含めない。
- **テスト**：`carouselImageRoutes.test.js` **95 passed／0 failed**（実HTTP listen 0・実DB 0・実Storage 0）。

### C-4 Production Environment / Image Review Preflight（read-only）

- 実環境接続に必要な残条件を棚卸しし、**REAL_ENABLED=true前の必須修正2件**を発見（C-5-preで修復）。
- **発見①**：C-3実装で `jobInput.billingLock = false` がhardcodeされており、既存 `/api/evidence/web-search`（EEA-3）が確立したbilling safety contractから逸脱していた。
- **発見②**：`/api/login` がsession／cookie／tokenを一切発行せず、ブラウザ側の `localStorage.enbisou_auth_v1='true'` のみでUI gateしていた。**server-side authorizationが存在しない**状態で、有料実行routeが実質無認証で公開されうる。
- **Supabase適用設計**：`schema.sql` 全文をSQL Editorへ貼ってはならない（他テーブルの裸の `CREATE POLICY` が31本あり、既存DBへの再実行で `policy already exists` となりトランザクション全体がrollbackする）。**`carousel_image_executions` ブロックのみ**を適用する。
- **`CREATE UNIQUE INDEX IF NOT EXISTS` の罠**：index名のみで存在判定しpredicate差を検出しない。適用後に `SELECT indexdef FROM pg_indexes WHERE indexname='uq_carousel_exec_active_output';` でpredicateを実測確認する必要がある。
- **Image Review UI**：`index.html` に `carouselAssets`／`carousel-image`／`imageReviewOk` の参照は**0件**（UIは存在しない）。
- **static-root finding**：`server.js` の `express.static(path.join(__dirname))` により repo root が広く静的公開対象。`.env.local` は serve-static の dotfiles 既定により配信されず **secret leakは確認されていない**が、over-broad static exposure として要レビュー（Carousel由来ではないため今回未修正）。

### C-5-pre Billing Lock Contract Repair ＋ Server-side Access Control（Decision 113）

- **Fix 1（billingLock）**：`handleGenerateRequest` へ **`if (body.billingLock !== false) return 403 billing_locked;`** を追加。`true`／欠落／`null`／`'false'`／`'true'`／`0`／`''` はすべて拒否。**拒否位置はreserve／provider／Storageより前**（実測：全カウンタ0）。既存EEA-3 contractと同一。
- **Fix 2（access control）**：**新規 `lib/webSession.js`**。HMAC-SHA256 stateless signed session（token `s1.<payload>.<sig>`・payloadは `{v,iat,exp}` のみ）／secretは独立の **`WEB_SESSION_SECRET`**（最小16文字・`WEB_APP_PASSWORD` を鍵へ流用しない）／cookie `enbisou_session`（`HttpOnly`／`SameSite=Strict`／`Path=/`／`Max-Age=24h`／`Secure`はrequest由来判定）／`crypto.timingSafeEqual`／**npm依存追加ゼロ**。
- **route適用**：POST 2route＝`session → Origin → handler`、`GET assets`＝`session → handler`。**適用は3routeのみ**（既存API群へ横展開しない）。
- **Origin検証**：allowlistは `WEB_TRUSTED_ORIGIN`（カンマ区切り）∪ `RENDER_EXTERNAL_URL`。**1つでも設定されていればlocalhostは許可しない**。**Origin欠落は403**（無条件acceptしない）。**Referer fallback不採用**。
- **fail-closed（実測）**：`WEB_SESSION_SECRET` 未設定状態で、cookie無し／偽造cookie／localStorage主張headerのいずれでも3routeが **401**。**環境変数の有無で認証をskipする経路は存在しない。**
- **session失敗は一律 `401 {ok:false, reason:'unauthorized'}`**（どのcheckで落ちたかを返さない）。
- **`server.js`**：`/api/login` 成功時に `Set-Cookie` を追加（既存レスポンス契約 `{ok:true}` は維持・観測用に `session` フィールド追記）。password不一致時はcookieを発行しない。**`POST /api/logout` を新設**しcookieを `Max-Age=0` で失効。
- **`index.html`**：`logout()` から `POST /api/logout` を呼ぶ4行を追加（localStorage削除のみではsessionが24時間残るため）。
- **認可の分離**：有料実行は **Session ∧ Approval Token ∧ `billingLock===false` ∧ Budget/Ledger** のAND。session／approval tokenは別domain・別secret・別prefix（`s1`／`v1`）。
- **限界**：`WEB_APP_PASSWORD` は社内共有の合言葉であり、本sessionは **individual identityではなく shared internal access boundary**。誰が操作したかの監査には使えない。
- **テスト**：`carouselAccessControl.test.js` **80 passed／0 failed**（新規）。既存 `carouselImageRoutes.test.js` は新contractに合わせ19箇所へ `billingLock: false` を追加し **95 passed／0 failed** を維持。

### Phase 2-E 現時点の未実装・未適用（正式記録）

| 項目 | 状態 |
|---|---|
| kill switch（`CAROUSEL_IMAGE_REAL_ENABLED` env二重鍵） | **実装Complete（PA-1・Decision 115）**。Render env未設定のため現状は常時disabled |
| Storage privileged production client | **未実装**（`buildProductionDeps()` の `storageClient` は意図的に `null`） |
| 実Supabase DB適用 | **未適用**（`carousel_image_executions` は実DBに存在しない） |
| `carousel-images` bucket | **未作成**／MIME・size制限未設定／policy未設定 |
| Render secrets（`SUPABASE_SECRET_KEY`／`CAROUSEL_APPROVAL_SECRET`／`WEB_SESSION_SECRET`／`WEB_TRUSTED_ORIGIN`） | **未設定** |
| Image Review UI | **未実装**（`imageReviewOk` を `true` へ更新するUI/APIも未実装） |
| commit／push／tag／deploy | **未実施** |
| `REAL_ENABLED` | **false 維持**（source constant・`lib/carouselImageClient.js:91`） |
| paid generation／実Image API call | **0件** |

### 運用手順（Decision化しない・記録のみ）

- **in_progress stuck時のmanual recovery**：`attempted_provider_calls = 0` が確認できる場合のみ `failed_before_charge` 候補。それ以外は `unknown_billing`。**行のDELETE禁止・nonce再利用禁止・課金不明時にSafety Lockを解除しない。**
- **orphan Storage**：partial失敗／metadata write失敗でorphanが残り得る。**自動削除は行わない。** 初回本番ではSupabase Storage UIによるmanual observabilityを用いる（admin cleanup routeは未実装）。
- **Image Review運用候補**：`生成成功（status:'ready', imageReviewOk:false）→ 人間の目視 → imageReviewOk:true → Publishing判断`。**`imageReviewOk` を更新するUI/APIは未実装のため、現時点で「実装済み」と扱わない。**
