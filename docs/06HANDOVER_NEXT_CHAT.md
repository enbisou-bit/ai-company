...
- **次工程**：プラファストAffiliate Evaluation登録・商品採用工程（ユーザー本人が本番UIで実施。Claude Codeは代理登録しない）。その後、A8実画面／広告主LPでユーザー本人が確認したFactをAPFR UIから登録する。APFR Step C（ASP/Product Intelligence接続）は**自動で開始しない・ユーザー承認後に着手**。Leader Case Context Phase2 は別系統・引き続き本番未 commit・未リリース。
