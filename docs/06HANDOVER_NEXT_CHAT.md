...
- **次工程**：EER登録3件を踏まえたAccount Creation Readiness最終確認、またはInstagram実運用側（別チャット）の進行。EER追加実装（`verified`状態・他executionType追加等）はユーザー承認後に別途判断。Leader Case Context Phase2 は別系統・引き続き本番未 commit・未リリース。
