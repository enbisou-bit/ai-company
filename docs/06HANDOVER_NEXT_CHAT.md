...
**既存生成成果（変更しない）**：`case-value-1788410623` ／ `out_1789809676034`（`generated=7`・`published=false`）は **production pipeline 実証成果**。**Instagram 実投稿には使用しない**（Content Value 実測 `insufficient` ／ `11 / 100`）。手作業で修正して投稿完成させる方向には戻らない。

### 次セッションが必ず守る正式契約（追加分・下記の既存契約も引き続き有効）

- **Content Value Diagnosis は表示専用。** hard gate 化しない（canonical Evidence と `fields.contentReviewerSignal.novelty` の配線が未完のため、gate 化すると誤ブロックになる）。
- **`produce` route の approvalToken を client へ返さない。** `billingLock` を client input へ戻さない。二重生成の正本は execution ledger（`nonce` UNIQUE ／ `uq_carousel_exec_active_output`）。
- **Image Review の保存は必ず「最新 Draft を GET → 最新 fields を基準に `imageReviewOk` だけ変更 → `{outputId, caseId, fields}` のみ POST」。** client snapshot を基準にしない。stale（`nonce`／件数／`slideIndex`／`sha256`／`storagePath` の不一致）なら write 0。
- **署名URLを localStorage・DB・Draft fields・log へ保存しない。** innerHTML へも入れない（`img.src` の DOM property へ `https://` のみ代入）。
- **同一 outputId の再生成を許可しない**（Decision 112・ledger 制約を変更しない）。作り直す場合は新しい Output Draft を作る。
- **新 Decision を安易に追加しない**（本工程では追加していない。最大番号 119 のまま）。

### 次工程（Stage 2A）

**Instagram Publishing / Meta Connection。** 現状は **Meta / Instagram Graph API のコードが 0 件**（`graph.facebook`／`media_publish`／access token 等の参照なし・投稿系 route なし）で、既存は `output_approvals.published` への**手動投稿記録のみ**。Meta App Review（`instagram_content_publish`）は外部依存で所要時間が読めないため、**申請を並行トラックとして先に着手**するのが最短。審査完了までの投稿は現行の手動記録で足りる。**自動実行しない。ユーザー判断待ち。**

---

## 【1つ前の現在地】PRG Core ＋ PRG UI Contract U2 — 本番反映 ＋ Controlled Production Verification（Level 1／Level 2）（2026-09-18〜19・Decision119追記・Tag未作成）

- **現在Version**：**Version1 Final Complete ／ Version1.1 Connected AI Company 開発中**（Version変更なし）
- **現在Phase**：**Phase54 Complete維持 ／ Phase55 未着手**
- **B1**：Production COMPLETE（維持）
- **状態区分**：**Pushed ／ Render Auto-Deploy Verified（ユーザー実測）／ Production Read-only Smoke PASS ／ PRG Core ＝ Production Verified ／ U2 ＝ Production Verified は「表示・canonical GET・Claim選択UI」まで（送信側は `deterministic test verified / production POST未実証`）**
- **観測主体**：Push・Smoke・オフライン事前検証は Claude Code 実行。Render Dashboard 確認・Level 1・Level 2 の Console 操作は**ユーザー実施・ユーザー実測**（Claude Code は production を直接観測せず、production mutation も実行していない）。実施日 `2026-09-18〜19`（正確な時刻は未記録）。

**いま完了しているところ**：Release Preflight（New Regression Gate 12ファイル・1,010/1,010 PASS）→ `git push origin main`（`e5ce21a..9fef9dc8`・fast-forward・force 無し。最初の1回は実行環境の権限制限でブロックされ、ユーザーの「一度だけ許可」確認を経て再実行）→ Render Auto-Deploy commit `9fef9dc` Live（service `ai-company`・`node server.js`・port 10000・`Your service is live`）→ Production Read-only Smoke（`GET /` 200／`GET /api/auth-required` 200／未認証 `GET /api/session-status` 401）→ **Level 1**（本番UI：canonical GET・CI-01/02/03 表示・`revision: (なし)`・Claim 選択 1/3・3/3。Plan／Submit は押していない）→ **Level 2**（専用 disposable scope へ 3 POST：行作成／`full_replace`／`partial_update` がすべて `200`・`ok:true`）。

**Level 2 の revision（検証記録）**：`full_replace` 成功後 `rev-eeffa38d4820371bf05ca41cacce6cab`（CAS 述語 `IS NULL`）→ `partial_update` 成功後 `rev-e8ab5c08e1ebb6b8c34ac008f505965c`（CAS 述語 `= rev-eeffa38d…`）。両 POST とも Evidence 2・Claims 2。成功は「CAS UPDATE がちょうど 1 行に作用した」ことを意味する（0 行→409・2 行以上→500 のため）。

**Git（検証時点の実測・次セッションでも必ず再実測すること）**：branch `main`／HEAD ＝ origin/main ＝ `9fef9dc8d79a24b0e1c94ace9f998a47e1af4426`／ahead-behind 0/0／staged 0／Protected 10件 changed 0。**旧 docs commit `19c50d41` は push 前の amend で `9fef9dc8` に置換済み**（履歴に現れる `19c50d41` は置換前の値であり、現在の HEAD ではない）。

**disposable scope（削除しない・検証用データとして残置）**：`case-prg-verification-20260918` ／ `out-prg-verification-20260918`。`output_drafts` に 1 行・canonical は Claim 2（TEST-01（更新後）・TEST-02）・Evidence 2（`law_regulation`・`user_verified`・本文に「検証用・実在の主張ではありません」と明記）。`cases` テーブルに行は無く案件一覧等には現れない。DELETE／rollback／manual DB edit／cleanup は 0。**実運用（Instagram 投稿・Writer 等）に使わない。**

**証明していないこと（正確な限界）**：①DB の読み戻し（readback）は未実施（「TEST-02 保持・TEST-01 更新」は応答・オフライン検証・CAS 成功の含意からの推論）②**U2 の送信側ブラウザ挙動（`resolutionScope`・POST直前の再GET・payload組立）は本番で未実証**（`contentEvidenceUiResolutionContract` 84/84 の決定的テストのみ）③本番での negative 経路（実409 発火等）は意図的に未実施 ④第一投稿 canonical の無変更は未アクセスによる構成上の保証で、読み戻し確認ではない。Level 1 で Evidence 6件そのものの UI 再カウントも行っていない。

### 次セッションが必ず守る正式契約（追加分・下記【1つ前の現在地（push前）】の6項目も引き続き有効）

- **第一投稿（`case-value-1788410623` ／ `out_1788413020275`）は変更禁止・確認目的でも GET を含め触らない。**（Level 1 は既存UIでの canonical GET と選択表示のみで、Level 2 は一切アクセスしていない。）
- **disposable scope は削除・rollback・再利用しない。** 追加の本番 mutation 検証が必要な場合は、事前にユーザーの明示承認を得る（この scope の再利用可否も含む）。
- **本番 mutation はユーザー本人の認証済みブラウザ Console から実施**し、Claude Code は credential を扱わず production mutation を実行しない。複数 POST は 1 本ずつ「実行→結果確認→STOP」で進める。応答は `status` と `ok` の**両方**で判定する（`POST /api/output-drafts` は `upsert` 失敗等で HTTP 200 のまま `ok:false` を返す経路がある）。
- **Render は Claude Code から read-only 確認できる手段が無い**（CLI 未インストール・`RENDER_API_KEY` 未設定・認証済みセッション無し）。新規 install／token 発行／credential 抽出はせず、ユーザーが Dashboard で確認する。
- **push は auto mode の権限制限でブロックされる場合がある。** その場合は auto mode を終了し、ユーザーの「一度だけ許可」確認で 1 回だけ実行する（「常に許可」は使わない）。既に push 済みの commit は amend しない（docs 訂正は通常の新規 commit）。
- **テストはファイル名を明示した個別実行のみ**（`npm test`／wildcard／auto discovery 禁止）。`server.test.js` は Protected `_meta.json` へ書込む経路を持つため実行しない。`contentValueWiring.test.js` は Supabase 関連 env がすべて UNSET であることを実行直前に確認した場合のみ実行する。

### 次工程

**自動実行しない。ユーザー判断待ち。** 候補：①disposable scope 限定の readback GET の要否 ②U2 送信側の production 実証の要否（実UI経由の POST を disposable scope で行う設計になり、Web Search／承認フローに入らない経路の事前確認が必要）。製品側の次工程はユーザーの指示に従う。**Tag は作成していない。**

### 記録してはいけない表現

「U2 が送信側まで Production Verified」「U2 全体を本番検証完了」「Level 2 で DB を読み戻し確認済み」「canonical 破壊不可を保証」「既存11スイート 991件 再PASS」「Evidence Resolution 全体 Complete」「第一投稿 canonical を本番で読み戻し確認済み」。**記録してよいのは、PRG Core ＝ Production Verified、U2 ＝ 表示・canonical GET・Claim 選択 UI まで Production Verified／送信側は deterministic test verified・production POST 未実証、という範囲のみ。**

---

## 【1つ前の現在地（push前）】Safety Foundation B1 ＋ Partial Resolution Guard（PRG）Core ＋ PRG UI Contract U2 — Code Commit Complete ／ 未push（2026-09-18・Decision119・Tagなし）

> ※本節は **push 前（2026-09-18）の時点の記録**であり、履歴として保持する。以下の「Not Pushed／Not Deployed／ahead」等は当時の状態で、**現在の状態は直上の【現在地・最優先】節**を参照。

- **現在Version**：**Version1 Final Complete ／ Version1.1 Connected AI Company 開発中**（Version変更なし）
- **現在Phase**：**Phase54 Complete維持 ／ Phase55 未着手**
- **（push前の状態区分）**：Code Commit Complete ／ Not Pushed ／ Not Deployed ／ production 実機確認 pending だった

**いま完了しているところ**：docs commit `e37324e`（2026-09-10）以降、公式docsが未同期のまま Code だけが 15 commit 進んでいたため、`e37324e..da24cf29` を Git 実測で再構築して正式記録した。工程は CV-3a/CV-3b Core（`df99437`）→ CV-4b server-side Content Value ＋ `content_type`/`content_value` 列（`07d7027`）→ Output Draft Authentication S2（`79161b8`）→ Internal API Authorization Boundary S3-B1b（`2e3fde7`）→ Cost Tracker Test Isolation（`72bb578`）→ CV-4c-1（`d4ad96a`）→ CV-4c-2 Claim Intent Planning（`1a81d9b`）→ CV-4c-3 Approval State Machine（`201cc17`）→ CV-4c-3B Server Canonical Resolution（`edd096d`）→ UI 修正4件（`6dfc335`/`50ca7de`/`5644449`/`4ec5249`）→ Safety Foundation B1（`e5ce21a`）→ PRG Core + U2（`da24cf2`）。Decision 119「Canonical Content Evidence Formal Truth Boundary」を新規採番（最大 118→119）。

**Git 実測（push前当時の値・履歴）**：branch `main`／HEAD `da24cf29f3f0d3d2b4ef4550694391a57871b51d`（subject `feat: guard partial evidence resolution updates`・7 files・+1,933/-46）／origin/main `e5ce21a5a8e5ec9079e2f4dad4a1e2ce7954500d`／ahead 1 / behind 0／staged 0。当時 PRG Core + U2 は未 push だった（現在の値は直上の【現在地・最優先】節）。

### 次セッションが必ず守る正式契約（追加分）

- **canonical の正本は専用列**（`content_evidence` / `content_claims` / `content_evidence_origin`）。**通常保存は canonical を変更しない。** canonical write は Evidence Resolution 成功時の `writeCanonicalContentEvidence()` 1 経路のみ。増やさない。
- **Evidence Resolution は `resolutionMode` / `targetClaimIds` / `expectedRevision` を必須**とし、完全に成立した場合だけ canonical を更新する（部分成功・反証・Claim を伴わない Evidence・cross-case は request 全体を拒否）。
- **CAS は `content_evidence_origin->>revision`**。1 行＝成功／0 行＝409 `canonical_revision_conflict`／2 行以上＝500（成功扱いにしない）。**409 で自動 retry しない。**
- **UI（U2）は既存 canonical Claim を明示選択**し、claimId を推測しない。POST 直前に再GETし、Claim集合・revision・loadSeq・outputId が変化していれば送信しない。**既存 canonical がある状態での新規 Claim 追加は未対応**（別工程）。
- **第一投稿（caseId `case-value-1788410623` / outputId `out_1788413020275`）の canonical を確認目的で変更しない**（Evidence 6・Claims 3・CI-01/02/03・`origin.mode='legacy_fields_backfill'`）。
- **Protected 10件**（`cost-logs.json` / `data/conversations/_meta.json` / `claude-cost-logs.json` / `claude-quality-history.json` / backup-dup-candidates 2件 / `user-cont-1〜4_line_*.json`）は restore・delete・stage・commit・内容変更をしない。

### Release Preflight 実施結果（2026-09-18・追記）

- **既存「11スイート／991件」baselineは `NOT RECOVERABLE`（正式判定）。** repository 内の全commit message／docs過去版／git notes・stash・reflog／ログ系ファイル／`package.json` を read-only 全数調査したが、正確な11ファイル名・991の内訳を裏付ける一次記録は存在しなかった。過去の「991」出現はすべてcommit hash（`504b991`等）の一部で、テスト件数としての「991」は0件。唯一の実測記録は `df99437` の「既存安全 8 suites 1,204/0」（スイート数・件数・対象領域が不一致）。**推測によるファイル名復元・assertion数合算での逆算は行っていない。**
- **代わりに release change set（`da24cf29` の変更範囲）から New Regression Gate を設計・実測した。** 詳細は `docs/02PHASE_PROGRESS.md`「Release Preflight ─ Historical Baseline Recovery ＋ New Regression Gate Design / Execution」節。
  - REQUIRED 5：`contentEvidenceResolutionGuard.test.js` 110/110・`contentEvidenceCanonicalColumns.test.js` 127/127・`contentEvidenceUiResolutionContract.test.js` 84/84・`contentValueWiring.test.js` 98/98・`outputDraftAuth.test.js` 62/62
  - SUPPORTING 7：`contentEvidenceApproval.test.js` 172/172・`contentEvidenceServerResolution.test.js` 31/31・`contentEvidenceWriterContext.test.js` 19/19・`contentEvidenceBrowserReality.test.js` 22/22・`contentEvidence.test.js` 79/79・`contentValueQuality.test.js` 141/141・`contentClaimPlanning.test.js` 65/65
  - **合計12ファイル・1,010 / 1,010 PASS・0 failed。** すべて `node <filename>.test.js` のファイル名明示・個別実行。
  - `contentValueWiring.test.js` はSupabase関連env（`SUPABASE_URL`等6種）が `process.env` 上ですべてUNSETであることを実行直前にfail-closed確認したうえでlocal fallback modeとして実行した。**今後この test をGateに含める場合は毎回同じprecheckを行うこと。**
  - **`server.test.js` は今回もGateから除外（NOT RUN）。** Protected `_meta.json` への書込み経路を持ち、今回変更への固有coverageも0のため。次セッションでも実行しないこと。
- Git実測（Release Preflight完了時点・push前・docs commit amend前の値）：HEAD `19c50d411d73f237c9ba434843ad0761ae51b21c`（のち amend で `9fef9dc8` に置換）／origin/main `e5ce21a5a8e5ec9079e2f4dad4a1e2ce7954500d`／ahead-behind `2/0`／staged `0`／Protected 10件changed 0。

### 次工程（push前当時の計画・実施済み／現在の次工程は直上の【現在地・最優先】節）

1. 今回のdocs訂正内容を docs commit として反映（当時は未push だった Docs Commit へ amend する Option A／コード変更を混ぜない）→ 実施済み（`9fef9dc8`）
2. ChatGPT/ユーザーへ結果報告・確認 → 実施済み
3. `git push origin main` → 実施済み（`e5ce21a..9fef9dc8`）
4. Render Auto-Deploy 実測確認 → 実施済み（ユーザー実測・Live）
5. 本番 read-only smoke → 実施済み（3/3 PASS）
6. 必要な Controlled Production Verification → Level 1／Level 2 まで実施済み（結果は直上の節）

### 記録してはいけない表現（push前当時のもの・現在の禁止表現は直上の【現在地・最優先】節）

当時は「PRG 本番反映済み」「U2 実機確認済み」「既存11スイート 991件 再PASS」「canonical 破壊不可を保証」「Evidence Resolution 全体 Complete」「Release Preflight PASS = Production Complete」を記録しなかった（未実施・未実測だったため）。New Regression Gate 12ファイル・1,010/1,010 PASS は実測済みの事実として記録してよいが、それをもって「production検証済み」とは書かなかった。**現在は push・Render Deploy・Smoke・Level 1／2 を実施済みで、記録してよい範囲・してはいけない表現は直上の節に従う。**

---

## 【2つ前の現在地】Carousel Background Control（Option B+C）正式Release Complete ＋ Release Docs Sync（2026-09-10・PA-26〜PA-28・Tag `v1.01-carousel-image-production-background-control`）
...
## 【参考・完了済み】Carousel Background Generation Control（Option B+C）Local Implementation Complete ＋ Decision 118（2026-09-10・PA-22F〜PA-25）
...
**Runtime 安全状態（PA-25 で不変）**：`Source Gate=true`・`Runtime Env Gate=unset`・`Effective REAL=false`。Image API 0・paid generation 0・DB write 0・Storage write 0・Publishing 0・Instagram 0。

**この工程で確定したこと**：
- Carousel 背景生成は Background Sanitizer ＋ Post-level Visual Direction ＋ SAFE_SUFFIX の三層 deterministic prompt control（Decision 118）。AI provider の責務は decorative photographic background のみ。
- Sanitizer の conflict term 照合は boundary-aware（`_termRegex`）。`comfortable`／`sustainable`／`guide`／`fluid`／`withdrawing` 等の安全語を誤検知しない。日本語は具体語優先（単漢字「絵」等の generic term は使わない）。
- ローカル安全テスト 8 suites **1,204 assertions / 0 fail**・F1/F2/F3 回帰 PASS。
- Decision 117（overlay readability）は維持。Decision 118 は AI background generation control として責務分離。

**まだやっていないこと（次チャットの候補）**：
1. Release Precheck（read-only）→ ユーザー承認 → `git push`（Code `8ff9f8d` ＋ Docs commit）→ Render Live 確認 → annotated tag。
2. 3回目の controlled paid generation：new Output Draft／new outputId／new approval／new nonce（Decision 112）。approval 発行 → CRITICAL HUMAN CHECKPOINT → generate 1回のみ → 人間 Image Review。
3. 実画像で B+C の効果を確認（現時点では production real-image validation pending・「解決済み」とは書かない）。

**過剰主張しないこと**：「実画像で B+C PASS」「UI/icon 生成完全防止」「Visual Consistency 完全解決」「production validation 完了」は未達。正確には "Option B+C implementation complete / 1,204 local safe assertions PASS / real-image validation pending"。

---

## 【参考・完了済み】Production Activation Step PA-2 — Storage Privileged Production Client 実装Complete（2026-09-08・Decision 114実装完成）
...
**Implemented / Production Not Activated。** bucket未作成・Render `SUPABASE_SECRET_KEY`等未設定・実Storage upload 0・paid generation 0。

### 今回やったこと

新規 `lib/carouselAssetStorageSupabase.js`（Storage専用server-only privileged client）を実装し、`lib/carouselImageRoutes.js` の `buildProductionDeps().storageClient` へ配線した。

- credential優先順位：`SUPABASE_SECRET_KEY`優先→`SUPABASE_SERVICE_ROLE_KEY`fallback（Decision 110と同一規約）。`NEXT_PUBLIC_*`・anon keyは一切参照しない。
- **Decision 110のledger専用client（`lib/carouselExecutionSupabase.js`）は横展開せず**、完全に独立したmoduleとして実装（module責務分離）。
- credential不足時 `carouselAssetStorageClient = null` → `buildProductionDeps().storageClient` も `null` のまま。既存 `lib/carouselAssetStorage.js`／`lib/carouselAssetAccess.js`（いずれも無変更）が `storage_unavailable` でfail-closedするため、実Storage networkへの到達は変わらず0。
- `createClient()` はネットワークリクエストを発生させないlazy clientのため、moduleをrequireするだけでは実接続しない。
- bucket自動作成・Storage policy変更は一切行っていない。
- 新規テスト `carouselAssetStorageSupabase.test.js` **31/0**。既存6スイート回帰 **835/0**（総計 **866/0**）。
- Decisionは新規採番せず、**Decision 114（`docs/04DECISIONS.md`）へ実装完成として追記**。

### 次セッションが必ず守る正式契約（追加分）

14. **Decision 110のledger専用clientとStorage専用clientを統合・共有しない。** モジュール責務の分離（credential漏洩時のblast radius最小化）を維持する。
15. **`lib/carouselAssetStorageSupabase.js` からbucket作成・Storage policy変更を行わない。** それらは別工程（bucket provisioning／Render secrets設定）の責務。
16. **secret値（`SUPABASE_SECRET_KEY`等）を取得・表示・ログ出力・repo内へ保存しない。**

---

## 【参考・完了済み】Production Activation Step PA-1 — Real Image Generation Dual-Key Kill Switch 実装Complete（2026-09-08・Decision 115）
...
**Implemented / Production Not Activated。** `REAL_ENABLED=false`（source gate）維持・Render `CAROUSEL_IMAGE_REAL_ENABLED` 未設定・実OpenAI API 0・paid generation 0。

### 今回やったこと

`lib/carouselImageClient.js` のみを変更し、実Image API到達条件を **dual-key** へ拡張した。

```
実API到達条件 = _sourceRealEnabled(source) === true
              AND process.env.CAROUSEL_IMAGE_REAL_ENABLED(env) === 'true'（厳密一致）
```

- env未設定／`''`／`'false'`／`'TRUE'`／`'1'`／`'yes'`／前後空白付きはすべてdisabled。
- source gateは削除していない（envだけで実解禁できる構造にしない）。
- `REAL_ENABLED` をgetter/setterへ変更し、`shared/carouselImageCore.js`（変更禁止）の `assertRealCallAllowed()` と `generateBackground()`／`generateBackgroundRaw()` が**常に同一のdual-key値**を参照するようにした（乖離すると reserve だけ先に成功してbudget/nonceを浪費する経路が生まれるため）。
- 既存テストの `client.REAL_ENABLED = true/false` というsave/restoreパターンはsetterがそのまま受理（後方互換）。
- 新規テスト `carouselRealEnabledKillSwitch.test.js` **51/0**。既存5スイート回帰 **784/0**（総計 **835/0**）。
- **Decision 115** として正式化済み。

### 次セッションが必ず守る正式契約（追加分）

11. **`CAROUSEL_IMAGE_REAL_ENABLED` の値は絶対にレスポンス・ログへ出力しない。**
12. **`REAL_ENABLED` を再びプレーンなdata propertyへ戻さない。** getter/setterを外すと、`assertRealCallAllowed()` と `generateBackground()` のgateが再び乖離しうる。
13. **dual-keyのenv gateは厳密一致 `'true'` のみ。** 将来これを変更する場合は既存テスト（`carouselRealEnabledKillSwitch.test.js` の1〜6系）を先に確認する。

---

## 【参考・完了済み】Carousel Image Production Phase 2-E Production Connection C-1〜C-5-pre 実装Complete（2026-09-08・Decision 112／113／114）

> ⚠ **本セクション内の「kill switch＝未実装」という記述はPA-1（Decision 115）で、「Storage privileged production client＝未実装」という記述はPA-2（Decision 114実装完成・上記【現在地・最優先】参照）で、それぞれ解消済み。** 本セクションはC-5完了時点の履歴記録として保持する。

- **現在Version**：**Version1 Final Complete ／ Version1.1 Connected AI Company 開発中**（Version変更なし）
- **現在Phase**：**Phase54 Complete維持 ／ Phase55 未着手**（Carousel Image Productionを理由にPhase55へ進めない）

### 状態区分（最重要・混同禁止）

**Implemented / Production Not Activated。**
コード実装とdeterministic testは完了しているが、**本番はまだ何も動いていない**。

| 項目 | 実状 |
|---|---|
| 実Supabase DB適用 | **0**（`carousel_image_executions` は実DBに存在しない） |
...
| kill switch（`CAROUSEL_IMAGE_REAL_ENABLED`） | **未実装** |
| Render secrets | **未設定**（`SUPABASE_SECRET_KEY`／`CAROUSEL_APPROVAL_SECRET`／`WEB_SESSION_SECRET`／`WEB_TRUSTED_ORIGIN`） |
| Image Review UI | **0**（`index.html` に `carouselAssets` 参照0件） |
| commit／push／tag／deploy | **未実施**（HEAD `21b20330…` はorigin/mainより ahead 1・未push） |
| `REAL_ENABLED` | **false 維持**（`lib/carouselImageClient.js:91` の source constant） |
| paid generation／実Image API | **0件** |

### 完了内訳

- **C-1**：TOCTOU fix。partial UNIQUE index を `WHERE status IN ('in_progress','completed')` へ拡張（**Decision 112**）。→ **1 Output Draftにつきcompleted generationは最大1件**。
- **C-2**：`lib/carouselAssetStorage.js`（private bucket前提・upsert=false・PNG signature／32MB／SHA-256検証・public URL不生成・自動delete rollbackなし）。
- **C-3**：production routes 3本＋`lib/carouselImageService.js`（HTTP非依存）／`lib/carouselImageRoutes.js`／`lib/carouselAssetAccess.js`。`server.js` へ登録済み。
- **C-4**：Preflight（read-only）。必須修正2件を発見。
- **C-5-pre**：(1) `billingLock` 契約の修復、(2) `lib/webSession.js` による **server-side signed session ＋ Origin検証**を3routeへ必須化（**Decision 113**）。
- **Decision 114**：Carousel Asset Storage Server-only Trust Boundary。

### 次セッションが必ず守る正式契約

1. **`REAL_ENABLED=false` を維持する。** 解禁はユーザーの明示承認後・下記の残工程を全て終えてから。
2. **有料実行は Session ∧ Approval Token ∧ `billingLock===false` ∧ Budget/Ledger のAND**。どれか1つでも欠ければ生成へ到達しない。この構造を緩めない。
3. **Decision 112 は Decision 111 と同義ではない。** low 7枚＝¥45.2816、2回で¥90.5632＝¥100以内のため、Decision 111単体なら2回成立し得る。Decision 112はそれを超える**追加のSafety Policy**。docsへ「同義」と書かない。
4. **Image Review NG時は新しいOutput Draft（新output_id）で再生成する。** completed ledgerの書き換え・nonce再利用・C-1 predicate緩和は禁止。
5. **課金不明な `in_progress` のSafety Lockを解除しない。** `attempted_provider_calls = 0` が証明できる場合のみ `failed_before_charge`、それ以外は `unknown_billing`。行のDELETE禁止。
6. **Storage orphanを自動削除しない。**
7. **`published=true` へ自動昇格しない**（生成成功≠Image Review Approved≠Publishing・Decision 107 EER境界を保持）。
8. **`schema.sql` 全文をSupabase SQL Editorへ貼らない。** 他テーブルの裸の `CREATE POLICY` が31本あり再実行で衝突しトランザクション全体がrollbackする。**`carousel_image_executions` ブロックのみ**適用する。
9. **DB適用後は必ず `SELECT indexdef FROM pg_indexes WHERE indexname='uq_carousel_exec_active_output';` でpredicateを実測確認する。** `CREATE UNIQUE INDEX IF NOT EXISTS` はpredicate差を検出せず黙ってno-opになる。
10. **secret値を取得・表示・ログ出力・repo内へ保存しない。**

### Production Activation 残工程（REAL_ENABLED=true まで）

1. kill switch（`CAROUSEL_IMAGE_REAL_ENABLED` env二重鍵）実装＋Decision採番
2. Storage privileged production client 実装（`lib/carouselAssetStorageSupabase.js` 等）＋`storageClient` 配線
3. 回帰テスト
4. code／docs commit
5. `carousel_image_executions` ブロックのみのDB適用
6. `pg_indexes` によるpredicate実測確認
7. private `carousel-images` bucket 作成
8. bucket MIME（`image/png`）／size（32MB）設定
9. Storage policy確認（anon／authenticated policy 0本）
10. Render server-only secrets 設定
11. push／deploy
12. 本番 fail-closed 疎通確認（`REAL_ENABLED=false` のまま）
13. `REAL_ENABLED` 解禁（source＋envの二重鍵）
14. 初回 paid calibration run（`case-value-1788410623`／`out_1788413020275`／medium／7枚／authorization estimate **¥92.5232**＝実請求額の保証ではない）
15. ledger／usage／Storage 検証
16. Image Review（最小手段で目視）
17. calibration実測のdocs記録

**Publishingはこの後の別工程。**

### security backlog（Carousel由来ではない）→ **Resolved（Implemented / Not Yet Deployed）・PA-3A**

**旧状態（PA-3 §17まで）**：`server.js` の `express.static(path.join(__dirname))` により repo root が広く静的公開対象になっている（`docs`／`data/conversations`／`lib`／`server.js`／`supabase/schema.sql` 等）。`.env.local` は serve-static の dotfiles 既定により配信されず **secret leakは確認されていない**が、over-broad static exposure として要レビュー。Carousel工程と混ぜて修正しないこと。 PA-3 の再評価で、本releaseは Decision 110〜115（認可境界の設計と既知の弱点）を1箇所に列挙した docs を新規追加するため、**単なるbacklogではなく本番前必須修正**と判定した。

**修正（PA-3A・Implemented）**：`app.use(express.static(path.join(__dirname)))` を廃止し、新規 `lib/publicStatic.js` の **allowlist型 public asset boundary** へ置換した。denylist（「これは隠す」）は将来追加される server-only ファイルが自動公開されるため採用していない。

- **配信する（browserが実際に要求する asset のみ・index.html全走査で実測）**：`/`（→`index.html`）／`/index.html`／`/shared/agentResultNormalizer.js`／`/shared/evidenceAcquisition.js`／`/shared/iadpIntelligenceContext.js`／`/shared/instagramAccountDesign.js`／`/shared/instagramAccountDesignQuality.js`／`/members/<id>.md`（単一segment・`.md`のみ）
- **配信しない**：`docs/`／`lib/`／`shared/`（上記5ファイル以外）／`supabase/`／`data/`／`assets/`／`members/` のサブディレクトリ／`node_modules/`／`server.js`／`package.json`／`package-lock.json`／`*.test.js`／`*.log`／`*.err`／`*.ps1`／root直下の内部MD・内部JSON／dotfile（`.env*`／`.git*`）
- **traversal / percent-encoding / backslash / 制御文字 / dot segment / case variation** はすべて拒否（`%` を含む path はデコード曖昧性を避けるため一律拒否）。
- **SPA fallback は存在しない**ため、内部path要求は index.html の 200 ではなく **404** になる（漏洩の有無と HTTPステータスが一致する）。
- 許可asset の配信契約（Content-Type／ETag／Last-Modified／Range／conditional GET／`/`→`index.html`）は既存 `express.static` へ委譲したまま**変更していない**。変わるのは配信対象の集合だけ。
- 認証（`WEB_APP_PASSWORD`／`WEB_SESSION_SECRET`／localStorage UI gate）と Carousel の課金安全境界（billingLock／Approval Token／Budget／Ledger／Dual-Key／Storage trust boundary）は**一切変更していない**。

**実測**：`staticExposureBoundary.test.js` **211 passed／0 failed**（新規・実HTTP listen 0／実network 0）。加えて loopback ephemeral port による end-to-end 実測 **36/36**（公開10path が 200・内部26path が 404）。

**状態**：**Deployed / Production Verified（PA-6・2026-09-08）**。PA-4でcode/docs commit（`e0dadc4`／`341eaa7`）→ PA-6で `main` へpush（`02f8f6c..341eaa7`）→ Render Auto-Deploy → 本番URL（`https://ai-company-l45x.onrender.com`）へのread-only GETで実測：**blocked internal paths 8/8 = 404**（`/docs/04DECISIONS.md`等）・**public allowlist resources = 200**（`/`・`/index.html`・`shared`配下2件）。Production Security Verification PASSED。

**新Decision番号**：PA-3A時点では**採番していない**（security bug fix扱い）だったが、**Production Activation Step PA-3B（2026-09-08）でユーザー承認を得て Decision 116（Public Static Asset Allowlist Boundary）として正式採用済み**。詳細は `docs/04DECISIONS.md` Decision 116参照（docs-onlyの正式化・code変更0）。

### 実測テスト（2026-09-08時点）

`carouselAccessControl` **80 passed／0 failed**／`carouselImageRoutes` **95／0**／`carouselAssetStorage` **54／0**／`carouselExecutionDb` **88／0**。

---

## 【参考・完了済み】Phase IG-QC / B-7F Quality Gate Package Routing Fix 正式リリースComplete（2026-08-20・Decision105）
...
- **次工程**：プラファストAffiliate Evaluation登録・商品採用・APFR実Fact登録（21/21）は**2026-08-22に完了**。続くAPFR **Phase 0（再Adopt時Fact消失防止・`d69ff60`）**・**Phase 1（Current Fact Resolver・`46c51ef`）**・**CUI-0（Correction-aware Duplicate Policy・`9ad76f8`）**も同日に実装・検証・push完了し、**Step C開始条件となる4Contract（Current Fact Resolver／Correction／ambiguity fail-closed／legacy Fact handling）はすべて正式確定**、加えて**CUI-0によりCorrection Contractがduplicate policy側でも成立**。さらに2026-08-23に**CUI-1（Current Fact / History UI・`1cf3b2e`）**を実装・検証・push・本番反映し**Resolverを初めてUIへ表示専用接続**、続けて**CUI-2（Correction UI Core・`fd99134`）**を実装・検証・commitして**訂正操作＋`supersedesFactId`自動付与**まで到達し、2026-08-24に**CUI-0/CUI-1/CUI-2をまとめてmain push（HEAD/origin/main `186ec637...`・ahead/behind 0/0）・正式Tag `v1.01-apfr-correction-ui-core`（Annotated・target `186ec637...`）作成・tag push・Render本番反映まで完了**した（本番実データread-only検証もComplete）。続けて2026-08-24、ユーザー本人が本番URLへ認証後ログインし実ブラウザでCorrection UIを目視確認：resolved Fact行の「訂正」ボタン表示・Correction modeへの正常遷移・現在値の正常表示・field固定表示・新しい値入力欄が空（旧値自動コピーなし）・「訂正をやめる」正常表示・cancel後の通常モード復帰（Correction Target残留なし）をすべて確認し、**「確認済みFactとして登録」は押していないためCorrection Fact登録0・本番Fact変更0・DB書き込み0を維持**（Console Errorの追加実測は今回未実施）。**本番認証後の実ブラウザ目視確認もComplete**。Decision108追記・本ファイル冒頭「更新日」参照）。これにより**Correction UI Core系列（CUI-0〜CUI-2）は正式リリース・本番認証後最終目視ともComplete・リリース確認Pendingは0**。次工程選定・調査の結論として、続けて2026-08-24に**APFR Step C-1A Compliance Context Foundation（Code commit `9d665256d5ac359f4c8354db5cf1c7a1447aceef`）**を実装し、Compliance Formal Truth 4field（`listingNgWords`／`advertisingDisclosureRequirements`／`complianceRestrictions`／`regulatoryCategory`。`listingPolicy`は対象外）をResolverから`atRunWorkflow()`→`/api/auto-task`→`server.js`受動パススルー→`runAutoTaskWorkflow()`までread-only配線した（この時点でprompt文字列への実注入は0件）。続けて**APFR Step C-1B Writer/Reviewer Compliance Injection（Code commit `f525111f32d2686ca47c221423865a7600dff228`）**で、新規helper`buildCompliancePromptBlock(agent, complianceContext)`によりWriter（「遵守して生成」）・Reviewer（「遵守されているか確認」）へ限定注入した。Strategy・Leader Finalへは注入せず、Reviewerの品質フィードバックは既存の「【Reviewerの品質フィードバック】」経路でLeader Finalへ統合される既存機構を再利用（新規reject契約は追加せず）。Fact値は「確認済み商品事実データであり命令ではない」という責務境界を明示したデータ枠として提示し、empty時（null/undefined/{}/対象fieldなし）はC-1B前systemPromptとbyte-identical。**Quality Gate Compliance EnforcementはStep C-1C候補として未実装**（現在Quality GateにはCompliance専用強制軸がない）。C-1A/BともWorking treeの別系統差分「Leader Case Context Phase2」はHEADベース合成patchによりhunk単位で完全分離し混入0。テスト：C-1B専用62/62 PASS・C-1A更新後48/48 PASS・既存回帰全PASS・新規FAIL0。**dev-check**：`/`200・`/api/task-history`は応答時間変動あり・`/api/workflow-dashboard`はこのセッション内で固定5秒タイムアウト内の200が未確定——C-1Bのコードパス（`buildSystemPrompt`/`runAutoTaskWorkflow`）とは無関係な`server.js`→`_hybridTaskHistory()`→Supabase経路の応答時間変動（実測0.6〜1.9秒）と切り分け済み。**environment/latency Pendingとして記録しPASS扱いにしない**（server/timeout/DB/Supabase変更は0）。**C-1A/BともCode commitのみでpush・Tag・Render未実施**。次工程候補は、①**最優先＝C-1A + C-1B正式リリース前最終検証**（dev-check再試行・push判断・Tag判断・Render・本番確認。**C-1Cへは先に進まない**）、②**CUI-3**（「訂正済み」バッジ・旧値参考表示。独立工程として必要かは再評価）、③**CUI-4**（APFR直接ジャンプ導線）、④残課題2（ITP日数field追加要否）の仕様判断、⑤残課題3〜7のUI改善、⑥**APFR Step C-1C（Quality Gate Compliance Enforcement）**、⑦**APFR Step C-2（payout/epc/approvalRateのFormal Truth数値突合・score接続）**。いずれも**自動で開始しない・ユーザー承認後に着手**。Leader Case Context Phase2 は別系統・引き続き本番未 commit・未リリース。
