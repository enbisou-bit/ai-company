...
- **次工程**：プラファストAffiliate Evaluation登録・商品採用・APFR実Fact登録（21/21）は**2026-08-22に完了**。続くAPFR **Phase 0（再Adopt時Fact消失防止・`d69ff60`）**と**Phase 1（Current Fact Resolver・`46c51ef`）**も同日に実装・検証・push完了し、**Step C開始条件となる4Contract（Current Fact Resolver／Correction／ambiguity fail-closed／legacy Fact handling）はすべて正式確定**（Decision108追記・本ファイル冒頭「更新日」参照）。以後の次工程候補は、①**Correction UI**（`supersedesFactId`を付与する訂正操作・現在値／履歴表示）の実装要否判断＝残課題1の残り、②残課題2（ITP日数field追加要否）の仕様判断、③残課題3〜7のUI改善、④**APFR Step C（ASP/Product Intelligence接続）**─ Resolver経由（`resolved`のみ）で読み取り、`ambiguous`は利用禁止・`facts`直接走査は禁止。いずれも**自動で開始しない・ユーザー承認後に着手**。Phase 0/1のCode commitはpush済みだが**Tag・Render反映は未実施**（正式リリース判断は関連工程とまとめて行う）。Leader Case Context Phase2 は別系統・引き続き本番未 commit・未リリース。
