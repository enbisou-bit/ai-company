...
- **⚠ 重要な原則（次工程への引き継ぎ）**：
  - Marketは**現在案件内の同一市場候補商材群を集約する説明層**。集約単位は**案C（案件内集約）**を正式採用（案件横断は`caseId`必須制約＋案件境界原則により不採用）。
  - 集約キーは既存`_aicNormalizeKeyPart`を再利用（ASP工程5-1と同一正規化）。**意味的表記揺れ（美容/ビューティー）は自動統合しない**。実運用でmarket欄に他項目のコピペ混入があると「別市場」として扱われる（誤入力の再登録で解消・削除不要＝再評価の冪等機構）ことを実機で確認。
  - **新規入力項目は追加しない**。既存Product Intelligence入力14項目のみを使用する。
  - Evidenceは新規生成しない。**共通ヘルパー`_intelSyncMarketGroupProductEvidence`**（`_aicBuildMarketForRow`・`adoptAffiliateForContentPlanning`の両方から共通利用）が市場内対象商材群の既存Product Evidenceに`usedBy:'market'`を冪等追記。
  - **★工程8-3B補正の教訓**：初回実装ではderived集計（productCount等）が複数商材対象なのに、Evidence/Confidence母集団は採用商材1件分のみという不整合が生じた（保存フローが採用商品のみEvidence同期していたため）。**「集計対象」と「Evidence母集団」が一致しているかは、複数商材/複数レコードを集約する層を作る際に必ず検証すること**。
  - `INTEL_MARKET_MIN_PRODUCT_COUNT=2`未満は強制insufficient（ASP同型）。**1商材市場でも構造・derived集計値は保持**し、statusのみinsufficientにする。
  - **★意味の固定**：Market Confidenceは登録候補情報の根拠充足度を示すのみで、実検索数・実トレンド・市場規模・市場成長率・実需要は示さない。Safety表記を画面上に必ず表示する。
  - 保存済み判定は**marketKey＋caseId一致**（productIdentifierではない＝Marketは単一商材ではなく市場単位）。表示位置はランキングカード・AICパネルともCompetition直下。
  - 採用時は7項目を同一Output Draftへ**七書き**し既存push1回のみ（採用1回=POST1回・Market専用POSTなし）。挿入位置は`nextIntelligenceContext.competition`構築の直後。
  - Copy Full Reportは`_aicBuildMarketReportText`をCompetitionの直後・Rankingの手前へ追記（順序：Leader→ASP→Content→Competition→Market→Ranking）。
- **Git現在地**：branch **main**／Code commit **2de9317（foundation）＋4ef70ca（ui）＋e61e7d5（persist）＋3b1e5b7（Evidence整合修正）**／docs commit＝本更新（以降の最新HEAD）／Annotated Tag **v1.01-affiliate-market-opportunity-persistence**（作成予定）／**main push・Render反映はこれから**（ユーザー承認後）。保護対象4件は未stage・未commitで保護。
- **検証済み**：純関数・UI・保存・補正テスト計115アサーション全PASS・dev-check 200/200/200・Console 0・回帰なし。**実Supabase検証（専用caseId `case-ms79vojuf7g1`・商材2件）**：七書き保存確認／productCount=2・productIdentifiers2件／Evidence総数28件中22件が両商材にまたがる（母集団整合の実証）／derived集計値が実値と完全一致／全7層のconfidence.calculatedAtが同一ミリ秒台（単一トランザクション実証）／F5復元完全一致／案件切替混入なし／Copy Full Report確認／Product・Revenue・ASP・Content・Competition・ranking回帰なし／**テストデータ限定削除 remaining=0（affiliate_evaluations・output_drafts・cases の3テーブルとも）**確認済み。**PC本番確認・iPhone実機確認はこれから実施**（push・Render反映後にユーザー実施予定）。
- **次工程**：未定（Market Opportunity Intelligence完成によりVersion2 Core 7層のうち6層完成・残るは⑦Self Improvement Intelligenceのみ。**Instagram実運用前のため実績データ供給条件を再確認してから着手判断**。またはVersion1.1 Realtime Sync残課題等）。実装順＝Product→Revenue→ASP→Content→Competition→Market（**表示・永続化まで完了**）→Self Improvement。**Phase55未着手のまま維持**（Decision 083）。
- **⚠ 既知の残テストデータ（スコープ外・未処理）**：実案件「インスタグラムアカウント設計」(`case-mry3oyqamqlp`)に別セッション由来と思われるテスト評価（evaluationId 19・`TEST_CONTENT_PROD_20260729`）が引き続き残存。本工程でも対象外のため未削除。整理する場合は`case_id='case-mry3oyqamqlp'`かつ該当product_identifier限定での削除可否をユーザーに確認すること（実案件本体は削除しない）。
