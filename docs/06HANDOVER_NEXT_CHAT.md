...
- **次工程**：External Execution Record（EER）実装工程（`FORMAL_CASE_FIELDS`への`externalExecution`追加・保存/復元配線・UIボタン実装）。**まだ実装開始しない**。実装指示は次のユーザー承認後に作成する。実装後、`case-msr9yckye65y`の現実側の実行事実（Instagram Account Created／A8.net Registered／A8.net Media Registered）を正式記録する。Leader Case Context Phase2 は別系統・引き続き本番未 commit・未リリース。
