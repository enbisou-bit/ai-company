...
- **次工程**：プラファストAffiliate Evaluation登録・商品採用・APFR実Fact登録（21/21）は**2026-08-22にすべて完了**（Decision108追記・本ファイル冒頭「更新日」参照）。以後の次工程候補は、①残課題1（同一fieldへの複数Fact存在時の最新採用ルール明文化＝**Step Cの前提**）の設計判断、②残課題2（ITP日数field追加要否）の仕様判断、③残課題3〜7のUI改善、④APFR Step C（ASP/Product Intelligence接続）。いずれも**自動で開始しない・ユーザー承認後に着手**。Leader Case Context Phase2 は別系統・引き続き本番未 commit・未リリース。
