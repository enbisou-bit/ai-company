...
- **⚠ 重要な原則（次工程への引き継ぎ）**：
  - **Constitution Gateは「Package複製の可否」のみを制御する**：Decision自体の採否判定（decisionStatus）やOutput Draftの生成そのものには影響しない。将来Gateの適用範囲を拡張する際は、この境界（第9条・正本一意性）を維持すること。
  - **通常運用ではApproved Decision Packageは常にnull**（Quality Gate・Completion Gate未定義のため）。今回追加したGate条件は通常運用下では実質的に発火しない設計であり、Approved到達条件が将来変わった際に初めて意味を持つ。
  - **`_constitutionValidation`はEDE内以外で代入しない**（Phase B-5C-1の原則を継続）。Gateの安全性はこの前提に依存している。
  - **AI社員カードの「期限」表示は本工程でも変更していない**（別途対応保留・本工程の対象外）。
  - Quality Gate・Completion Gate・Decision Ledger・Executive Memory・Validator違反時のユーザー通知UIはいずれも未実装（Phase B-6後段以降候補）。
...
- **Git現在地**：branch **main**／Code commit **9436fec**＋docs commit（本更新）／Annotated Tag **v1.01-executive-constitution-gate**／main push・Render反映。
- **次工程候補（優先順位未確定・比較対象として並列記録）**：Validator違反時の制御設計／Quality Gate調査・設計／Completion Gate調査・設計／Decision Ledger／AI社員カード期限表示廃止。**特定の1つを自動確定せず、正式な次工程はユーザー承認後に決定する**。**Phase55未着手のまま維持**（Decision091）。
