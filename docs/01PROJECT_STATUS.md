...
### Git・反映
Code commit **28fa51c / 1d04f31 / 3ef7495**（index.htmlのみ）／docs commit＝本更新／Annotated Tag **v1.01-affiliate-product-intelligence-persistence**／**main push・Render反映**。**iPhone実機確認はユーザー実施**（本番URLで採用/保存/復元/既存Workflow回帰なしを確認）。保護対象4件（`cost-logs.json`・`claude-cost-logs.json`・`claude-quality-history.json`・`backup-dup-candidates-20260714/`）は未stage・未commitで保護（Decision 077）。
