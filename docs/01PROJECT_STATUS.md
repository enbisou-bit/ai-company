...
- **現在Phase**：**Phase54 Complete維持 ／ Phase55 未着手**（Phase 境界の変更なし）
- **背景**：Carousel Image Production は backend/API としては完成していたが、**Output Engine UI へ一度も接続されていなかった**（`index.html` に `carousel-image`／`approvalToken`／`carouselAssets`／`imageReviewOk` の参照が 0 件・git 履歴にも追加記録なし）。そのため実運用に DevTools Console が必須だった。本工程はこの接続だけを行う。
- **Code commit（実測）**：
  - `2a27c12b487b792222fd7963b03e4c8082ecd53c` — `feat: surface content value diagnosis before approval`（`index.html` のみ・+173/−0）
  - `b738e82cd22da1589672a9edf018f7babb2a3ad2` — `feat: add server-side carousel production flow`（4ファイル・+945/−0）
  - `01d31bc10725983de8998b011cbe3572a7345f6b` — `feat: add carousel image review panel`（3ファイル・+774/−5）
- **Stage 0（Content Value Diagnosis）**：server が保存時に再計算して `output_drafts.content_value` へ保存済みの評価結果を、**Mobile Approval の直前に表示のみ**で提示する。score／status／FAIL 評価軸／改善ポイント上位3件を通常表示し、前提条件・全 blockingReasons・実測値・4件目以降の改善ポイントは折りたたみ。**hard gate ではない**（Mobile Approval・Output Draft・Carousel Generate・Publishing のいずれの可否条件にも影響しない）。理由：canonical Evidence と Reviewer novelty（`fields.contentReviewerSignal.novelty`）の配線が未完のため、現段階の gate 化は誤ブロックを生む。**`shared/contentValueQuality.js` は変更していない**（評価ロジック・閾値とも無変更）。
- **Stage 1a（Console-free Carousel Production）**：`GET /api/carousel-image/quote`（read-only・token 発行 0・課金 0・DB write 0）と `POST /api/carousel-image/produce`（既存 approval → generate を **server 内部で連結**）を追加し、Output Engine から見積り確認 → 明示承認 → 生成まで到達可能にした。**approvalToken は response・log・client のどこにも出さない**。`billingLock` は client から受け取らず produce route 内部で `false` 固定。`confirmedCostJpy` が server 再計算額と一致しない場合は 409 `estimated_cost_mismatch` で停止し、自動で課金へ進まない。二重生成の正本は既存 execution ledger（`nonce` UNIQUE ／ `uq_carousel_exec_active_output`）。auto retry・loop・timer は 0。
- **Stage 1b（Image Review）**：既存 `GET /api/carousel-image/assets` のみで7枚サムネイル・拡大・前後移動・URL 更新を提供し、`fields.carouselAssets[i].imageReviewOk` を正本に OK／未OK を記録する。**保存直前に必ず `GET /api/output-drafts` で最新 Draft を取得し、最新 fields を基準に `imageReviewOk` だけを変更**して `{outputId, caseId, fields}` のみ送る（whole-fields data loss 防止）。表示時と保存直前で `carouselGeneration.nonce`／件数／`slideIndex`／`sha256`／`storagePath` のいずれかが異なれば **write 0 で fail-closed**。署名URLは localStorage・DB・fields・log へ出さず、innerHTML にも入れず描画後に `img.src`（DOM property・`https://` のみ）へ代入する。再生成ボタンは置かない（Decision 112・ledger 制約とも無変更）。
- **非接触（git 差分 0 で確認）**：`server.js`／`shared/`／`supabase/schema.sql`／`lib/carouselImageClient.js`／`lib/carouselExecutionDb.js`／`lib/carouselAssetStorage.js`／`lib/webSession.js`。新 DB 列・migration・新 paid route は追加していない。既存3route（approval／generate／assets）の middleware・handler・契約も無変更。
- **テスト（ローカル実測）**：Stage 1b `imageReviewPanel.test.js` 89/0、Stage 1a `carouselImageProduceRoute.test.js` 98/0、既存回帰 `carouselImageProduction` 593/0・`staticExposureBoundary` 215/0・`carouselImageRoutes` 95/0・`carouselExecutionDb` 88/0・`carouselAccessControl` 81/0・`outputDraftAuth` 62/0・`carouselAssetStorage` 54/0・`carouselRealEnabledKillSwitch` 51/0。**合計 1,433 assertions / 0 failed**（ファイル名を明示した個別実行のみ）。
...
**状態区分：Pushed ／ Render Auto-Deploy Verified（ユーザー実測）／ Production Read-only Smoke PASS ／ PRG Core ＝ Production Verified ／ U2 ＝ Production Verified は「表示・canonical GET・Claim選択UI」まで。U2 送信側は `deterministic test verified / production POST未実証`。**

- **現在Version**：**Version1 Final Complete ／ Version1.1 Connected AI Company 開発中**（Version変更なし）
- **現在Phase**：**Phase54 Complete維持 ／ Phase55 未着手**
- **B1**：Production COMPLETE（維持）
- **実施日と観測主体**：`2026-09-18〜19`（正確な時刻は未記録）。Push・Smoke・オフライン事前検証は **Claude Code 実行**。Render Dashboard確認・Level 1・Level 2（Console操作）は **ユーザー実施・ユーザー実測**（Claude Code は production を直接観測していない）。
- **Git（検証時点の実測）**：branch `main`／HEAD ＝ origin/main ＝ `9fef9dc8d79a24b0e1c94ace9f998a47e1af4426`／ahead-behind 0/0。Push `e5ce21a..9fef9dc8`（fast-forward・force 無し。最初の1回は実行環境の権限制限でブロックされ remote は変化せず、ユーザーの「一度だけ許可」確認を経て再実行した1回のみが反映）。push対象は `da24cf29`（Code：`feat: guard partial evidence resolution updates`）と `9fef9dc8`（Docs：`docs: sync evidence safety and resolution guard status`）の2 commits。**旧 docs commit `19c50d41` は push 前の amend で `9fef9dc8` に置換されており、現在の履歴には含まれない**（以前の記録に現れる `19c50d41` は置換前の値）。
- **Render Auto-Deploy（ユーザー実測）**：service `ai-company`／branch `main`／deployed commit `9fef9dc`／trigger Auto-Deploy／status Live／`node server.js`・port 10000・`Your service is live`／起動失敗・crash なし。Claude Code は Render を read-only で確認できる手段（CLI／API key／認証済みセッション）を持たないため未観測。
- **Production Read-only Smoke（Claude Code 実行・GET 3回・認証なし・retry 0）**：`GET /` 200／`GET /api/auth-required` 200（`{"required":true}`）／未認証 `GET /api/session-status` 401（`{"ok":false,"reason":"unauthorized"}`）。実行前にコードで3 endpoint の DB write 0・AI/provider 到達 0 を確認。
- **Level 1（本番UI・ユーザー実測）**：第一投稿案件で Content Evidence パネルの「既存Evidenceを確認」（GET）を1回押下 → canonical Claim **CI-01（洗顔時の摩擦）／CI-02（洗顔後の保湿）／CI-03（日常の紫外線対策）** が表示され `revision: (なし)`。CI-01 のみ選択（1/3）→ CI-01＝selected・他 unselected、続けて CI-02・CI-03 を追加選択（3/3）を確認。**「選択したClaimでPlanを作成」「Evidence確定（serverへ送信）」は押していない**（Plan／Submit／Resolution POST／canonical mutation／Output Draft保存 すべて 0）。**Evidence 6件そのものの UI 再カウントは行っていない**（Evidence 6件は B1 本番実測の正式 baseline としてのみ扱う）。
- **Level 2（専用 disposable scope・ブラウザConsole の `fetch`・ユーザー実測／Claude Code は production mutation 0）**：対象 `case-prg-verification-20260918` ／ `out-prg-verification-20260918`（第一投稿とは完全に別。第一投稿の ID は一切送信していない）。

  | POST | 内容 | 結果 | CAS 述語 |
  |---|---|---|---|
  | 1 | disposable 行の作成（通常保存・PRG経路に入らない） | `200`／`ok:true` | ― |
  | 2 | `full_replace`（TEST-01／TEST-02・`expectedRevision: null`） | `200`／`ok:true`／Evidence 2・Claims 2／revision `rev-eeffa38d4820371bf05ca41cacce6cab` | `content_evidence_origin->>revision` **IS NULL** |
  | 3 | `partial_update`（`targetClaimIds: ['TEST-01']`・`expectedRevision` ＝ POST 2 の revision） | `200`／`ok:true`／Evidence 2・Claims 2／revision `rev-e8ab5c08e1ebb6b8c34ac008f505965c`（POST 2 の値から変化） | `content_evidence_origin->>revision` **= `rev-eeffa38d…`** |

  Level 2 の含意（コード実測に基づく）：`writeCanonicalContentEvidence` は更新行数がちょうど 1 のときだけ `ok:true` を返し（0 行→409・2 行以上→500）、ルートは canonical write 失敗時に 409／500 で即 return する。したがって POST 2・POST 3 の成功は、実本番DBで CAS UPDATE がそれぞれちょうど 1 行に作用したことを意味する。POST 3 は DB から読んだ既存 revision が `rev-eeffa38d…` であることを前提に通過しており、POST 2 の書込が永続化されていたことの間接的な独立証拠にもなる。事前に、実サーバー module に対するオフライン検証（DB・ネットワーク・ファイル書込なし）で validate→resolve→plan の通過、非対象 TEST-02 の Claim／Evidence が deep-equal で保持されること、新 revision の形式（`rev-[0-9a-f]{32}`）を確認した。
- **fixture**：TEST-01／TEST-02 は `law_regulation`・`manual_user_input`・`user_verified`・出典は `.go.jp` ドメイン（Tier1・ホスト名の文字列判定のみで URL は fetch されない）・本文に「検証用・実在の主張ではありません」と明記。paid provider／AI API／Web Evidence 到達 0。
- **証明していないこと（正確な限界）**：①DB の読み戻し（readback）は未実施 ― 「TEST-02 保持・TEST-01 更新」は応答とオフライン検証とCAS成功の含意からの推論で、DB実データの直接確認ではない ②**U2 の送信側ブラウザ挙動（`resolutionScope` 判定・POST直前の再GET・payload組立）は本番で未実証**（`deterministic test verified` のみ・`contentEvidenceUiResolutionContract` 84/84）③本番での negative 経路（実409 発火等）は意図的に未実施（JS層の判定は決定的テスト、`.eq()` 不一致→0行は標準DB動作）④第一投稿 canonical の無変更は「アクセスしていない」ことによる構成上の保証で、読み戻しによる確認ではない。
- **disposable scope（削除しない・検証用データとして残置）**：`output_drafts` に1行（`out-prg-verification-20260918`）。canonical は Claim 2件（TEST-01・TEST-02）・Evidence 2件。`cases` テーブルには行を作っておらず、`output_drafts` を全件列挙する reader も無い（`case_id`／`output_id` 指定のみ）ため、案件一覧等には現れない。DELETE／rollback／manual DB edit／cleanup はすべて 0。以後この scope を実運用（Instagram 投稿等）に使わない。
- **第一投稿（`case-value-1788410623` ／ `out_1788413020275`）**：変更禁止を継続。Level 2 を通じて GET を含め一切アクセスしていない。
- **Decision**：Decision 最大番号 119（**Decision 120 は追加していない**）。Decision 119 本文は不変・「状態」と検証エビデンスを Decision 119 へ追記（`docs/04DECISIONS.md`）。
- **テスト（Regression Gate・前工程実測）**：12ファイル・1,010 / 1,010 PASS・0 failed（詳細は後続の既存記録）。本工程（Level 1／2 の docs 同期）では test 実行 0。
- **次工程**：自動実行しない。ユーザー判断待ち（候補：disposable scope 限定の readback GET の要否／U2 送信側の production 実証の要否）。**U2 を「送信側まで Production Verified」と扱うことはしない。**

---

## 【push前の記録】Evidence-Based Content Value Quality ＋ Safety Foundation B1 ＋ Partial Resolution Guard（PRG）Core / UI Contract U2 — Code Commit Complete ／ 未push（2026-09-18・Decision119・Tagなし）

> ※本節は **push 前（2026-09-18）の時点の記録**であり、履歴として保持する。以下の「未push／Not Deployed／ahead 1」等は当時の状態で、**現在の状態は直上の節**を参照。

**（push前の状態区分）PRG Core + U2 は Code Commit Complete ／ Not Pushed ／ Not Deployed ／ production 実機確認 pending だった。** 公式docsが docs commit `e37324e`（2026-09-10）で停止していたため、今回 `e37324e..da24cf29` の未同期 Code Commit **15件** を Git 実測で再構築し正式記録した。B1 までの commit（`e5ce21a` まで）は origin/main に到達済みで、PRG Core + U2（`da24cf2`）のみが当時 push 前だった（その後 push 済み）。

- **現在Version**：**Version1 Final Complete ／ Version1.1 Connected AI Company 開発中**（Version変更なし）
- **現在Phase**：**Phase54 Complete維持 ／ Phase55 未着手**
- **Git 実測（push前当時）**：branch `main`／HEAD `da24cf29f3f0d3d2b4ef4550694391a57871b51d`（subject `feat: guard partial evidence resolution updates`・7 files・+1,933/-46）／origin/main `e5ce21a5a8e5ec9079e2f4dad4a1e2ce7954500d`／ahead 1 / behind 0／staged 0（当時の値。現在は直上節のとおり HEAD ＝ origin/main ＝ `9fef9dc8`）。
- **未同期 Code Commit 15件（`e37324e`→`da24cf29`）**：`df99437`（CV-3a/CV-3b Core）→`07d7027`（CV-4b server-side Content Value ＋ `content_type`／`content_value` 列）→`79161b8`（Output Draft Authentication S2）→`2e3fde7`（Internal API Authorization Boundary S3-B1b）→`72bb578`（Cost Tracker Test Isolation）→`d4ad96a`（CV-4c-1 Activation Wiring）→`1a81d9b`（CV-4c-2 Claim Intent Planning Core）→`201cc17`（CV-4c-3 Evidence Approval State Machine）→`edd096d`（CV-4c-3B Server Canonical Resolution）→`6dfc335`／`50ca7de`／`5644449`／`4ec5249`（Content Evidence UI 修正4件）→`e5ce21a`（Safety Foundation B1 canonical 列）→`da24cf2`（PRG Core + UI Contract U2）。
- **Content Value Quality（CV-3a〜CV-4c-1）**：`shared/contentEvidence.js`（1 record = 1 claim × 1 source・cross-case guard・`contradicts` を supports として数えない・AI生成物を Evidence enum に含めない）＋ `shared/contentValueQuality.js`（6軸：evidenceGrounding／specificity／informationGain／actionability／saveValue／nonGeneric・gate と score の分離）。`lib/contentValueService.js` が **client 供給 contentValue を一切採用せず server 側で再計算**する。投稿種別の唯一の SoT は `output_drafts.content_type`（`value`／`bridge`／`product`・**atomic first-write-wins**・downgrade 不可・未宣言 NULL は `unknown` として **fail-closed**）。DB 側の失敗のみ fail-open（Output Draft 本体保存を巻き込まない）。
- **認可境界（S2／S3-B1b）ならびに Test Isolation**：`GET /api/output-drafts` の `requireSession()` 化と 401／200-empty の区別（`79161b8`）、有料AI実行・破壊的 API route の認証境界（`2e3fde7`）、`server.test.js` が Protected の本番 `cost-logs.json` へ書き込まないための `COST_TRACKER_STORAGE_PATH` 分離（`72bb578`）。
- **Evidence Grounding（CV-4c-2／4c-3／4c-3B）**：Claim Intent（まだ claim ではない「問い」）→ Evidence 取得 → Evidence が支持する範囲だけ claim 確定、という順序を強制。medical／treatment 等の Safety Filter は fail-closed。Web Search 実行は常に独立した明示承認が必要（`shared/contentEvidenceApproval.js` の状態機械・`NO_AUTO_TRIGGER_EVENTS`＝Auto Task／Writer実行／Output Draft保存／Case開始を承認として扱わない・自動retry禁止）。`lib/contentEvidenceResolutionService.js` は claimType／supportType／verificationStatus／reliability／sourceTier／grounded のいずれも client 値を採用しない。
- **Safety Foundation B1（`e5ce21a`・本番到達済み）**：canonical 3列（`content_evidence`／`content_claims`／`content_evidence_origin`）を `fields`（client が全置換する JSONB）の外側へ分離。**通常保存では canonical を変更しない**／canonical write は Evidence Resolution 成功経路（`writeCanonicalContentEvidence()`）のみ／cross-case は `case_id` 一致条件で二重防御／列未追加DBでも fail-safe。legacy fields は削除せず保全。
- **B1 本番実績（ユーザー実測・引継ぎ事実）**：canonical 3列 migration 適用済み。第一投稿（caseId `case-value-1788410623`／outputId `out_1788413020275`）へ controlled backfill 実施＝**Evidence 6件（verified 6）／Claims 3件（CI-01・CI-02・CI-03・grounded）**・`origin.mode='legacy_fields_backfill'`・canonical を持つ row は 1件・legacy fields 削除 0。**Post-Deploy Controlled Save Verification COMPLETE**（通常保存後も canonical Evidence 6／Claims 3／origin／fields 不変・`content_type` NULL 維持・変化したのは `content_value.evaluatedAt` と `updated_at` のみ）。
- **CAS R-a（read-only production verification：PASS）**：CAS 条件に `content_evidence_origin->>revision` を採用。revision キーが無い既存 row では `IS NULL` が最初の CAS 条件として機能することを実測（production SELECT のみ・write 0）。**migration 不要・backfill 不要**。
- **PRG Core + UI Contract U2（`da24cf2`・push前記録／現在は push 済み）**：`full_replace`／`partial_update`、`targetClaimIds`、`expectedRevision` を必須化し、CAS 更新行数 1＝成功／0＝`cas_conflict`（409 `canonical_revision_conflict`）／2以上＝`invariant_multiple_rows`（500）。部分成功・反証・Claim を伴わない Evidence・evidenceId 重複・cross-case・`full_replace_would_drop_claims` は request 全体を拒否（canonical write 0）。UI（U2）は既存 canonical Claim を**明示選択**し、claimId を topic／行番号から推測せず、POST 直前に canonical を再GETして Claim集合・revision・loadSeq・outputId のいずれかが変化していれば送信しない。全選択＝`full_replace`／一部選択＝`partial_update`。既存 canonical がある scope での新規 Claim 追加は禁止（別工程）。409 は自動 retry しない。
- **Decision**：**Decision 119「Canonical Content Evidence Formal Truth Boundary」を新規採番**（最大 118 → 119・既存 Decision 本文は無変更・衝突なし）。本文は `docs/04DECISIONS.md`。
- **テスト（引継ぎ実測時点・本セッションでは実行 0）**：前セッションの NUL 修正後に対象3スイートを実行し `contentEvidenceUiResolutionContract` 84/84・`contentEvidenceResolutionGuard` 110/110・`contentEvidenceCanonicalColumns` 127/127 PASS。当時の引継ぎには「既存11スイート 991件は release前Preflightで再実行する」という記載があったが、**この「11スイート／991件」という過去baselineは、その後の Release Preflight で repository 内に一次記録（正確な11ファイル名・991の内訳）が存在しないことを確認し、`C — BASELINE NOT RECOVERABLE` と正式判定した**（commit message・docs過去版・git notes／stash／reflog・ログ系ファイルを read-only 全数調査。唯一の実測記録は `df99437` の「既存安全 8 suites 1,204/0」で、スイート数・件数・対象領域とも不一致）。推測によるファイル名復元・件数合わせは行っていない。
- **New Regression Gate（release change set から再設計・実測済み）**：991 baseline を使わず、`da24cf29` の変更範囲（`server.js`／`lib/outputDraftsDb.js`／`lib/contentEvidenceResolutionGuard.js`／`index.html`）と依存関係から **12ファイルを明示manifest化**して個別実行した。REQUIRED 5（`contentEvidenceResolutionGuard.test.js` 110/110・`contentEvidenceCanonicalColumns.test.js` 127/127・`contentEvidenceUiResolutionContract.test.js` 84/84・`contentValueWiring.test.js` 98/98・`outputDraftAuth.test.js` 62/62）＋ SUPPORTING 7（`contentEvidenceApproval.test.js` 172/172・`contentEvidenceServerResolution.test.js` 31/31・`contentEvidenceWriterContext.test.js` 19/19・`contentEvidenceBrowserReality.test.js` 22/22・`contentEvidence.test.js` 79/79・`contentValueQuality.test.js` 141/141・`contentClaimPlanning.test.js` 65/65）。**合計 12ファイル・1,010 / 1,010 PASS・0 failed。** `contentValueWiring.test.js` は実行直前に Supabase 関連 env（`SUPABASE_URL`／`SUPABASE_ANON_KEY`／`SUPABASE_SERVICE_ROLE_KEY`／`SUPABASE_SECRET_KEY`／`NEXT_PUBLIC_SUPABASE_URL`／`NEXT_PUBLIC_SUPABASE_ANON_KEY`）がすべて `process.env` 上で UNSET であることを fail-closed 確認したうえで local fallback mode として実行した（`.env.local` にキー自体は存在するが、この test の require chain に `dotenv` 読込は無く到達しない。値は docs へ一切記載しない）。**`server.test.js` は今回も実行せず、Regression Gate にも含めない**（Protected `data/conversations/_meta.json` への書込み経路を持ち、かつ現在のPRG/U2/B1変更に対する固有coverageが0であるため、安全上の理由で除外）。テスト実行はすべて `node <filename>.test.js` のファイル名明示・個別実行のみ（`npm test`／wildcard／auto discoveryは不使用）。
- **次工程（push前当時の計画・その後 push／Render Deploy／Smoke／Level 1／Level 2 まで実施済み。現在の状態は直上節）**：ChatGPT確認 → docs commit → release前Preflight最終確認（本Gate結果の反映後）→ `git push origin main` → Render Auto-Deploy → 本番 read-only smoke → 必要な Controlled Production Verification。当時は push／tag／Render／production write をいずれも未実施で、Release Preflight PASS は Production Complete を意味しなかった。

---

## Carousel Background Control（Option B+C）正式Release Complete ＋ Release Docs Sync（2026-09-10・PA-26〜PA-28・Tag `v1.01-carousel-image-production-background-control`）

**状態区分：Production Release Complete ／ production real-image validation pending。** PA-26 Release Precheck PASS（判定A）→ PA-27 `git push origin main`（`e2288ea..9239e83`）→ Render Auto-Deploy `9239e83` Live（ユーザー Dashboard 実測・Deploy succeeded・Auto-Deploy・2026-09-10 14:48:37 JST）→ production read-only smoke PASS → annotated tag `v1.01-carousel-image-production-background-control`（target `9239e839a2eb6006fa8e210dca75fd5b125f2320`）作成・push・remote verification PASS → PA-28 で本 Release を公式 docs へ同期（docs-only）。**HEAD==origin/main==`9239e839a2eb6006fa8e210dca75fd5b125f2320`・ahead0/behind0。**
...
- **production smoke（read-only GET のみ）**：`GET /`=200・`/api/approvals`=200・`/api/carousel-image/assets`=401（route mounted）・`/shared/carouselImageCore.js`／`carouselBackgroundSanitizer.js`／`carouselVisualDirection.js`／`/server.js`／`/.env.local`＝404（server-only 維持）・`/shared/instagramAccountDesign.js`=200（公開 allowlist 無傷）。startup error なし＝新規モジュール require 解決。
- **Decision**：Decision 最大番号 **118 維持**（Decision 119 追加なし・Decision 118 本文無変更・Decision 117／112 維持）。
- **正確な状態**：Option B+C implementation complete ／ local safe tests PASS（8 suites・1,204 assertions・0 fail）／ **production release complete** ／ **production real-image validation pending**。「実画像で PASS」「F4 完全解決」「UI/icon 生成完全防止」「Visual Consistency 完全解決」「Image Review PASS」「Publishing Ready」「Instagram 投稿済み」「monetization started」は未達・記録しない。
- **次にやること**：**Third Controlled Paid Generation Preparation（未着手）** — new caseId → new Output Draft → new outputId → approved Value Content 本文の content-preserving copy → new Mobile Approval → Runtime Gate 一時 ON → new carousel approval → new nonce → quality=low・7 slides → generate POST exactly 1・retry 0 → Runtime Gate 再 OFF → Human Image Review。Decision 112 により completed 済み `out_1788413020275`／`out_1788992344384` は再利用しない。今回はこの準備・生成を実行していない。

---

## Carousel Background Generation Control（Option B+C）Local Implementation Complete ＋ Decision 118（2026-09-10）

**状態区分：Local Implementation Complete（未push）。** Code commit `8ff9f8d`（`feat: harden carousel background visual consistency`）＋ Docs commit（本工程）。origin/main=`e2288eab…` のまま・local mainのみ ahead 2。`Source Gate=true`・`Runtime Env Gate=unset`・`Effective REAL=false`・Image API 0・paid generation 0・DB write 0・Storage write 0・Publishing 0 を維持。

- **背景**：PA-22F の3回目相当 controlled paid generation（trial case・7 assets／published=false維持）→ PA-22G 人間Image Review **B FAIL**（slide 5/6/7 背景に UI/infographic）。PA-22H root cause = 肯定的日本語 imagePrompts と SAFE_SUFFIX の意味衝突 ＋ 7 slides 独立生成で Visual Consistency 層が不在。
- **対応（PA-23〜PA-25）**：Background Sanitizer（`shared/carouselBackgroundSanitizer.js` 新規）＋ Post-level Visual Direction（`shared/carouselVisualDirection.js` 新規）＋ SAFE_SUFFIX の三層 deterministic prompt control を `shared/carouselImageCore.js` の `buildBackgroundPrompt` に実装。Output Draft 本文は不変。
- **PA-25 code review**：ASCII substring 誤爆bug を発見→ boundary-aware matching（`_termRegex`）で是正。raw control byte 除去。8 safe suites **1,204 assertions / 0 fail**・F1/F2/F3 回帰 PASS。
- **Decision 118**「Carousel Background Generation Control Boundary」正式採用（Decision最大 117→118）。
- **正確な状態**：deterministic prompt-control implementation complete ／ local safe tests PASS ／ **production real-image validation pending**。
- **次にやること**：PA-25 の Code commit ＋ Docs commit を前提に、Release Precheck（read-only：fast-forward 可否・push 対象 path・Protected 不変・Effective REAL=false）→ ユーザー承認のうえ `git push` → Render Live 確認 → 3回目の実画像 controlled trial（approval 発行 → CRITICAL HUMAN CHECKPOINT → generate 1回）→ 人間 Image Review。いずれも都度ユーザー承認が必要。
