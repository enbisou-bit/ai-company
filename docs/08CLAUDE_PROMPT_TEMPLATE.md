...
更新日: 2026-06-29（v1.1 — 最終出力形式 正式仕様追加）
...
Claude Code へ渡す最終実装指示書は、必ず**通常テキスト形式**で出力する。

Markdown コードブロック（` ```markdown ～ ``` `）で全体を囲まない。

ChatGPT の「編集」「コピー」ボタンが表示される通常テキスト形式を標準とする。

---

### ヘッダー

最終指示書は必ず以下のヘッダーから開始する。
...
- 見出し・改行・空行を適切に使用し、長文でも読みやすくする
- 不要な装飾・過剰なインデントは使用しない
- コードブロックで全体を囲まない
...
- ユーザーが「コピー」ボタンを 1 回押すだけで、そのまま Claude Code へ貼り付けられる状態を維持する
- 途中で説明文を挟まない
- 途中で別の指示書へ分割しない
...
- 毎 Phase 同じフォーマット・同じ品質で出力する
- テンプレートはプロジェクトの正式仕様とし、改善案があれば各 Phase 終了時に更新を検討する
