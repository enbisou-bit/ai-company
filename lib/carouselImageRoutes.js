...
// Stage 1a-1: 見積り（read-only・token 発行 0・課金 0・DB write 0）
function createQuoteHandler(deps) {
  return async function (req, res) {
    var result = await service.handleQuoteRequest(req.query || {}, deps);
    res.status(result.status).json(result.body);
  };
}

// Stage 1a-2: 有料生成（approval → generate を server 内部で連結。approvalToken は応答に含めない）
function createProduceHandler(deps) {
  return async function (req, res) {
    var result = await service.handleProduceRequest(req.body || {}, deps);
    res.status(result.status).json(result.body);
  };
}
...
function registerCarouselImageRoutes(app, deps) {
  ...
  // Stage 1a: Console を使わずに Output Engine から見積り→承認→生成へ到達するための2route。
  //   ★ Security Boundary は既存と同一に保つ:
  //     - GET quote  … session のみ（GET assets と同方針。書き込みではないため Origin 検証は課さない）
  //     - POST produce … session → Origin → handler（POST 2route と完全に同じ並び）
  //   ★ 既存3routeの middleware・handler・契約は一切変更しない（追加のみ）。
  app.get('/api/carousel-image/quote', session, createQuoteHandler(deps));
  app.post('/api/carousel-image/produce', session, origin, createProduceHandler(deps));
}
...
module.exports = {
  ...
  createProduceHandler: createProduceHandler,
