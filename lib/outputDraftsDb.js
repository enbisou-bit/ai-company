...
//   ★ Partial Resolution Replacement Guard: revision CAS（content_evidence_origin.revision / R-a）。
//     expectedRevision は必須（null = revision 未設定の初期状態）。更新条件に
//     `content_evidence_origin->>revision IS NULL` または `= expectedRevision` を含める。
//     更新行数 1 = 成功 / 0 = cas_conflict（stale）/ 2以上 = invariant_multiple_rows（成功扱いしない）。
//   戻り値: { ok, applied, reason, rowCount }
async function writeCanonicalContentEvidence({ caseId, outputId, contentEvidence, contentClaims, origin, expectedRevision } = {}) {
  if (!caseId || !outputId) return { ok: false, applied: false, reason: 'caseId_and_outputId_required', rowCount: 0 };
  if (!Array.isArray(contentEvidence) || contentEvidence.length === 0
      || !Array.isArray(contentClaims) || contentClaims.length === 0) {
    return { ok: false, applied: false, reason: 'canonical_arrays_required', rowCount: 0 };
  }
  if (!origin || typeof origin !== 'object' || Array.isArray(origin)) {
    return { ok: false, applied: false, reason: 'origin_required', rowCount: 0 };
  }
  if (expectedRevision === undefined || (expectedRevision !== null && (typeof expectedRevision !== 'string' || !expectedRevision))) {
    return { ok: false, applied: false, reason: 'expected_revision_required', rowCount: 0 };
  }
  if (typeof origin.revision !== 'string' || !origin.revision || origin.revision === expectedRevision) {
    return { ok: false, applied: false, reason: 'next_revision_invalid', rowCount: 0 };
  }
  if (!supabase) return { ok: false, applied: false, reason: 'supabase_unavailable', rowCount: 0 };
  try {
    let query = supabase
      .from('output_drafts')
    ...
    query = expectedRevision === null
      ? query.is('content_evidence_origin->>revision', null)
      : query.eq('content_evidence_origin->>revision', expectedRevision);
    const { data, error } = await query.select('output_id');
    if (error) return { ok: false, applied: false, reason: error.message, rowCount: 0 };
    const rowCount = Array.isArray(data) ? data.length : 0;
    if (rowCount === 1) return { ok: true, applied: true, reason: null, rowCount };
    if (rowCount === 0) return { ok: false, applied: false, reason: 'cas_conflict', rowCount };
    return { ok: false, applied: true, reason: 'invariant_multiple_rows', rowCount };
  } catch (e) { return { ok: false, applied: false, reason: e.message, rowCount: 0 }; }
}
