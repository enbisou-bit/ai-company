...
const crypto = require('crypto'); // A-2-8b-2: source_fingerprint（SHA-256）生成用（標準モジュールのみ）
...
// ══════════════════════════════════════════════════════════════
// A-2-8b-2: ensureOpeningBalance(options)
//   移行前料金を api_cost_opening_balance へ冪等登録する専用関数。
//   冪等キー＝DB の source_fingerprint UNIQUE（金額は含めない＝別金額での二重計上を防ぐ）。
//   migrated_at は DB DEFAULT now() に委譲（payload に含めない）。id / voided_at も含めない。
//   失敗を成功扱いしない・0円行や代替行を作らない。事前SELECT＋UNIQUE(23505)を最終防衛線とする。
//   ※ Opening Balance は Legacy14キー/停止ゲートに影響しない（getCumulativeSummary の opening/grand のみ）。
// ══════════════════════════════════════════════════════════════
function _isNonEmptyStr(v) { return typeof v === 'string' && v.trim() !== ''; }

async function ensureOpeningBalance(options) {
  if (!supabase) return _fail('Supabase未設定', 'no_client', { source: 'fallback' });
  const o = options || {};

  // ── validation ──
  if (!_isNonEmptyStr(o.balanceType)) return _fail('balanceType は必須（非空文字列）', 'VALIDATION_ERROR');
  if (!_isNonEmptyStr(o.provider))    return _fail('provider は必須', 'VALIDATION_ERROR');
  const providerNorm = o.provider.trim().toLowerCase();
  if (providerNorm !== 'openai' && providerNorm !== 'claude') {
    return _fail("provider は 'openai' / 'claude' のみ（unknown / all / 空は不可）", 'VALIDATION_ERROR');
  }
  if (typeof o.amountJpy !== 'number' || !Number.isFinite(o.amountJpy) || o.amountJpy < 0) {
    return _fail('amountJpy は 0 以上の有限数（NaN / Infinity / 負数は不可）', 'VALIDATION_ERROR');
  }
  const amt = o.amountJpy;
  if (!_isNonEmptyStr(o.currency)) return _fail('currency は必須', 'VALIDATION_ERROR');
  const currencyNorm = o.currency.trim().toUpperCase();
  if (currencyNorm !== 'JPY') return _fail("currency は 'JPY' のみ", 'VALIDATION_ERROR');
  if (!_isNonEmptyStr(o.source))           return _fail('source は必須', 'VALIDATION_ERROR');
  if (!_isNonEmptyStr(o.migrationKey))     return _fail('migrationKey は必須', 'VALIDATION_ERROR');
  if (!_isNonEmptyStr(o.migrationVersion)) return _fail('migrationVersion は必須', 'VALIDATION_ERROR');

  let recordedAtNorm = null;
  if (o.recordedAt != null) {
    const t = Date.parse(o.recordedAt);
    if (Number.isNaN(t)) return _fail('recordedAt は有効なISO日時', 'VALIDATION_ERROR');
    recordedAtNorm = new Date(t).toISOString();
  }
  if (o.isVerified != null && typeof o.isVerified !== 'boolean') return _fail('isVerified は boolean', 'VALIDATION_ERROR');
  if (o.isActive   != null && typeof o.isActive   !== 'boolean') return _fail('isActive は boolean', 'VALIDATION_ERROR');

  // ── 正規化 ──
  const balanceTypeNorm      = o.balanceType.trim();
  const sourceNorm           = o.source.trim();
  const migrationKeyNorm     = o.migrationKey.trim();
  const migrationVersionNorm = o.migrationVersion.trim();
  const isVerified = (o.isVerified == null) ? false : o.isVerified;
  const isActive   = (o.isActive   == null) ? true  : o.isActive;
  const migratedBy = _isNonEmptyStr(o.migratedBy) ? o.migratedBy.trim() : 'ENBISOU_AI_COMPANY';
  const sourceFile         = (o.sourceFile         == null) ? null : String(o.sourceFile);
  const sourceValue        = (o.sourceValue        == null) ? null : _num(o.sourceValue);
  const verificationSource = (o.verificationSource == null) ? null : String(o.verificationSource);
  const note               = (o.note               == null) ? null : String(o.note);

  // ── source_fingerprint（SHA-256・金額は含めない）──
  const raw = [
    balanceTypeNorm,
    providerNorm,          // 小文字
    currencyNorm,          // 大文字
    sourceNorm,
    migrationKeyNorm,
    migrationVersionNorm,
  ].join('|');
  const sourceFingerprint = crypto.createHash('sha256').update(raw, 'utf8').digest('hex');

  try {
    // 1) 事前SELECT（冪等）
    const sel = await supabase.from('api_cost_opening_balance').select('*').eq('source_fingerprint', sourceFingerprint).maybeSingle();
    if (sel.error) return _fail(sel.error.message, sel.error.code);
    if (sel.data) return _ok({ inserted: false, existing: true, sourceFingerprint: sourceFingerprint, row: sel.data });

    // 2) INSERT（id / migrated_at / voided_at は含めない）
    const payload = {
      balance_type:        balanceTypeNorm,
      ...
      migration_key:       migrationKeyNorm,
      migration_version:   migrationVersionNorm,
      migrated_by:         migratedBy,
      recorded_at:         recordedAtNorm,
      note:                note,
      is_verified:         isVerified,
      verification_source: verificationSource,
      is_active:           isActive,
    };
    const ins = await supabase.from('api_cost_opening_balance').insert(payload).select().single();
    if (ins.error) {
      // A-2-8c: UNIQUE競合（23505）を二段階SELECTで分類（INDEX名の文字列解析には依存しない）。
      if (ins.error.code === '23505' || /duplicate key|unique/i.test(ins.error.message || '')) {
        // ① fingerprint 再SELECT ＝ 同一登録の再実行（冪等）。現行挙動を維持。
        const re = await supabase.from('api_cost_opening_balance').select('*').eq('source_fingerprint', sourceFingerprint).maybeSingle();
        if (!re.error && re.data) return _ok({ inserted: false, existing: true, sourceFingerprint: sourceFingerprint, row: re.data });

        // ② 業務キー（provider, balance_type, is_active）＝ 別fingerprintでのactive競合。existing扱いにしない。
        const be = await supabase.from('api_cost_opening_balance').select('*')
          .eq('provider', providerNorm).eq('balance_type', balanceTypeNorm).eq('is_active', true).maybeSingle();
        if (!be.error && be.data) {
          return {
            ok: false, data: null, source: 'db', stale: true,
            error: {
              code: 'OPENING_BALANCE_ACTIVE_CONFLICT',
              message: `Active opening balance already exists for provider=${providerNorm}, balance_type=${balanceTypeNorm}`,
              conflict: true,
              provider: providerNorm,
              balanceType: balanceTypeNorm,
              existing: be.data,
              attempted: {
                amountJpy: amt, currency: currencyNorm, source: sourceNorm,
                migrationKey: migrationKeyNorm, migrationVersion: migrationVersionNorm,
                sourceFingerprint: sourceFingerprint,
              },
              dbError: {
                code: ins.error.code || '23505',
                message: ins.error.message || null,
                details: (ins.error.details == null) ? null : ins.error.details,
                hint: (ins.error.hint == null) ? null : ins.error.hint,
              },
            },
          };
        }

        // ③ どちらでも既存行を特定できない → INSERT時の元23505を保持（lookup結果で上書きしない）。秘匿情報は載せない。
        return {
          ok: false, data: null, source: 'db', stale: true,
          error: {
            code: ins.error.code || '23505',
            message: ins.error.message || 'Unique conflict could not be resolved',
            details: (ins.error.details == null) ? null : ins.error.details,
            hint: (ins.error.hint == null) ? null : ins.error.hint,
            fingerprintLookupError: re.error ? { code: re.error.code || null, message: re.error.message || null } : null,
            businessLookupError: be.error ? { code: be.error.code || null, message: be.error.message || null } : null,
          },
        };
      }
      return _fail(ins.error.message, ins.error.code);
    }
    return _ok({ inserted: true, existing: false, sourceFingerprint: sourceFingerprint, row: ins.data });
  } catch (ex) {
    return _fail(ex && ex.message, 'exception');
  }
}
...
module.exports = {
  ...
  ensureOpeningBalance,         // A-2-8b-2: Opening Balance 冪等登録（source_fingerprint UNIQUE）
  getCostSettings,
  updateCostSettings,
  getDailyCostSummary,
