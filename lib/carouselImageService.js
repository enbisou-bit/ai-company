...
var STATUS_BY_REASON = {
  ...
  // Stage 1a: quote / produce の advisory reason（既存 reason の意味は一切変更しない・追加のみ）
  already_generated: 409,   // 対象 Output Draft に carouselAssets が既に存在する（Decision 112 の再生成禁止と整合）
  approval_failed: 500,     // produce 内部の approval 発行が既存 reason を返さず失敗した場合のみ
};
...
// ══════════════════════════════════════════════════════════════
// Stage 1a-1: GET /api/carousel-image/quote
//   input: { caseId, outputId, quality }
//
//   責務: 「いま生成してよいか」と「そのときの認可額」を、課金・token 発行・DB write を
//   一切伴わずに返す read-only route。UI が有料実行の前にユーザーへ金額と可否を提示するために使う。
//
//   ★ approval token を発行しない（shared/carouselApproval.js を呼ばない）。
//   ★ 新しい認可ロジックを作らない。可否判定は既存 core.planCarouselImageJob()
//     （scope / approval / published / slides / quality）と既存 budget 計算
//     （client.estimateAuthorizedTotalJpy + deps.getCumulativeSpentJpyByOutputId）の
//     戻り値をそのまま伝えるだけで、ここで独自の許可判断を足さない。
//   ★ 戻り値の eligible=false は「助言」であり authorization ではない。実際の認可は
//     既存 approval/generate の全ゲート（変更なし）が最終決定する。
// ══════════════════════════════════════════════════════════════
async function handleQuoteRequest(input, deps) {
  var q = input || {};
  var caseId = q.caseId;
  var outputId = q.outputId;
  ...
  var assets = Array.isArray(fields.carouselAssets) ? fields.carouselAssets : [];
  var approvalDecision = approvalRow
    ? (approvalRow.approval_decision != null ? approvalRow.approval_decision : approvalRow.approvalDecision)
    : null;
  var published = !!(approvalRow && (approvalRow.published === true || approvalRow.published === 'true'));

  // 既定の骨格（plan が通らない場合でも UI が状態を表示できるよう、読めた値は返す）
  var body = {
    ok: true,
    eligible: false,
    reason: null,
    caseId: caseId,
    outputId: outputId,
    quality: (quality === undefined || quality === null) ? null : String(quality),
    slideCount: slides.length,
    estimatedTotalCostJpy: null,
    cumulativeSpentJpy: null,
    budgetJpyPerPost: null,
    approved: String(approvalDecision) === 'approved',
    published: published,
    assetsReady: assets.length > 0,
  };

  // 既に生成済み（Decision 112: 同一 output_id の再生成は行わない）
  if (body.assetsReady) {
    body.reason = 'already_generated';
    return { status: 200, body: body };
  }

  // quality は request shape として扱う（既存 approval route と同じ語彙・同じ扱い）
  var qv = client.validateQuality(quality);
  if (!qv.ok) return _err(reasonToHttpStatus(qv.reason), qv.reason, 'quality');
  ...
  if (!plan.ok) {
    body.reason = plan.reason;   // no_approval / not_approved / already_published / cross_case 等（既存語彙）
    return { status: 200, body: body };
  }

  body.quality = plan.quality;
  body.slideCount = plan.totalSlides;
  body.budgetJpyPerPost = plan.budgetJpyPerPost;

  var estimatedCostJpy = client.estimateAuthorizedTotalJpy(plan.quality, plan.totalSlides);
  if (estimatedCostJpy === null) return _err(503, 'estimate_unavailable');
  body.estimatedTotalCostJpy = estimatedCostJpy;

  var cumulative;
  try { cumulative = await deps.getCumulativeSpentJpyByOutputId(outputId); }
  catch (e) { cumulative = { ok: false, reason: 'budget_read_unavailable' }; }
  if (!cumulative || cumulative.ok !== true) return _err(503, 'budget_read_unavailable');
  body.cumulativeSpentJpy = cumulative.cumulativeJpy;

  if (cumulative.cumulativeJpy + estimatedCostJpy > plan.budgetJpyPerPost) {
    body.reason = 'budget_exceeded';
    return { status: 200, body: body };
  }
...
// ══════════════════════════════════════════════════════════════
// Stage 1a-2: POST /api/carousel-image/produce
//   input: { caseId, outputId, quality, confirmedCostJpy }
//
//   責務: 既存 approval → generate を **server 内部で連結**し、approval token を
//   ブラウザへ一切出さずに有料生成を 1 回だけ実行する。
//
//   ★ approvalToken は response・log・client のどこにも出さない（本関数のローカル変数のみ）。
//   ★ billingLock は client から受け取らない。既存契約どおり boolean false を内部固定する。
//   ★ 新しい認可・課金・token・ledger ロジックを作らない。実際の認可は既存
//     handleApprovalRequest / handleGenerateRequest（いずれも無変更）が行う。
//   ★ confirmedCostJpy: UI がユーザーへ表示した金額。server 再計算値と一致しない場合は
//     estimated_cost_mismatch（409）で fail-closed し、自動で課金へ進まない。
//     丸めは既存 approval.normalizeCostJpy() を再利用する（新しい丸め規則を作らない）。
//   ★ retry を一切持たない。失敗はそのまま返し、呼び出し側へ再送を促さない。
// ══════════════════════════════════════════════════════════════
async function handleProduceRequest(input, deps) {
  var body = input || {};
  var caseId = body.caseId;
  var outputId = body.outputId;
  var quality = body.quality;
  ...
  // ── 1. 事前確認（read-only）: quote と同一の判定を再利用する（二重実装しない） ──
  var quote = await handleQuoteRequest({ caseId: caseId, outputId: outputId, quality: quality }, deps);
  if (quote.status !== 200 || !quote.body || quote.body.ok !== true) return quote;   // 400/404/503 はそのまま
  if (quote.body.eligible !== true) {
    var r = quote.body.reason || 'not_approved';
    return _err(reasonToHttpStatus(r), r);
  }

  // ── 2. 金額の明示確認（UI 表示額 == server 再計算額） ──
  var confirmed = approval.normalizeCostJpy(body.confirmedCostJpy);
  var computed = approval.normalizeCostJpy(quote.body.estimatedTotalCostJpy);
  if (confirmed === null || computed === null || confirmed !== computed) {
    return { status: 409, body: { ok: false, reason: 'estimated_cost_mismatch', estimatedTotalCostJpy: quote.body.estimatedTotalCostJpy } };
  }

  // ── 3. approval 発行（server 内部のみ・token は外へ出さない） ──
  var appr = await handleApprovalRequest({ caseId: caseId, outputId: outputId, quality: quote.body.quality }, deps);
  var token = (appr && appr.body && typeof appr.body.approvalToken === 'string') ? appr.body.approvalToken : null;
  if (appr.status !== 200 || !appr.body || appr.body.ok !== true || !token) {
    var ar = (appr && appr.body && appr.body.reason) ? appr.body.reason : 'approval_failed';
    return _err(appr && appr.status ? appr.status : reasonToHttpStatus(ar), ar);   // ★ token を含めない
  }

  // ── 4. generate（billingLock は内部固定 false・retry なし） ──
  var gen = await handleGenerateRequest({ approvalToken: token, billingLock: false }, deps);

  // ── 5. 結果のみ返す（token は含めない） ──
  if (gen.status === 200 && gen.body && gen.body.ok === true) {
    return {
      status: 200,
      body: {
        ok: true,
        generated: gen.body.generated,
        published: gen.body.published,
        caseId: caseId,
        outputId: outputId,
        quality: quote.body.quality,
        slideCount: quote.body.slideCount,
        estimatedTotalCostJpy: quote.body.estimatedTotalCostJpy,
      },
    };
  }
  return { status: gen.status, body: gen.body };
}
